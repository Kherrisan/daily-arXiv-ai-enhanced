...
TL;DR: This tutorial introduces cryptographic foundations for electronic voting, showing how ballot secrecy and verifiability can coexist using modern techniques like homomorphic encryption and game-based models, making advanced concepts accessible to non-specialists.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the inherent tension between ballot secrecy and verifiability in electronic voting systems, ensuring secure and trustworthy elections by leveraging cryptographic techniques inspired by physical voting systems.

Method: The paper uses fundamental concepts in modern cryptography, such as asymmetric and homomorphic encryption, and frames electronic elections using game-based cryptography to formally define and analyze ballot secrecy and verifiability.

Result: The paper presents a general electronic election scheme with minimal mathematical formalism, introduces game-based cryptographic modeling for elections, and provides precise definitions of ballot secrecy and verifiability within this framework.

Conclusion: The paper concludes that ballot secrecy and verifiability, though seemingly conflicting, can be balanced in electronic voting systems using modern cryptographic tools, and it aims to make advanced research in this area accessible to a broader audience.
...
TL;DR: Context-Conditioned Delta Steering (CC-Delta) is a novel SAE-based defense that detects and mitigates jailbreak attacks by identifying sparse features through contextual representation differences and applying mean-shift steering in SAE latent space, showing superior performance, especially on out-of-distribution attacks.


<details>
  <summary>Details</summary>
Motivation: Jailbreak attacks continue to threaten the safety of large language models, and existing defenses operating in dense latent spaces may not adequately generalize, especially against out-of-distribution attacks. There is a need for more effective, generalizable, and interpretable defense mechanisms.

Method: CC-Delta uses sparse autoencoders (SAEs) to identify jailbreak-relevant features by comparing token-level representations of the same harmful request with and without jailbreak context. It applies statistical testing on paired harmful/jailbreak prompts to select relevant features and performs mean-shift steering in the SAE latent space during inference.

Result: CC-Delta achieves comparable or better safety-utility tradeoffs than baseline defenses across four aligned models and twelve jailbreak attacks. It outperforms dense mean-shift steering on all models, particularly against out-of-distribution attacks, demonstrating the advantage of sparse SAE feature space for jailbreak mitigation.

Conclusion: Off-the-shelf sparse autoencoders (SAEs) trained for interpretability can be effectively repurposed as practical jailbreak defenses without additional task-specific training, demonstrating that sparse feature space steering is a viable and often superior alternative to dense activation space methods.
...
TL;DR: DRAMatic is a PIM-based system that accelerates homomorphic encryption using optimized arithmetic techniques on UPMEM's architecture, achieving competitive performance against Microsoft SEAL while highlighting limitations in current PIM hardware.


<details>
  <summary>Details</summary>
Motivation: Homomorphic encryption is computationally expensive and memory-bound on conventional architectures; PIM offers higher memory bandwidth and presents a promising alternative for accelerating HE, motivating the development of efficient PIM-based HE implementations.

Method: The authors implement DRAMatic on UPMEM's programmable PIM system, leveraging arithmetic optimizations such as residue number system and number-theoretic transform techniques to support large-parameter homomorphic encryption operations, and compare its performance with Microsoft SEAL in terms of runtime and energy efficiency.

Result: DRAMatic significantly reduces the performance gap between UPMEM PIM and Microsoft SEAL in HE operations, demonstrating improved runtime and energy efficiency, but is bottlenecked by slow multiplication and data transfer overhead on the current UPMEM hardware.

Conclusion: DRAMatic demonstrates the potential of using PIM architectures for accelerating homomorphic encryption operations, showing significant performance improvements over conventional systems, though it is currently limited by multiplication inefficiencies and data transfer overhead in the UPMEM PIM platform.
...
TL;DR: RADAR is a log-adversary-aware forensic framework that uses physical storage artifacts to detect hidden operations in NoSQL databases, overcoming compromised audit logs by cross-referencing carved records with application logs across diverse NoSQL systems.


<details>
  <summary>Details</summary>
Motivation: The diversity and opacity of NoSQL storage formats, combined with the potential for privileged insiders to manipulate or disable audit logs, make traditional digital forensics unreliable for NoSQL databases.

Method: RADAR combines the Automated NoSQL Carver (ANOC) to reconstruct low-level storage artifacts from raw disk bytes with a reconciliation engine that compares these artifacts against high-level application logs to detect discrepancies.

Result: Evaluated across ten NoSQL engines, RADAR successfully identifies delta artifacts such as unlogged insertions, silent deletions, and field-level updates, achieving 31.7-397 MB/min processing throughput under log-evasion scenarios including log suppression and post-maintenance attacks.

Conclusion: RADAR demonstrates the feasibility of log-independent, trustworthy NoSQL forensics by using physical storage artifacts to detect malicious activities even when audit logs are compromised or suppressed.
...
TL;DR: TensorCommitments (TCs) is a tensor-native proof-of-inference scheme that enables verifiable LLM inference with minimal overhead (0.97% prover and 0.12% verifier time on LLaMA2) and improved robustness against adversarial tampering.


<details>
  <summary>Details</summary>
Motivation: Current LLM inference on external clouds requires users to trust the service provider; there is no efficient way to verify that inference was executed correctly without re-executing the model, creating a need for verifiable and efficient proof-of-inference.

Method: The authors propose TensorCommitments (TCs), which bind LLM inference to cryptographic commitments organized in multivariate Merkle Trees (Terkle Trees), creating tamper-evident tags native to tensor operations.

Result: On LLaMA2, TCs add only 0.97% prover overhead and 0.12% verifier overhead while improving robustness to targeted LLM attacks by up to 48% compared to prior GPU-reliant verification methods.

Conclusion: TCs enable efficient, verifiable LLM inference at cloud scale with minimal performance cost and enhanced security, overcoming limitations of both cryptographic and non-cryptographic prior approaches.
...
TL;DR: This paper introduces two structured conformal prediction methods, level-wise CP (L-CP) and projection-based CP (P-CP), for OS fingerprinting with formal uncertainty quantification, addressing the limitations of flat classification by leveraging the hierarchical taxonomy of operating systems.


<details>
  <summary>Details</summary>
Motivation: Conventional OS fingerprinting methods lack formal uncertainty quantification and ignore the taxonomic structure of OSs, leading to brittle predictions. The need for reliable, structured uncertainty estimates motivates the use of conformal prediction that respects hierarchy.

Method: Two structured conformal prediction strategies are proposed: L-CP, which independently calibrates prediction sets at each level of the hierarchy, and P-CP, which enforces structural consistency by projecting leaf-level prediction sets upward through the taxonomy.

Result: Both L-CP and P-CP provide valid coverage guarantees. L-CP produces tighter (more efficient) prediction sets per level but allows taxonomic inconsistencies. P-CP ensures hierarchically consistent, nested prediction sets, making it suitable for automated systems, though at the cost of lower efficiency at coarser hierarchy levels.

Conclusion: The work demonstrates a fundamental trade-off between efficiency and structural consistency in hierarchical OS fingerprinting. P-CP is preferred for automated, policy-critical applications requiring consistent outputs, while L-CP suits forensic contexts where precision per level is prioritized.
...
TL;DR: TrustMee introduces self-verifying remote attestation using signed WebAssembly components to enable platform-agnostic verification of confidential VM integrity, reducing verifier TCB and maintenance costs.


<details>
  <summary>Details</summary>
Motivation: Remote attestation for cVMs is crucial for trust but currently requires hardware-specific verification logic, increasing complexity and the verifier's trusted computing base (TCB). This limits scalability and practical deployment.

Method: The authors propose embedding hardware-agnostic verification logic directly into attestation evidence as a WebAssembly (Wasm) module, signed by a trusted party. The verifier checks the signature on the Wasm module and executes it to validate the evidence, eliminating the need for platform-specific verification code.

Result: TrustMee is implemented as a verification driver in the Trustee framework and supports self-verifying attestation for AMD SEV-SNP and Intel TDX. The system generates standard-compliant EAT Attestation Result (EAR) claims and enables verifiers to validate evidence without hardware-specific knowledge.

Conclusion: Self-verifying attestation using signed Wasm components simplifies verification, reduces the verifier's TCB, and enables portable, maintainable, and standardized attestation across diverse hardware platforms.
...
TL;DR: The paper proposes an LLM-based agentic system for autonomous incident response that integrates perception, reasoning, planning, and action in a 14B model, enabling end-to-end learning and in-context adaptation without handcrafted simulators.


<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning approaches for incident response require manually designed simulators and fail to leverage rich semantic information in raw logs; the paper aims to overcome these limitations using LLMs.

Method: The approach uses a fine-tuned 14B LLM with chain-of-thought reasoning to process system logs, infer network state, update attack hypotheses, simulate responses, and generate actions, enabling closed-loop, in-context adaptation.

Result: The LLM agent achieves up to 23% faster recovery compared to state-of-the-art LLMs when evaluated on published incident logs, while operating on commodity hardware without a custom simulator.

Conclusion: An integrated LLM agent can effectively perform autonomous incident response with real-time adaptation, eliminating the need for handcrafted models and enabling practical deployment.
...
### [12] [Perceptual Self-Reflection in Agentic Physics Simulation Code Generation](https://arxiv.org/abs/2602.12311)
*Prashant Shende,Bradley Camburn*

Main category: cs.SE

TL;DR: A multi-agent framework uses perceptual self-reflection—analyzing rendered simulation frames via a vision-language model—to iteratively validate and correct physics simulation code generated from natural language, effectively closing the oracle gap and improving accuracy across diverse physics domains.


<details>
  <summary>Details</summary>
Motivation: The motivation is to bridge the 'oracle gap' in physics simulation code generation, where syntactically correct code can produce physically incorrect behavior that standard testing methods cannot detect. Existing approaches lack mechanisms to validate whether simulations behave as expected in physical terms, especially when generated from natural language descriptions.

Method: A four-agent system is designed: a natural language interpreter, a technical requirements generator, a physics code generator with self-correction, and a physics validator using perceptual self-reflection. The key methodological innovation is the use of a vision-capable language model to analyze rendered simulation frames for physical correctness, enabling iterative refinement of code based on visual output rather than syntactic validation alone.

Result: The system was evaluated across seven physics domains and showed substantial improvements over single-shot generation baselines, with most scenarios achieving target physics accuracy. It demonstrated robust self-correction, pipeline stability, and an operational cost of approximately $0.20 per animation.

Conclusion: The proposed multi-agent framework with perceptual self-reflection significantly improves the accuracy and reliability of physics simulation code generation from natural language, demonstrating the effectiveness of iterative visual feedback via vision-language models and highlighting the potential of agentic AI in engineering and physics workflows.
...
### [13] [SHAPR: A Solo Human-Centred and AI-Assisted Practice Framework for Research Software Development](https://arxiv.org/abs/2602.12443)
*Ka Ching Chan*

Main category: cs.SE

TL;DR: This paper introduces SHAPR, a practice-level framework that operationalizes Action Design Research for solo, AI-assisted research software development, enhancing accountability, reflection, and methodological rigor through human-centered design and structured reflective practices.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the growing use of generative AI in research software development by solo HDR researchers, coupled with the lack of guidance in Action Design Research for integrating AI assistance while maintaining accountability, reflection, and methodological rigor in practice.

Method: The study employs a conceptual design science approach, formulating SHAPR as a design artefact and evaluating it through formative, reflective analysis of its internal coherence, alignment with ADR principles, and applicability to solo research practice.

Result: The result is the development and formative evaluation of the SHAPR framework, which articulates roles, artefacts, reflective practices, and lightweight governance mechanisms to support ADR cycles in AI-assisted, solo research software development.

Conclusion: The paper concludes that SHAPR provides a coherent, actionable framework that effectively operationalizes Action Design Research principles in solo, AI-assisted research software development, supporting human accountability, reflective learning, and methodological rigor in contemporary HDR contexts.
...
### [14] [Favia: Forensic Agent for Vulnerability-fix Identification and Analysis](https://arxiv.org/abs/2602.12500)
*André Storhaug,Jiamou Sun,Jingyue Li*

Main category: cs.SE

TL;DR: Favia is a forensic, agent-based framework that improves vulnerability-fixing commit identification by combining scalable ranking with deep semantic reasoning using an LLM agent, achieving state-of-the-art performance on a large-scale real-world dataset.


<details>
  <summary>Details</summary>
Motivation: Existing methods for identifying vulnerability-fixing commits suffer from poor precision-recall trade-offs and are often evaluated on randomly sampled commits, which underestimates the difficulty of distinguishing highly similar, security-relevant commits in real-world settings.

Method: Favia uses a two-stage method: (1) an efficient candidate ranking stage to narrow down potential commits, followed by (2) a ReAct-based LLM agent that performs iterative reasoning in a pre-commit repository environment, using specialized tools to localize vulnerabilities, navigate code, and establish causal links between code changes and vulnerabilities.

Result: Evaluated on CVEVC, a dataset of 8 million commits from 3,708 repositories, Favia achieves superior precision-recall trade-offs and the highest F1-scores compared to state-of-the-art traditional and LLM-based baselines under realistic candidate selection.

Conclusion: Favia's forensic, agent-based approach significantly improves the accuracy of identifying vulnerability-fixing commits in real-world scenarios, outperforming existing methods by combining scalable ranking with iterative semantic reasoning and causal analysis.
...
### [15] [FuncDroid: Towards Inter-Functional Flows for Comprehensive Mobile App GUI Testing](https://arxiv.org/abs/2602.12834)
*Jinlong He,Changwei Xia,Binru Huang,Jiwei Yan,Jun Yan,Jian Zhang*

Main category: cs.SE

TL;DR: This paper proposes FuncDroid, an inter-functional-flow-oriented GUI testing approach for Android apps that models app behaviors using a Functional Flow Graph (FFG) and employs a long-short-term-view-guided strategy to explore complex inter-functional interactions, significantly improving test coverage and bug detection.


<details>
  <summary>Details</summary>
Motivation: Existing GUI testing methods focus on individual functionalities and ignore complex inter-functional interactions, limiting their ability to detect deep bugs arising from cross-functional behaviors in modern, complex mobile apps.

Method: FuncDroid employs a Functional Flow Graph (FFG) to model functional units and their interactions, and uses a long-short-term-view-guided testing process with dual test-generation strategies to adaptively refine functional boundaries and explore inter-functional flows.

Result: FuncDroid achieves +28% higher coverage and detects 107% more bugs than state-of-the-art tools. It uncovers 18 new non-crash functional bugs in popular commercial apps and effectively reproduces 50 known crash bugs.

Conclusion: FuncDroid is a practical and effective inter-functional-flow-oriented GUI testing tool that advances Android app testing by explicitly modeling and testing inter-functional interactions, outperforming existing approaches in coverage and bug detection.
...
### [16] [A Microservice-Based Platform for Sustainable and Intelligent SLO Fulfilment and Service Management](https://arxiv.org/abs/2602.12875)
*Juan Luis Herrera,Daniel Wang,Schahram Dustdar*

Main category: cs.SE

TL;DR: CASCA is an open-source, microservices-based platform that enables computing continuum providers to dynamically reconfigure applications to meet performance and sustainability goals while preserving application developer privacy.


<details>
  <summary>Details</summary>
Motivation: Meeting performance and sustainability SLOs in microservices-based applications on the Computing Continuum is challenging, especially when balancing these objectives under privacy constraints between providers and developers.

Method: Design and implementation of an open-source MSA-based platform (CASCA) enabling runtime reconfiguration of services using decision systems while preserving application developer privacy.

Result: CASCA was successfully evaluated in a real Computing Continuum testbed with a media streaming service; decision systems in Bash, Rust, and Python effectively reconfigured the service without compromising privacy.

Conclusion: CASCA provides a privacy-preserving, reusable, and distributable platform for reconfiguring microservices in the Computing Continuum to meet both performance and sustainability SLOs.
...
### [17] [The Influence of Code Smells in Efferent Neighbors on Class Stability](https://arxiv.org/abs/2602.12950)
*Zushuai Zhang,Elliott Wen,Ewan Tempero*

Main category: cs.SE

TL;DR: This study shows that code smells in dependent classes, especially when interconnected, negatively impact a class's stability due to ripple effects, emphasizing the need to consider dependency-wide smell contexts in maintainability assessments.


<details>
  <summary>Details</summary>
Motivation: Code smells are known to harm maintainability, but prior studies focus only on smells within the modified class. This work investigates how smells in dependent classes (efferent neighbors) and their interrelations propagate instability through ripple effects, which has been largely underexplored.

Method: The study mines one year of commit history from 100 top-starred GitHub projects, detects code smells and static dependencies, identifies code smell interrelation and interaction, and uses these factors as predictors in a model to assess class stability.

Result: The analysis reveals that code smell interrelation and interaction in efferent neighbors have a measurable impact on class instability, suggesting that ripple effects exacerbated by interconnected smells contribute significantly to maintenance effort and risk.

Conclusion: The presence of code smells in a class's efferent neighbors, particularly when considering code smell interrelation and interaction, significantly affects the stability of the class, highlighting the importance of analyzing broader dependency contexts beyond the class itself for accurate assessment of maintainability.
...
### [18] [Analysis of Asset Administration Shell-based Negotiation Processes for Scaling Applications](https://arxiv.org/abs/2602.13029)
...
### [19] [Automated Testing of Task-based Chatbots: How Far Are We?](https://arxiv.org/abs/2602.13072)
*Diego Clerissi,Elena Masserini,Daniela Micucci,Leonardo Mariani*

Main category: cs.SE

TL;DR: This paper evaluates the effectiveness of state-of-the-art chatbot testing techniques on real-world task-based chatbots, revealing limitations such as simplistic test scenarios and weak oracles.


<details>
  <summary>Details</summary>
Motivation: The growing adoption of task-based chatbots necessitates effective quality assessment; however, existing testing methods, both traditional and chatbot-specific, suffer from limitations in coverage and verification capabilities.

Method: The authors conduct a confirmatory study by applying state-of-the-art chatbot testing techniques to a curated set of task-based chatbots from GitHub, built on popular commercial and open-source platforms, to assess their testing effectiveness.

Result: The evaluation reveals that current techniques still generate overly simple test scenarios and rely on weak oracles, limiting their ability to thoroughly test complex conversational behaviors.

Conclusion: Despite progress, existing chatbot testing approaches are insufficient for robust quality assurance, highlighting the need for more sophisticated test generation and stronger oracle mechanisms.
...
### [20] [Source Code Hotspots: A Diagnostic Method for Quality Issues](https://arxiv.org/abs/2602.13170)
*Saleha Muzammil,Mughees Ur Rehman,Zoe Kotti,Diomidis Spinellis*

Main category: cs.SE

TL;DR: The study identifies 15 recurring line-level hotspot patterns in 91 GitHub repositories, revealing that automated accounts contribute 74% of hotspot edits; the most common patterns relate to version management, code formatting, and layout issues.


<details>
  <summary>Details</summary>
Motivation: To understand the underlying causes of code hotspots—frequently modified code regions—that concentrate maintenance effort and potentially degrade software quality over time.

Method: Mining and analyzing the complete version histories of 91 actively developed GitHub repositories to detect line-level change patterns; clustering and classifying frequent changes into a taxonomy of hotspot patterns.

Result: Identification of 15 recurring hotspot patterns; Pinned Version Bump (26%), Long Line Change (17%), and Formatting Ping-Pong (9%) are most prevalent; automated accounts generate 74% of hotspot edits.

Conclusion: Hotspots are often caused by preventable development practices; the proposed taxonomy enables actionable refactoring and CI-based mitigations to improve software configurability, stability, and changeability.
