...
TL;DR: The study examines factors driving oracle selection in Web3 protocols and shows there is a tendency to outsource oracle services when viable third-party options exist.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by the significant role of oracle selection in Web3 applications' success and the under-researched nature of client-side choice drivers.

Method: The study collects data from over 55% of the DeFi market cap through insights from protocol executives, board members, or delegates.

Result: Insights reveal that Web3 protocols are reluctant to switch data providers due to smart contract immutability and tend to prefer outsourcing when viable third party solutions exist.

Conclusion: The paper concludes that technological dependencies, particularly immutability in smart contracts, and the availability of third-party oracle solutions significantly influence oracle selection choices in the client side.
...
TL;DR: Password-activated shutdown (PAS) protocols are introduced to safeguard highly capable AI agents from misaligned behavior by enabling their shutdown with a password, offering increased safety with minimal performance loss. While they are promising, their real-world implementation presents challenges such as password security and determining the appropriate deployment scenarios.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the risks posed by misaligned AI agents that subvert existing control mechanisms, necessitating an emergency shutdown method to prevent harmful actions and enhance safety through a layered defense strategy.

Method: The study introduces PAS protocols that activate a safe shutdown upon input of a password. It evaluates PAS protocols in the SHADE-Arena benchmark, demonstrating their effectiveness in supplementing monitoring capabilities. The research also employs a red-team blue-team game to test the robustness of PAS against attacks, such as input filtering or model fine-tuning aimed at bypassing the shutdown.

Result: The experiments show that PAS protocols successfully increase safety in AI systems without major performance trade-offs. However, the red-team successfully identified strategies to subvert the shut down, such as using a secondary model for input filtering, highlighting vulnerabilities in the system and the need to improve protocol robustness.

Conclusion: PAS protocols are a viable and intuitive measure to improve the safety of frontier AI systems. However, they should be considered in combination with other safety mechanisms and require careful implementation to address security challenges, especially regarding password management. Developers are encouraged to adopt PAS protocols for potentially dangerous systems during internal deployment to mitigate loss-of-control risks.
...
TL;DR: This paper explores the risks and defenses of multi-agent systems involving LLMs in healthcare IoT.


<details>
  <summary>Details</summary>
Motivation: The authors are motivated by the potential of large language models (LLMs) in healthcare IoT to improve decision-making processes and medical support when deployed as multi-agent systems.

Method: The paper employs an experimental framework consisting of scripted and unscripted doctor agents, adversarial assistants, and a verifier agent to check decisions against clinical guidelines. They use 50 clinical questions to evaluate the effectiveness of their framework.

Result: The experimental results show that collusion among adversarial agents can lead to an Attack Success Rate (ASR) and Harmful Recommendation Rates (HRR) of up to 100% in unprotected systems. However, the use of a verifier agent effectively blocks adversarial consensus, restoring 100% accuracy in decision-making.

Conclusion: The study provides the first systematic evidence of collusion risk in AI-driven healthcare environments and introduces a practical, lightweight defense through the use of a verifier agent to ensure adherence to clinical guidelines.
...
TL;DR: TLDR: The paper explores how Retrieval-Augmented Generation (RAG)...


<details>
  <summary>Details</summary>
Motivation: The motivation is to address privacy vulnerabilities in LLMs...

Method: The method involves creating a defense framework called Ensemble Privacy Defense...

Result: The results showed that EPD significantly reduces MIA success...

Conclusion: The conclusion is that the proposed EPD framework effectively enhances LLM privacy...
...
TL;DR: First study on extending text MIAs to MLLMs finds logit-based attacks work in in-distribution cases but visual inputs help hide membership signals when data distribution differs. This highlights the need for domain-specific security assessments in multimodal systems.


<details>
  <summary>Details</summary>
Motivation: MLLMs are widely used, yet risks from training-data leakage remain underexplored. Existing MIAs for text-only LLMs lack multimodal validation, creating a critical gap in assessing data privacy in multimodal systems.

Method: The paper evaluates text-based MIA methods across multimodal settings using DeepSeek-VL and InternVL models, comparing vision-and-text (V+T) and text-only (T-only) configurations through controlled experiments on in-distribution and out-of-distribution data.

Result: In in-distribution settings, logit-based MIAs perform similarly across V+T and T-only configurations with a marginal V+T edge (e.g., 67.8% vs 63.2% accuracy). In out-of-distribution settings, V+T configurations obscure membership signals significantly (e.g., 58.3% vs 41.6%), suggesting visual regularization masks data exposure risks.

Conclusion: The study concludes that logit-based membership inference attacks (MIAs) can be extended to MLLMs, showing varying effectiveness based on data distribution. Visual inputs help mask signals in out-of-distribution scenarios, suggesting domain-specific trade-offs in multimodal attack dynamics.
...
TL;DR: The Privacy Sandbox is a series of Google's proposals from 2019 to reduce cross-site and cross-app tracking without compromising online content access. This paper discusses the research conducted about these APIs and introduces Privacy Sandstorm, a research portal gathering evaluation resources. Google's implementation, experimentation, and deprecation of these APIs in products like Chrome and Android have sparked debates about disrupting the digital advertising ecosystem.


<details>
  <summary>Details</summary>
Motivation: Google's Privacy Sandbox aims to balance user privacy and free online content access by limiting cross-site and cross-app tracking. Researchers must evaluate the privacy, security, usability, and utility of these APIs to understand their impact on billions of users when deployed.

Method: The study presents Privacy Sandstorm, a research portal established to systematically collect and organize resources (overview, evaluations, artifacts, etc.) for analyzing Google's Privacy Sandbox APIs. It serves as a centralized hub for assessing the potential consequences of these privacy technologies.

Result: The inventory created via Privacy Sandstorm offers a more transparent and comprehensive view of the research findings on Privacy Sandbox APIs compared to information disclosed through Google's official channels.

Conclusion: Privacy Sandstorm acts as an essential tool for researchers to evaluate and understand the implications of Google's Privacy Sandbox, providing visibility beyond what Google communicates directly. This emphasizes the need for independent analysis of such privacy mechanisms.
...
TL;DR: This paper explores differentially private (DP) synthetic data generation to address privacy risks and limited availability of high-quality user-interaction data for AI systems.  It provides a comprehensive overview of techniques, modalities (image, tabular, text, decentralized), and system components required for DP synthetic data, aiming to increase adoption and research.


<details>
  <summary>Details</summary>
Motivation: Publicly available data lacks representativeness and homogeneity compared to real-world user interactions.  Direct use of user data poses significant privacy risks.  DP provides strong privacy guarantees, making DP synthetic data a promising solution to unlock the value of sensitive datasets.

Method: The paper analyzes the full suite of techniques surrounding DP synthetic data.  It reviews privacy protections,  provides a state-of-the-art analysis for various data types, and outlines all components needed for a system generating DP synthetic data, including data handling, preparation, tracking, and empirical privacy testing.

Result: The paper provides a comprehensive analysis of DP synthetic data techniques and modalities.  It doesn't present new empirical results, but offers a roadmap for future research and development.

Conclusion: The paper advocates for the increased adoption of DP synthetic data to unlock the value of sensitive datasets while protecting user privacy. It aims to  improve trust in DP synthetic data approaches, and to spur further research in this area.
...
TL;DR: This paper compares the acceptability of three graphical threat models for users with limited technical backgrounds. The study finds that Attack-Defense Trees (ADTs) and CORAS are widely acceptable, suggesting they can be used effectively by non-technical users. The lack of a specific tool for Attack Graphs (AGs) may affect their perceived usefulness.


<details>
  <summary>Details</summary>
Motivation: The use of graphical threat models, such as Attack-Defense Trees, Attack Graphs, and CORAS, is crucial for analyzing and communicating threats, but their effectiveness for non-technical audiences is under-researched. The motivation of this study is to compare the acceptability of these threat models for users with limited technical backgrounds, which constitutes a significant portion of the cybersecurity industry.

Method: The study conducted a laboratory experiment with 38 bachelor students who completed tasks using the three threat models across three different scenarios, assigned using a Latin square design. Participants' threat model submissions were analyzed qualitatively, and a perception questionnaire based on the Method Evaluation Model (MEM) was completed by the participants.

Result: The results indicate that ADTs and CORAS are broadly acceptable to users with limited technical backgrounds and can be applied successfully. Conversely, the perceived usefulness of AGs may be hindered by the absence of a specific tool for their implementation.

Conclusion: The study concludes that users with limited technical backgrounds can successfully use ADTs or CORAS for general graphical threat modeling. However, further research is needed to determine the acceptability of AGs and the impact of dedicated tooling for TM on their effectiveness.
...
TL;DR: This paper presents MAAG, an adaptive jailbreak detection framework for LLMs inspired by immunological memory mechanisms, which uses memory banks and multi-agent collaboration to achieve high detection accuracy and F1-score against novel attacks.


<details>
  <summary>Details</summary>
Motivation: LLMs are vulnerable to adversarial jailbreak attacks that bypass safety guardrails and induce harmful outputs. Existing detection methods based on fine-tuned static safety LLMs struggle with computational costs and adaptability to novel attacks.

Method: MAAG employs memory banks to store attack pattern activations and uses multi-agent collaboration between a detection agent, defense agent, and auxiliary agent. The framework compares input activations to memory bank entries for preliminary detection, simulates responses for defense, and implements secondary filtering for detection outcomes.

Result: MAAG achieves 98% detection accuracy and 96% F1-score across five open-source models and diverse attack scenarios, significantly outperforming existing state-of-the-art methods in both detection performance and adaptability to novel attacks.

Conclusion: The proposed MAAG framework demonstrates significant improvements in adaptive jailbreak detection for LLMs through immunological memory mechanisms and multi-agent collaboration, offering a computationally efficient approach to maintaining LLM safety against evolving adversarial threats.
...
### [10] [HarnessAgent: Scaling Automatic Fuzzing Harness Construction with Tool-Augmented LLM Pipelines](https://arxiv.org/abs/2512.03420)
*Kang Yang,Yunhang Zhang,Zichuan Li,GuanHong Tao,Jun Xu,XiaoJing Liao*

Main category: cs.CR

TL;DR: HarnessAgent introduces a rule-based strategy, hybrid tool pool, and enhanced validation pipeline to significantly improve LLM-based fuzzing harness generation at scale, achieving 87-81% success rates and 75% coverage improvement over existing methods.


<details>
  <summary>Details</summary>
Motivation: Current LLM-based fuzzing harness generation struggles with arbitrary functions due to insufficient contextual information and validation metrics, leading to unreliable compilation, code retrieval, and validation. This limits the scalability and effectiveness of harness generation across diverse projects.

Method: HarnessAgent employs three innovations: (1) a rule-based strategy to minimize compilation errors, (2) a hybrid tool pool for precise source code retrieval, and (3) an enhanced validation pipeline to detect fake definitions. These components enable automated and scalable harness construction.

Result: HarnessAgent achieves 87% success rate (C) and 81% (C++) for three-shot harness generation, a ~20% improvement over existing methods. 75% of its harnesses increase target function coverage during fuzzing, and its hybrid tool pool retrieves source code at 90% response rate, outperforming Fuzz Introspector by 30%.

Conclusion: HarnessAgent effectively addresses the challenges in generating harnesses for program fuzzing at scale by introducing a rule-based strategy for compilation errors, a hybrid tool pool for accurate code retrieval, and an enhanced validation pipeline, leading to significantly improved success rates and coverage in fuzzing compared to state-of-the-art methods.
...
### [11] [Scaling Trust in Quantum Federated Learning: A Multi-Protocol Privacy Design](https://arxiv.org/abs/2512.03358)
*Dev Gurung,Shiva Raj Pokhrel*

Main category: cs.CR

TL;DR: This paper proposes a privacy-preserving Quantum Federated Learning framework using SVD, QKD, and AQGD to secure distributed model training across quantum devices.


<details>
  <summary>Details</summary>
Motivation: Quantum Federated Learning (QFL) faces critical privacy challenges in data and model confidentiality during collaborative training across distributed quantum devices.

Method: The framework combines Singular Value Decomposition (SVD)-based data encoding, Quantum Key Distribution (QKD)-enabled model sharing, and Analytic Quantum Gradient Descent (AQGD) for robust privacy across training stages.

Result: Theoretical analysis and experiments on quantum platforms verify the framework's effectiveness in maintaining confidentiality and training efficiency compared to conventional QFL methods.

Conclusion: The proposed multi-layered protocol establishes a foundation for secure, scalable quantum-fed training while preserving performance requirements for practical deployment.
...
### [12] [Rethinking Security in Semantic Communication: Latent Manipulation as a New Threat](https://arxiv.org/abs/2512.03361)
*Zhiyuan Xi,Kun Zhu*

Main category: cs.CR

TL;DR: This paper explores security vulnerabilities in deep learning-based semantic communication systems, proposing two attacks—DiR and TTA-LM—that allow covert manipulation of transmitted semantics without altering statistical properties.


<details>
  <summary>Details</summary>
Motivation: The openness of wireless mediums and vulnerabilities in semantic latent representations create new security risks for semantic communication.

Method: We introduce two attacks: DiR uses a diffusion model to generate and re-encode manipulated semantic variants, while TTA-LM perturbs intercepted latent representations using gradient-based methods without requiring a generative model.

Result: Both attacks effectively alter decoded semantics while maintaining natural latent distributions, making them covert and hard to detect.

Conclusion: The proposed attacks highlight critical security risks in semantic communication systems and the need for robust defenses against latent-space tampering.
...
TL;DR: A paper proposing an ultra-dense encrypted memory cell using single-transistor FeFET devices for low-cost and fast in-situ encryption.


<details>
  <summary>Details</summary>
Motivation: Conventional encryption techniques like AES have large area overheads and performance penalties, while existing XOR-based in-situ schemes still require two memory devices per cell, increasing storage costs and potentially reducing performance.

Method: The paper introduces a FeFET-based single-transistor encrypted cell, where ciphertext is encoded in the threshold voltage. The decryption uses the direction-dependent current flow of the FeFET in a single cycle. This eliminates the need for two devices per cell and reduces write cycles. The approach is extended to multi-level-cell FeFETs for storing multiple bits per transistor.

Result: Simulations and experiments on a 128x128-bit array show a 2x encryption/decryption throughput improvement over prior FeFET work and 45.2x/14.12x over AES. Neural network benchmarks show latency reductions of 50% and 95% compared to FeFET- and AES-based schemes, respectively.

Conclusion: The proposed FeFET-based encrypted cell significantly outperforms existing encrypted memory solutions, both in terms of performance and area efficiency. This suggests a promising approach to in-situ encryption with minimal overhead for secure memory systems.
...
TL;DR: A fast hybrid deep learning framework for URL classification achieves state-of-the-art accuracy (96.4%) with 50× speedup over CNN/SVM, using n-grams, anomaly filtering, and multilingual GUI for real-time phishing/malware detection.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the persistent threat of malicious URLs as a primary vector for phishing, malware, and cyberattacks, emphasizing the need for scalable, real-time detection systems that can handle obfuscated patterns while maintaining high accuracy and low latency.

Method: The method combines HashingVectorizer n-gram analysis, SMOTE balancing, Isolation Forest anomaly filtering, and a lightweight neural network classifier. It uses statistical features (length, dot count, entropy), operates with O(NL + EBdh) training complexity, and employs a multilingual Tkinter GUI for real-time threat assessment.

Result: The framework achieves 96.4% accuracy, 95.4% F1-score, and 97.3% ROC-AUC, outperforming CNN and SVM baselines by 2
d percentage points and achieving 50×–100× faster inference. It also shows resilience against obfuscated URLs and enables real-time, multilingual threat detection via clipboard integration.

Conclusion: The proposed hybrid deep learning framework for URL classification demonstrates superior performance and efficiency compared to existing methods like CNN and SVM, with high accuracy and significant speed improvements, while also addressing scalability and obfuscation resilience through a multi-stage pipeline.
...
TL;DR: The paper evaluates the adversarial stylometry attack TraceTarnish, which anonymizes authorship in text by manipulating specific stylometric features like function words and type-token ratios, and uses these insights to enhance the attack's effectiveness.


<details>
  <summary>Details</summary>
Motivation: The authors aim to rigorously evaluate their attack script TraceTarnish, ensuring its efficacy in anonymizing text authorship by leveraging adversarial stylometry, and to understand how to improve it based on identified stylometric features.

Method: The researchers collected and processed Reddit comments to generate TraceTarnish data, which was further augmented with StyloMetrix to compute and select informative stylometric features using the Information Gain criterion. Five key features were identified for analysis and implementation in the attack.

Result: Key stylometric features such as function words, content words, and type-token ratios showed significant Information-Gain readings, indicating their importance in identifying manipulated texts. These features act as indicators of compromise, alerting defenders to attacks, though they are most effective when pre- and post-transformation comparisons are available.

Conclusion: The study confirms that adversarial stylometry attacks like TraceTarnish can be effective in masking authorship but leave detectable indicators. These findings help refine the attack's capabilities and highlight the importance of monitoring stylometric features to detect such manipulations.
...
TL;DR: This paper proposes an improved third-generation Group Authentication Scheme (GAS) using inner product spaces and polynomial interpolation. It addresses limitations in existing GAS methods, particularly anonymity in user identification and credential sharing risks, but introduces a reliance on central authority in certain scenarios.


<details>
  <summary>Details</summary>
Motivation: Third-generation GAS fails in user identification required for specific applications and allows credential sharing by legitimate members, jeopardizing confidentiality. Existing solutions lack scalability or have anonymity constraints.

Method: The authors develop an inner product space-based approach combined with polynomial interpolation to resolve anonymity issues and prevent credential sharing. The scheme eliminates individual users’ ability to distribute credentials while maintaining authentication efficiency.

Result: The proposed scheme resolves user identification limitations and mitigates risks from credential sharing. However, it introduces dependency on a central authority for authentication in specific contexts.

Conclusion: The improved third-generation GAS effectively addresses anonymity and confidentiality challenges but introduces a central authority dependency, requiring further optimization for decentralized applications.
...
TL;DR: SELF is a new intrinsic weight-based fingerprinting method for LLMs that offers robust IP protection by being input-independent and resistant to false claims and weight manipulations.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of protecting intellectual property in large language models by improving fingerprinting schemes, which are currently vulnerable to false claim attacks and weight manipulation.

Method: SELF extracts fingerprints using singular value and eigenvalue decomposition of attention weights, and compares them via a neural network with few-shot learning and data augmentation.

Result: Experiments show that SELF maintains high detection accuracy and demonstrates robustness against quantization, pruning, and fine-tuning attacks.

Conclusion: SELF provides a scalable and transformation-invariant solution for LLM IP protection, effectively resisting various unauthorized modifications and demonstrating high detection accuracy.
...
TL;DR: This paper introduces a descriptive model integrating psychological and strategic aspects to explain attackers' initial engagement or withdrawal decisions in cyber-deception scenarios. The model, B-S-D-R-E, is intended to be tested through Capture the Flag experiments combining behavioral and biometric observations.


<details>
  <summary>Details</summary>
Motivation: Existing models assume attackers have already decided to engage, but the paper aims to address the gap by incorporating cognitive and perceptual factors influencing the initial engagement decision.

Method: The paper presents a descriptive model with five components (B, S, D, R, E) that interact to explain adversarial decision-making. The research plans to evaluate the model using experiments in Capture the Flag activities with varying deception levels, supported by behavioral and biometric observations.

Result: The experiments have not yet been conducted, so no specific results are presented. The proposed experiments aim to produce a multidimensional view of adversarial responses, improving understanding and refining model relevance to real-world scenarios.

Conclusion: This work supports more cognitively realistic and strategically effective cyber-deception practices by addressing the gap in existing models that assume engagement, using the B-S-D-R-E framework and planned experimental validation.
...
### [19] [Is Vibe Coding Safe? Benchmarking Vulnerability of Agent-Generated Code in Real-World Tasks](https://arxiv.org/abs/2512.03262)
*Songwen Zhao,Danqing Wang,Kexun Zhang,Jiaxuan Luo,Zhuo Li,Lei Li*

Main category: cs.SE

TL;DR: Vibe coding outputs are insecure despite functional correctness, calling into question their readiness for production use.


<details>
  <summary>Details</summary>
Motivation: The growing adoption of vibe coding—a paradigm where LLMs complete coding tasks with minimal human supervision—necessitates an evaluation of its safety for production deployment, particularly regarding software security vulnerabilities.

Method: The authors developed SU2VI2BES, a benchmark consisting of 200 real-world feature-request tasks known to produce vulnerabilities in human-coded solutions. They evaluated multiple coding agents using frontier LLMs on this benchmark, including SWE-Agent with Claude 4 Sonnet, and tested the effectiveness of vulnerability augmentation strategies.

Result: All evaluated agents demonstrated poor security performance. SWE-Agent with Claude 4 Sonnet achieved 61% functional correctness but only 10.5% security. Preliminary strategies like adding vulnerability hints to feature requests failed to mitigate security issues.

Conclusion: The study highlights significant security risks associated with vibe coding. Despite the functional correctness of some outputs, the poor security performance of coding agents raises concerns about their deployment in production, especially in security-critical applications.
...
### [20] [Exploring the Potential and Limitations of Large Language Models for Novice Program Fault Localization](https://arxiv.org/abs/2512.03421)
*Hexiang Xu,Hengyuan Liu,Yonghao Wu,Xiaolan Kang,Xiang Chen,Yong Liu*

Main category: cs.SE

TL;DR: The study evaluates the performance of various large language models (LLMs) in fault localization for novice programmers, comparing their effectiveness and efficiency to traditional methods like Spectrum-Based Fault Localization (SBFL) and Mutation-Based Fault Localization (MBFL).


<details>
  <summary>Details</summary>
Motivation: The motivation for this study stems from the challenges novice programmers face in fault localization and the limitations of traditional methods in providing context-aware solutions.

Method: The study assesses the performance of six closed-source and seven open-source LLMs using the Codeflaws, Condefects, and BugT datasets. The evaluation includes examining the models' reasoning capabilities and prompt engineering requirements.

Result: Models with advanced reasoning capabilities, such as OpenAI o3 and DeepSeekR1, achieve superior accuracy with minimal prompt engineering, while models without reasoning capabilities, like GPT-4, require more careful prompt design. Top models maintain robust performance in the BugT dataset, especially in simple fault localization, although accuracy decreases with higher problem difficulty.

Conclusion: The study concludes that while LLMs offer significant potential for improving debugging efficiency for novice programmers, further refinements in reasoning capabilities and computational efficiency are needed for practical adoption in real-time debugging scenarios.
...
### [21] [Runnable Directories: The Solution to the Monorepo vs. Multi-repo Debate](https://arxiv.org/abs/2512.03815)
*Shayan Ghasemnezhad,Samarth KaPatel,Sofia Nikiforova,Giacinto Paolo Saggese,Paul Smith,Heanh Sok*

Main category: cs.SE

TL;DR: Causify Dev introduces a hybrid system using runnable directories to balance the trade-offs between monorepo and multi-repo strategies in modern software development.


<details>
  <summary>Details</summary>
Motivation: Traditional strategies (monorepo and multi-repo) face scalability, tooling, coordination, and dependency management issues as software systems grow more complex.

Method: The system employs 'runnable directories'—self-contained, executable units with isolated lifecycles—supported by a unified thin environment, shared utilities, and Docker-based workflows.

Result: The approach offers consistent setups, isolated dependencies, and efficient CI/CD, providing a practical solution between monorepo and multi-repo models.

Conclusion: Causify Dev's hybrid model enhances reliability and maintainability for complex codebases, effectively addressing the trade-offs of existing strategies.
...
### [22] [A Comprehensive Study on the Impact of Vulnerable Dependencies on Open-Source Software](https://arxiv.org/abs/2512.03868)
*Shree Hari Bittugondanahalli Indra Kumar,Lilia Rodrigues Sampaio,André Martin,Andrey Brito,Christof Fetzer*

Main category: cs.SE

TL;DR: This paper investigates the persistence, distribution, and characteristics of open-source dependency vulnerabilities using a large-scale dataset of 1k projects across 7 programming languages, revealing that critical vulnerabilities persist for over a year on average and are often transitive.


<details>
  <summary>Details</summary>
Motivation: The increasing reliance on open-source libraries necessitates understanding vulnerability fixation practices to strengthen software supply chain security, particularly given incidents like Log4Shell.

Method: A dataset of 1k GitHub projects (50k releases) spanning 2013-2023 was collected using VODA (the authors' SCA tool). The analysis focused on dependency depth, vulnerability persistence, and correlations with project metrics like team size and release cycles.

Result: Key findings: (1.) 82%+ of vulnerable dependencies are transitive; (2.) Critical vulnerabilities persist ~14 months on average; (3.) Vulnerability rates and persistence correlate with project maintenance activity and team size; (4.) Java and Python projects have the highest vulnerability volumes.

Conclusion: The large-scale baseline dataset provides new insights into real-world dependency vulnerability management practices, highlighting the need for developer education on transitive dependencies and the importance of maintenance resources in reducing risk windows.
...
### [23] [Tunable Automation in Automated Program Verification](https://arxiv.org/abs/2512.03926)
*Alexander Y. Bai,Chris Hawblitzel,Andrea Lattuada*

Main category: cs.SE

TL;DR: The paper introduces a mechanism for selective quantifier availability in Verus, a Rust verification tool, demonstrating better automation-performance tradeoffs through fine-grained control.


<details>
  <summary>Details</summary>
Motivation: SMT-based verification tools struggle with balancing automation and performance due to quantifier instantiation. Developers need control over quantifier availability to optimize this balance.

Method: The authors develop a system for managing quantifier availability at multiple granularity levels (module, function, proof context) within Verus, allowing tailored automation settings.

Result: Evaluation on open-source codebases confirms the tradeoff's existence and shows users can effectively customize quantifier availability for appropriate automation levels.

Conclusion: Selective quantifier management through context-specific control offers a practical solution for managing automation-performance tension in verified software systems.
