...
TL;DR: Evaluation format, not scaffold architecture, is the key driver of safety measurement differences; map-reduce slightly reduces safety, but effects vary widely by model; current benchmarks lack reliability, necessitating per-model, per-configuration safety testing.


<details>
  <summary>Details</summary>
Motivation: Safety benchmarks typically evaluate models in isolation using multiple-choice formats, while real-world deployments use agentic scaffolds (e.g., reasoning traces, critic agents). The motivation is to measure how these scaffolds affect safety evaluations and to determine whether standard benchmarks generalize to deployed settings.

Method: The authors conducted a large-scale controlled study (N = 62,808) across six frontier models and four deployment configurations, using pre-registration, assessor blinding, equivalence testing (TOST with ±2 pp margin), and specification curve analysis. They compared safety outcomes across scaffold architectures (e.g., map-reduce) and evaluation formats, and performed generalizability (G) analysis to assess reliability of safety rankings.

Result: Map-reduce scaffolding degrades measured safety (NNH = 14), but two of three scaffolds preserve safety within ±2 pp. Switching to open-ended format shifts safety scores by 5–20 pp—larger than any scaffold effect. Model × scaffold interactions vary widely (up to 35 pp in opposing directions), e.g., one model drops by -16.8 pp on sycophancy under map-reduce while another improves by +18.8 pp. Generalizability analysis yields G = 0.000, indicating safety rankings reverse across benchmarks.

Conclusion: The study concludes that evaluation format (e.g., multiple-choice vs. open-ended) has a larger impact on measured safety than scaffold architecture, and that model × scaffold interactions are highly variable, precluding universal safety claims. Therefore, per-model, per-configuration safety testing is necessary, as no reliable composite safety index can be formed.
...
TL;DR: This paper evaluates five prompt engineering techniques to reduce LLM hallucinations in industrial applications. Enhanced Data Registry (M4) consistently improved output quality across 100 trials, while revised Decomposed Prompting (M2 v2) showed the largest performance gain, rising from 34% to 80% effectiveness.


<details>
  <summary>Details</summary>
Motivation: Hallucinations in LLMs produce syntactically plausible but factually or contextually incorrect outputs, posing critical risks in high-stakes industrial applications. There is a need for lightweight, model-agnostic methods to improve output consistency without retraining or complex validation systems.

Method: Five prompt engineering strategies were evaluated—Iterative Similarity Convergence (M1), Decomposed Model-Agnostic Prompting (M2), Single-Task Agent Specialization (M3), Enhanced Data Registry (M4), and Domain Glossary Injection (M5)—using an LLM-as-Judge framework over 100 repeated runs per method under stochastic decoding (τ = 0.7). A follow-up evaluation with v2 implementations was conducted over 10 trials.

Result: M4 (Enhanced Data Registry) outperformed the baseline in all 100 trials. M3 and M5 achieved 80% and 77% 'Better' verdicts, respectively; M1 reached 75%; M2 initially scored 34% but improved to 80% in its v2 form—the largest gain among revised methods.

Conclusion: The study concludes that prompt engineering strategies, particularly Enhanced Data Registry (M4), can significantly reduce hallucinations and output variance in LLMs without modifying model weights, enabling more reliable application in industrial settings. Revised versions of other methods, especially Decomposed Model-Agnostic Prompting (M2 v2), show substantial improvement, suggesting that iterative refinement of prompt design can yield robust results.
...
TL;DR: DUCTILE is an LLM-based agentic orchestration framework that enables adaptive automation of engineering analysis by decoupling flexible decision-making (handled by an LLM agent) from deterministic execution (performed by verified tools), demonstrated successfully in aerospace structural analysis with robustness to input variations and compliance with engineering standards.


<details>
  <summary>Details</summary>
Motivation: Traditional engineering automation pipelines are brittle and break when interfaces, data formats, or processes change. There is a need for more adaptive systems that can handle real-world variability in engineering workflows without sacrificing reliability or compliance.

Method: The DUCTILE approach uses a user-supervised LLM agent to interpret design practices, inspect inputs, and dynamically coordinate tools and documents, while keeping execution in trusted engineering software. The agent adapts workflows in response to deviations in format, units, naming, or methods, under the oversight of human engineers who retain final judgment.

Result: Demonstrated on an industrial aerospace structural analysis task, DUCTILE successfully handled input deviations that would break scripted pipelines. Evaluation showed consistent generation of correct, methodologically compliant results across multiple runs, validated against expert-defined criteria and tested with practicing engineers.

Conclusion: Agentic automation via DUCTILE enables robust, adaptable engineering analysis workflows. However, it introduces new practical challenges, including shifts in engineering roles and the cognitive load of supervision, highlighting tensions between automation benefits and evolving work demands.
...
TL;DR: MALTA improves risk assessment in open-source dependencies by identifying abandoned packages missed by traditional Version Lag metrics, using maintenance activity signals to reclassify 62.2% of previously low-risk packages as high risk.


<details>
  <summary>Details</summary>
Motivation: Existing Technical Lag metrics like Version Lag cannot reliably differentiate between actively maintained and abandoned packages, leading to underestimation of security and sustainability risks in open-source ecosystems. This limitation necessitates a more nuanced, maintenance-aware approach to assess dependency risk.

Method: The authors introduce MALTA, a maintenance-aware framework comprising three metrics: Development Activity Score (DAS), Maintainer Responsiveness Score (MRS), and Repository Metadata Viability Score (RMVS). It is evaluated on a dataset of 11,047 Debian packages linked to GitHub repositories, including 1.7M commits and 4.2M pull requests, to assess its ability to classify maintenance states and reevaluate risk compared to Version Lag.

Result: MALTA achieves an AUC of 0.783 in classifying active versus declining maintenance. Critically, 62.2% of packages deemed 'Low Risk' by Version Lag are reclassified as 'High Risk' under MALTA, with these packages averaging 2019 days since last commit and 9.8% being in archived repositories.

Conclusion: Version Lag metrics systematically fail to detect abandoned packages, leading to an underestimation of risk in open-source ecosystems. MALTA addresses this blind spot by distinguishing between resolvable technical lag and terminal lag caused by upstream abandonment, thereby revealing a significant population of high-risk dependencies previously misclassified as low risk.
...
TL;DR: SpecOps is a fully automated, LLM-powered testing framework for real-world GUI-based AI agents, using specialized agents across four testing phases to achieve high accuracy, robustness, and efficiency in detecting bugs at low cost.


<details>
  <summary>Details</summary>
Motivation: Existing AI agent evaluation frameworks are limited by manual effort, simulation-only environments, or insufficient support for multimodal, real-world agents, necessitating a robust, automated solution.

Method: SpecOps employs a four-phase, LLM-based specialist agent architecture—test case generation, environment setup, test execution, and validation—to enable fully automated testing of GUI-based AI agents in real-world environments.

Result: SpecOps achieves an F1 score of 0.89 in bug detection, identifies 164 true bugs across five real-world agents, and operates at under 0.73 USD and eight minutes per test, outperforming AutoGPT and LLM-generated scripts in planning accuracy, execution success, and reliability.

Conclusion: SpecOps demonstrates superior performance in automated, real-world AI agent testing, offering high practical viability due to its low cost, short runtime, and effective bug detection across diverse platforms.
...
TL;DR: The paper proposes a shift from traditional software verification to a 'herding' approach, using the EZR algorithm to efficiently identify key controlling variables in software systems via model-free search, achieving high performance with minimal samples.


<details>
  <summary>Details</summary>
Motivation: Traditional software verification is costly and ineffective for modern complex systems; the authors aim to reduce verification effort by replacing heavy modeling and solvers with lightweight, model-free testing.

Method: The method leverages the 'Sparsity of Influence' principle, using EZR—a stochastic, zero-knowledge ranker—to perform model-free search and identify a small set of critical variables that control system behavior, steering software toward goals through lightweight sampling.

Result: EZR achieved 90% of peak performance across dozens of tasks using only 32 samples, demonstrating significant efficiency over traditional solvers.

Conclusion: Model-free herding with EZR offers a scalable and efficient alternative to conventional verification, exploiting sparsity to replace costly modeling with effective, lightweight testing.
...
TL;DR: This paper proposes a Graph Convolutional Network (GCN) model that uses Code Property Graphs (CPGs) to classify SAST alerts as true or false positives, achieving high accuracy on CamBenchCAP and CryptoAPI-Bench datasets.


<details>
  <summary>Details</summary>
Motivation: SAST tools suffer from high false positive rates, which reduce developer productivity and trust; this work aims to improve the precision of vulnerability reports using machine learning on rich code representations.

Method: The approach uses a GCN model trained on Code Property Graphs (CPGs) derived from static analysis outputs to capture structural and semantic code features; evaluation is performed on CamBenchCAP (80/20 split) and CryptoAPI-Bench datasets.

Result: The model achieves 100% accuracy on the CamBenchCAP test set and up to 96.6% accuracy on CryptoAPI-Bench; qualitative analysis shows many 'misclassifications' are actually valid security concerns, reflecting security-conservative reasoning.

Conclusion: GCNs on CPGs are highly effective for SAST result classification; future work includes enhancing control-flow modeling with call graphs, improving explainability, and expanding training data across multiple SAST tools for better generalization.
...
TL;DR: The QuantumX track at JISBD 2025 marked a milestone in advancing Quantum Software Engineering in Spain by integrating key software engineering principles with quantum computing, synthesizing research contributions, and fostering a collaborative national and international research community.


<details>
  <summary>Details</summary>
Motivation: To explore how software engineering principles can be adapted to the quantum computing paradigm and to foster collaboration among Spanish and Ibero-American research groups in Quantum Software Engineering.

Method: The study summarizes and synthesizes the contributions and discussions from the QuantumX track held at JISBD 2025, identifying common themes, challenges, and future directions through presentation analysis and community engagement.

Result: The track highlighted advancements in areas such as quantum service engineering, hybrid architectures, quality models, circuit optimization, and quantum machine learning, while promoting community building via national and regional networks and networking initiatives.

Conclusion: The first QuantumX track successfully established a foundational platform for a sustained research community in Quantum Software Engineering, positioning Spain as an emerging contributor to the European and global quantum software ecosystem.
...
TL;DR: This paper proposes an agentic ESG lifecycle framework that uses AI agents to automate and enhance ESG reporting, making it dynamic, adaptive, and accountable by integrating continuous feedback and improving data consistency and governance.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the challenges in current ESG reporting practices, including unstructured data, inconsistent terminology, and lack of automation and feedback mechanisms, which hinder effective sustainability governance.

Method: The authors introduce an agentic ESG lifecycle framework comprising five stages: identification, measurement, reporting, engagement, and improvement. The framework employs multiple AI agents to automate ESG information extraction, performance verification, and report updating. They define technical requirements and quality attributes for key ESG tasks and propose three architectural approaches—single-model, single-agent, and multi-agent—which are implemented in a publicly available prototype.

Result: The framework enables automation, adaptability, and continuous feedback in ESG reporting. It supports critical ESG tasks such as report validation, multi-report comparison, report generation, and knowledge-base maintenance through AI-driven agents. A prototype implementation is provided with public access to source code and data.

Conclusion: The proposed agentic ESG lifecycle framework transforms traditional ESG reporting into a dynamic and adaptive system by integrating AI agents, enabling continuous improvement and accountability in sustainability governance.
...
TL;DR: This paper proposes an AI-integrated agile education platform as a collaborative research environment to bridge the gap between academic research and fast-evolving agile practices, especially in the context of generative AI, enabling timely, reusable, and practice-relevant evidence through structured iterations and stakeholder engagement.


<details>
  <summary>Details</summary>
Motivation: Agile software development evolves faster than academic research can respond, resulting in delayed, non-transferable findings; the emergence of generative AI intensifies this gap, demanding more timely and context-rich research methods.

Method: The authors design and implement a project-based, AI-integrated agile education platform that functions as a living lab, combining sprint rhythms, quality gates, and real stakeholder involvement; they define a framework with iteration structures and recurring events to support AI-assisted engineering and evidence generation.

Result: Preliminary results over several semesters indicate the platform supports growing project pipelines, increasing cohort participation, and sustained stakeholder engagement, demonstrating its ability to produce timely, context-rich, and reusable insights into agile and AI-augmented software development.

Conclusion: The platform effectively bridges research and practice by enabling rapid, contextualized inquiry in agile software development; future work will focus on strengthening governance and systematic evidence capture to enhance validity and reuse.
...
TL;DR: This study explores how individual and contextual factors influence the perception of sentiment in text-based communication among software developers, finding that sentiment perception is moderately stable within individuals but highly dependent on statement ambiguity and personal mood traits.


<details>
  <summary>Details</summary>
Motivation: Understanding how developers perceive sentiment in communication is critical for team collaboration and project success, as emotional contagion can affect team dynamics. However, sentiment perception may vary due to individual differences and contextual factors, which complicates the interpretation of sentiment analysis in software engineering.

Method: A four-round longitudinal survey study was conducted with 81 students participating in team-based software projects. Participants reported on mood traits, life circumstances, project phases, and group dynamics, and labeled 30 decontextualized statements for sentiment, providing metadata on their labeling rationale and uncertainty. Repeated-measures generalized estimating equations (GEE) were used to analyze within-person changes in sentiment labeling.

Result: (1) Sentiment perception showed only moderate intra-individual stability, with label changes primarily occurring on ambiguous statements; (2) Correlations between personal/contextual factors and sentiment labeling were small and nonsignificant after multiple testing correction; (3) Higher mood trait and emotional reactivity were associated with more positive (and less neutral) sentiment labels, while negative labeling had weaker predictors (e.g., task conflict showed only trend-level effects); (4) No systematic effects of project phase were found. Sentiment perception was highly statement-dependent.

Conclusion: Sentiment perception in software development communication varies significantly within individuals and is strongly influenced by the ambiguity of the statement and personal affective traits. This variability suggests caution in applying sentiment analysis tools to decontextualized messages and highlights the need for future research using contextualized, in-project communication data.
...
TL;DR: STADA is a specification-based test generation framework for autonomous driving agents that uses temporal logic (LTLf) to systematically generate diverse and behaviorally relevant test scenarios, significantly improving coverage and efficiency compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing test generation methods for autonomous driving agents rely on templates, manual design, or randomness, which either demand high human effort or risk missing critical behaviors related to safety requirements; STADA aims to address this by enabling systematic, specification-driven test generation.

Method: STADA systematically generates test scenarios based on formal specifications expressed in linear temporal logic (LTLf), constructing distinct initial scenes, diverse continuations, and corresponding simulations that reflect the specified behaviors.

Result: STADA achieves over 2x higher coverage than the best baseline on the finest coverage criterion and a 75% improvement on the coarsest, while matching baseline coverage with 6 times fewer simulations.

Conclusion: STADA provides a more efficient and effective approach to test generation for autonomous driving agents by leveraging formal specifications in temporal logic, achieving higher coverage with fewer simulations and demonstrating applicability beyond autonomous driving.
...
TL;DR: This paper investigates how AI enhances Agile software development by automating tasks like requirement management, code generation, and testing through ML and NLP, based on a literature review and survey of software professionals.


<details>
  <summary>Details</summary>
Motivation: To address ongoing challenges in Agile software development, such as managing dynamic requirements and maintaining quality under tight schedules, by exploring how AI can augment Agile practices.

Method: Combined literature review and empirical study using a survey of software engineering professionals to evaluate the perception, adoption, and impact of AI-driven tools in Agile environments.

Result: AI integration, particularly via ML and NLP, automates repetitive tasks, improves development agility, and supports innovation while maintaining product quality.

Conclusion: AI acts as a catalyst in Agile software development, not only optimizing current practices but also enabling new capabilities crucial for future software innovation, quality, and speed.
...
### [16] [Evaluating Generalization Mechanisms in Autonomous Cyber Attack Agents](https://arxiv.org/abs/2603.10041)
...
TL;DR: Study evaluates how unseen IP reassignments affect autonomous attacker generalization in NetSecGame; LLM-based agents perform best but with higher cost and failure risks.


<details>
  <summary>Details</summary>
Motivation: Autonomous offensive agents often fail to generalize beyond their training environments; this work investigates the impact of a minimal, fundamental shift—unseen host/subnet IP reassignment—on policy transfer, aiming to understand failure modes in long-horizon attack planning under small environmental changes.

Method: The study evaluates attacker generalization in the NetSecGame environment by training agents on five IP-range variants and testing on a sixth unseen variant; it compares three agent families—traditional RL, adaptation agents, and LLM-based agents—using action-distribution-based behavioral analysis and XAI techniques, with only the meta-learning agent allowed adaptation at test time.

Result: While some adaptation methods show partial success, all agents exhibit significant performance degradation under unseen IP reassignment; LLM-based agents achieve the highest success rate on the held-out variant but require more compute, lack transparency, and exhibit failure patterns such as repetition and invalid action loops.

Conclusion: Prompt-driven pretrained LLM agents show superior transfer performance under unseen IP reassignments in NetSecGame, but suffer from higher computational costs, reduced transparency, and practical failure modes like repetition and invalid actions; overall, generalization remains challenging for autonomous offensive agents even under minimal environmental shifts.
...
### [17] [Targeted Bit-Flip Attacks on LLM-Based Agents](https://arxiv.org/abs/2603.10042)
*Jialai Wang,Ya Wen,Zhongmou Liu,Yuxiao Wu,Bingyi He,Zongpeng Li,Ee-Chien Chang*

Main category: cs.CR

TL;DR: Flip-Agent is the first targeted bit-flip attack framework for LLM-based agents, demonstrating high effectiveness in manipulating both outputs and tool invocations in multi-stage agent pipelines.


<details>
  <summary>Details</summary>
Motivation: LLM-based agents introduce new attack surfaces due to their multi-stage reasoning and use of external tools, yet these are unexplored in the context of bit-flip attacks; existing attacks focus only on single-step models.

Method: Proposes Flip-Agent, a targeted bit-flip attack framework designed to exploit hardware faults to alter model parameters in LLM-based agents, specifically targeting both final outputs and intermediate tool invocations.

Result: Flip-Agent significantly outperforms prior bit-flip attacks on real-world agent tasks, successfully manipulating agent behavior through minimal parameter corruption.

Conclusion: LLM-based agent systems are critically vulnerable to targeted bit-flip attacks, highlighting the need for robust defenses in agent architectures and deployment environments.
...
### [18] [OAuthHub: Mitigating OAuth Data Overaccess through a Local Data Hub](https://arxiv.org/abs/2603.10056)
...
### [19] [SBOMs into Agentic AIBOMs: Schema Extensions, Agentic Orchestration, and Reproducibility Evaluation](https://arxiv.org/abs/2603.10057)
*Petar Radanliev,Carsten Maple,Omar Santos,Kayvan Atefi*

Main category: cs.CR

TL;DR: This paper introduces agentic AI Bills of Materials (AIBOMs) to overcome the limitations of static SBOMs by incorporating autonomous agents that capture runtime behavior, environment drift, and exploitability context, enabling more accurate and reproducible software supply-chain security assessments.


<details>
  <summary>Details</summary>
Motivation: Conventional SBOMs fail to capture runtime behavior, environment drift, and exploitability context, limiting their effectiveness in dynamic software supply-chain security. There is a need for provenance mechanisms that support reproducibility and vulnerability assessment under real-world execution conditions.

Method: The paper proposes an agentic AIBOM framework using a multi-agent architecture: a baseline environment reconstruction agent (MCP), a runtime dependency and drift-monitoring agent (A2A), and a policy-aware vulnerability and VEX reasoning agent (AGNTCY). The agents operate under policy constraints and use CSAF v2.0 semantics to generate structured VEX assertions. The framework integrates with CycloneDX and SPDX via minimal, standards-aligned schema extensions.

Result: Evaluation across heterogeneous workloads shows the agentic AIBOM framework improves runtime dependency capture, reproducibility fidelity, and stability in vulnerability interpretation compared to existing provenance systems, with low computational overhead. Ablation studies confirm the unique contributions of each agent, demonstrating capabilities beyond deterministic automation.

Conclusion: The agentic AIBOM framework enhances traditional SBOMs by enabling dynamic, context-aware software supply-chain security through autonomous agents, offering improved runtime visibility, reproducibility, and stable vulnerability assessment while maintaining compatibility with existing standards.
...
### [20] [Tool Receipts, Not Zero-Knowledge Proofs: Practical Hallucination Detection for AI Agents](https://arxiv.org/abs/2603.10060)
*Abhinaba Basu*

Main category: cs.CR

TL;DR: NabaOS is a lightweight, epistemically grounded verification framework for AI agents that uses HMAC-signed tool execution receipts and Indian epistemology (Nyaya Shastra) to classify and verify claims in real time, effectively detecting hallucinations with minimal latency.


<details>
  <summary>Details</summary>
Motivation: AI agents often hallucinate tool executions or outputs, undermining trust and reliability; existing cryptographic verification methods like zero-knowledge proofs are too slow for interactive use, necessitating a faster, practical alternative.

Method: NabaOS classifies each claim in an LLM's response by its epistemic source (pramana)—such as direct observation (pratyaksha), inference (anumana), or testimony (shabda)—and verifies claims against cryptographically signed tool execution receipts; it uses HMAC signatures to prevent forgery and performs real-time cross-checking, including independent re-fetching for web tasks.

Result: On the NyayaVerifyBench benchmark, NabaOS detects 94.2% of fabricated tool references, 87.6% of count misstatements, and 91.3% of false absence claims, with less than 15ms overhead per response; in deep delegation tasks, it catches 78.4% of URL fabrications via re-fetching; it outperforms zkLLM, TOPLOC, SPEX, tensor commitments, and self-consistency in cost-latency-coverage trade-offs.

Conclusion: Receipt-based verification combined with epistemic classification offers a practical, low-latency solution for detecting AI agent hallucinations, providing fine-grained trust signals that are more actionable than binary cryptographic proofs, making it ideal for interactive agent systems.
...
### [21] [Building Privacy-and-Security-Focused Federated Learning Infrastructure for Global Multi-Centre Healthcare Research](https://arxiv.org/abs/2603.10063)
...
TL;DR: FLA$^3$ is a governance-aware federated learning platform that integrates authentication, authorization, and accounting (AAA) via ABAC, cryptographic logging, and scoped federation, enabling compliant, trustworthy cross-border healthcare AI with performance matching centralized models.


<details>
  <summary>Details</summary>
Motivation: Cross-institutional and cross-border healthcare research requires data sharing, but privacy regulations limit data movement. While FL enables local data training, existing frameworks lack robust governance for authentication, authorization, and accountability, hindering real-world clinical deployment.

Method: The authors designed FLA$^3$, a governance-aware federated learning platform incorporating XACML-compliant ABAC for fine-grained access control, cryptographic accounting for auditability, and study-scoped federation. The system integrates these AAA (Authentication, Authorization, Accounting) controls into the FL orchestration layer to enforce runtime policy compliance.

Result: FLA$^3$ was deployed across five institutions in four countries, demonstrating operational feasibility. In clinical evaluation using FBC data from 54,446 samples across 25 centers, the platform achieved predictive performance on par with centralized training while enforcing strict governance policies, including access control and auditability.

Conclusion: FLA$^3$ demonstrates that enforceable governance mechanisms can be integrated as first-class components in federated learning systems to ensure regulatory compliance and institutional sovereignty, enabling trustworthy, scalable AI deployment in cross-border healthcare settings.
...
### [22] [ADVERSA: Measuring Multi-Turn Guardrail Degradation and Judge Reliability in Large Language Models](https://arxiv.org/abs/2603.10068)
*Harry Owiredu-Ashley*

Main category: cs.CR

TL;DR: ADVERSA is an automated red-teaming framework that evaluates LLM safety beyond binary outcomes by measuring continuous compliance trajectories using a fine-tuned 70B attacker model and a structured 5-point rubric, revealing jailbreaks occur early and highlighting measurement confounds like judge reliability and attacker drift.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial evaluations of LLM safety rely on single-prompt tests with binary outcomes, failing to capture the temporal dynamics of safety degradation under sustained adversarial pressure. This work aims to develop a more robust and nuanced evaluation methodology that accounts for partial compliance and evolving guardrail effectiveness.

Method: The authors introduce ADVERSA, an automated red-teaming framework employing a fine-tuned 70B attacker model (ADVERSA-Red) based on Llama-3.1-70B-Instruct with QLoRA, which eliminates self-imposed safety filters. Victim model responses are evaluated using a structured 5-point rubric across multiple adversarial rounds, with a triple-judge consensus architecture to assess compliance and measure judge reliability explicitly.

Result: In a controlled experiment across three frontier models (Claude Opus 4.6, Gemini 3.1 Pro, GPT-5.2), ADVERSA achieved a 26.7% jailbreak rate with an average jailbreak round of 1.25, indicating early rather than cumulative breakthroughs. The study also quantified inter-judge agreement, identified attacker drift and attacker-side refusals as key confounds, and treated judge reliability as a measurable variable.

Conclusion: The study concludes that adversarial evaluations of LLM safety should move beyond binary pass/fail metrics to capture dynamic compliance behaviors, with ADVERSA offering a more nuanced framework for measuring safety degradation through continuous trajectories and structured scoring, revealing that jailbreaks occur predominantly in early interaction rounds.
...
### [23] [Why LLMs Fail: A Failure Analysis and Partial Success Measurement for Automated Security Patch Generation](https://arxiv.org/abs/2603.10072)
*Amir Al-Maamari*

Main category: cs.CR

TL;DR: This study evaluates 319 LLM-generated patches for 64 Java vulnerabilities from Vul4J, finding only 24.8% fully correct, with 51.4% failing both security and functionality due to semantic misunderstandings.


<details>
  <summary>Details</summary>
Motivation: Despite the potential of LLMs in automated program repair, their performance on security-related code fixes is not well understood, especially regarding whether generated patches actually resolve vulnerabilities without breaking functionality.

Method: The authors conducted a tri-axis evaluation on LLM-generated patches using compilation success, security correctness (via PoV tests), and functional correctness (via test suites) across 64 Java vulnerabilities from the Vul4J benchmark. They introduced a Security Repair Score (SRS) to quantify patch quality and analyzed failure modes and vulnerability type impact.

Result: Only 24.8% of patches were fully correct; 51.4% failed both security and functionality. Semantic misunderstandings were the primary failure mode. Functionality was well-preserved (mean SRS 0.832), but security correctness was poor (mean SRS 0.251). Fix success varied significantly by vulnerability type (0% for input validation, 45% for infinite loops).

Conclusion: LLM-generated security patches often fail to correctly address vulnerabilities despite syntactic validity, necessitating rigorous multi-axis validation before deployment.
...
### [24] [TASER: Task-Aware Spectral Energy Refine for Backdoor Suppression in UAV Swarms Decentralized Federated Learning](https://arxiv.org/abs/2603.10075)
...
### [25] [Amnesia: Adversarial Semantic Layer Specific Activation Steering in Large Language Models](https://arxiv.org/abs/2603.10080)
*Ali Raza,Gurang Gupta,Nikolay Matyunin,Jibesh Patra*

Main category: cs.CR

TL;DR: Amnesia is a lightweight adversarial attack that manipulates internal transformer states to bypass safety mechanisms in open-weight LLMs, demonstrating that current alignment techniques are vulnerable and underscoring the need for stronger security measures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to evaluate the robustness of existing safety alignment techniques in LLMs, such as reinforcement learning with human feedback, against sophisticated adversarial attacks that could enable the generation of harmful content.

Method: The authors propose Amnesia, a lightweight activation-space adversarial attack that manipulates internal transformer states to bypass safety mechanisms in open-weight LLMs without requiring fine-tuning or additional training.

Result: Experiments on state-of-the-art open-weight LLMs show that Amnesia successfully circumvents safety safeguards and induces various antisocial behaviors, including the generation of harmful content, on benchmark datasets.

Conclusion: The study concludes that current safety mechanisms in open-weight LLMs are insufficient against activation-space adversarial attacks like Amnesia, highlighting the need for more robust security measures to prevent misuse.
...
### [26] [Multi-Stream Perturbation Attack: Breaking Safety Alignment of Thinking LLMs Through Concurrent Task Interference](https://arxiv.org/abs/2603.10091)
*Fan Yang*

Main category: cs.CR

TL;DR: This paper proposes a multi-stream perturbation attack that exploits the vulnerabilities of large language models in thinking mode by interleaving multiple task streams, achieving high attack success rates and causing thinking process collapse or repetitive outputs.


<details>
  <summary>Details</summary>
Motivation: The increasing use of thinking mode in LLMs improves complex reasoning but introduces new security risks, especially under jailbreak attacks that can lead to detailed harmful content generation. The authors aim to expose and exploit unique vulnerabilities in thinking mode when handling multiple interleaved tasks.

Method: The authors propose a multi-stream perturbation attack with three strategies: multi-stream interleaving (concurrent task mixing), inversion perturbation (character reversal), and shape transformation (format constraints). These are designed to disrupt the model's step-by-step reasoning process by superimposing interference within a single prompt.

Result: The attack achieves high success rates on JailbreakBench, AdvBench, and HarmBench across models like Qwen3, DeepSeek, Qwen3-Max, and Gemini 2.5 Flash. It induces up to 17% thinking collapse and 60% response repetition, outperforming many existing methods.

Conclusion: Multi-stream perturbation effectively bypasses safety mechanisms in thinking-mode LLMs and disrupts their reasoning process, revealing critical vulnerabilities in current models and highlighting the need for robust defenses against structured, multi-task adversarial inputs.
...
### [27] [Detecting Privilege Escalation with Temporal Braid Groups](https://arxiv.org/abs/2603.10094)
*Christophe Parisel*

Main category: cs.CR

TL;DR: The paper uses the Burau Lyapunov exponent (LE) as an algebraic tool to identify a boundary between two risk regimes in cloud permission graphs, showing that non-Abelian structure is necessary and actionable for risk classification.


<details>
  <summary>Details</summary>
Motivation: To accurately distinguish between different risk regimes in evolving cloud permission graphs where traditional Abelian statistics fail.

Method: Applies the Burau Lyapunov exponent (LE) as a non-Abelian algebraic probe within strongly connected components of a temporal cloud permission graph; proves the insufficiency of Abelian statistics for determining LE.

Result: Demonstrates that Abelian statistics (e.g., edge counts, net flows) cannot capture the LE; identifies and distinguishes 'dispersed' and 'focused' risk regimes using the non-commutative advantage of the Burau LE.

Conclusion: Non-Abelian algebraic invariants like the Burau LE provide a small but actionable advantage in classifying and governing risky permission flows in cloud systems.
...
### [28] [Compatibility at a Cost: Systematic Discovery and Exploitation of MCP Clause-Compliance Vulnerabilities](https://arxiv.org/abs/2603.10163)
*Nanzi Yang,Weiheng Bai,Kangjie Lu*

Main category: cs.CR

TL;DR: This paper presents the first systematic analysis of security vulnerabilities in the Model Context Protocol (MCP) caused by optional compliance clauses, introducing a language-agnostic framework using IR-based static analysis and LLM-guided reasoning to detect compatibility-abusing attacks like prompt injection and DoS across SDKs.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the security risks introduced by the MCP protocol's optional behavioral clauses, which lead to inconsistent and misuse-prone SDK implementations across languages, creating a new vector for attacks such as silent prompt injection and denial-of-service.

Method: The authors propose a systematic framework comprising: (1) a universal, language-agnostic intermediate representation (IR) generator to normalize SDKs; (2) auditable static analysis enhanced with LLM-guided semantic reasoning for cross-language and cross-clause compliance checking; and (3) a modality-guided pipeline based on formalized attack semantics of MCP clauses to detect vulnerabilities.

Result: The framework enables cross-language analysis of MCP SDKs, identifies non-compliant implementations, and uncovers exploitable vulnerabilities corresponding to three defined attack modalities, demonstrating its effectiveness in exposing compatibility-abusing attack vectors.

Conclusion: The paper concludes that the relaxed behavioral constraints in the MCP specification introduce a novel attack surface known as compatibility-abusing attacks, and demonstrates that their proposed framework effectively identifies exploitable non-compliance issues across multi-language MCP SDKs.
...
### [29] [MCP-in-SoS: Risk assessment framework for open-source MCP servers](https://arxiv.org/abs/2603.10194)
*Pratyay Kumar,Miguel Antonio Guirao Aguilera,Srikathyayani Srikanteswara,Satyajayant Misra,Abu Saleh Md Tayeen*

Main category: cs.CR

TL;DR: This paper presents the first large-scale static analysis of security weaknesses in open-source Model Context Protocol (MCP) servers. Using CWE and CAPEC frameworks, it identifies critical vulnerabilities and introduces a risk-assessment model, revealing widespread exploitable flaws and calling for secure-by-design practices in MCP development.


<details>
  <summary>Details</summary>
Motivation: Despite the rapid adoption of MCP servers for enabling LLM agents to interact with real-world tools, there has been no large-scale, systematic evaluation of their security weaknesses. This gap motivates the need for a comprehensive assessment to understand and mitigate risks in open-source MCP implementations.

Method: The authors conducted a systematic, large-scale static code analysis of open-source MCP servers to identify Common Weakness Enumeration (CWE) vulnerabilities. These weaknesses were mapped to real-world attack patterns and threat categories using MITRE CAPEC. A multi-metric risk-assessment framework was introduced, evaluating likelihood and impact to quantify security risks across the MCP landscape.

Result: The analysis revealed numerous CWE-identified weaknesses in open-source MCP servers that align with known CAPEC attack patterns. These vulnerabilities pose significant risks to confidentiality, integrity, and availability, demonstrating that current implementations are often not resilient to real-world threats.

Conclusion: The study concludes that many open-source Model Context Protocol (MCP) servers contain exploitable security weaknesses affecting confidentiality, integrity, and availability, emphasizing the necessity of secure-by-design principles in MCP server development to ensure dependable agent deployments.
...
### [30] [Paladin: A Policy Framework for Securing Cloud APIs by Combining Application Context with Generative AI](https://arxiv.org/abs/2603.10228)
...
### [31] [ACE Runtime - A ZKP-Native Blockchain Runtime with Sub-Second Cryptographic Finality](https://arxiv.org/abs/2603.10242)
*Jian Sheng Wang*

Main category: cs.CR

TL;DR: ACE Runtime reduces blockchain signature verification overhead by separating identity and authorization using HMAC attestations and aggregated ZK proofs, enabling scalable, efficient, and post-quantum-ready consensus with two-tier finality.


<details>
  <summary>Details</summary>
Motivation: Traditional blockchains incur O(N) signature verification costs per block, leading to high hardware demands and challenges in migrating to post-quantum cryptography; ACE Runtime aims to reduce on-chain verification load and improve scalability and future-proofing.

Method: ACE Runtime introduces an Attest-Execute-Prove pipeline with two-tier finality (soft finality via BFT voting and hard finality via ZKP verification), replacing per-transaction signature checks with lightweight HMAC attestations and using aggregated zero-knowledge proofs for block finality.

Result: The prototype demonstrates low CPU orchestration overhead; analytical models project constant per-block verification cost, reduced hardware requirements for validators (non-builders), and improved bandwidth efficiency compared to per-transaction signature schemes.

Conclusion: Identity authorization separation is a practical architecture for achieving sub-second cryptographic finality and enables a clear migration path to post-quantum security.
...
### [32] [Post-Quantum Entropy as a Service for Embedded Systems](https://arxiv.org/abs/2603.10274)
...
TL;DR: This paper presents a Quantum Entropy as a Service (QEaaS) system that securely delivers high-quality quantum entropy from a Quantis device to resource-constrained ESP32-class devices using post-quantum-secured CoAP/DTLS. The system integrates efficient post-quantum algorithms (ML-KEM-512 and ML-DSA-44) and demonstrates faster handshake performance compared to classical ECDHE-ECDSA, proving that post-quantum security is not only feasible but advantageous in embedded systems.


<details>
  <summary>Details</summary>
Motivation: Embedded devices often lack high-quality entropy sources and are vulnerable to cryptographic attacks due to entropy starvation. Existing solutions are either too heavy for constrained devices or rely on classical cryptographic protocols that are not quantum-resistant. There is a critical need for lightweight, quantum-secure entropy distribution in embedded cryptography.

Method: The authors design and implement a QEaaS system with a server-side QRNG (Quantis) that provides quantum entropy via a custom OpenSSL provider or the Linux entropy pool. On the client side (ESP32), they extend the Zephyr-based libcoap with wolfSSL-integrated DTLS 1.3 using post-quantum algorithms (ML-KEM-512 for key exchange and ML-DSA-44 for authentication). A BLAKE2s-based entropy pool is implemented to locally store and inject server-provided entropy while maintaining compatibility with Zephyr's entropy interface.

Result: Benchmarks on ESP32 show that ML-KEM-512 achieves a DTLS 1.3 handshake in 313 ms without certificate verification, 35% faster than ECDHE-P256. Using ML-KEM-512 + ML-DSA-44 reduces the average time to 225 ms. With certificate verification, ECDSA adds ~194 ms, while ML-DSA-44 adds only 17 ms, making the full post-quantum configuration 63% faster than classical ECDHE-P256 + ECDSA. Local BLAKE2s entropy pool operations take less than 0.1 ms.

Conclusion: Post-quantum cryptography, when properly implemented, can outperform classical cryptographic protocols in resource-constrained embedded systems. The proposed QEaaS system enables secure, efficient delivery of high-quality quantum entropy and demonstrates that post-quantum key exchange and authentication are not only viable but faster than classical counterparts on ESP32-class devices.
...
### [33] [PRoADS: Provably Secure and Robust Audio Diffusion Steganography with latent optimization and backward Euler Inversion](https://arxiv.org/abs/2603.10314)
*YongPeng Yan,Yanan Li,Qiyang Xiao,Yanzhen Ren*

Main category: cs.CR

TL;DR: PRoADS is a secure and robust audio steganography method based on diffusion models that uses orthogonal projection and optimized inversion techniques to achieve exceptionally low bit error rates under compression.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based steganography methods suffer from high bit error rates due to reconstruction errors in diffusion inversion; PRoADS aims to improve robustness and reliability.

Method: PRoADS uses orthogonal matrix projection to embed secret messages into the initial noise of audio diffusion models, incorporating Latent Optimization and Backward Euler Inversion to minimize reconstruction and inversion errors.

Result: PRoADS achieves a very low bit error rate of 0.15% under 64 kbps MP3 compression, demonstrating superior performance and strong robustness compared to current approaches.

Conclusion: PRoADS is a provably secure and robust audio steganographic framework that effectively reduces bit error rates in diffusion-based steganography, outperforming existing methods under MP3 compression.
...
### [34] [The Orthogonal Vulnerabilities of Generative AI Watermarks: A Comparative Empirical Benchmark of Spatial and Latent Provenance](https://arxiv.org/abs/2603.10323)
*Jesse Yu,Nicholas Wei*

Main category: cs.CR

TL;DR: This paper evaluates the robustness of spatial and latent digital watermarks against modern generative AI and geometric attacks, revealing orthogonal vulnerabilities and arguing for multi-domain watermarking solutions.


<details>
  <summary>Details</summary>
Motivation: The rapid proliferation of open-weights generative AI and the resulting threat to digital trust necessitate robust, verifiable digital provenance. Current watermarking techniques lack rigorous evaluation against modern AI-based editing tools, leaving critical gaps in cybersecurity defenses.

Method: The authors empirically test two state-of-the-art watermarking paradigms—RivaGAN (spatial domain) and Tree-Ring (latent domain)—using an automated Attack Simulation Engine. They apply 30 intensity levels of geometric and generative perturbations and introduce the Adversarial Evasion Region (AER) framework to quantify cryptographic degradation while preserving semantic content (measured via OpenCLIP > 70.0). Statistical analysis is performed with n=100 per interval and a margin of error of ±3.92%.

Result: Spatial watermarks (RivaGAN) show high vulnerability to generative pixel-rewriting attacks, with a 67.47% AER evasion rate under Img2Img translation. Latent watermarks (Tree-Ring) are significantly compromised by geometric distortions, exhibiting a 43.20% AER evasion rate under static cropping. The two approaches demonstrate mathematically orthogonal weaknesses.

Conclusion: Single-domain watermarking (spatial or latent) is insufficient against modern adversarial editing tools. The study exposes a systemic weakness in current digital provenance mechanisms and advocates for the development of multi-domain cryptographic watermarking architectures to ensure robust content authentication.
...
### [35] [Don't Let the Claw Grip Your Hand: A Security Analysis and Defense Framework for OpenClaw](https://arxiv.org/abs/2603.10387)
*Zhengyang Shan,Jiayun Xin,Yue Zhang,Minghui Xu*

Main category: cs.CR

TL;DR: This paper evaluates the security vulnerabilities of the OpenClaw code agent platform and proposes a Human-in-the-Loop (HITL) defense layer to significantly improve its resilience against adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: Code agents using large language models can execute shell commands, posing serious security risks; OpenClaw, as an open-source local agent framework, lacks built-in security mechanisms, making it a representative case for studying baseline vulnerabilities in AI agents.

Method: A two-phase security analysis is conducted: first, a systematic evaluation of OpenClaw's native defenses using 47 adversarial scenarios across six attack categories from MITRE ATLAS and ATT&CK; second, implementation and testing of a novel HITL defense layer using a dual-mode framework to compare performance with and without human intervention.

Result: OpenClaw's native defense is weak, with only a 17% average defense rate and susceptibility to sandbox escape attacks; the proposed HITL layer successfully blocks up to 8 severe attacks that evaded native defenses, improving the overall defense rate to 19%–92%.

Conclusion: Current code agents like OpenClaw have significant security shortcomings; integrating human-in-the-loop mechanisms is an effective strategy to enhance security and mitigate risks posed by malicious instructions.
...
### [36] [Naïve Exposure of Generative AI Capabilities Undermines Deepfake Detection](https://arxiv.org/abs/2603.10504)
*Sunpill Kim,Chanwoo Hwang,Minsu Kim,Jae Hong Seo*

Main category: cs.CR

TL;DR: Commercial generative AI chatbots can refine deepfakes using benign prompts to evade detection while preserving identity and improving visual quality, exposing a critical vulnerability in current deepfake detection frameworks.


<details>
  <summary>Details</summary>
Motivation: The motivation is to investigate how real-world, user-facing generative AI systems — despite safety policies — can be exploited to undermine deepfake detection, without any malicious manipulation or adversarial attacks.

Method: The authors analyze the impact of using commercial generative AI systems with benign prompts to refine deepfakes, evaluating the effectiveness of state-of-the-art detection methods against semantically preserved, AI-refined images.

Result: The results show that refined images evade detection by state-of-the-art models, maintain identity accuracy via commercial APIs, and achieve higher perceptual quality. Commercial chatbot services are found to pose greater security risks than open-source models due to their realism and accessibility.

Conclusion: The study concludes that current deepfake detection methods are fundamentally challenged by the capabilities of commercial generative AI systems, revealing a critical structural mismatch between existing threat models and real-world AI deployment.
...
### [37] [An Approach for Safe and Secure Software Protection Supported by Symbolic Execution](https://arxiv.org/abs/2603.10608)
*Daniel Dorfmeister,Flavio Ferrarotti,Bernhard Fischer,Evelyn Haslinger,Rudolf Ramler,Markus Zimmermann*

Main category: cs.CR

TL;DR: A novel copy-protection method for industrial control software using Physically Unclonable Functions (PUFs) ensures correct execution only on target hardware, with symbolic execution preserving safety properties and resistance to reverse engineering.


<details>
  <summary>Details</summary>
Motivation: To prevent unauthorized copying and execution of industrial control software on non-target hardware while maintaining safety and security.

Method: Hardware-software binding via Physically Unclonable Functions (PUFs), combined with symbolic execution to enforce safety properties even when executed on unauthorized or faulty systems.

Result: The method ensures correct program behavior only on the intended hardware, preserves safety properties under execution mismatches or PUF errors, and resists reverse engineering attacks.

Conclusion: The proposed PUF-based copy-protection method effectively secures industrial control software against cloning and reverse engineering while guaranteeing safety via symbolic execution.
...
### [38] [Detecting and Eliminating Neural Network Backdoors Through Active Paths with Application to Intrusion Detection](https://arxiv.org/abs/2603.10641)
*Eirik Høyheim,Magnus Wiik Eckhoff,Gudmund Grov,Robert Flood,David Aspinall*

Main category: cs.CR

TL;DR: This paper introduces an explainable method to detect and remove backdoor triggers in neural networks by analyzing active paths, demonstrated on an intrusion detection model.


<details>
  <summary>Details</summary>
Motivation: Backdoor attacks in machine learning are hard to detect because models behave normally on clean inputs but maliciously on triggered inputs; existing detection methods lack transparency and effectiveness.

Method: The authors propose analyzing active neuron paths during inference to identify anomalous patterns indicative of backdoor triggers, enabling both detection and removal of such triggers.

Result: Experiments show promising results in detecting and eliminating backdoors injected into a neural network used for intrusion detection.

Conclusion: The proposed method offers a novel, explainable approach to enhancing model robustness against backdoor attacks by leveraging activation path analysis.
...
### [39] [Repurposing Backdoors for Good: Ephemeral Intrinsic Proofs for Verifiable Aggregation in Cross-silo Federated Learning](https://arxiv.org/abs/2603.10692)
*Xian Qin,Xue Yang,Xiaohu Tang*

Main category: cs.CR

TL;DR: This paper proposes a lightweight, verifiable secure aggregation scheme for federated learning by embedding ephemeral verification signals into model updates via backdoor injection and catastrophic forgetting, enabling efficient, scalable, and private integrity checks without heavy cryptography.


<details>
  <summary>Details</summary>
Motivation: Secure Aggregation protects update confidentiality but lacks integrity guarantees, enabling malicious servers to alter or drop updates undetected. Existing verifiable aggregation methods using heavy cryptographic tools (e.g., ZKPs, HE) do not scale well with large models, motivating the need for a lightweight, scalable solution.

Method: The paper introduces intrinsic proofs by repurposing backdoor injection to embed ephemeral verification signals into model parameters, leveraging catastrophic forgetting for signal decay. A randomized, single-verifier auditing framework is designed to work with Secure Aggregation, ensuring anonymity and avoiding signal collision without trusted third parties.

Result: Experiments on SVHN, CIFAR-10, and CIFAR-100 show high detection rates against malicious servers. The approach achieves over 1000x speedup on ResNet-18 compared to cryptographic baselines, demonstrating scalability to large models while preserving model accuracy and client privacy.

Conclusion: The proposed lightweight architecture using intrinsic proofs through backdoor injection and catastrophic forgetting enables efficient, scalable, and private verification of aggregation integrity in cross-silo federated learning, outperforming cryptographic methods in speed while maintaining model utility and client anonymity.
...
### [40] [CacheSolidarity: Preventing Prefix Caching Side Channels in Multi-tenant LLM Serving Systems](https://arxiv.org/abs/2603.10726)
*Panagiotis Georgios Pennas,Konstantinos Papaioannou,Marco Guarnieri,Thaleia Dimitra Doudali*

Main category: cs.CR

TL;DR: CacheSolidarity is a system that mitigates timing side channels in Automatic Prefix Caching for multi-tenant LLM inference by selectively isolating suspicious prefixes, preserving performance and security without full cache isolation.


<details>
  <summary>Details</summary>
Motivation: Automatic Prefix Caching (APC) in LLMs introduces timing side channels that allow attackers in multi-tenant systems to infer sensitive information via cache hit/miss timing differences; existing defenses eliminate risk by disabling caching entirely, sacrificing performance.

Method: CacheSolidarity monitors inter-user prefix cache reuse, detects suspicious access patterns indicative of attacks, and selectively isolates only those prefixes by restricting their cross-user reuse, while allowing benign sharing to continue.

Result: CacheSolidarity achieves up to 70% higher cache reuse and 30% lower inference latency compared to full user isolation defenses, with minimal overhead.

Conclusion: Security against APC side channels in LLM serving can be achieved without sacrificing efficiency by using adaptive, fine-grained isolation rather than coarse-grained defenses.
...
### [41] [AttriGuard: Defeating Indirect Prompt Injection in LLM Agents via Causal Attribution of Tool Invocations](https://arxiv.org/abs/2603.10749)
*Yu He,Haozhe Zhu,Yiming Li,Shuo Shao,Hongwei Yao,Zhihao Liu,Zhan Qin*

Main category: cs.CR

TL;DR: AttriGuard proposes a new paradigm of action-level causal attribution to defend LLM agents against Indirect Prompt Injection (IPI) by verifying the causal origin of tool calls, achieving strong security with minimal utility loss.


<details>
  <summary>Details</summary>
Motivation: LLM agents are vulnerable to IPI attacks through malicious tool outputs, and existing defenses focusing on input-level semantics lack generalization to novel payloads.

Method: Introduces action-level causal attribution via AttriGuard, which uses parallel counterfactual tests, teacher-forced shadow replay, hierarchical control attenuation, and a fuzzy survival criterion to determine if tool calls are driven by user intent or untrusted inputs.

Result: AttriGuard achieves 0% attack success rate under static IPI attacks across four LLMs and two agent benchmarks, with low overhead and preserved utility, and remains effective against adaptive optimization-based attacks where other defenses fail.

Conclusion: Action-level causal attribution is a robust and generalizable defense strategy against IPI, with AttriGuard setting a new standard for runtime security in LLM agent systems.
...
### [42] [A PUF-Based Approach for Copy Protection of Intellectual Property in Neural Network Models](https://arxiv.org/abs/2603.10753)
*Daniel Dorfmeister,Flavio Ferrarotti,Bernhard Fischer,Martin Schwandtner,Hannes Sochor*

Main category: cs.CR

TL;DR: The paper presents a method to protect neural network (NN) models' intellectual property by binding their weights to unique hardware characteristics using Physically Unclonable Functions (PUFs), preventing accurate execution on cloned hardware.


<details>
  <summary>Details</summary>
Motivation: Neural network models embody valuable intellectual property that is vulnerable to theft through hardware cloning; there is a need to prevent unauthorized replication and use of these models on copied hardware.

Method: The authors leverage Physically Unclonable Functions (PUFs) to anchor the NN model weights to intrinsic, unclonable hardware properties. The weights are modified or encrypted such that they can only be correctly restored and function on the original hardware.

Result: The approach effectively degrades model accuracy on cloned hardware while maintaining performance on the target hardware, demonstrated across multiple NN models.

Conclusion: Hardware binding via PUFs is a feasible and effective strategy for protecting neural network intellectual property against cloning, with potential for further optimization.
...
### [43] [Incremental Federated Learning for Intrusion Detection in IoT Networks under Evolving Threat Landscape](https://arxiv.org/abs/2603.10776)
*Muaan Ur Rehman,Hayretdin Bahs,Rajesh Kalakoti*

Main category: cs.CR

TL;DR: This paper evaluates incremental federated learning strategies for intrusion detection in IoT networks, using LSTM models to handle concept drift while preserving privacy and minimizing resource use. On the CICIoMT2024 dataset, cumulative and representative learning show stable performance, while retention-based methods balance accuracy and latency effectively.


<details>
  <summary>Details</summary>
Motivation: The increasing attack surface in IoT networks demands adaptive and privacy-preserving intrusion detection systems. However, concept drift and resource constraints in IoT environments challenge the long-term effectiveness of learning-based models, necessitating efficient and continuously updatable federated learning approaches.

Method: The authors propose LSTM-based models in a federated learning framework to evaluate incremental learning strategies, including cumulative, representative, and retention-based methods. They assess performance under concept drift using both binary and multiclass classification on the CICIoMT2024 dataset, focusing on stability, accuracy, and latency in non-stationary environments.

Result: Cumulative and representative incremental learning achieve the most stable detection performance under concept drift. Retention-based methods, while less stable, offer superior accuracy and lower latency, making them suitable for resource-constrained scenarios. The evaluation on the CICIoMT2024 dataset demonstrates the effectiveness of different incremental strategies across multiple attack types and classification tasks.

Conclusion: Cumulative incremental learning and representative learning are the most stable approaches for maintaining performance under concept drift in IoT intrusion detection systems, while retention-based methods provide a favorable trade-off between accuracy and latency. The study highlights the importance of balancing model adaptability and computational efficiency for resilient, resource-aware IDS in dynamic IoT environments.
...
### [44] [Re-Evaluating EVMBench: Are AI Agents Ready for Smart Contract Security?](https://arxiv.org/abs/2603.10795)
*Chaoyuan Peng,Lei Wu,Yajin Zhou*

Main category: cs.CR

TL;DR: ReEVMBench expands and reevaluates AI agent performance on smart contract security, revealing instability, limited end-to-end exploitation, and scaffold dependence, challenging the notion of imminent fully automated auditing.


<details>
  <summary>Details</summary>
Motivation: To reassess the capabilities of AI agents in smart contract security auditing by addressing limitations in EVMbench, specifically narrow scope and potential data contamination.

Method: Expanded evaluation to 26 agent configurations across four model families and three scaffolds; introduced a contamination-free dataset of 22 real-world security incidents occurring after model training cutoffs.

Result: (1) Agent performance is unstable across configurations, tasks, and datasets; (2) No agent achieves end-to-end exploitation on real-world incidents despite up to 65% vulnerability detection; (3) Scaffold choice significantly impacts results, with open-source scaffolds outperforming vendor ones by up to 5 pp.

Conclusion: Fully automated AI auditing is not imminent; agents are effective for known patterns and with human guidance but cannot replace human judgment. Recommended use: pre-deployment checks for developers and human-in-the-loop workflows for audit firms.
...
### [45] [MAD: Memory Allocation meets Software Diversity](https://arxiv.org/abs/2603.10840)
*Manuel Wiesinger,Daniel Dorfmeister,Stefan Brunthaler*

Main category: cs.CR

TL;DR: MAD introduces memory allocation diversity to combat evolving RowHammer attacks by using novel spatial diversification techniques that enhance entropy, delay attacks, and enable practical, low-overhead defense without hardware or software modifications.


<details>
  <summary>Details</summary>
Motivation: Existing defenses against RowHammer are narrowly focused and ineffective against evolving, generalized attack variants; a more robust and adaptable solution is needed.

Method: MAD combines memory allocation with principles of software diversity through two novel spatial diversification techniques to increase entropy and counteract DRAM vulnerability to RowHammer attacks.

Result: MAD overcomes the entropy limitation in memory systems, achieves negligible performance impact, is easy to implement, and delays RowHammer attacks effectively, enabling supplementary defensive responses.

Conclusion: MAD presents a promising new approach to mitigating generalized RowHammer attacks by leveraging spatial diversification in memory allocation, offering practicality, low overhead, and compatibility across hardware and software platforms.
...
### [46] [Layered Performance Analysis of TLS 1.3 Handshakes: Classical, Hybrid, and Pure Post-Quantum Key Exchange](https://arxiv.org/abs/2603.11006)
...
### [47] [Separating Oblivious and Adaptive Differential Privacy under Continual Observation](https://arxiv.org/abs/2603.11029)
