...
TL;DR: This paper proposes using Digital Object Identifiers (DOIs) to identify algorithms, especially in AI and multimodal LLMs, to improve transparency, accountability, and reproducibility, and introduces a cryptographic protocol to secure their application in APIs.


<details>
  <summary>Details</summary>
Motivation: The rapid proliferation of AI and LLM-based algorithms across critical domains necessitates stronger accountability, transparency, and ethical oversight, which current systems lack.

Method: The paper examines the applicability of Digital Object Identifiers (DOIs) to algorithms, analyzing their role in improving traceability, auditability, and reproducibility, and proposes a cryptographic authentication protocol for securing DOI-identified algorithms in API environments.

Result: DOI-based identification enables reliable provenance tracking, bias mitigation, auditability, and reproducibility of algorithms; the proposed cryptographic protocol enhances security in API usage of DOI-referenced algorithms.

Conclusion: The integration of DOIs for algorithm identification enhances accountability, transparency, and ethical standards in AI systems, particularly in LLMs and AI agents, while the proposed cryptographic authentication protocol supports secure and reliable algorithm deployment.
...
TL;DR: Tru-RM is a privacy-preserving radar-based respiratory monitoring system that uses variational mode decomposition and adversarial loss-based encryption to decouple and anonymize user identity information while accurately monitoring breathing.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the privacy risks in consumer-grade radar-based respiratory monitoring, where non-contact sensing collects sensitive user identity information (USI) that could be exploited, necessitating a trustworthy framework that preserves privacy without compromising monitoring performance.

Method: The method involves three core components: Attribute Feature Decoupling (AFD) to separate radar signals into respiratory, personal, and unrelated components; Flexible Perturbation Encryptor (FPE) using phase noise and learning-based intensity to encrypt identity information; and Perturbation Tolerable Network (PTN) with domain-independent design to ensure robust respiration monitoring on perturbed signals.

Result: Experiments across varying distances, respiratory patterns, and durations show that Tru-RM achieves strong USI anonymity and high accuracy in respiratory waveform detection, even under significant signal perturbations.

Conclusion: Tru-RM establishes a trusted, privacy-preserving radar-based respiratory monitoring framework that successfully decouples and anonymizes user identity information while maintaining high respiratory detection accuracy.
...
TL;DR: FC-GUARD is a privacy-preserving fiat-to-cryptocurrency exchange system that uses zero-knowledge proofs and verifiable credentials to protect user anonymity while supporting regulatory requirements like KYC and tax auditability through a controlled de-anonymization mechanism.


<details>
  <summary>Details</summary>
Motivation: Existing fiat-to-cryptocurrency exchange platforms often expose users' personally identifiable information and crypto addresses, enabling identity linkage and compromising privacy; FC-GUARD aims to preserve anonymity without sacrificing regulatory compliance.

Method: FC-GUARD uses verifiable credentials and zero-knowledge proofs to enable privacy-preserving transactions, and integrates a lawful de-anonymization mechanism for regulatory auditability.

Result: The system was implemented on desktop and mobile platforms and evaluated for feasibility, demonstrating practical deployability while maintaining strong privacy and compliance guarantees.

Conclusion: FC-GUARD successfully balances user privacy and regulatory compliance in fiat-to-cryptocurrency exchanges by breaking the link between real-world identities and crypto addresses while enabling lawful de-anonymization for compliance purposes.
...
TL;DR: NOIR is a privacy-preserving framework for LLM-based code generation that protects client prompts and code from the cloud using client-side encoding, local differential privacy, and randomized tokenization, achieving strong security with negligible performance degradation.


<details>
  <summary>Details</summary>
Motivation: LLM-powered code generation poses intellectual property and data security risks as cloud providers can observe client prompts and generated code, necessitating a privacy-preserving solution.

Method: NOIR employs a client-side encoder-decoder architecture with indistinguishability, local differential privacy at the token embedding level, and a randomized, data-independent tokenizer to prevent the cloud from inferring sensitive information.

Result: NOIR achieves strong privacy against reconstruction and frequency analysis attacks with minimal performance loss, achieving Pass@1 scores of 76.7 (MBPP) and 77.4 (HumanEval) on Evalplus, and 38.7 (BigCodeBench), only 1.77% below the original LLM.

Conclusion: NOIR is the first framework that effectively protects client prompts and generated code from cloud providers in LLM-powered code generation, offering strong privacy guarantees while maintaining high performance.
...
TL;DR: Ringmaster enables secure, time-sensitive programs in TEEs to safely access rich OS services via io_uring, falling back to a minimal TrustZone kernel when under attack, achieving high performance and low overhead on embedded hardware.


<details>
  <summary>Details</summary>
Motivation: Safety-critical systems require timely processing and access to OS services, but existing isolation methods (e.g., hypervisors) prevent access to rich OS features, creating a trade-off between security and functionality.

Method: Ringmaster leverages Linux's io_uring for asynchronous access to untrusted OS services from enclaves, while maintaining a minimal ARM TrustZone kernel to ensure continued execution during denial-of-service; it supports large unmodified programs within TEEs.

Result: Ringmaster achieves nearly 1 GiB/sec data throughput into enclaves on a Raspberry Pi 4B with 0-3% overhead compared to non-enclave tasks, enabling secure, high-performance operation in a UAV use case.

Conclusion: Ringmaster provides a secure and efficient solution for time-sensitive applications by enabling TEEs to access rich OS services while maintaining safety through a minimal trusted computing base in TrustZone, demonstrating practicality and low overhead in real-world UAV scenarios.
...
TL;DR: PyGuard is a knowledge-driven framework that leverages hierarchical pattern mining and large language models to enable semantic understanding of code, drastically reducing false positives in malware detection on PyPI and demonstrating cross-ecosystem effectiveness on NPM.


<details>
  <summary>Details</summary>
Motivation: Existing PyPI malware detection tools suffer from high false positive rates (15-30%) due to reliance on syntactic rules, leading to incorrect flagging of legitimate packages; this undermines trust and usability in package ecosystems.

Method: PyGuard extracts behavioral patterns from false positives and negatives using hierarchical pattern mining, applies Large Language Models to derive semantic abstractions of code behavior, and integrates exact pattern matching with contextual reasoning in a knowledge-driven detection system.

Result: PyGuard achieves 99.50% accuracy with only 2 false positives (vs. 1,927-2,117 in prior tools), maintains 98.28% accuracy on obfuscated code, detects 219 new malicious packages in production, and generalizes to NPM with 98.07% accuracy.

Conclusion: Semantic understanding of code behavior through knowledge-driven analysis enables highly accurate, robust, and transferable malware detection across package ecosystems.
...
TL;DR: AgentSemSteCom is a novel semantic steganographic communication scheme leveraging Agentic AI to eliminate reliance on cover images and private semantic keys, enhancing both steganographic capacity and security in semantic communications.


<details>
  <summary>Details</summary>
Motivation: Semantic Communication (SemCom) is vulnerable to intelligent attacks like semantic eavesdropping; existing diffusion-based steganographic methods still depend on private semantic keys, which can be inferred, creating security risks.

Method: The proposed AgentSemSteCom scheme employs Agentic AI for semantic extraction, digital token-controlled reference image generation, coverless steganography, semantic codec, and optional task-oriented enhancement modules, removing the need for cover images and private keys.

Result: Simulations on open-source datasets show that AgentSemSteCom outperforms baseline methods in transmission quality and achieves higher security levels.

Conclusion: AgentSemSteCom effectively enhances both the capacity and security of semantic steganographic communication by eliminating dependencies on cover images and private semantic keys through an intelligent, agent-driven framework.
...
TL;DR: DeMark is a query-free black-box attack that effectively removes defensive watermarks from deepfake images by exploiting latent-space vulnerabilities in encoder-decoder watermarking models, reducing detection accuracy from 100% to 32.9% on average while preserving visual quality.


<details>
  <summary>Details</summary>
Motivation: The increasing use of deepfakes has led to the deployment of defensive watermarks for detection and provenance tracking; however, the assumption that these watermarks are resistant to removal has not been thoroughly challenged.

Method: DeMark uses a compressive sensing-based sparsification process to manipulate the latent space of encoder-decoder watermarking models, suppressing watermark signals without queries or access to the watermarking model, while maintaining perceptual and structural realism.

Result: DeMark reduces watermark detection accuracy from 100% to 32.9% on average across eight state-of-the-art watermarking schemes and outperforms existing attacks; tested defenses (super resolution, sparse watermarking, adversarial training) show limited effectiveness.

Conclusion: Current encoder-decoder watermarking schemes are vulnerable to latent-space attacks, highlighting the need for more robust watermarking techniques to ensure reliable deepfake detection and provenance tracking.
...
TL;DR: This paper presents a high-performance, efficient crypto-processor for FrodoKEM, a lattice-based post-quantum KEM, achieving low latency, reduced resource usage, and full protocol support via novel execution and memory optimization techniques.


<details>
  <summary>Details</summary>
Motivation: FrodoKEM is a promising post-quantum cryptographic scheme standardized by ISO, but existing hardware implementations suffer from high latency, high resource consumption, and limited flexibility, limiting practical deployment in diverse applications.

Method: The authors propose a crypto-processor featuring a multiple-instruction overlapped execution scheme for efficient scheduling, a reconfigurable parallel multiplier array for fast matrix operations, and a compact memory scheduling strategy to minimize storage overhead; the design supports all FrodoKEM security levels and protocol phases and is implemented on an Artix-7 FPGA.

Result: The implementation consumes 13,467 LUTs, 6,042 FFs, and 14 BRAMs, achieving the fastest reported execution time and improving the area-time product (ATP) by 1.75-2.00x over state-of-the-art designs.

Conclusion: The proposed hardware architecture effectively addresses FrodoKEM's performance and resource challenges, offering a highly efficient, flexible, and scalable solution suitable for practical post-quantum cryptographic applications.
...
TL;DR: SafeThinker is an adaptive safety framework for LLMs that uses a gateway classifier to route inputs to specialized defense mechanisms, improving robustness against disguised attacks without sacrificing utility.


<details>
  <summary>Details</summary>
Motivation: Existing LLM safety measures often achieve only shallow alignment, leaving models vulnerable to disguised attacks like prefilling while reducing model utility; SafeThinker aims to overcome this trade-off.

Method: SafeThinker employs a lightweight gateway classifier to assess input risk and dynamically route queries to one of three components: a Standardized Refusal Mechanism for clear threats, a Safety-Aware Twin Expert (SATE) to detect deceptive attacks, and a Distribution-Guided Think (DDGT) module for adaptive intervention in uncertain cases.

Result: Experiments demonstrate that SafeThinker significantly reduces attack success rates across various jailbreak strategies while maintaining or improving model utility compared to baseline defenses.

Conclusion: By coordinating intrinsic risk awareness throughout the generation process via adaptive resource allocation, SafeThinker effectively balances robustness and practicality in LLM safety.
...
TL;DR: First end-to-end eclipse attack on Ethereum 2.0 execution-layer nodes, exploiting bootstrapping and peer management via DNS poisoning, discovery table poisoning, and connection slot hijacking; demonstrated on testnet and mainnet with over 80% of nodes vulnerable.


<details>
  <summary>Details</summary>
Motivation: Eclipse attacks have been well-studied in Bitcoin and Monero, but their practicality against Ethereum, especially post-Merge, remains unclear. This work fills the gap by evaluating the vulnerability of Ethereum (2.0) execution-layer nodes to such attacks.

Method: Proposes a multi-stage attack: (i) poisoning the node's discovery table using unsolicited messages, (ii) infiltrating DNS-based peer lists by manipulating the official DNS crawler, and (iii) hijacking idle incoming connection slots to block legitimate peers. Implemented and tested on Sepolia testnet and Ethereum mainnet.

Result: DNS poisoning achieved with only 28 IP addresses over 100 days; outgoing redirection success increased from 45% to 95%. Measurements show over 80% of public nodes lack sufficient idle capacity, making slot hijacking effective. Attack successfully isolates nodes upon restart.

Conclusion: Ethereum nodes are highly susceptible to eclipse attacks due to weaknesses in peer discovery and connection management. The work highlights practical threats post-Merge and proposes countermeasures, with findings responsibly disclosed to Ethereum's security team.
...
TL;DR: This survey explores security and privacy challenges in neuromorphic computing, focusing on vulnerabilities in spiking neural networks and memristive hardware, while reviewing attack methods, side-channels, and countermeasures like PUFs and TRNGs.


<details>
  <summary>Details</summary>
Motivation: The increasing adoption of brain-inspired neuromorphic architectures and memristive devices necessitates a thorough evaluation of their security vulnerabilities due to unique features like asynchronous processing and stochastic behavior that introduce new attack surfaces.

Method: The paper conducts a systematic survey of the security landscape in neuromorphic systems, examining both hardware and software-level threats, including attack methodologies, side-channel vulnerabilities, and the use of hardware security primitives such as PUFs and TRNGs.

Result: The analysis identifies key vulnerabilities in neuromorphic systems, particularly in SNNs and memristor-based implementations, and categorizes existing countermeasures that balance energy efficiency and security.

Conclusion: The survey provides a foundation for developing secure and trustworthy neuromorphic architectures by mapping current threats and highlighting the need for integrated security solutions in brain-inspired computing.
...
TL;DR: TracExp is the first automated framework that synthesizes verifiable proof-of-concept (PoC) exploits from on-chain attack traces by combining trace-driven reverse engineering and LLM-based code generation, achieving 93% success rate on real-world attacks.


<details>
  <summary>Details</summary>
Motivation: On-chain attacks in blockchain systems are fast and stealthy, and manually creating proof-of-concept (PoC) exploits to reproduce them is time-consuming, error-prone, and does not scale. There is a critical need for automated, accurate, and cost-effective methods to systematically analyze and reproduce such attacks for security auditing and defense.

Method: TracExp uses trace-driven reverse engineering to extract attacker logic from low-level transaction traces across multiple contracts. It introduces a dual-decompiler to convert noisy execution traces into semantically enriched exploit pseudocode, which guides large language models (LLMs) to generate executable PoCs. The framework iteratively refines the generated code to preserve exploit-relevant semantics and ensure verifiability.

Result: Evaluated on 321 real-world attacks over 20 months, TracExp synthesized PoCs for 93% of cases, with 58.78% being directly verifiable. The average cost per case was only $0.07. The system enabled public release of many previously unavailable PoCs, leading to a $900 bounty and significant community impact.

Conclusion: TracExp demonstrates that combining reverse engineering of transaction traces with LLM-based code synthesis enables highly effective, low-cost automation of exploit reproduction, advancing blockchain security analysis and response capabilities.
...
TL;DR: A risk-based access control system for Linux uses function-level kernel tracing and machine learning to detect and block malicious encryption in real time, integrating with SELinux for enforceable, explainable, and context-aware encryption control.


<details>
  <summary>Details</summary>
Motivation: Unauthorized encryption by ransomware requires fine-grained controls that can distinguish malicious from legitimate cryptographic activity without disrupting normal system operations.

Method: The system leverages Linux's ftrace function_graph tracer to collect high-resolution kernel function traces with resource and I/O counters. These traces are used to train a supervised classifier and extract interpretable rules that inform SELinux policies via lightweight booleans, enabling real-time, context-sensitive access control at encryption initiation.

Result: The approach achieves high detection accuracy with rule-like responsiveness, effectively gating suspicious encryption while preserving legitimate crypto workflows. It provides finer behavioral granularity than syscall-based methods and avoids the overhead of sandboxing or VMI. The user-space prototype shows detectable overhead under burst I/O, which a kernel-space implementation is expected to mitigate.

Conclusion: The architecture offers a practical, explainable, and risk-proportionate method for enforcing encryption control on production Linux systems by combining behavioral tracing, machine learning, and mandatory access control.
...
TL;DR: This paper proposes a cyber-risk management framework that models strategic interactions between attackers and defenders in networked systems, incorporating heterogeneous node valuations and contagion dynamics to determine optimal cybersecurity resource allocation.


<details>
  <summary>Details</summary>
Motivation: Cyber risk poses a significant financial threat in the digital economy, and existing models often fail to capture the strategic asymmetry between attackers and defenders or the impact of network-based contagion. This work aims to bridge that gap by integrating game theory, network structure, and risk propagation.

Method: The authors use a security game framework to model attacker-defender interactions, incorporating heterogeneous node valuations and contagion dynamics. They develop a method for optimal resource allocation using weighted network metrics based on risk profiles and introduce risk measures grounded in contagion paths. Numerical experiments analyze trade-offs between risk and cost under varying topologies and behavioral profiles.

Result: The framework identifies patterns in resource allocation that maximize resilience and reveals cyber deception effects, where strategic misalignment between attacker and defender priorities can be exploited for defense. Risk-cost efficiency frontiers show how topology and risk profiles influence optimal defense strategies.

Conclusion: By combining strategic behavior, contagion modeling, and network metrics, the proposed framework provides actionable insights for enhancing resilience in digital systems and effectively mitigating systemic cyber risk.
...
TL;DR: This paper proposes a linguistic-pattern-based framework to automatically identify concurrency bug reports by leveraging patterns across word, phrase, sentence, and report levels, and shows that fine-tuning pre-trained language models with these patterns achieves high precision (91-93%) on real-world datasets.


<details>
  <summary>Details</summary>
Motivation: Manually labeling concurrency-related bug reports is labor-intensive and error-prone; existing methods lack precision and interpretability, motivating the need for automated, accurate classification using domain-specific linguistic knowledge.

Method: The authors extract 58 linguistic patterns from 730 manually labeled concurrency bug reports across four levels (word, phrase, sentence, report), and evaluate four approaches—matching, learning, prompt-based, and fine-tuning—using machine learning and language models on 10,920 issue reports from 12 open-source projects.

Result: Fine-tuning pre-trained language models with linguistic-pattern-enriched inputs achieves the highest performance: 91% precision on GitHub, 93% on Jira, and sustained 91% precision on post cut-off data, outperforming other methods.

Conclusion: Integrating domain-specific linguistic patterns into fine-tuned PLMs significantly improves the automation, accuracy, and interpretability of concurrency bug report classification, offering a reproducible foundation for future research via a public dataset and taxonomy.
...
TL;DR: The software engineering research community faces systemic challenges that require a holistic, ecosystem-level analysis to identify leverage points for coordinated reform.


<details>
  <summary>Details</summary>
Motivation: Current issues in software engineering research—such as overloaded review processes, metric-driven incentives, and AI-driven disruptions—are often addressed in isolation, ignoring their shared root causes in the structural dynamics of the research ecosystem.

Method: The authors apply frameworks from complex systems, ecosystems, and theory of change to model the interrelated challenges in software engineering research as a unified system.

Result: This systems-level perspective reveals non-linear feedback loops that perpetuate dysfunction and identifies high-impact leverage points for systemic reform.

Conclusion: Sustainable progress requires coordinated, multi-level interventions across the SE research ecosystem rather than isolated fixes.
...
TL;DR: Introduces the concept of an Agentic Project Manager (PM) as a multi-agent system for Software Engineering 3.0, operating in four autonomy modes to support human teams while addressing ethical and accountability concerns.


<details>
  <summary>Details</summary>
Motivation: The rise of agentic AI necessitates evolution in Software Project Management (SPM) to maintain project success and keep humans central in the process.

Method: Proposes a vision of an Agentic PM as a multi-agent system with four operational modes of varying autonomy, informed by preliminary work and existing literature.

Result: Outlines a framework for agentic SPM with defined working modes, role evolution for human PMs, and new skill sets focusing on strategic leadership and coaching.

Conclusion: Establishes a foundation for agentic SPM research and presents a research agenda for the community to advance the field responsibly.
...
TL;DR: RubberDuckBench is a multilingual benchmark of real-world code-related questions from GitHub pull requests, used to evaluate 20 LLMs; even top models perform poorly, with frequent hallucinations and no cost-performance correlation.


<details>
  <summary>Details</summary>
Motivation: To evaluate AI coding assistants on real-world, contextual coding questions and assess their reliability, correctness, and consistency across diverse models.

Method: Curated a benchmark (RubberDuckBench) from GitHub pull request comments; included multilingual questions with detailed evaluation rubrics; tested 20 LLMs (open-source and proprietary) using human evaluation with partial credit scoring.

Result: Top models (Grok 4, Claude Opus 4, GPT-5) scored around 68-69%, but no significant performance gap among top 10; most answers received only partial credit; best models fully answered at most 2 questions correctly; 58.3% of responses contained hallucinated content; no correlation between model cost/size and performance.

Conclusion: Current AI coding assistants are inconsistent and unreliable despite advances; RubberDuckBench provides a rigorous target for improving correctness and trustworthiness in future systems.
...
TL;DR: IntelGuard is a novel framework using retrieval-augmented generation (RAG) and expert knowledge to detect malicious open-source packages with high accuracy and low false positives.


<details>
  <summary>Details</summary>
Motivation: Existing detection methods for malicious packages in open-source ecosystems rely on brittle handcrafted rules or data-driven features that cannot adapt to evolving attack semantics, creating a need for more robust and interpretable solutions.

Method: IntelGuard builds a structured knowledge base from over 8,000 threat intelligence reports, linking malicious code with behavioral descriptions and expert reasoning. For new packages, it retrieves similar malicious examples and uses LLM-guided reasoning to evaluate if code behavior matches claimed functionality.

Result: IntelGuard achieves 99% accuracy and a 0.50% false positive rate on 4,027 real-world packages, maintains 96.5% accuracy on obfuscated code, and discovered 54 previously unknown malicious packages when deployed on PyPI.org.

Conclusion: By integrating expert reasoning via a RAG-based approach, IntelGuard enables accurate, interpretable, and robust detection of malicious packages in open-source ecosystems.
...
TL;DR: EvoConfig is an efficient environment configuration framework that enhances multi-agent collaboration with fine-grained error analysis and self-evolving feedback, achieving state-of-the-art performance in building reliable runtime environments for LLMs in software engineering tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for environment configuration in software engineering tasks lack fine-grained analysis of agent actions, leading to inefficiencies and failures in handling complex errors. The need for reliable and efficient executable environments motivates the development of a more robust solution.

Method: EvoConfig introduces an expert diagnosis module for detailed post-execution analysis and a self-evolving mechanism that enables expert agents to self-feedback and dynamically adjust error-fixing priorities in real time. It optimizes multi-agent collaboration to accurately configure runtime environments.

Result: On Repo2Run's 420 repositories, EvoConfig matches the state-of-the-art performance. On the more challenging Envbench, it achieves a 78.1% success rate, outperforming Repo2Run by 7.1%. It also shows superior debugging capabilities, including higher accuracy in error identification and more effective repair recommendations.

Conclusion: EvoConfig significantly improves the reliability and efficiency of environment configuration for LLM-based software engineering tasks by enabling fine-grained analysis and adaptive error resolution, setting a new standard for runtime environment setup.
...
TL;DR: REprompt is a multi-agent framework that applies requirements engineering principles to automate and optimize prompt generation for large language model-based coding agents, improving alignment with formal software requirements.


<details>
  <summary>Details</summary>
Motivation: Effective prompt design in coding agents is crucial but challenging, requiring expertise in both prompt engineering and software requirements engineering; existing automated methods often overlook established requirements engineering principles, leading to suboptimal prompt quality in real-world software development contexts.

Method: REprompt employs a multi-agent architecture guided by requirements engineering methodologies to systematically generate, refine, and validate both system and user prompts, ensuring traceability and conformance to formal requirement specifications.

Result: Experiments show that REprompt successfully optimizes prompts by integrating requirements engineering principles, leading to improved performance and alignment with user requirements compared to baseline methods.

Conclusion: Incorporating structured requirements engineering into prompt generation enhances the effectiveness and reliability of LLM-based coding agents, bridging a critical gap between automated prompt engineering and professional software development practices.
...
TL;DR: This paper conducts a large-scale study on the impact of code comments in improving code translation accuracy across five programming languages and proposes COMMENTRA, a method that leverages purpose-describing comments to potentially double LLM-based translation performance.


<details>
  <summary>Details</summary>
Motivation: Despite the potential relevance of natural language comments in aiding code translation, existing benchmarks largely omit them, leaving their influence on translation quality poorly understood.

Method: The authors perform an empirical study on over 80,000 translations of 1,100+ code samples from two benchmarks, comparing translation performance with and without comments across C, C++, Go, Java, and Python. They analyze comment types and propose COMMENTRA, a comment-aware translation approach.

Result: Code comments—especially those describing the overall purpose of the code—significantly improve translation accuracy. COMMENTRA demonstrates the potential to double the performance of LLM-based code translation.

Conclusion: The study establishes the value of high-level code comments in code translation and introduces COMMENTRA as an effective approach, marking the first comprehensive evaluation of comment impact across multiple languages and benchmarks.
...
TL;DR: LuaJIT significantly improves energy efficiency and performance over standard Lua interpreters, approaching C's efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the growing need for energy-efficient software as highlighted by the UN's 2030 Agenda, and to understand how language implementation affects energy consumption, particularly in interpreted languages like Lua.

Method: Empirical evaluation across 25 official Lua interpreter versions and JIT compilers using a comprehensive benchmark suite to measure execution time and energy consumption; comparisons with C and analysis of JIT impact.

Result: LuaJIT outperforms standard Lua interpreters by consuming ~7x less energy and running ~7x faster. LuaJIT approaches C's efficiency, using ~6x more energy and running ~8x slower than C.

Conclusion: JIT compilation substantially enhances energy efficiency and performance in interpreted languages, making LuaJIT a much greener and faster alternative to traditional Lua interpreters.
...
TL;DR: LLMs improve requirement elicitation by helping stakeholders articulate needs more clearly and completely, especially when domain knowledge is limited.


<details>
  <summary>Details</summary>
Motivation: Stakeholders often struggle to express requirements accurately due to cognitive or domain knowledge limitations, leading to misalignment and inefficiencies in traditional elicitation methods.

Method: A study with 26 participants producing 130 requirements; participants first wrote requirements unaided, then assessed LLM-generated revisions contextualized to their input.

Result: LLM revisions were rated significantly higher than original statements in intent alignment, readability, reasoning, and unambiguity; qualitative feedback indicated enhanced self-understanding and surfacing of tacit knowledge.

Conclusion: LLM-assisted reformulation enhances requirements completeness, clarity, and intent alignment; involving stakeholders in validation ensures responsible AI use in Requirements Engineering.
...
TL;DR: This study investigates GenAI adoption among German software engineers through interviews and a survey, revealing that experience level and organizational size significantly influence tool effectiveness, with limited project context awareness being a major barrier.


<details>
  <summary>Details</summary>
Motivation: Despite growing use of GenAI in software development, little is known about adoption dynamics in regulated environments like Germany; this study addresses the gap by examining how experience, organization size, and regulatory constraints affect GenAI use.

Method: A mixed-methods approach was employed, including 18 exploratory interviews with practitioners and a survey of 109 German software developers, analyzing tool adoption patterns, prompting strategies, and organizational influences.

Result: Experience level moderates perceived benefits of GenAI, with uneven productivity gains; organizational size impacts tool selection and usage intensity; lack of project context awareness is the primary barrier to effective use.

Conclusion: The findings highlight the need for tailored support for developers based on experience, organizational adaptation, and improved tool design to enhance contextual understanding, offering actionable insights for developers, organizations, and tool vendors in regulated environments.
...
TL;DR: This paper investigates the challenges and best practices in designing REST API guidelines through interviews with 16 industry experts, identifying key factors affecting API usability, developer resistance to rigid guidelines, and the importance of organizational fit, evolution, and automated linting with educational feedback.


<details>
  <summary>Details</summary>
Motivation: REST APIs are critical business assets, but designing usable APIs remains challenging despite numerous existing guidelines. Many companies develop custom guidelines due to perceived inadequacies in public ones, leading to inconsistencies and adoption issues. This work aims to empirically understand how guidelines are adopted, used, and created to improve API design practices.

Method: Semi-structured interviews with 16 experienced REST API practitioners from industry were conducted. Thematic analysis was used to identify factors influencing API usability, challenges in guideline adoption and creation, and best practices for effective guideline design and implementation.

Result: Eight factors influencing REST API usability were identified, with adherence to conventions being the most critical. Developers show resistance to overly strict or large guidelines. Guideline effectiveness depends on organizational fit, maintainability, stakeholder involvement, and adaptability. Automated linting with explanatory feedback is shown to support both compliance and learning.

Conclusion: Effective REST API guidelines must balance standardization with flexibility, evolve with the organization, involve all stakeholders, and leverage automation not just for enforcement but also for education. One-size-fits-all guidelines are insufficient; contextual adaptability is key to widespread adoption and improved API usability.
...
TL;DR: SWE-Pruner is a task-aware context pruning framework for LLM agents in software development that uses goal-guided, adaptive compression to reduce long context lengths with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: Long interaction contexts in LLM-based software development agents lead to high API costs and latency; existing compression methods use fixed metrics and often discard critical code structure and logic.

Method: SWE-Pruner introduces a goal-driven, self-adaptive pruning framework where an agent defines a task-specific goal (e.g., 'focus on error handling') to guide context selection; a lightweight 0.6B-parameter neural skimmer dynamically prunes irrelevant code lines based on this goal.

Result: SWE-Pruner achieves 23-54% token reduction on agent tasks like SWE-Bench Verified and up to 14.84x compression on single-turn tasks such as LongCodeQA, with minimal impact on performance across multiple models and benchmarks.

Conclusion: Task-aware, goal-guided pruning significantly improves context compression for coding agents by preserving syntactic and logical structure, reducing costs and latency while maintaining effectiveness.
...
TL;DR: This paper evaluates foundation models (GPT-OSS-20B, GEMINI 3 PRO) for detecting and fixing variability-induced compilation errors in configurable C systems, showing high precision and recall compared to traditional tools like TYPECHEF.


<details>
  <summary>Details</summary>
Motivation: Variability-induced compilation errors in configurable software are hard to detect with traditional compilers and existing variability-aware tools due to high setup complexity and cost; foundation models offer a potentially simpler, more scalable alternative.

Method: Empirical evaluation of GPT-OSS-20B and GEMINI 3 PRO on 5,000 small configurable C systems, 14 real-world GitHub commits, and 42 mutation testing scenarios; comparison with TYPECHEF, a state-of-the-art variability-aware parser; assessment of error detection (precision, recall, accuracy) and repair capability (compilable fixes).

Result: GPT-OSS-20B achieved 0.97 precision, 0.90 recall, and 0.94 accuracy on small systems, outperforming TYPECHEF in coverage and matching GEMINI 3; it produced compilable fixes in over 70% of cases; on real commits, CHATGPT-5.2 detected all but two injected faults and found a potential real bug in a large Linux commit.

Conclusion: Foundation models are effective and practical for detecting and repairing variability-induced compilation errors, offering a low-effort complement to traditional variability-aware analysis tools.
...
TL;DR: AI-generated code in open-source projects survives longer than human-written code, contradicting the 'disposable code' hypothesis, though modification patterns differ slightly.


<details>
  <summary>Details</summary>
Motivation: To test the hypothesis that AI agent-generated code is disposable—quickly merged but rapidly discarded—potentially shifting maintenance burdens in software development.

Method: Survival analysis on over 200,000 code units from 201 open-source projects, comparing AI agent-authored and human-authored code in terms of modification rates and hazards; used statistical models and machine learning to predict modifications.

Result: AI-authored code has a 15.8 pp lower modification rate and 16% lower hazard of modification (HR = 0.842, p < 0.001); shows higher corrective modifications (26.3% vs 23.0%) but lower adaptive ones; small effect sizes (Cramér's V = 0.116), with more variation within agents than between agent and human; predictive models achieve AUC-ROC = 0.671 for modification likelihood but low F1 = 0.285 for timing.

Conclusion: AI-generated code is not more disposable than human code and often persists longer; differences in modification types are minor and overwhelmed by intra-agent variation; code evolution is more influenced by organizational dynamics than code origin, suggesting current bottlenecks lie in processes, not AI quality.
...
TL;DR: This paper introduces AIDev, a large-scale dataset of AI agent-authored pull requests, to study the impact of AI coding agents on build system quality, finding that while AI agents can introduce build code smells, they also sometimes remove them, and over 61% of their pull requests are merged with minimal human intervention.


<details>
  <summary>Details</summary>
Motivation: As AI coding agents become increasingly prevalent in software development, there is growing concern about the quality and maintainability of the code they generate, particularly in critical but under-researched areas like build systems. Existing studies focus on source code, leaving a gap in understanding how AI agents affect build code quality.

Method: The authors created AIDev, the first large-scale open dataset of real-world agent-authored pull requests (Agentic-PRs) from GitHub. They analyzed the dataset to investigate: (RQ1) whether AI agents introduce build code smells; (RQ2) whether agents can remove existing smells; and (RQ3) the acceptance rate of Agentic-PRs. They identified and categorized build smells related to maintainability and security across severity levels.

Result: The study identified 364 build smells in AI-generated code, including issues like lack of error handling and hardcoded paths. However, AI agents also performed refactorings (e.g., Pull Up Module, Externalize Properties) that removed existing smells. Over 61% of Agentic-PRs were approved and merged with little human intervention.

Conclusion: AI coding agents have a dual impact on build system code: they can both introduce quality issues and improve code through refactoring. The high merge rate highlights their practical integration into development workflows. The findings call for AI-aware build code quality tools and further research to assess, guide, and govern AI-generated build systems.
...
TL;DR: Proposes a selective instrumentation method for code coverage in AAA games, reducing overhead while maintaining stability and performance.


<details>
  <summary>Details</summary>
Motivation: Code coverage is important for testing, but full instrumentation in large C++ game engines causes performance issues and test instability, hindering its use in strict production environments.

Method: Develops a selective instrumentation framework tailored to C++ game engines that limits instrumentation scope while preserving coverage relevance to developer commits; integrates into an industrial testing pipeline.

Result: Minimal build overhead (doubling time only after 2,000+ commits), maintains >50% frame rate even in worst case, and causes no test failures—unlike full instrumentation which destabilizes tests.

Conclusion: Commit- or build-level coverage in large C++ game engines is feasible with low overhead and high stability using selective instrumentation, enabling practical integration into AAA game development workflows.
