...
TL;DR: This paper demonstrates effective adversarial attacks (UAP) against RFFI systems using CNNs/LSTMs/GRUs, achieving 81.7% success with minimal prior knowledge, revealing critical security vulnerabilities in wireless IoT authentication methods.


<details>
  <summary>Details</summary>
Motivation: Deep learning-based RFFI systems lack robustness against adversarial attacks, but no comprehensive analysis of adversarial attacks across hardware-impairment-based classifiers exists.

Method: The authors evaluated adversarial attack methods (FGSM, PGD, UAP) on RFFI systems using CNNs, LSTMs, GRUs. They tested real-time wireless attack factors like temporal attack effectiveness and implemented practical attacks with UAP.

Result: UAP achieved 81.7% success rate against RFFI systems even with minimal attacker knowledge, while FGSM/PGD also reduced classification accuracy. Temporal attack effectiveness remained high (65-75%) over 15 weeks.

Conclusion: The study concludes that adversarial attacks using UAP are effective against RFFI systems, revealing significant vulnerabilities in current deep learning-based authentication methods for wireless IoT devices.
...
TL;DR: MVP-ORAM is a new wait-free protocol that enables concurrent clients to access a data store without revealing their access patterns and can be integrated into BFT data stores.


<details>
  <summary>Details</summary>
Motivation: Existing ORAM protocols are not suitable for concurrent clients and fail to support BFT. Access patterns can be exploited for inference attacks, so there is a need for a protocol that protects data privacy while allowing concurrent and fault-tolerant operations.

Method: MVP-ORAM avoids trusted proxies, inter-client communication, and distributed locks by enabling clients to perform concurrent requests and merging conflicting updates as they happen. It introduces a weaker obliviousness guarantee for asynchronous concurrent ORAM services.

Result: MVP-ORAM实现了高吞吐量，能够处理每秒数百次4KB访问，并被集成到机密BFT数据存储中，创造出第一个BFT ORAM构造。

Conclusion: MVP-ORAM represents a practical approach to achieving concurrent and wait-free access to a private data store while maintaining security and performance.
...
TL;DR: This paper introduces two novel models to address the limitations of IP Camouflage by using a Graph-Matching algorithm and a DNAS-based NAND Array model, facilitating scalable and resilient IP design against RE attacks.


<details>
  <summary>Details</summary>
Motivation: The motivation for the paper arises from the significant annual losses due to semiconductor IP theft and the limitations of existing solutions like IP Camouflage in scalability and compatibility with standard analysis flows.

Method: The paper proposes a Graph-Matching algorithm to tackle the logic representation mismatch and a DNAS-based NAND Array model for scalability. These are complemented by a mimicry-aware partitioning method for large designs.

Result: The models show resilience against SAT and GNN-RE attacks and provide an efficient, scalable approach for deceptive IP design at the module level.

Conclusion: The introduced models offer a robust and scalable solution to expand IP Camouflage, addressing post-generation overhead and logic representation issues, and are effective against various RE attacks.
...
TL;DR: The paper proposes a new framework called Representational Contrastive Scoring (RCS) to detect jailbreak attacks in Large Vision-Language Models (LVLMs) by analyzing internal representations. It outperforms existing methods in generalization and efficiency.


<details>
  <summary>Details</summary>
Motivation: LVLMs are susceptible to multimodal jailbreak attacks, and current defenses either lack generalization or are computationally expensive. Traditional anomaly-detection methods also struggle with distinguishing between novel benign and malicious inputs.

Method: RCS uses the internal representations of LVLMs to learn a lightweight projection that maximally separates benign and malicious inputs in safety-critical layers. Two instantiations, MCD and KCD, use statistical methods to calculate contrastive scores for detection.

Result: MCD and KCD achieve state-of-the-art performance on a challenging evaluation protocol that tests generalization to unseen attack types. They offer reliable detection with low computational overhead.

Conclusion: The paper shows that effective jailbreak detection can be achieved by applying simple statistical methods to LVLM's internal representations. This provides a practical approach to improving the safety of LVLM deployments.
...
TL;DR: This paper proposes a three-stage RFFI approach using contrastive learning and Siamese networks to robustly identify IoT devices despite channel and receiver impairments, achieving over 90% accuracy with minimal fine-tuning data.


<details>
  <summary>Details</summary>
Motivation: The motivation arises from the need for reliable authentication of IoT devices through RFFI, facing challenges such as insufficient public datasets and the impact of channel and receiver effects on authentication accuracy.

Method: The method includes a three-stage RFFI approach: using spectrograms for signal representation to separate transmitter from channel and receiver effects, an unsupervised contrastive learning method for pretraining, and a Siamese network-based classification enhanced by data augmentation and contrastive loss to reduce the impact of impairments.

Result: The approach was tested with three public LoRa datasets and a self-collected one. The results showed the method can effectively mitigate channel and receiver impairments and significantly reduce the required fine-tuning data, achieving over 90% accuracy in dynamic NLOS scenarios with just 20 packets per device.

Conclusion: The proposed three-stage RFFI approach successfully addresses the issues of channel and receiver impairments with high accuracy and low need for labeled data, demonstrating the effectiveness of contrastive learning and Siamese networks in RFFI.
...
TL;DR: The paper highlights the procedural semantic gap in current CTI standards like MITRE ATT&CK, designed descriptively rather than procedurally for adversary emulation, and proposes a three-stage methodology to convert CTI into executable steps using MITRE Caldera, showing the gap can only be bridged with explicit analyst parameters.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by the observation that while MITRE ATT&CK is a key framework for cyberthreat intelligence, it lacks procedural semantics required for multi-stage adversary emulation, limiting its utility for automated defense systems and exercises.

Method: The methodology involves analyzing the MITRE ATT&CK framework’s structured data to evaluate if it provides enough behavioral detail to emulate adversaries. They conduct systematic measurements, then propose a three-stage methodology to operationalize CTI artifacts into executable steps, tested by emulating adversary campaigns with MITRE Caldera.

Result: Campaign objects in ATT&CK Enterprise encode only fragmented behaviors, with 35.6% of techniques appearing in campaigns and no reusable procedural order found via clustering or sequence analyses. Intrusion sets cover more techniques but miss procedural semantics. The authors’ methodology demonstrates that while CTI can enable multi-stage APT emulation (as shown in cases of ShadowRay and Soft Cell), it requires explicit analyst input which exposes semantic gaps in CTI standards.

Conclusion: The paper concludes that current CTI standards have a ‘procedural semantic gap’—describing what adversaries do but not how to operationalize it. Bridging this gap requires explicit parameters and assumptions by analysts, and the methodology presented reveals the need for CTI improvement to support fully automated multi-stage emulation.
...
TL;DR: The paper presents a method to verify LN channel balances using TEEs and zkTLS for secure, hardware-backed guarantees.


<details>
  <summary>Details</summary>
Motivation: There is a need for auditors and network participants to verify the financial capacity of Lightning Network channels against potentially malicious node operators.

Method: The framework combines TEEs for software integrity and zkTLS to prove delivery authenticity, with a TEE signing variant and a distinction between Hot and Cold proofs.

Result: The paper proposes a secure verification method for LN channels, with analysis of different implementations and their trade-offs.

Conclusion: The paper offers a framework for secure LN balance verification and highlights key security and performance considerations.
...
TL;DR: This paper introduces BRIDG-ICS, an AI-powered Knowledge Graph (KG) framework that enhances threat analysis and quantitative assessment of cyber resilience in Industry 5.0 environments. The framework integrates Industrial Control Systems (ICS) and cybersecurity data, leverages Large Language Models (LLMs) for semantic enrichment, and enables multi-stage attack path simulations for improved resilience.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by the increasing cybersecurity challenges in Industry 5.0, where IT and OT systems are integrated, leading to expanded cyber-physical attack surfaces. Traditional siloed security approaches fail to provide comprehensive, cross-domain threat insights, necessitating a more holistic approach to threat analysis and quantifying cyber resilience in smart manufacturing environments.

Method: BRIDG-ICS is an AI-driven Knowledge Graph (KG) framework that (1) fuses heterogeneous industrial and cybersecurity data into a unified Industrial Security Knowledge Graph, (2) uses Large Language Models (LLMs) to extract cybersecurity entities, predict relationships, and enrich the graph with semantic depth, and (3) simulates multi-stage attack paths using graph-analytic techniques for context-aware threat reasoning and risk assessment.

Result: In simulated industrial scenarios, BRIDG-ICS demonstrates strong scalability, reduces potential attack exposure, and enhances the resilience of cyber-physical systems in Industry 5.0 settings.

Conclusion: The paper concludes that BRIDG-ICS offers a robust solution for context-aware threat analysis and quantitative cyber resilience assessment, leveraging AI and Knowledge Graphs with LLMs for Industry 5.0 environments. The framework improves visibility into complex attack chains and supports data-driven mitigation strategies.
...
TL;DR: This paper introduces a plug-in framework for detecting adversarial examples in time-series large language models (TS-LLMs) used for energy forecasting. The method uses sampling-induced divergence to distinguish benign from adversarial sequences. It is evaluated on three TS-LLMs across three energy datasets, showing effective detection in both black-box and white-box attack settings.


<details>
  <summary>Details</summary>
Motivation: Accurate time-series forecasting is crucial for low-carbon power systems. TS-LLMs enable scalable forecasting without retraining but are vulnerable to adversarial examples (AEs), which are hard to detect due to global dependencies and variable input lengths.

Method: The proposed method leverages the TS-LLM's variable-length input feature to generate shortened input variants. It then detects AEs by measuring forecast consistency: benign inputs yield stable forecasts, while adversarial inputs produce divergent forecasts due to structure-dependent perturbations.

Result: The approach was tested on TimeGPT, TimesFM, and TimeLLM using three energy datasets (ETTh2, NI, and Consumption). It demonstrated strong detection performance against both black-box and white-box attacks.

Conclusion: The plug-in framework effectively detects AEs in TS-LLMs by exploiting sampling-induced forecast divergence. It offers a practical solution to secure time-series forecasting in energy systems. Future work may refine the method for different model architectures or attack types.
...
TL;DR: EIP-7702 introduces a new delegation-based phishing attack vector in Ethereum that allows attackers to gain persistent control over user accounts. The paper identifies multiple attack pathways, provides empirical evidence of its usage, and proposes protocol-level defenses.


<details>
  <summary>Details</summary>
Motivation: This research is motivated by the need to understand and mitigate security risks introduced by EIP-7702's new authorization mechanism, which enables novel phishing attacks that could lead to large-scale asset theft.

Method: The research combines formal analysis of the EIP-7702 design, controlled experiments to demonstrate attack methods, and empirical analysis of on-chain data from major EVM chains. The authors analyze over 150k authorization events involving 26k wallets to assess real-world impact.

Result: The study identifies three reliable phishing attack pathways enabled by EIP-7702 that can lead to full account takeover. It shows that these attacks are not hypothetical - our data reveals active exploitation with significant asset theft across Ethereum and other EVM chains. The analysis also finds that EIP-7702 usage is highly centralized among a small set of contracts associated with criminal activity.

Conclusion: The paper concludes that EIP-7702 creates significant new security risks that are already being exploited at scale. The authors propose protocol-level defenses to address this critical vulnerability in the Ethereum ecosystem's authorization architecture.
...
TL;DR: This paper introduces a code slicing framework for LLM-based npm malware detection, cutting input size by 99% and achieving 87.04% accuracy through taint-based slicing and heuristic backtracking for asynchronous pattern analysis.


<details>
  <summary>Details</summary>
Motivation: Traditional detection methods and basic LLM approaches struggle with npm malware's obfuscation and complex logic. LLMs' potential is hindered by context window constraints and computational costs, necessitating optimized input strategies.

Method: A taint-based code slicing technique with heuristic backtracking is developed to isolate security-critical code segments in npm packages, enabling LLMs (e.g., DeepSeek-Coder-6.7B) to analyze asynchronous patterns like callbacks and Promises with reduced input size.

Result: The framework reduces input volume by 99% while achieving 87.04% detection accuracy on 5,000+ packages, outperforming a token-splitting baseline by 11.63% and traditional static analysis methods.

Conclusion: The proposed code slicing framework effectively mitigates LLM context-window limitations while enhancing detection accuracy, offering an efficient defense against sophisticated npm malware through semantically optimized input representations.
...
TL;DR: UniMark offers a unified, open-source solution for multimodal AI content governance with both hidden and visible marking strategies, along with standardized evaluation benchmarks to improve trust and compliance.


<details>
  <summary>Details</summary>
Motivation: Address the trust crisis caused by AI-generated content proliferation and the limitations of fragmented identification tools, which lack compliance-friendly visible markers.

Method: Introduces UniMark, an open-source framework featuring a modular unified engine for multimodal governance and a dual-operation strategy for Hidden Watermarking and Visible Marking. Establishes standardized evaluation benchmarks (Image/Video/Audio-Bench).

Result: Development of a unified framework with dual-operation capabilities, standardized benchmarks for evaluation, and implementation of open-source tools.

Conclusion: UniMark bridges the gap between advanced algorithms and engineering implementation, fostering a more transparent and secure digital ecosystem.
...
### [13] [Vibe Coding in Practice: Flow, Technical Debt, and Guidelines for Sustainable Use](https://arxiv.org/abs/2512.11922)
*Muhammad Waseem,Aakash Ahmad,Kai-Kristian Kemell,Jussi Rasku,Sami Lahti,Kalle Mäkelä,Pekka Abrahamsson*

Main category: cs.SE

TL;DR: Vibe Coding speeds up software prototyping but risks long-term technical debt from AI-generated code. Addressing this requires balancing speed with deliberate design and maintenance.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the growing adoption of AI-assisted development practices like VC, which prioritize speed but lack clear frameworks for managing technical debt. Understanding these trade-offs is critical to ensuring long-term software quality and developer productivity.

Method: The analysis combines insights from internal MVP development experiences and a review of recent industry reports to evaluate the flow-debt trade-offs inherent in VC. This approach identifies systemic issues arising from AI-generated code and their root causes in model biases, process weaknesses, and hardware limitations.

Result: The study highlights that VC introduces risks such as architectural inconsistencies, security vulnerabilities, and maintenance challenges due to process-level flaws, biased training data, and overemphasis on rapid code generation. Proposed countermeasures include mitigating model biases, enforcing design rationale documentation, and integrating iterative human oversight.

Conclusion: The paper concludes that while Vibe Coding (VC) accelerates MVP development, its reliance on AI-generated code introduces significant technical debt through architectural issues and security risks. Addressing these challenges requires collaborative efforts to refine models, platforms, and workflows toward sustainable VC practices that balance efficiency with maintainability.
...
### [14] [A Systematic Mapping Study on Risks and Vulnerabilities in Software Containers](https://arxiv.org/abs/2512.11940)
*Maha Sroor,Teerath Das,Rahul Mohanani,Tommi Mikkonen*

Main category: cs.SE

TL;DR: The paper conducts a systematic mapping study to identify and organize security issues in software container systems, proposing a novel taxonomy and suggesting mitigation strategies.


<details>
  <summary>Details</summary>
Motivation: The study is motivated by the widespread adoption of software containers and the identified lack of reviewed, aggregated, and organized knowledge regarding their security risks, vulnerabilities, practices, and tools in the SE research literature.

Method: The researchers performed a Systematic Mapping Study (SMS) based on 129 selected primary studies to explore and organize existing knowledge on security issues in container-based systems.

Result: The study identifies critical risks and vulnerabilities across the container life-cycle, categorizes them using a novel taxonomy, and provides mitigation techniques, security practices, and tools to enhance container system security.

Conclusion: The paper concludes by emphasizing the need for future SE research to focus on security enhancement practices and effective mitigation strategies to address existing risks and vulnerabilities in software container systems.
...
### [15] [Evidence-Driven Decision Support for AI Model Selection in Research Software Engineering](https://arxiv.org/abs/2512.11984)
*Alireza Joonbakhsh,Alireza Rostami,AmirMohammad Kamalinia,Ali Nazeri,Farshad Khunjush,Bedir Tekinerdogan,Siamak Farshidi*

Main category: cs.SE

TL;DR: This paper proposes ModelSelect, an evidence-driven framework for AI model selection using MCDM principles, validated through 50 case studies to improve reproducibility and transparency in research software engineering.


<details>
  <summary>Details</summary>
Motivation: Current AI model selection practices rely on fragmented metadata and individual expertise, undermining reproducibility and software quality in research workflows.

Method: The authors conceptualize model selection as a Multi-Criteria Decision-Making (MCDM) problem, integrating automated data pipelines, a structured knowledge graph, and MCDM principles. The framework was validated using Design Science Research methodology with 50 real-world case studies and experiments against generative AI systems.

Result: ModelSelect achieved reliable, interpretable recommendations with high coverage (93%) and rationale alignment (88-91%) across case studies, performing comparably to generative AI assistants while offering superior traceability and consistency.

Conclusion: By formalizing AI model selection as an MCDM problem, the work establishes a rigorous foundation for transparent, reproducible decision support in research software engineering, enabling scalable evidence-based recommendations.
...
### [16] [Re-opening open-source science through AI assisted development](https://arxiv.org/abs/2512.11993)
*Ling-Hong Hung,Ka Yee Yeung*

Main category: cs.SE

TL;DR: The paper introduces STAR-Flex, an open-source fork of STAR, which allows processing of 10x Flex data by adding 16,000 lines of C++ code while maintaining full original functionality. This work is part of the NIH funded MorPHiC consortium.


<details>
  <summary>Details</summary>
Motivation: Large-scale modifications of scientific software can be challenging. The complexity of scientific software makes it closed to modification, but recent advances in AI technology allow for faster and more robust modifications, enabling re-opening scientific software to the community for review and further development.

Method: The paper demonstrates the modification of the STAR codebase through the use of an agentic AI team led by a single human, adding the necessary code to process 10x Flex data. The process resulted in STAR-Flex, an open-source fork with the new functionality.

Result: STAR-Flex was successfully created, adding the ability to process 10x Flex data to the original STAR capabilities, without losing any of the original functionality. This marks the first open-source processing software for Flex data and is a part of the NIH funded MorPHiC consortium.

Conclusion: The use of AI-assisted development enables more efficient modification of complex scientific software, allowing for the addition of new capabilities as demonstrated by STAR-Flex. This approach can re-open scientific software to community-based review and improvement.
...
### [17] [A Reference Architecture for Embedding Quantum Software Into Enterprise Systems](https://arxiv.org/abs/2512.12009)
*Marc Uphues,Sebastian Thöne,Herbert Kuchen*

Main category: cs.SE

TL;DR: Quantum computing holds promise for enterprise systems, but their software architectures must be designed with specific quality attributes. This paper proposes a modular reference architecture that uses loosely coupled and distributed services to embed quantum software into enterprise systems. The proposed architecture is demonstrated through two case studies involving combinatorial challenges in operations research.


<details>
  <summary>Details</summary>
Motivation: Quantum computing can offer significant performance improvements for certain enterprise applications, but current software architectures may not be designed with the necessary quality attributes to effectively integrate with quantum services.

Method: The authors propose a modular reference architecture that incorporates quantum software into enterprise systems through loosely coupled, distributed services that handle both quantum-independent and quantum-specific tasks. The architecture is defined as an executable BPMN model to form a stable, reusable pipeline.

Result: The proposed reference architecture was successfully demonstrated and evaluated in two case studies that address combinatorial challenges in operations research.

Conclusion: The reference architecture provides a scalable and adaptable solution for integrating quantum computing capabilities into enterprise systems.
...
### [18] [Hyper model checking for high-level relational models](https://arxiv.org/abs/2512.12024)
*Nuno Macedo,Hugo Pacheco*

Main category: cs.SE

TL;DR: The paper introduces HyperPardinus, an extension of Alloy's Pardinus backend, enabling verification of hyperproperties in design models by integrating with existing model checkers, with evaluation demonstrating its effectiveness on complex scenarios.


<details>
  <summary>Details</summary>
Motivation: Security and concurrency properties often require hyperproperties for multi-trace reasoning, yet existing tools lack high-level specification languages for early-stage verification. Alloy, while suitable, lacks natively hyperproperty support.

Method: HyperPardinus extends Pardinus to verify hyperproperties by leveraging low-level model checkers and conservatively extends Alloy for specification and visualization of (counter-)examples.

Result: Evaluation shows successful modeling and finding (counter-)examples for complex hyperproperties with alternating quantifiers, applicable to real-world scenarios.

Conclusion: HyperPardinus effectively bridges the gap in Alloy for hyperproperty verification, enabling practical verification at early design stages with high-level abstractions.
...
### [19] [Instruction-Tuning Open-Weight Language Models for BPMN Model Generation](https://arxiv.org/abs/2512.12063)
*Gökberk Çelikmasat,Atay Özgövde,Fatma Başak Aydemir*

Main category: cs.SE

TL;DR: Open-weight models instruction-tuned via InstruBPM generate high-quality BPMN models from natural language, outperforming baselines with lower resource needs and reduced modeling effort.


<details>
  <summary>Details</summary>
Motivation: Addresses modeling effort barriers by investigating cost-effective, privacy-preserving BPMN model generation from natural language using open-weight models.

Method: InstruBPM uses parameter-efficient fine-tuning and quantization on paired text-diagram data, evaluated via BLEU/ROUGE, graph edit distance, tool checks, and expert review.

Result: Tuned model outperforms baselines across metrics with lower resource use; generates BPMN diagrams conforming to best practices, reducing manual modeling effort.

Conclusion: Instruction tuning enhances structural accuracy and reduces prompt scaffolding reliance, with models and scripts shared for reproducibility.
...
### [20] [Citation-Grounded Code Comprehension: Preventing LLM Hallucination Through Hybrid Retrieval and Graph-Augmented Context](https://arxiv.org/abs/2512.12117)
*Jahidul Arafat*

Main category: cs.SE

TL;DR: TL;DR: The paper proposes a hybrid retrieval system combining BM25, BGE, and Neo4j graph expansion to address LLM hallucinations in code comprehension, achieving 92% citation accuracy with zero hallucinations.


<details>
  <summary>Details</summary>
Motivation: Motivation: Large language models for code comprehension often suffer from hallucinations—creating false citations. Reliable, verifiable assistance requires systems that accurately and comprehensively cite code, especially across files.

Method: Method: The authors develop a hybrid system integrating BM25 for sparse matching, BGE for dense embeddings, and Neo4j for structural graph expansion. This combines lexical, semantic, and structural approaches to improve citation grounding.

Result: Result: The hybrid system outperforms single-mode baselines by 14-18 percentage points in citation accuracy and identifies cross-file evidence missed in 62% of architectural queries, achieving 92% accuracy with no hallucinations.

Conclusion: Conclusion: The study emphasizes the necessity of integrating code structure and retrieval for verifiable code comprehension, advocating for citation grounded generation to combat hallucinations and enhance reliability.
...
### [21] [Training Versatile Coding Agents in Synthetic Environments](https://arxiv.org/abs/2512.12216)
*Yiqi Zhu,Apurva Gandhi,Graham Neubig*

Main category: cs.SE

TL;DR: The paper introduces SWE-Playground, a pipeline for generating synthetic coding environments and tasks to train versatile software engineering agents, addressing limitations of prior work by eliminating reliance on existing repositories and expanding task variety.


<details>
  <summary>Details</summary>
Motivation: Prior approaches to training software engineering agents face limitations due to their reliance on existing GitHub repositories and a narrow focus on issue resolution tasks, which restricts flexibility and applicability to a wider range of coding tasks.

Method: SWE-Playground is a pipeline that synthetically generates projects and tasks from scratch using strong language models and agents, without relying on external data sources. This enables a diverse range of coding tasks, such as generating unit tests and implementing libraries from scratch.

Result: The effectiveness of SWE-Playground is demonstrated on three distinct benchmarks. The results show that the generated trajectories provide dense training signals, enabling agents to achieve comparable performance with significantly fewer trajectories than previous works.

Conclusion: SWE-Playground addresses the limitations of prior methods by generating synthetic environments and tasks, thereby supporting the training of versatile coding agents with improved efficiency and applicability to a broader range of software engineering tasks.
