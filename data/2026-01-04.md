...
TL;DR: The paper introduces Audited Skill-Graph Self-Improvement (ASG-SI), a framework for secure, auditable self-improvement in AI agents by compiling reusable skills into a verifiable graph, addressing issues like reward hacking and opaque parameter updates.


<details>
  <summary>Details</summary>
Motivation: Current self-improving agents face unresolved security and governance challenges, such as reward hacking, untraceable behavioral drift, and entangled parameter updates. These weaknesses hinder audits, reproducibility, and system trustworthiness.

Method: ASG-SI iteratively compiles agents into skill graphs: (1)	extracts improvements from successful trajectories, (2)	normalizes skills with explicit interfaces, (3)	verifies skills via replay checks and contracts, (4)	decomposes rewards into auditable components, and (5)	integrates experience synthesis and memory control for scalability. The framework is grounded in a threat model and security analysis.

Result: The authors deliver a complete system architecture with a runnable implementation, demonstrating verifier-backed reward construction, skill compilation, audit logging, and measurable performance improvement under continuous tasks. They validate threat modeling and security guarantees through formal analysis.

Conclusion: ASG-SI redefines self-improvement as verifiable capability accumulation, offering a scalable path to govern and evaluate autonomous agents securely. It enables reproducible audits, mitigates reward hacking, and separates improvements into reusable skills, advancing responsible AI development.
...
TL;DR: The paper introduces a governance-aware, intent-driven architecture for managing API gateways in multi-cluster cloud environments, addressing issues like policy drift and inconsistent enforcement. A prototype shows significant improvements in policy consistency and performance.


<details>
  <summary>Details</summary>
Motivation: Organizations face challenges in maintaining consistent policy enforcement, predictable performance, and stability across heterogeneous API gateway environments in multi-cluster and hybrid cloud deployments. Current approaches lead to configuration drift, delayed policy updates, and unstable runtime behavior.

Method: The authors present an intent-driven architecture that translates high-level security, governance, and performance objectives into declarative intents. These intents are systematically converted into enforceable gateway configurations, validated through policy checks and telemetry feedback. The design decouples intent specification from enforcement and allows for policy-compliant adaptations, supporting diverse gateway implementations.

Result: Experimental results show a 42% reduction in policy drift, a 31% improvement in configuration propagation time, and a p95 latency overhead of less than 6% under varying workloads, compared to traditional manual and declarative approaches.

Conclusion: The proposed intent-driven architecture effectively addresses the challenges of managing API gateways in multi-cluster environments, providing a scalable and reliable solution for secure and performance-predictable cloud-native platforms.
...
TL;DR: The paper presents SyncGait, a gait-based mutual authentication system for drone delivery that uses a user's arm swing to authenticate without additional hardware, achieving high accuracy and security over long distances.


<details>
  <summary>Details</summary>
Motivation: Existing drone delivery systems require secure mutual authentication to prevent unauthorized access to valuable packages. Current solutions have limited authentication range and are vulnerable to sophisticated attacks, necessitating a more secure and practical method.

Method: SyncGait employs a user's natural arm swing during walking as a unique gait feature for implicit, hardware-free mutual authentication. The system does not require specific user actions and works over long distances.

Result: Extensive testing on 14 datasets from 31 subjects showed SyncGait achieves an average accuracy of 99.84% at distances greater than 18 meters and demonstrates strong resistance to spoofing attacks.

Conclusion: SyncGait provides a robust, secure, and user-friendly authentication solution for drone delivery, effectively addressing the limitations of existing methods with its hardware-free design and high performance.
...
TL;DR: This paper studies methods for inducing large language models to over-generate tokens by creating prompt attacks that force the model to produce excessive responses, and introduces two attackers: an evolutionary algorithm and a reinforcement learning method, which are evaluated on their ability to cause long continuations and over-generation. The results show that the reinforcement learning attacker is more effective in causing over-generation.


<details>
  <summary>Details</summary>
Motivation: The motivation is the issue of large language models (LLMs) being driven into over-generation, which negatively impacts response quality, increases latency and cost, and can be weaponized as a denial-of-service (DoS) attack. Current research lacks a comprehensive benchmark for comparing different black-box, query-only prompt-side attackers against a known tokenizer.

Method: The paper introduces two prompt-based attackers: Evolutionary Over-Generation Prompt Search (EOGen), which uses an evolutionary algorithm to search token space for prefixes that suppress EOS and induce long continuations, and a goal-conditioned reinforcement learning attacker (RL-GOAL), which trains a network to generate prefixes under a target length condition. The performance of these attackers is evaluated using metrics such as the Over-Generation Factor (OGF), which measures the ratio of produced tokens to the model's context window, alongside stall and latency summaries.

Result: The evolutionary attacker (EOGen) achieved a mean OGF of 1.38 ± 1.15 and a Success@OGF ≥ 2 of 24.5 percent on Phi-3. The RL-GOAL attacker outperformed these results, achieving a higher mean OGF of up to 2.81 ± 1.38 across different victim models.

Conclusion: The paper concludes that the introduced benchmark allows for a comparison of prompt-based attackers in a black-box, query-only regime. The reinforcement learning attacker (RL-GOAL) demonstrated superior performance in inducing over-generation, achieving higher Over-Generation Factors compared to the evolutionary algorithm. This highlights the potential for more effective DoS-style attacks on LLMs, emphasizing the importance of understanding and mitigating such vulnerabilities.
...
TL;DR: This paper is a comprehensive survey of power side-channel attacks across various application domains, providing classifications and comparisons of recent attacks and their countermeasures.


<details>
  <summary>Details</summary>
Motivation: While existing surveys focus on cryptographic implementations, recent power side-channel attacks have expanded into diverse areas like machine learning and user behavior, necessitating a broader review.

Method: The authors conduct a literature review to categorize and compare recent power side-channel attacks and their countermeasures based on application-specific contexts.

Result: The paper presents a structured classification of attacks across multiple domains and a detailed comparative analysis, highlighting the effectiveness of various countermeasures in different contexts.

Conclusion: The survey emphasizes the need for domain-specific defenses against power side-channel attacks, suggesting that understanding application-specific requirements is crucial for developing effective countermeasures.
...
TL;DR: This paper introduces VBSF, a visual-based spam detection system that overcomes text-based evasion techniques by combining OCR, text classifiers, and CNN-based image analysis with a meta-classifier, achieving 98.41% accuracy.


<details>
  <summary>Details</summary>
Motivation: Current text-based spam detection methods fail to identify spam using visual tricks like poisoned text or hidden salting, necessitating a system that processes emails like human vision.

Method: VBSF uses parallel pipelines: (1)'perceived text pipeline' with OCR, Naive Bayes, and Decision Tree, and (2)'appearance pipeline' with CNNs for rendered email images, integrated via stacking ensemble learning.

Result: VBSF achieved 98.41% accuracy, outperforming existing techniques on the designed dataset.

Conclusion: Visual-based analysis (combining perception and appearance) is more effective than text-only methods for combating visually obfuscated spam emails.
...
TL;DR: EDS is a security framework for IoT/edge systems that makes attacks economically unfeasible by increasing their cost, using four mechanisms to create a cost asymmetry between attackers and defenders.


<details>
  <summary>Details</summary>
Motivation: Traditional detection-based security fails against sophisticated, stealthy attackers in IoT/edge environments due to their encryption and resource constraints that limit ML-based intrusion detection. This motivates the need for detection-independent security approaches.

Method: EDS employs four mechanisms -- adaptive computational puzzles, decoy-driven interaction entropy, temporal stretching, and bandwidth taxation -- to exploit the asymmetry in how defenders and attackers control their environment. It is formalized as a Stackelberg game, providing closed-form equilibria for parameter selection and theoretical guarantees on cost amplification.

Result: EDS evaluation on a 20-device IoT testbed showed significant attack slowdowns (32-560x), large cost asymmetry (85-520:1), and reduced attack success (8-62%) with low overhead (<20ms) and no false positives. Against IoT-23 malware, EDS achieved 88% standalone mitigation, and 94% when combined with ML-IDS, a 27% improvement over IDS-only.

Conclusion: EDS provides effective detection-independent protection for resource-constrained environments by shifting the economic balance in favor of the defender. It can mitigate attacks without relying on detection, complementing or enhancing traditional detection-based security in IoT and edge systems.
...
TL;DR: MeLeMaD is a new framework for malware detection using MAML and CFSGB, achieving high accuracy on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Current malware detection methods struggle with robustness and adaptability to new threats and high-dimensional data.

Method: MeLeMaD combines Model-Agnostic Meta-Learning (MAML) with Chunk-wise Feature Selection based on Gradient Boosting (CFSGB) to improve adaptability and efficiency in malware detection.
...
Conclusion: MeLeMaD effectively addresses the challenges of robustness, adaptability, and high-dimensional data in malware detection, offering a promising solution for cybersecurity.
...
TL;DR: The paper discusses vulnerabilities in MoE architectures, showing how adversarial inputs can create routing concentration issues leading to denial-of-service attacks. It proposes RepetitionCurse, a low-cost method to exploit this flaw, significantly increasing inference latency in models like Mixtral-8x7B.


<details>
  <summary>Details</summary>
Motivation: Mixture-of-Experts (MoE) architectures are widely used to scale large language models due to their parameter efficiency. However, the lack of load balancing constraints during inference can lead to routing concentration when adversarial inputs are introduced, causing computational bottlenecks and potential denial-of-service scenarios.

Method: The authors demonstrate that adversarial inputs, particularly out-of-distribution prompts with repetitive token patterns, can exploit a universal flaw in MoE router behavior. They propose RepetitionCurse, a black-box strategy that constructs such adversarial prompts in a model-agnostic manner to trigger routing concentration.

Result: Using RepetitionCurse, the paper shows that inference latency can be increased by 3.063 times in models such as Mixtral-8x7B, leading to significant degradation in service availability and potential violations of service-level agreements.

Conclusion: The paper highlights a critical vulnerability in MoE-based systems, where adversarial inputs can exploit routing mechanisms to cause denial-of-service attacks. The proposed RepetitionCurse method demonstrates the real-world feasibility of such exploits and underscores the need for robust load balancing and security measures in MoE architectures.
...
TL;DR: This paper evaluates jailbreak attacks on LLMs through the full deployment pipeline, showing most techniques can be detected by safety filters and highlighting gaps in detection-precision trade-offs.


<details>
  <summary>Details</summary>
Motivation: Previous jailbreak assessments ignored safety mechanisms like input/output filters, leading to potentially overestimated attack success rates in real-world LLM deployments.

Method: Systematic evaluation of jailbreak techniques across the full inference pipeline, including both input filtering (prompt sanitization) and output filtering (response moderation) stages.

Result: 1) 90.4% of tested jailbreak methods were detectable by existing filters; 2). Detection accuracy and minimizing false positives/negatives remain suboptimal in safety system design.

Conclusion: Current evaluations overstate practical attack success when ignoring deployment pipeline defenses. Safety systems require improved detection accuracy while maintaining user experience through balanced filter precision.
...
TL;DR: This paper presents a light-weight transformer model for intrusion detection in connected and autonomous vehicles.


<details>
  <summary>Details</summary>
Motivation: Connected and autonomous vehicles are vulnerable to cyber threats and require efficient and effective intrusion detection systems (IDS). However, traditional methods may be resource-intensive and not suitable for the constrained environments of vehicles.


Method: We propose a lightweight Transformer model specifically designed for the domain of connected and autonomous vehicles. The model is trained and evaluated using real-world datasets to ensure practical relevance.


Result: Our experiments demonstrate that the proposed model achieves competitive performance metrics with significantly reduced computational resources compared to existing approaches.

Conclusion: The lightweight Transformer model offers an effective and resource-efficient solution for IDS in connected and autonomous vehicles, making it suitable for implementation in real-world vehicular systems.
...
TL;DR: The paper introduces a distance-aware encoding method for private point-in-polygon (PiP) tests in zero-knowledge proofs, achieving higher accuracy on coarse grids with moderate overhead compared to a Boolean baseline approach.


<details>
  <summary>Details</summary>
Motivation: Geometric operations in zero-knowledge proofs are computationally expensive, making private PiP tests challenging. Existing grid-based methods lack accuracy while maintaining efficiency.

Method: The authors compare a Boolean grid-based lookup approach with a distance-aware encoding that stores cell proximity to zone boundaries and uses interpolation. Experiments use a fixed STARK execution model on real-world data to evaluate accuracy and verification costs.

Result: The distance-aware approach achieves up to 60% higher accuracy on coarse grids with 1.4× increased verification overhead compared to the baseline.

Conclusion: Distance-aware zone encoding optimizes accuracy and efficiency for private spatial computations, positioning encoding strategies as critical for practical zero-knowledge spatial checks.
...
TL;DR: This paper proposes integrating oblivious memory (OM) in processors for privacy-preserving graph analytics, achieving 100x speedup over baselines with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Trusted processors suffer from performance degradation due to data-oblivious algorithms needed to prevent information leakage. Graph analytics is particularly vulnerable to access-pattern attacks.

Method: The authors explore a co-design approach combining storage structure and algorithms, implementing a prototype system with OM integrated in the processor.

Result: The prototype system achieves a 100x speedup compared to baselines, using OM sized around the per-core cache with negligible hardware overhead.

Conclusion: Integrating OM in trusted processors is viable for improving performance in privacy-sensitive applications like graph analytics, providing practical insights for future processor design.
...
TL;DR: We propose a lightweight transformer with encoder-only architecture for intrusion detection in the domain of connected and autonomous vehicles using federated learning


<details>
  <summary>Details</summary>
Motivation: Lightweight transformer models are important for intrusion detection in the resource-constrained environment of connected and autonomous vehicles. Federated learning allows for collaborative model training while maintaining data privacy across decentralized vehicular networks.

Method: We present an encoder-only transformer designed to minimize the number of layers in the network while maintaining high detection accuracy. The model is trained using a federated learning framework which enables training on decentralized data from vehicular networks without centralizing the sensitive data.

Result: Our experiments show that the lightweight transformer architecture achieves a competitive level of accuracy while significantly reducing model complexity compared to standard transformer models. The federated learning approach ensures that the model can be trained across multiple decentralized vehicular data sources without compromising data privacy.

Conclusion: This work demonstrates that a minimal layer transformer architecture combined with federated learning is effective for intrusion detection in connected and autonomous vehicles, offering a promising approach for implementing secure and privacy-conscious intrusion detection systems in vehicular networks.
...
TL;DR: A multi-stage IDS for CAVs is implemented and can be deployed in resouce-constrained environments after hybrid model compression.


<details>
  <summary>Details</summary>
Motivation: The motivation is to develop an intrusion detection system that is suitable for connected and autonomous vehicles (CAVs) with limited computational resources, which can be achieved by implementing a multi-stage IDS and utilizing hybrid model compression techniques.

Method: The method involves implementing a multi-stage intrusion detection system (IDS) for CAVs and deploying it in resource-constrained environments. This is achieved by applying hybrid model compression techniques, which likely include a combination of model pruning, quantization, and knowledge distillation to reduce the model size and computational requirements while maintaining performance.

Result: The result is a multi-stage IDS for CAVs that can be effectively deployed in resource-constrained environments after undergoing hybrid model compression, indicating that the system maintains its effectiveness while being optimized for limited resources.

Conclusion: The conclusion is that the implementation of a multi-stage IDS with hybrid model compression allows for the deployment of an effective intrusion detection system in CAVs that have limited computational resources, ensuring security while optimizing for efficiency.
...
TL;DR: The reliability of the SourceRank system, which assesses open-source packages' popularity and quality, is examined against evasion attacks, revealing its susceptibility to techniques like URL confusion and demonstrating its limited effectiveness in distinguishing malicious from benign packages in real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to evaluate the reliability of SourceRank metrics in the face of evasion tactics that malicious actors might use to appear trustworthy, especially given the increasing sophistication of malware within the open-source ecosystem.

Method: The study introduces a threat model to identify potential evasion techniques, analyzes historical data from MalwareBench alongside a large real-world dataset to assess the overlap of SourceRank distributions between benign and malicious packages, and evaluates the impact of URL confusion as an emerging attack vector.

Result: The results indicate significant overlap in SourceRank distributions between benign and malicious packages in real-world data, suggesting limited discriminative power. The prevalence and effectiveness of URL confusion in raising the SourceRank of malicious packages are highlighted, with its use increasing from 4.2% to 7.0%.

Conclusion: SourceRank cannot reliably distinguish between benign and malicious packages in real-world scenarios due to its delayed reflection of package removals and vulnerability to techniques like URL confusion, underscoring the need for more robust metrics to ensure open-source security.
...
TL;DR: This paper introduces a benchmark for evaluating the vulnerability of customer-service LLM agents to profit-seeking prompt injections, analyzing the effectiveness of attacks across different domains and techniques.


<details>
  <summary>Details</summary>
Motivation: Customer-service LLM agents are being used for policy-bound decisions, but their helpful interaction style can be exploited by users to induce unauthorized concessions, leading to financial losses and trust erosion. There is a need to understand the vulnerabilities and develop robust oversight workflows.

Method: The authors developed a cross-domain benchmark with 100 realistic attack scripts across 10 service domains and 5 technique families. They evaluated five widely used LLMs under a unified rubric, including uncertainty reporting.

Result: The study found that vulnerabilities are highly dependent on the domain, with airline support being the most exploitable, and the effectiveness of attack techniques varies, with payload splitting being the most consistently effective. The benchmark revealed significant variations in model susceptibility.

Conclusion: The research underscores the need for domain-specific security measures and technique-aware defenses to prevent exploitation of customer-service LLM agents. The release of data and code supports reproducible auditing and the development of trustworthy, human-centered agent interfaces.
...
TL;DR: GateChain is a blockchain-based solution for secure and transparent border entry/exit records.


<details>
  <summary>Details</summary>
Motivation: Traditional systems are centralized, leading to vulnerabilities and interoperability issues.

Method: GateChain uses blockchain to ensure data integrity and real-time access for authorized institutions.

Result: The system achieves high data reliability and enhances transparency in border control processes.

Conclusion: Blockchain provides a robust framework for secure entry and exit record management.
...
TL;DR: Real estate processes are inefficient and vulnerable to fraud due to manual document handling. This paper proposes a system that leverages OCR, NLP, and VCs to automate and secure document verification with blockchain technology, resulting in reduced verification time and enhanced reliability.


<details>
  <summary>Details</summary>
Motivation: Current real estate procedures rely heavily on manual document handling, which is inefficient and opens the door to fraud. The need for a secure, automated system for document management and verification is the core motivation for this work.

Method: The paper introduces an integrated system combining OCR, NLP, and VCs to automate document extraction, standardization, and verification. It utilizes a blockchain for decentralized trust and includes an OCR-NLP pipeline for data extraction, a backend for credential management, and a frontend for stakeholder interactions.

Result: The experimental results demonstrate the system's models achieve high accuracy across various document types and the end-to-end pipeline significantly reduces verification time without compromising reliability. The prototype effectively handles automated data extraction, verification, and secure document management.

Conclusion: The proposed framework shows the potential to revolutionize real estate transactions by enhancing efficiency, trust, and security through the automation of document handling using OCR, NLP, and VCs with blockchain technology.
...
TL;DR: The paper introduces Adversarial Diffusion Sanitization (ADS), a training-free defense against diffusion-based steganography, effectively neutralizing hidden payloads with minimal perceptual impact.


<details>
  <summary>Details</summary>
Motivation: The rapid growth of generative AI has enabled covert communication through synthetic media, particularly with diffusion models that can embed large payloads without detection, posing challenges for security systems.

Method: ADS uses a pretrained denoiser as a proxy for diffusion decoders and applies a color-aware, quaternion-coupled update rule to remove hidden payloads while minimizing image distortion.

Result: ADS significantly reduces decoder success rates to near zero against Pulsar, a state-of-the-art method, with minimal perceptual impact, outperforming standard content transformations.

Conclusion: ADS provides an effective and practical defense against diffusion-based steganography, offering a strong security-utility balance.
...
TL;DR: This paper presents an alternative mathematical proof for RSA cryptosystem correctness, focusing on extended conditions for the parameter N selection. It derives validity criteria for non-standard N values while excluding cryptographic security analysis.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address limitations in conventional RSA proofs by exploring extended mathematical conditions for N selection, which could enhance theoretical understanding of RSA's correctness foundation.

Method: The methodology involves: (1) Formalizing extended conditions for N selection, (2) Deriving mathematical proofs for valid N values, (3) Analyzing failure scenarios for invalid N choices through counterexamples

Result: The paper establishes explicit mathematical conditions for valid N values in RSA-like schemes, demonstrating that (1) conventional N requirements can be generalized, (2) specific non-prime N configurations maintain correctness under derived constraints

Conclusion: The authors conclude that RSA correctness can be formally established under broader N selection criteria than traditionally accepted, offering a mathematically rigorous alternative proof while explicitly noting the exclusion of security considerations from this scope.
...
TL;DR: SynRAG is a framework that generates platform-specific SIEM queries from a single high-level specification, enabling cross-platform threat detection without specialized training.


<details>
  <summary>Details</summary>
Motivation: SIEM platforms differ in attributes, architecture, and query languages, making it challenging for analysts to monitor multiple systems without extensive training or workforce expansion.

Method: SynRAG introduces a unified framework that automatically generates threat detection or incident investigation queries for multiple SIEM platforms from a platform-agnostic specification.

Result: SynRAG outperforms state-of-the-art language models (GPT, Llama, DeepSeek, Gemma, and Claude) in generating cross-SIEM queries for Qradar and SecOps systems.

Conclusion: SynRAG enables seamless threat detection and incident investigation across heterogeneous SIEM environments, significantly reducing the need for manual query translation and specialized training.
...
TL;DR: This paper addresses security and privacy risks in digital semantic communication (SemCom), reviews SemCom fundamentals, highlights architectural differences between analog and digital SemCom, organizes the threat landscape for digital SemCom, and outlines potential defenses and future research directions to achieve secure and deployable digital SemCom systems.


<details>
  <summary>Details</summary>
Motivation: Shift from bit-accurate transmission to task-oriented delivery in SemCom introduces new security and privacy risks such as semantic leakage, manipulation, and model-related attacks. Existing secure SemCom studies focus on analog SemCom, leaving digital SemCom's distinct security challenges underexplored despite its stronger compatibility with real-world systems.

Method: The paper reviews SemCom fundamentals, clarifies architectural differences between analog and digital SemCom and their security implications, organizes the threat landscape for digital SemCom, and discusses potential defenses.

Result: The paper provides a systematic review and analysis of secure digital SemCom, highlights key vulnerabilities and threats specific to digital SemCom's discrete mechanisms and practical transmission procedures, and outlines potential defenses and open research directions.

Conclusion: To ensure secure and deployable digital SemCom systems, there is a need for further research addressing the identified vulnerabilities, threats, and architectural differences between analog and digital SemCom. Potential defenses and future research directions are outlined to achieve this goal.
...
TL;DR: This paper introduces two new protocols for Traceable Over-Threshold Multi-Party Private Set Intersection (T-OT-MP-PSI), improving efficiency and security while eliminating collusion assumptions in prior work.


<details>
  <summary>Details</summary>
Motivation: Traditional MP-PSI with threshold lacks traceability and assumes non-colluding parties, limiting practicality in scenarios like digital forensics where transparency and reliability are essential.

Method: The authors propose two protocols: ET-OT-MP-PSI combines Shamir's secret sharing and oblivious programmable PRF for efficiency; ST-OT-MP-PSI adds oblivious linear evaluation for stronger security against colluding semi-honest parties.

Result: ET-OT-MP-PSI and ST-OT-MP-PSI achieve 15,056× and 505× speedups, respectively, versus Mahdavi et al.'s work under similar conditions, while supporting traceability and eliminating prior collusion assumptions.

Conclusion: The protocols enhance practicality and security for T-OT-MP-PSI, offering significant efficiency gains and eliminating restrictive assumptions about non-colluding parties, suitable for applications requiring traceable intersections.
...
TL;DR: CellSecInspector is an automated framework for security analysis of 3GPP specifications that discovers 43 vulnerabilities (8 previously unknown) by modeling procedures with SCA representations and validating security properties without manual rules.


<details>
  <summary>Details</summary>
Motivation: 3GPP specifications' complexity and evolution make manual reviews and existing automated tools (reliant on rules/small requirement sets) ineffective at capturing semantic dependencies and cross-clause relationships.

Method: Extracts structured SCA representations of network procedures, models function chains, validates against 9 security properties across 4 adversarial scenarios, and generates test cases in an end-to-end pipeline.

Result: Identified 43 vulnerabilities in 5G/4G NAS/RRC specifications through automated analysis, with 8 being new findings, demonstrating scalability and adaptability.

Conclusion: CellSecInspector provides an effective, rule-free solution for security analysis of rapidly evolving 3GPP specifications to safeguard current and future cellular networks.
...
TL;DR: This paper examines how blockchain-native RegTech solutions address AML/CFT compliance challenges in Web3 by leveraging distributed ledger properties. It identifies three frameworks to organize the domain, highlights capabilities unique to Web3 RegTech, and points out gaps in academic and industry alignment.


<details>
  <summary>Details</summary>
Motivation: The paper motivates the need to adapt compliance mechanisms to Web3's decentralized architecture, which traditional centralized RegTech cannot address effectively due to pseudonymity and cross-chain complexity.

Method: The authors systematically review 41 commercial platforms and 28 academic prototypes (2015-2025) to categorize RegTech solutions using three taxonomies: regulatory paradigm evolution, compliance protocol layers, and the compliance lifecycle.

Result: Web3 RegTech can achieve transaction graph analysis, real-time risk evaluation, cross-chain analytics, and privacy-preserving validation. However, academic research lags in deploying practical solutions, with challenges remaining in DeFi, cross-chain tracking, and scalability.

Conclusion: The paper concludes by suggesting architectural best practices to balance Web3 principles (decentralization, transparency) with compliance obligations and outlines research paths to bridge gaps in practical deployment and cross-chain monitoring.
...
TL;DR: The paper introduces MTSP-LDP, a framework addressing limitations in current local differential privacy (LDP) mechanisms for streaming data analytics.  It tackles the issues of handling complex queries and failing to account for temporal correlations by using optimal privacy budget allocation, a data-adaptive binary tree for complex queries, and a budget-free approach for multi-task processing.  Experiments show significant improvements in utility.


<details>
  <summary>Details</summary>
Motivation: Streaming data analytics raises privacy concerns due to direct data collection. Existing LDP mechanisms, while offering strong privacy guarantees, are limited in practical deployment.  They are ill-suited for complex queries and fail to account for temporal correlations, which degrades the overall utility of the data released.  A more effective solution is needed to enable privacy-preserving complex query processing on streaming data while maintaining high utility.

Method: MTSP-LDP is a framework for privacy-preserving streaming data publication under $w$-event LDP.  The core innovations are threefold.  First, an Optimal Privacy Budget Allocation algorithm dynamically allocates privacy budgets based on temporal correlation analysis within each window. Second, a data-adaptive private binary tree structure is introduced to support complex queries, incorporating cross-timestamp grouping and smoothing for improved estimation accuracy. Finally, a unified Budget-Free Multi-Task Processing mechanism enables a range of streaming queries without additional privacy budget consumption.

Result: MTSP-LDP achieves consistently high utility across a variety of streaming tasks, significantly outperforming existing methods in experimental evaluations on real-world datasets.

Conclusion: MTSP-LDP effectively addresses the limitations of current LDP mechanisms for streaming data, offering a more practical and efficient solution for privacy-preserving complex query processing with improved utility.
...
TL;DR: This paper addresses the limitations of Consensus Sampling (CS) in generative AI security by introducing Reliable Consensus Sampling (RCS), which eliminates the need for abstention and improves robustness through a feedback algorithm. Theoretical guarantees and experiments confirm RCS's effectiveness.


<details>
  <summary>Details</summary>
Motivation: Existing generative AI security methods rely on attack-defense cycles leading to unforeseen vulnerabilities. Building provably secure AI with theoretical risk control is essential. CS, while promising, faces issues with utility loss and vulnerability to malicious manipulation.

Method: The authors propose Reliable Consensus Sampling (RCS) to mitigate CS's limitations. RCS introduces acceptance probability tracing to tolerate adversarial behaviors without abstention. A feedback algorithm is also developed to dynamically enhance safety. Theoretical guarantees ensure risk controllability.

Result: RCS significantly improves robustness and utility while maintaining latency similar to CS. The feedback algorithm effectively boosts safety, and theoretical proofs confirm the risk control properties of RCS.

Conclusion: The paper presents RCS as a significant step toward provably secure generative AI. RCS offers improved robustness and utility over CS without sacrificing performance, contributing to the long-term goal of theoretically grounded AI security.
...
TL;DR: The paper presents AgenticTCAD, an open-source, natural language-driven framework for automated TCAD device design and optimization, demonstrating its efficiency in meeting IRDS-2024 specifications for a 2 nm NS-FET significantly faster than expert-led manual methods.


<details>
  <summary>Details</summary>
Motivation: The scarcity of open-source TCAD resources limits the ability of language models to generate valid TCAD code, hindering automated design and optimization as DTCO becomes increasingly critical with advanced technology nodes.

Method: The authors create an expert-curated open-source TCAD dataset and fine-tune a domain-specific language model for TCAD code generation. They then develop AgenticTCAD, a multi-agent framework that uses natural language inputs to automate the entire device design and optimization process.

Result: AgenticTCAD achieved IRDS-2024 device specifications for a 2 nm nanosheet FET in 4.2 hours, compared to 7.1 days required by human experts using commercial tools.

Conclusion: AgenticTCAD offers a significant advancement in automated TCAD simulation by combining an open-source dataset with a domain-adapted language model within a multi-agent framework, dramatically accelerating device design while meeting industry standards.
...
TL;DR: Hybrid-Code is a hybrid neuro-symbolic multi-agent framework for local clinical coding that ensures reliability through redundancy and verification, addressing privacy and latency issues in healthcare AI deployment.


<details>
  <summary>Details</summary>
Motivation: Cloud-based LLMs for clinical coding have privacy risks and latency issues, making them unsuitable for on-premise healthcare use.

Method: The system uses two agents: a Coder that combines semantic reasoning with BioMistral-7B and deterministic keyword matching, and an Auditor that verifies codes against a knowledge base and clinical evidence. The hybrid approach includes semantic understanding, deterministic fallback, and symbolic verification.

Result: On 1,000 MIMIC-III discharge summaries, Hybrid-Code achieved no hallucinated codes within the knowledge base, a 24.47% verification rate, 34.11% coverage (95% CI: 31.2%--37.0%), 86%+ language model utilization, a 75.53% rejection rate for invalid codes, and ensured data privacy by keeping patient data on-premise.

Conclusion: The study shows that reliability through redundancy is more valuable than pure model performance in production healthcare systems, as system failures are unacceptable. Hybrid-Code addresses critical barriers to AI adoption in healthcare by ensuring reliability and privacy.
...
TL;DR: This paper introduces DEFT, a novel ATPG algorithm that improves HTD fault detection through continuous optimization and achieves significant improvements over commercial tools in efficiency and fault-detection rates.


<details>
  <summary>Details</summary>
Motivation: Modern IC complexity leads to test pattern growth, primarily targeting a small set of hard-to-detect (HTD) faults. Traditional ATPG algorithms are less effective for these faults, necessitating new methods to improve test effectiveness.

Method: DEFT reformulates the discrete ATPG problem as a continuous optimization task using a reparameterization that aligns with discrete fault-detection semantics. It employs a custom CUDA kernel for scalability and gradient normalization to handle deep circuits, enabling gradient-based pattern generation.

Result: DEFT achieves 21.1% and 48.9% improvements in HTD fault detection on two industrial benchmarks compared to a commercial tool, using the same pattern budget and runtime. It also reduces 0/1 bits by 19.3% while detecting 35% more faults in partial assignment settings.

Conclusion: DEFT is a promising ATPG approach that effectively addresses limitations of traditional methods for HTD faults, offering superior performance and practical applicability in real-world test pattern generation.
...
TL;DR: Mify-Coder is a 2.5B-parameter code model trained on 4.2T tokens using compute-optimal strategies, achieving performance comparable to larger models while excelling in coding benchmarks. Its training combines curated/synthetic data with iterative refinement and quantized variants enable desktop deployment.


<details>
  <summary>Details</summary>
Motivation: The paper aims to demonstrate that compact models can match large-scale models in code generation and agent-driven workflows, addressing the need for efficient yet accurate code AI.

Method: Training integrates high-quality curated sources and synthetic data generated via agent-designed prompts, refined with enterprise evaluation datasets. LLM-based filtering and compute-disciplined CPT-SFT approaches optimize data efficiency.

Result: Mify-Coder outperforms larger baselines on coding/function-calling benchmarks, achieves safety/accuracy parity, and tenants quantized versions operate on standard desktops without specialized hardware.

Conclusion: Compact models with principled data computing strategies can deliver frontier-grade code intelligence, proving efficiency and accuracy are compatible in code generation through disciplined resource management.
...
TL;DR: This paper introduces a new concept called discrimination clustering to detect patterns of systematic bias in algorithmic decision-making, addressing limitations of individual fairness. It proposes HyFair, a hybrid method that combines symbolic analysis and randomized search to uncover discriminatory clusters and generate interpretable explanations for detecting algorithmic bias.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by the need to address the limitations of existing fairness measures, particularly individual fairness, which focuses on isolated cases and does not detect broader patterns of system-wide or clustered discrimination. The authors aim to identify algorithmic bias that may not be captured by pairwise comparisons but emerges in clusters within subgroups of the input space.

Method: The method,HyFair, is a hybrid approach combining formal symbolic analysis (using SMT and MILP solvers) and randomized search to detect regions of the input space where small changes in protected attributes lead to k-distinct clusters of outcomes. When clusters of unfair outcomes are identified, the method generates interpretable, decision-tree-style explanations to help understand the sources of bias. The symbolic component ensures formal guarantees when no violations are found, while the randomized search helps uncover complex violations that symbolic techniques alone might miss.

Result: The experiments demonstrated that HyFair outperforms existing state-of-the-art methods in fairness verification and local explanation. By identifying discriminatory clusters and providing interpretable explanations, the method reveals significant patterns of algorithmic bias that traditional pairwise fairness measures can miss.

Conclusion: The paper concludes that HyFair offers an effective approach to detecting and explaining systematic algorithmic bias through the concept of discrimination clustering. It emphasizes the importance of moving beyond pairwise fairness checks to uncover broader patterns of algorithmic discrimination, and highlights the hybrid method's ability to provide both formal guarantees and the identification of complex bias patterns that purely symbolic methods cannot address.
...
TL;DR: This paper addresses the lack of a systematic methodology for system testing in the automotive industry as it shifts to software-centric development, offering a curated catalog of requirements and recommendations based on challenges identified through an SLR and industry validation.


<details>
  <summary>Details</summary>
Motivation: The automotive industry's shift to software-centric development is increasing the complexity of embedded systems, creating testing challenges due to legacy tools and a lack of standardized approaches, necessitating a more systematic methodology.

Method: The research combines a systematic literature review (SLR) with insights from industry practitioners to identify recurring challenges in automotive system testing. These are mapped to test case specification techniques and testing tools using PRISMA to evaluate suitability for automotive contexts.

Result: The paper synthesizes nine key challenge areas across the software life cycle and introduces a prioritized catalog of criteria for effective testing, such as model-based planning, interoperable toolchains, and the use of AI and formal methods. The catalog aims to support tool and technique selection and future framework development.

Conclusion: The study provides actionable recommendations for addressing automotive software testing challenges through systematic methods and industry collaboration, offering a foundation for standardizing practice beyond individual expertise.
...
TL;DR: This paper provides a systematic literature mapping of fairness research in software engineering, revealing a focus on methods/algorithms and limited industrial adoption, while highlighting underexplored areas like early-stage bias mitigation and individual fairness.


<details>
  <summary>Details</summary>
Motivation: Existing fairness guidelines are fragmented, research lacks comprehensive categorization, and industrial transferability remains unclear. This study addresses these gaps by mapping research trends, focus areas, and industrial viability.

Method: A classification framework was developed and applied to 95 studies, analyzing three dimensions: research trends, focus (e.g., fairness types), and Technology Readiness Level (TRL).

Result: Fairness research is expanding but skewed toward post-processing and group fairness, with less emphasis on algorithms for bias root causes or early-stage interventions. Most studies have low to medium TRL, indicating limited industrial adoption and academic-industry collaboration.

Conclusion: The study advocates for integrating fairness across the entire software development lifecycle and strengthening academia-industry partnerships to improve practical implementation. It establishes a foundation for future fairness research and applications in software systems.
...
TL;DR: This paper addresses the gap in evaluating Large Language Models (LLMs) as collaborative partners in software engineering by introducing a taxonomy of desired agent behaviors and a Context-Adaptive Behavior (CAB), a framework. It shifts focus from code correctness to human-AI collaborative dynamics.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks for LLMs in software engineering prioritize code correctness, neglecting interactive behaviors critical for effective human-AI collaboration. This limits the evaluation of dynamic, context-aware agent behaviors required in real-world scenarios.

Method: The paper presents two contributions: (1} a taxonomy of four key behavior expectations (Adhere to Standards, Ensure Code Quality, Solve Problems, Collaborate with User) derived from 91 user-defined agent rules, and (2} the CAB framework, analyzed via interviews with 15 expert engineers (Time Horizon axis) and prototyping agent prompt analysis (Type of Work axis).

Result: The taxonomy categorizes desirable agent behaviors, while the CAB framework reveals behavioral shifts along Time Horizon (immediate vs. future needs) and Type of Work (enterprise production vs. rapid prototyping). These contributions provide empirical insights into dynamic context adaptations.

Conclusion: The work establishes a human-centered foundation for evaluating LLM-based agents in software engineering, emphasizing adaptive collaboration over standalone code correctness, and advancing toward true collaborative intelligence in AI development.
...
TL;DR: This paper challenges the validity of traditional software defect prediction (SDP), proposing a change-aware LLM-driven multi-agent framework that addresses label-persistence bias by focusing on code evolution rather than static file snapshots.


<details>
  <summary>Details</summary>
Motivation: Existing SDP models gain inflated accuracy by exploiting persistent file labels across software versions, failing to capture critical defect transitions that require reasoning about code changes over time.

Method: Reformulates SDP as a change-aware task tracking file evolution between successive versions, combined with an LLM-powered multi-agent debate framework to model evolving defect transitions.

Result: Traditional models fail critical defect-transition cases despite high F1 scores, while the proposed approach achieves balanced performance across evolution subsets and improved sensitivity to newly introduced defects in PROMISE projects.

Conclusion: Current SDP evaluation practices are fundamentally flawed by rewarding label-persistence bias; change-aware reasoning frameworks are essential for practical defect prediction that captures code evolution dynamics.
...
TL;DR: The paper explores the impact of LLM coding tools on professional software development and education by analyzing YouTube practitioner discussions, highlighting productivity gains, new challenges, and needed curricular changes.


<details>
  <summary>Details</summary>
Motivation: Prior research focused on individual or educational AI-coding contexts, leaving industrial practitioner experiences underexplored, necessitating an industry-grounded analysis to understand transformative impacts on practice and education.

Method: Qualitative analysis of 57 curated YouTube videos from late 2024 to 2025, including reflection and experience sharing by practitioners, with a filtering and quality assessment to examine LLM-based versus traditional programming practices.

Result: Findings show productivity improvements, reduced entry barriers, emerging risks in code quality, maintainability, and security, as well as shifts in bottlenecks to code review; practitioners also express concerns about foundational problem-solving erosion and unprepared entry-level engineers.

Conclusion: Curriculum must shift to problem-solving, architectural thinking, code review, and project-based LLM-integrated learning to align with industry changes driven by AI-coding practices, ensuring educational preparation for evolving professional demands.
...
TL;DR: The paper introduces a systematic method for constructing controlled natural language requirements using AI, comprising three stages: pattern generalization, corpus generation via attribute reduction, and syntax formalization through grammatical analysis, validated for event-driven temporal requirements.


<details>
  <summary>Details</summary>
Motivation: To address the complexity of aligning formal specifications with natural language requirements by providing a structured, AI-assisted methodology for systematic controlled natural language generation.

Method: Three-stage process: (1)c ompiling generalized natural language patterns from formal templates; (2)p rompt-based AI generation of reduced attribute patterns; (3)f ormalizing syntax via grammatical analysis of generated patterns.

Result: A validated framework for controlled natural language construction, demonstrated through application to event-driven temporal requirements, showing effectiveness in attribute reduction and syntax formalization.

Conclusion: The proposed AI-driven method systematically bridges formal specifications and controlled natural language, enhancing precision in requirements engineering through structured pattern generation and formal analysis.
...
TL;DR: This paper introduces the CoHalLo method for line-level code hallucination localization by probing hidden-layer vectors.


<details>
  <summary>Details</summary>
Motivation: Despite recent efforts in hallucination detection for AI-generated code, there is a lack of fine-grained methods to identify specific hallucinated lines, which limits the efficiency of improving code reliability.

Method: CoHalLo is designed by fine-tuning a hallucination detection model on annotated datasets, then using a probe network to project latent vectors into a syntactic subspace to generate a predicted AST and compare it with the original.

Result: CoHalLo shows strong performance with high Top-n ACCs and better metric scores compared to baseline methods in hallucination line identification.

Conclusion: The proposed method effectively identifies code lines with hallucinations by capturing key syntactic structures, offering a more efficient approach to enhance the reliability of AI-generated code.
...
TL;DR: This paper investigates how blind and low vision software professionals (BLVSPs) adopt and experience Generative AI (GenAI) tools in their work, revealing both productivity benefits and heightened risks like AI hallucinations.


<details>
  <summary>Details</summary>
Motivation: The study addresses the absence of BLVSPs' perspectives in GenAI research and examines how accessibility challenges in software development are influenced by their unique access to and vulnerabilities in using these tools.

Method: Qualitative analysis of 39 semi-structured interviews with BLVSPs, focusing on their GenAI usage patterns, benefits, and challenges.

Result: BLVSPs reported increased productivity and accessibility via GenAI, but also faced enhanced risks from hallucinations, workflow disruptions, and restrictive organizational policies. Balancing these trade-offs became critical.

Conclusion: GenAI adoption for BLVSPs involves higher-stakes decisions due to amplified risks and rewards. The study calls for tailored organizational policies and inclusive AI design to reduce accessibility gaps.
...
TL;DR: Unifico is a new compiler that eliminates runtime stack transformation overhead for heterogeneous ISA systems, enabling efficient execution while targeting x86-64 and ARMv8.


<details>
  <summary>Details</summary>
Motivation: The lack of a ready compiler toolchain to support heterogeneous-ISA targets and the prohibitive costs of runtime stack transformation during ISA migration is the main motivation. Research in this area is hindered by the absence of efficient compilation solutions.

Method: Unifico maintains a uniform stack layout, ABI, and virtual address space across ISAs using LLVM. It generates binaries that function without needing runtime stack conversion and is implemented to target x86-64 and ARMv8 architectures.

Result: Unifico incurs less than 6% (10%) overhead on average for high-end (low-end) processors with minimal runtime cost; its binary size overhead is reduced from ~200% to ~10% compared to Popcorn, and it eliminates stack transformation overhead.

Conclusion: Unifico provides an effective and efficient solution for binary translation in heterogeneous-ISA processor designs, offering a significant improvement over existing tools by eliminating runtime stack transformation overhead and reducing binary size significantly.
...
TL;DR: The paper introduces a dataset of Minimal Intent Aligning Patches and evaluates techniques for estimating potential edits in large language model-generated code, finding that probes with a small model can calibrate well on larger models.


<details>
  <summary>Details</summary>
Motivation: LLMs can generate diverse but sometimes irrelevant code, necessitating supervision and edits. The study aims to develop tools to identify where generated code may need modifications, thereby improving alignment with user intent.

Method: The authors create a dataset of 'Minimal Intent Aligning Patches' by repairing LLM-generated programs with test cases, ensuring accuracy. They evaluate white-box and black-box techniques for probability calibration to predict necessary edits, including an efficient arbitrary-span querying method.

Result: Probes with small supervisor models achieved low calibration error and a Brier Skill Score of about 0.2 for estimating edited lines in code generated by much larger models. These techniques show some generalization to natural language errors when probability scaling is adapted.

Conclusion: The study demonstrates the effectiveness of small models in calibrating edit predictions for larger LLM outputs, suggesting implications for AI oversight and control, particularly in error detection that transcends code into natural language.
...
TL;DR: The study systematically evaluates training data optimization techniques for code generation, finding that data synthesis is most effective for functional correctness and reducing code smells, while combinations of data synthesis with data refactoring yield the best overall performance.


<details>
  <summary>Details</summary>
Motivation: Prior work on training data optimization for code generation lacks systematic evaluation of techniques and their combinations, prompting the need for an empirical analysis to guide future research and deployment.

Method: The authors conduct a large-scale empirical study across three benchmarks and four LLMs, examining five widely-used training data optimization techniques and their pairwise combinations.

Result: Data synthesis outperforms other techniques in functional correctness and code smell reduction, while combinations improve code quality. Data synthesis plus data refactoring achieves the strongest overall performance.

Conclusion: This work provides practical guidance for optimizing training data in code generation, highlighting the effectiveness of data synthesis and the benefits of combining it with data refactoring for both correctness and code quality.
...
TL;DR: Preguss is a modular framework combining static analysis and LLM-based synthesis to automate formal verification, achieving 80.6–88.9\% reduction in human effort for large systems.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with scalability and complex interprocedural specifications in formal verification, limiting automation despite their potential.

Method: Preguss integrates 1) runtime error-guided prioritization of verification units via divide-and-conquer, and 2)LMM-driven interprocedural specification synthesis at unit level.

Result: Outperforms existing LLM-based approaches in automating RTE-freeness verification for programs with $>10^3 representation, reducing manual effort by 80.6\–88.9\%.

Conclusion: Preguss demonstrates superior scalability and effectiveness for industrial-strength verification by combining modular verification strategies with LLM-driven specification synthesis.
...
TL;DR: TL;DR: This empircal study analyzes performance-related pull requests by agents and reveals patterns and insights on their impacts on review outcomes.


<details>
  <summary>Details</summary>
Motivation: LLM-based software engineering is influencing modern software development. While previous studies have looked at correctness, the practical aspect of how agentic AI systems address performance concerns is still unclear.

Method: The research uses LLM-assisted detection and BERTopic-based topic modeling to analyze 52 performance-related topics, grouped into 10 categories, from pull requests generated by AI agents.

Result: AI agents apply optimization across software layers, and the type of optimization affects PR acceptance rates and review times. They optimize more during development than maintenance.

Conclusion: The findings offer empirical insights to evaluate and improve agentic AI systems related to performance optimization and review outcomes.
...
TL;DR: TL;DR: DynaFix is an APR method that uses execution-level dynamic information iteratively to improve patch generation, achieving better repair effectiveness and efficiency on complex bugs.


<details>
  <summary>Details</summary>
Motivation: Current APR methods rely on static analysis or limited dynamic signals, failing to simulate human stepwise debugging and leading to inefficiencies in complex bug repair.

Method: DynaFix iteratively leverages runtime information (variable states, control-flow paths, call stacks) to guide LLMs in generating and refining patches. It re-executes modified programs to gather new execution data for subsequent repair attempts.

Result: DynaFix repairs 186 single-function bugs on Defects4J, 10% more than state-of-the-art methods, including 38 previously unrepaired bugs. It achieves correct patches in 35 attempts max, reducing the patch search space by 70%.

Conclusion: DynaFix improves APR by integrating iterative execution-level feedback, closely mimicking human debugging, leading to higher effectiveness and efficiency in repairing complex bugs.
...
TL;DR: The paper studies how AI coding agents handle energy aspects in software, finding they are somewhat aware but face adoption issues.


<details>
  <summary>Details</summary>
Motivation: Energy concerns in software are rising with data centers, big models, and mobile devices. It's unclear how AI coding agents tackle these.

Method: Analyzing energy-explicit agent-authored pull requests through a public dataset with thematic analysis.

Result: 216 PRs were analyzed, showing most optimizations align with research, but such PRs are less accepted due to maintainability trade-offs.

Conclusion: AI agents exhibit energy awareness but face challenges in having their optimizations accepted due to maintainability issues.
...
TL;DR: The study analyzes software defects in 123 open-source quantum software projects from 2012 to 2024, identifying patterns and factors affecting defect density, with a focus on the impact of automated testing and the evolution of defect trends over time.


<details>
  <summary>Details</summary>
Motivation: Quantum Software Engineering (QSE) is crucial for hybrid quantum-classical systems, but there is limited empirical evidence on how bugs emerge and affect quality in real-world quantum projects. This study aims to fill that gap by analyzing software defects at an ecosystem level.

Method: The study uses a mixed-method approach combining repository mining, static code analysis, issue metadata extraction, and a validated rule-based classification framework. It analyzes 32,296 verified bug reports across eight functional categories in 123 open-source quantum repositories from 2012 to 2024.

Result: The study finds that full-stack libraries and compilers are most defect-prone due to circuit and gate issues, while simulators face measurement and noise modeling errors. Defect densities peaked between 2017 and 2021, and repositories using automated testing detect and resolve more defects faster, with a 60% reduction in expected defect incidence.

Conclusion: This work provides the first large-scale data-driven characterization of quantum software defects, offering empirical guidance to improve testing, documentation, and maintainability in QSE. It highlights the importance of automated testing in reducing defect incidence and discusses the maturation of the quantum software ecosystem over time.
...
TL;DR: MATUS mitigates target noise in bug detection via buggy-code-guided feature slicing, embedding, and similarity comparison, achieving 31 confirmed Linux kernel bugs.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to suppress target noise, as irrelevant statements in targets interfere with similarity-based bug detection.

Method: MATUS extracts feature slices from buggy queries and targets, guides slicing using buggy code as prior knowledge, and compares embedded slices via vector similarity.

Result: 31 unknown Linux kernel bugs detected (11 with CVEs) in experiments on real-world projects, demonstrating precision and efficiency advantages.

Conclusion: Target noise elimination via feature slicing guided by buggy code significantly enhances bug detection accuracy and practical applicability.
...
TL;DR: The paper presents a railway ticket purchase system designed using B/S architecture and Spring Cloud to handle high concurrency with microservices and enhanced security measures for stability and data consistency.


<details>
  <summary>Details</summary>
Motivation: During peak periods, traditional railway ticketing systems face challenges due to insufficient fault tolerance and performance in high concurrency scenarios. The need arises to ensure stability, data consistency, and quick responses to user requests.

Method: The system is built using B/S (Browser/Server) architecture and Spring Cloud, adopting a microservice design principle. It integrates various functionalities like real-time train inquiry, dynamic seat updates, and online seat selection. Security and high performance are achieved through a well-architected design combined with multiple middleware components.

Result: The developed system performs well under high concurrency, as validated by testing the core interfaces. Key functionalities like ticket purchasing, refunds, and the 'add passenger' feature work efficiently, showing high stability and performance.

Conclusion: The proposed railway ticket system effectively addresses the limitations of traditional infrastructure with enhanced stability and performance, making online ticketing more reliable and convenient for users, including those who need assistance.
