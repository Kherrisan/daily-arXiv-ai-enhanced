...
TL;DR: This survey examines security and privacy risks in RIS-enabled practical systems (homes, vehicles), identifies attacks (e.g., eavesdropping) and defenses, and provides tools for future research on secure RIS deployments.


<details>
  <summary>Details</summary>
Motivation: RIS technology shows promise for wireless environments, but practical deployments in smart systems (homes, vehicles, industrial) lack comprehensive security and privacy analysis. Existing research focuses on theoretical benefits, neglecting real-world threat modeling.

Method: The authors analyze scenarios involving systems with/without legitimate RIS and attackers with/without malicious RIS. They categorize threats (e.g., eavesdropping, jamming, spoofing) and defenses (e.g., security algorithms, attacker disruption, unauthorized RIS detection). They also curate open-source tools and datasets for further research.

Result: The survey reveals critical security risks introduced by RIS, such as novel attack vectors, and evaluates defensive strategies. It provides a structured overview of vulnerabilities and mitigation techniques while offering an online repository of resources to advance future work.

Conclusion: The paper emphasizes the importance of understanding and addressing security and privacy challenges in RIS-enabled systems to facilitate secure and reliable deployments in real-world scenarios. It serves as a guide for future research and development.
...
TL;DR: This paper introduces PHANTOM, an adversarial variational framework for creating realistic synthetic cyberattack data to improve intrusion detection systems.


<details>
  <summary>Details</summary>
Motivation: The lack of real-world cyberattack data limits the development of effective intrusion detection systems, prompting the need for synthetic data generation without compromising privacy.

Method: The PHANTOM framework employs a dual-path VAE-GAN architecture with progressive training and domain-specific feature matching to retain attack semantics accurately.

Result: Models trained using PHANTOM data achieved 98% weighted accuracy on real attacks, and statistical analyses showed that the synthetic data preserved the true distributions and diversity, though rare attack types were underrepresented.

Conclusion: PHANTOM successfully advances the ability to generate synthetic attack data, providing a privacy-preserving solution to improve intrusion detection models, albeit with challenges in handling rare attack cases due to class imbalance.
...
TL;DR: This paper examines backdoor attacks in generative data pipelines, particularly diffusion models, which can reproduce backdoor triggers during synthetic data generation, leading to security risks in downstream models even under clean-label attack scenarios.


<details>
  <summary>Details</summary>
Motivation: The increasing use of diffusion models for synthetic data augmentation, while cost-effective, introduces new security risks due to potential backdoor propagation. The study aims to highlight and address these concerns.

Method: Researchers analyze how open-source diffusion models act as carriers of backdoors by memorizing and reproducing triggers during generation. They observe an Early-Stage Trigger Manifestation (ESTM), where backdoor patterns become more explicit in early, high-noise stages of the generation process.

Result: The research reveals that diffusion models, due to their strong distribution-fitting ability, can produce backdoor-infected synthetic data. This poses a significant security threat, especially under clean-label attack conditions, with minimal impact on data utility.

Conclusion: The study highlights a critical threat in generative data pipelines and begins to provide insights into mitigating backdoor risks in synthetic data generation.
...
TL;DR: This paper introduces the Variable Record Table (VRT), a hardware-assisted framework that unifies three key security mechanisms—spatial memory safety, back-edge control-flow integrity, and speculative execution attack detection—into a single structure, offering comprehensive protection with minimal performance overhead.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by the need to address multiple contemporary security threats (e.g., buffer overflows, CFI violations, speculative execution attacks) collectively rather than individually. Existing solutions often suffer from performance overhead or leave security gaps, prompting the development of a unified framework like VRT.

Method: The proposed solution, VRT, dynamically constructs a protection table through runtime instrumentation of system instructions. This table tracks memory addresses, bounds metadata, and back-edge control-flow signatures. The hardware-assisted framework leverages this table to enforce spatial memory safety, CFI, and detect speculative execution attacks simultaneously.

Result: The evaluation of VRT on the MiBench and SPEC benchmarks demonstrates its effectiveness in detecting all tested attack variants, with the notable results including zero additional instruction overhead and minimal memory and power overhead (25KB memory usage for 512 entries, and less than 8% area overhead and 11.65 μW power overhead).

Conclusion: The VRT provides an efficient and comprehensive security framework that integrates three critical protections in a unified hardware structure. The evaluation results confirm minimal performance impact while achieving full attack detection, positioning VRT as a promising approach for securing modern computing systems against a diverse range of security threats.
...
TL;DR: The paper introduces RAMBO, a hardware-induced bit-flip attack framework targeting Mamba-based State-space models (SSMs), demonstrating their vulnerability to single-bit flips that severely degrade performance.


<details>
  <summary>Details</summary>
Motivation: SSMs, such as Mamba, are gaining popularity due to their efficiency and scalability. However, their susceptibility to software- and hardware-level threats, particularly hardware-induced bit-flip attacks (BFAs), remains underexplored, necessitating an investigation into their robustness.

Method: The authors propose RAMBO, the first BFA framework designed for Mamba-based architectures. They conduct experiments on the Mamba-1.4b model using the LAMBADA benchmark to evaluate the impact of bit-flip attacks on model accuracy and perplexity.

Result: Experiments show that flipping a single critical bit can reduce accuracy from 74.64% to 0% and increase perplexity from 18.94 to 3.75 x 10^6, highlighting the severe fragility of SSMs to such attacks.

Conclusion: The paper concludes that SSMs, despite their efficiency and scalability, are highly vulnerable to hardware-induced bit-flip attacks. The findings underscore the need for robust security measures to protect these models in real-world applications.
...
TL;DR: This study evaluates and optimizes various machine learning models on the KDD CUP 1999 dataset to enhance NIDS accuracy, achieving 99.12% with SVM after hyperparameter tuning.


<details>
  <summary>Details</summary>
Motivation: As threats grow increasingly sophisticated, improving NIDS with ML methods to detect emerging threats and anomalies is crucial for network security.

Method: A variety of ML models were assessed using the KDD CUP 1999 intrusion dataset before and after optimization with hyperparameter tuning techniques. Recursive Feature Elimination was used for feature selection, and tenfold cross-validation was used for validation.

Result: SVM, after optimization, achieved the highest accuracy of 99.12% with a low false alarm rate of 0.0091, surpassing all other tested models and its default configuration. All models were validated using tenfold cross-validation.

Conclusion: ML classifiers, particularly SVM with optimized parameters and feature selection, significantly improve NIDS accuracy, demonstrating adaptability and reliability for network intrusion detection.
...
TL;DR: The project introduces a detection framework combining a complete corpus of Microsoft Graph permissions, consistent LLM risk scoring, and real-time detection of malicious OAuth consent activity.


<details>
  <summary>Details</summary>
Motivation: OAuth consent is a frequent attack vector exploited through phishing and other deceptive means, necessitating a comprehensive detection solution.

Method: The framework creates a comprehensive Microsoft Graph permission corpus, employs an LLM to generate consistent risk scores, and integrates these into a real-time detection engine.

Result: The proposed framework effectively identifies malicious OAuth activity through the integration of LLM-based risk assessments within a real-time detection system.

Conclusion: This unified approach enhances the detection and mitigation of OAuth-related security threats by leveraging both structured data and advanced AI techniques.
...
TL;DR: The paper presents an approach to optimize safety guardrails for LLMs by treating them as hyperparameters. It compares grid search and Optuna optimization over prompt combinations and filter modes on three benchmarks.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by the common use of ad-hoc, brittle safety guardrails in LLM deployments and aims to find a systematic optimization approach given constraints on model weights modification.

Method: The authors wrap Mistral-7B-Instruct with modular jailbreak and malware prompts plus a harmfulness classifier, evaluating configurations on three benchmarks using attack success rates, harmful response rates, and latency. They compare grid search and Optuna's black-box optimization.

Result: Optuna reliably finds best configurations with 10x fewer evaluations and 8x less time than grid search, maintaining good performance on multiple metrics.

Conclusion: Viewing safety guardrails as optimizable hyperparameters provides a feasible way to strengthen LLM safety under computational and time constraints, suggesting this optimization approach should be considered.
...
TL;DR: The paper introduces XAMT, a framework for covert memory tampering in heterogeneous MAS by using bilevel optimization to generate stealthy, minimal perturbations.


<details>
  <summary>Details</summary>
Motivation: The growing dependence on complex MAS in safety-critical domains requires rigorous adversarial robustness assessment, but current systems have shared vulnerabilities in centralized memory components.

Method: XAMT formulates attack generation as a bilevel optimization problem, with the Upper Level focusing on covertness by minimizing perturbations and the Lower Level maximizing behavior divergence toward an adversarial target. It provides mathematical instantiations for CTDE MARL algorithms and RAG-based LLM agents.

Result: XAMT demonstrates effectiveness with sub-percent poison rates in MARL (≤1%) and RAG (≤0.1%) using SMAC and SafeRAG benchmarks, showing it can create stealthy perturbations that evade detection heuristics.

Conclusion: XAMT defines a new class of training-time threats for secure MAS development, highlighting the need for intrinsic safety mechanisms rather than perimeter-based detection.
...
TL;DR: The paper investigates how cybersecurity practices, data protection, and corporate reputation are interconnected during digital transformation, based on a survey of Polish enterprises.


<details>
  <summary>Details</summary>
Motivation: The motivation of the paper is to understand how effective cybersecurity management and data protection strategies influence a company's reputation, especially amidst the challenges of digital transformation. This is important as cybersecurity threats increasingly affect trust and stability in organizations.

Method: The research employed a quantitative design, using an online diagnostic survey to collect data from enterprises in Poland across different industries. The survey focused on cybersecurity strategies, technical safeguards, incident response practices, and adoption of international standards.

Result: The results show that while most organizations have formal cybersecurity practices and invest in awareness programs, 75% experienced cybersecurity incidents in the last year. These incidents led to reputational damage and loss of customer trust, along with operational and financial impacts. Cybersecurity is increasingly seen as a strategic investment rather than just compliance.

Conclusion: The paper concludes that effective cybersecurity governance must be integrated with corporate communication and reputation management. Data protection is highlighted as a vital component of digital trust and organizational resilience during the digital age.
...
TL;DR: This study examines India's DPDP Act through AI forensics challenges, revealing regulatory gaps in addressing adversarial AI threats like deepfakes. It proposes XAI-integrated frameworks and legislative updates to balance privacy, forensic integrity, and AI accountability.


<details>
  <summary>Details</summary>
Motivation: The integration of generative AI into digital systems creates a critical gap in India's computational forensics framework, particularly regarding the DPDP Act's compatibility with anti-forensics, deepfakes, and AI's dual role as both forensic tool and cyber threat vector.

Method: Employed doctrinal legal analysis, synthesizing statutory evaluation of India's Digital Personal Data Protection Act, 2023 with global ethical frameworks (IEEE, EU) to assess regulatory preparedness for adversarial AI threats.

Result: Identified tensions between data minimization principles and forensic retention needs, vulnerabilities in ML (data poisoning/bias), and inadequate legal definitions for AI-driven 'tool crimes.' Demonstrated gaps in current statutes to address synthetic media risks and algorithmic accountability.

Conclusion: The paper advocates for a 'human-centric' forensic model using explainable AI (XAI) to reconcile India's DPDP Act with forensic needs, while urging legislative alignment with international standards to address synthetic media risks and AI-driven crimes.
...
TL;DR: Analyzing 415 zero-day vulnerabilities from ZDI (2024), this study examines disclosure trends, severity patterns, and machine learning approaches to predict vulnerability severity, aiming to enhance cybersecurity defenses against zero-day threats.


<details>
  <summary>Details</summary>
Motivation: Zero-day vulnerabilities pose critical threats due to their exploitability before patches are available. This work aims to improve patch prioritization, vulnerability management, and organizational readiness by understanding disclosure trends and severity indicators.

Method: The research analyzed 415 ZDI vulnerabilities from Jan-Apr 2024, using CVSS scores, publication dates, and descriptions. It compared classical machine learning and deep learning models for severity classification, leveraging structured metadata and unstructured textual data.

Result: The study identifies trends in zero-day disclosures, vendor-specific severity distributions, and key vulnerability characteristics correlated with high severity. Predictive modeling comparisons reveal effective approaches for severity classification, supporting better mitigation strategies.

Conclusion: The study concludes that analyzing zero-day vulnerability disclosures aids in understanding trends and severity distributions, which enhances patch prioritization strategies and organizational preparedness against zero-day threats.
...
TL;DR: The paper proposes a system to reduce bandwidth and latency in AR/VR streaming by downsampled point cloud content and partial encryption, with client-side ML upscaling.


<details>
  <summary>Details</summary>
Motivation: Immersive formats like 360° and 6DoF point cloud videos demand high bandwidth and low latency, challenging real-time AR/VR streaming.

Method: The system downsamples point cloud content at the origin server and applies partial encryption; clients decrypt and upscale using an ML-based super-resolution model.

Result: Evaluation shows nearly linear reduction in bandwidth/latency and encryption/decryption overhead with lower downsampled resolutions, with effective reconstruction of full-resolution point clouds.

Conclusion: The proposed system effectively reduces bandwidth consumption and encryption/decryption delay while maintaining quality through ML super-resolution, making real-time AR/VR streaming more feasible.
...
TL;DR: This paper introduces VET (Verifiable Execution Traces), a framework for achieving host-independent authentication and autonomy for large language model (LLM) agents. It proposes the Agent Identity Document (AID) and evaluates implementation using Web Proofs and TEE Proxy for different scenarios.


<details>
  <summary>Details</summary>
Motivation: The paper emphasizes the need for trust in autonomous agents operating in high-stakes domains like financial management and governance, even when the host infrastructure may not be reliable. Current LLM agents, despite their capabilities, still rely on the host infrastructure, leading to vulnerabilities in autonomy and security.

Method: The paper proposes VET (Verifiable Execution Traces), a formal framework for authentication and autonomy. A key element is the Agent Identity Document (AID), which outlines the agent's configuration and verification systems. VET is designed to be compositional, meaning it supports various verification mechanisms, such as trusted hardware, succinct cryptographic proofs, and Web Proofs. The paper also evaluates an implementation with realistic workloads and use cases like a trading agent.

Result: For secret-bearing API calls, Web Proofs are found to be a practical option with an overhead typically under 3x compared to regular API calls. For public API calls, a TEE Proxy offers lower overhead. The paper demonstrates the deployment of a verifiable trading agent that successfully produces and composes proofs for actions.

Conclusion: The paper concludes that VET proves practical, host-independent authentication is achievable using existing technology, opening possibilities for future systems to realize full autonomy in agent operations across unreliable environments.
...
TL;DR: The paper introduces Private Virtual Tree Networks (PVTNs), a security-enhanced extension of the VIRGO network, which allows scalable and secure organization of distributed systems by encrypting join requests with managers' public keys and using signed delegation certificates for membership. The approach limits cryptographic information sharing to direct manager-member relationships, ensuring privacy and dynamic scalability without global PKI.


<details>
  <summary>Details</summary>
Motivation: The need to establish scalable, secure, and privately organized virtual hierarchical networks for distributed computing systems, where traditional infrastructure like global PKI is not feasible or efficient.

Method: The method leverages the existing structure of the VIRGO network but integrates cryptographic tools for confidentiality and authentication. Join requests are encrypted with manager's public keys, and membership is authorized via delegation certificates signed by the manager. Public keys are treated as organizational secrets, shared only within direct relationships, creating private virtual trees.

Result: The paper presents a system model, protocols, security analysis, and rationale, showing that PVTNs achieve scalable, dynamically manageable virtual tree structures with strong security and privacy without relying on global PKI.

Conclusion: The proposed PVTN extension to VIRGO effectively provides scalable and secure hierarchical overlay networks, suitable for real-world organizational hierarchies, by enforcing cryptographic privacy and dynamic management within direct relationships.
...
TL;DR: This paper analyzes the security of the ISO 15118 DC fast charging standard, reveals a vulnerability in its plug and charge functionality allowing one vehicle to be charged while a second is billed, and proposes a more secure and efficient authentication scheme.


<details>
  <summary>Details</summary>
Motivation: The rise of electric vehicles necessitates minimizing charging times. The paper aims to address the insufficient security measures in the ISO 15118 standard, which is prevalent in Europe, to ensure secure and accurate billing for charging.

Method: The authors methodically examine all security controls defined in ISO 15118, identify their shortcomings, demonstrate a new plug and charge vulnerability, and propose an alternative authentication scheme for improvement.

Result: The researchers successfully documented and demonstrated a vulnerability in ISO 15118's plug and charge feature, enabling unauthorized charging at a victim's billing expense. They also introduced a novel authentication method to enhance security and efficiency.

Conclusion: The study highlights the critical need for updated security measures in the ISO 15118 standard to prevent exploitation. Implementing the proposed authentication scheme is recommended to achieve secure and future-proof fast charging.
...
TL;DR: ContextLeak is introduced to measure privacy leakage in ICL methods, revealing flaws in existing approaches that either leak data or worsen performance while providing theoretical validation through ε correlation.


<details>
  <summary>Details</summary>
Motivation: Despite existing privacy-preserving methods for in-context learning (ICL), no systematic framework exists to audit their effectiveness, making it difficult to assess whether sensitive data leaks while maintaining model utility.

Method: We developed ContextLeak, a framework using canary insertion (embedding unique tokens in exemplars) and targeted querying to empirically measure worst-case information leakage in privacy-preserving ICL methods.

Result: ContextLeak closely correlates with theoretical privacy budgets (ε) and reliably detects leakage across methods. Existing ICL privacy techniques exhibit poor privacy-utility trade-offs, with some failing to prevent leakage and others drastically reducing task performance.

Conclusion: ContextLeak demonstrates the need for improved privacy-preserving methods in ICL, as current techniques either leak sensitive data or degrade utility. The framework provides a reliable tool for auditing such methods and their privacy-utility trade-offs.
...
TL;DR: BondMM-A is proposed to enhance DeFi with multi-maturity lending through extended mathematical invariants, improving interest rate stability and capital efficiency.


<details>
  <summary>Details</summary>
Motivation: DeFi fixed-income lending AMMs face time-related complexities and are limited to single-maturity support. Multi-maturity solutions are lacking.

Method: Generalizes BondMM’s mathematical invariants for arbitrary maturities, designing BondMM-A to integrate multiple maturity instruments under one contract.

Result: BondMM-A demonstrates superior interest rate stability, financial robustness, and capital efficiency via experiments across varied maturity scenarios.

Conclusion: BondMM-A effectively generalizes BondMM for multi-maturity lending, offering a robust and efficient DeFi solution for fixed-income markets.
...
TL;DR: Autoencoder-based denoising mitigates adversarial attacks on object detectors, offering partial performance recovery without model retraining.


<details>
  <summary>Details</summary>
Motivation: Deep learning object detection models face vulnerabilities in real-world applications due to adversarial examples, necessitating lightweight, retraining-free defenses.

Method: An adversarial attack using Perlin noise is applied to vehicle images from the COCO dataset. A single-layer convolutional autoencoder is then used to remove perturbations, followed by YOLOv5-based detection performance evaluation.

Result: Adversarial attacks reduce(bbox mAP) from 0.2890 to 0.1640 (-43.3%). Autoencoder defense recovers it to 0.1700 (+3.7%) with a 10.8% improvement in(bbox mAP@50) from 0.2780 to 0.3080.

Conclusion: The study demonstrates that an autoencoder-based denoising defense can partially restore object detection performance under adversarial attacks without requiring model retraining, highlighting its practicality for real-world deployment.
...
TL;DR: DualGuard is a new LLM watermarking algorithm that can defend against both paraphrase and piggyback spoofing attacks, enhancing the reliability and trustworthiness of watermark detection.


<details>
  <summary>Details</summary>
Motivation: The study is motivated by the need to address the limitations of existing LLM watermarking algorithms, which focus on paraphrase attacks but neglect piggyback spoofing attacks. This neglect leaves models vulnerable to harmful content injection and attribution issues.

Method: The method introduces DualGuard, an adaptive dual-stream watermarking algorithm that dynamically injects two complementary watermark signals based on the semantic content of the text. This approach is designed to both detect and trace spoofing attacks.

Result: Extensive experiments showed that DualGuard offers excellent detectability, robustness, traceability, and maintains high text quality across multiple datasets and language models.

Conclusion: DualGuard represents a significant advancement in LLM watermarking by providing effective defenses against both paraphrase and spoofing attacks, thereby enhancing the reliability and trustworthiness of watermarking techniques in practical applications.
...
TL;DR: Romance-baiting scams, often run by criminal organizations, use prolonged text conversations to pressure victims into fraudulent cryptocurrency investments. The paper explores the use of Large Language Models (LLMs) in automating these scams, revealing that LLMs are already widely deployed and outperform human operators in eliciting trust and compliance. Additionally, common safety filters fail to detect these scams, indicating a pressing need for better defenses.


<details>
  <summary>Details</summary>
Motivation: The paper highlights the global threat posed by romance-baiting scams, driven by the need to understand the role of LLMs in their automation and the potential for large-scale harm if such automation is not addressed. The authors aim to assess the extent to which LLMs are employed in these scams and evaluate the effectiveness of current safety measures.

Method: The authors conduct an analysis using interviews with 145 insiders and five scam victims, a long-term blinded conversation study comparing LLM scam agents to human operators, and an evaluation of commercial safety filters for their ability to detect romance-baiting dialogues.

Result: The study finds that 87% of scam labor involves systematized tasks amenable to LLM automation. LLM agents were more effective than humans in building trust and prompting compliance (46% vs. 18%). Moreover, popular safety filters failed to detect any of the romance-baiting dialogues.

Conclusion: Romance-baiting scams are highly suited for automation using LLMs, and current safety filters are ineffective at detecting them, suggesting a critical gap in defenses that needs to be addressed to prevent the scaling of such fraudulent practices.
...
TL;DR: This paper proposes a framework to evaluate the effectiveness of privacy quantification methods for synthetic data by inserting controlled risks.


<details>
  <summary>Details</summary>
Motivation: Synthetic data is a promising PET; however, the lack of clear privacy metrics makes it difficult for practitioners to assess privacy protection.

Method: The paper evaluates and benchmarks privacy measures using no-box threat models and deliberate risk insertion on public datasets.

Result: Analysis of current privacy quantification methods reveals insights into their effectiveness under controlled risk scenarios.

Conclusion: The proposed framework provides a structured approach to examining and comparing tabular synthetic data privacy through empirical evaluation.
...
TL;DR: This paper introduces ICP-MIA, a new black-box framework for membership inference attacks (MIAs) on fine-tuned large language models (LLMs), leveraging optimization dynamics and a training-free method called In-Context Probing (ICP). Experimental results show improved effectiveness over prior methods.


<details>
  <summary>Details</summary>
Motivation: Prior techniques for MIAs are flawed as they rely on signals entangled with content properties like difficulty or rarity, leading to poor generalization and low signal-to-noise ratios. This paper aims to provide a more reliable and generalizable MIA framework for privacy auditing.

Method: The authors propose ICP-MIA, which uses the Optimization Gap as a signal, derived from the loss-reduction potential of member vs. non-member samples. In-Context Probing estimates this gap using two strategies: reference-data-based and self-perturbation-based, without requiring training.

Result: ICP-MIA significantly outperforms existing black-box MIAs, especially in low false positive rate scenarios, across three tasks and multiple LLMs. Effects of reference data alignment, model type, and training schedules on attack performance are demonstrated.

Conclusion: ICP-MIA is a practical and theoretically supported framework for auditing privacy risks in deployed LLMs, offering improved performance through optimization dynamics and context-based probing strategies.
...
### [25] [XBIDetective: Leveraging Vision Language Models for Identifying Cross-Browser Visual Inconsistencies](https://arxiv.org/abs/2512.15804)
*Balreet Grewal,James Graham,Jeff Muizelaar,Jan Honza Odvarko,Suhaib Mujahid,Marco Castelluccio,Cor-Paul Bezemer*

Main category: cs.SE

TL;DR: this study introduces XBIDetective, a tool using vision language models to detect cross-browser inconsistencies with 79% accuracy.


<details>
  <summary>Details</summary>
Motivation: browser rendering bugs are hard to detect due to specific triggering conditions, and existing XBI detection methods struggle with dynamic elements.

Method: the tool captures website screenshots in mozilla firefox and google chrome and analyzes them with a vision language model to identify XBIs.

Result: XBI detectieve achieved 79% accuracy in detecting XBIs and 84% and 85% accuracy in detecting dynamic elements and advertisements, respectively, when using a fine-tuned VLM on 1,052 websites.

Conclusion: XBI detectieve can serve multiple practical purposes, including automated regression testing and XBI bug report triaging, and its effectiveness demonstrates the potential of VLMs in improving browser rendering error detection.
...
### [26] [CodeMem: Architecting Reproducible Agents via Dynamic MCP and Procedural Memory](https://arxiv.org/abs/2512.15813)
*Nishant Gaurav,Adit Akarsh,Tejas Ravishankar,Manoj Bajaj*

Main category: cs.SE

TL;DR: CodeMem addresses AI agent instability in workflows by introducing procedural memory through code, achieving deterministic reliability for repetitive tasks.


<details>
  <summary>Details</summary>
Motivation: Current AI agents fail to handle repetitive tasks reliably due to limited action space, context inefficiency, and probabilistic instability. Platforms like n8n and Zapier demonstrate superior reliability for such tasks, highlighting the need for deterministic workflow execution in AI agents.

Method: The proposed method, CodeMem, implements procedural memory via code, allowing reusable agentic workflows to be built and executed with deterministic reliability. This is achieved by encoding procedural knowledge into code, which standardizes agent behavior across identical tasks.

Result: CodeMem enables consistent task execution by eliminating probabilistic variation in agent behaviors, allowing agents to follow deterministic trajectories for the same task in identical environments.

Conclusion: The paper concludes that CodeMem's architecture, which integrates procedural memory via code, addresses the probabilistic instability of current tool-using AI agents, enabling deterministic and reliable agentic workflows akin to those on n8n and Zapier.
...
### [27] [OLAF: Towards Robust LLM-Based Annotation Framework in Empirical Software Engineering](https://arxiv.org/abs/2512.15979)
*Mia Mohammad Imran,Tarannum Shaila Zaman*

Main category: cs.SE

TL;DR: The paper introduces the OLAF framework to improve the reliability, reproducibility, and transparency of LLM-based annotations in ESE research.


<details>
  <summary>Details</summary>
Motivation: LLM-based annotations in empirical software engineering lack standardized measures for reliability, calibration, and drift, with insufficient configuration details.

Method: The paper proposes OLAF, a conceptual framework that organizes key constructs like reliability, calibration, drift, consensus, aggregation, and transparency.

Result: OLAF provides a structured approach for treating LLM-based annotation as a measurement process, facilitating methodological discussions and future empirical work.

Conclusion: The paper advocates for methodological improvements in LLM-based annotation within ESE research through the OLAF framework to enhance transparency and reproducibility.
...
### [28] [Embedding Software Intent: Lightweight Java Module Recovery](https://arxiv.org/abs/2512.15980)
*Yirui He,Yuqi Huai,Xingyu Chen,Joshua Garcia*

Main category: cs.SE

TL;DR: ClassLAR uses language models and class name semantics to efficiently recover JPMS modules from monolithic Java systems, outperforming existing approaches in speed and accuracy.


<details>
  <summary>Details</summary>
Motivation: Modern Java systems require language-level modularization via JPMS, but existing architecture recovery techniques are ineffective for monolithic projects, creating a critical gap in scalability solutions.

Method: ClassLAR employs language models to extract semantic information from package and class names, enabling architectural recovery through structural and functional intent analysis.

Result: ClassLAR achieved 3.99-10.50× faster execution and superior architectural similarity metrics across 20 Java projects compared to state-of-the-art methods.

Conclusion: ClassLAR presents an effective solution for modularizing monolithic Java projects with JPMS, offering superior accuracy and speed compared to existing techniques.
...
### [29] [LLM4Perf: Large Language Models Are Effective Samplers for Multi-Objective Performance Modeling (Copy)](https://arxiv.org/abs/2512.16070)
...
### [30] [Analysis of Design Patterns and Benchmark Practices in Apache Kafka Event-Streaming Systems](https://arxiv.org/abs/2512.16146)
*Muzeeb Mohammad*

Main category: cs.SE

TL;DR: This paper synthesizes 42 studies on Apache Kafka, identifying nine design patterns and highlighting issues in current research reproducibility and benchmarking.


<details>
  <summary>Details</summary>
Motivation: Despite Kafka's maturity, there is fragmented research on design patterns and benchmarking methodologies, limiting reuse and practical application.

Method: The study analyzes 42 peer-reviewed papers from 2015-2025, categorizing design patterns, evaluating benchmarking practices, and identifying trends in configurations and evaluations.

Result: Nine recurring Kafka design patterns were identified. Findings show significant inconsistencies in study reproducibility, evaluation rigor, and configuration documentation.

Conclusion: The paper provides a unified taxonomy, pattern benchmark matrix, and decision heuristics to guide architects and researchers in building reliable, high-performance Kafka systems.
...
### [31] [Beyond Blind Spots: Analytic Hints for Mitigating LLM-Based Evaluation Pitfalls](https://arxiv.org/abs/2512.16272)
*Ora Nova Fandina,Eitan Farchi,Shmulik Froimovich,Raviv Gal,Wesam Ibraheem,Rami Katan,Alice Podolsky*

Main category: cs.SE

TL;DR: This paper addresses domain-specific blind spots in Large Language Models (LMMs, 'LaaJs') used as code evaluation judges. It proposes a hybrid approach combining analytic tools with LMM prompts to improve error detection in COBOL legacy code modernization, achieving up to 94% error coverage.


<details>
  <summary>Details</summary>
Motivation: LMMs acting as code judges miss domain-critical errors in industrial tasks like COBOL modernization, raising reliability concerns for critical evaluation pipelines

Method: 1. Constructed a taxonomy of domain-specific COBOL issues via expert analysis 2. Developed a lightweight static checker for 30 domain issues 3. Created injection prompts to integrate checker outputs as 'analytic hints' into LMM judging workflows

Result: LaaJ alone detected 45%, but LaaJ with hints achieved 94%. Hybrid approach delivered both higher coverage and richer explanations across 4 production judges and 100 programs

Conclusion: Analytic-LLM hybrids substantially enhance evaluation reliability for domain-specific code tasks. Released dataset and prompts enable replicable evaluation of code generation pipelines.
...
### [32] [Using a Sledgehammer to Crack a Nut? Revisiting Automated Compiler Fault Isolation](https://arxiv.org/abs/2512.16335)
*Yibiao Yang,Qingyang Li,Maolin Sun,Jiangchang Wu,Yuming Zhou*

Main category: cs.SE

TL;DR: This paper compares a BIC-based strategy (Basic) for compiler bug debugging against SBFL techniques using 60 GCC and 60 LLVM bugs and finds Basic to be effective and often superior in identifying faulty files.


<details>
  <summary>Details</summary>
Motivation: The study addresses the gap between the effectiveness of sophisticated SBFL techniques and the BIC-based strategies commonly used in practical compiler debugging. Developers frequently rely on BIC-based techniques, but their comparative performance remains understudied.

Method: The method involves implementing the Basic strategy, which identifies BICs using binary search and marks modified files as potentially faulty. The paper benchmarks Basic against SBFL techniques using GCC and LLVM bugs, with metrics to evaluate performance in ranking faulty files.

Result: The results show that Basic performs as well as or better than SBFL techniques, especially on Top-1 and Top-5 ranking metrics, indicating its practical effectiveness in compiler debugging for critical faults.

Conclusion: Basic is recommended as a baseline for future research on compiler fault isolation due to its strong performance and alignment with practical developer workflows.
...
### [33] [An Empirical Study of the Realism of Mutants in Deep Learning](https://arxiv.org/abs/2512.16741)
*Zaheed Ahmed,Philip Makedonski,Jens Grabowski*

Main category: cs.SE

TL;DR: The study compares pre-training and post-training mutation approaches in deep learning regarding realism, finding pre-training mutants more realistic but computationally expensive.


<details>
  <summary>Details</summary>
Motivation: To assess how well mutation analysis techniques in deep learning reflect real faults, focusing on the realism of pre-training versus post-training mutation methods.

Method: An empirical comparison using a statistical framework with four public bug datasets and state-of-the-art mutation tools for both pre-training and post-training approaches.

Result: Pre-training mutants show stronger coupling and higher behavioral similarity to real faults than post-training mutants, but pre-training is more computationally costly.

Conclusion: Pre-training mutations are more realistic but require optimization; there is a need for better post-training operators to match this realism efficiently.
...
### [34] [Inside Out: Uncovering How Comment Internalization Steers LLMs for Better or Worse](https://arxiv.org/abs/2512.16790)
*Aaron Imani,Mohammad Moshirpour,Iftekhar Ahmed*

Main category: cs.SE

TL;DR: This work reveals how LLMs internally represent code comments as distinct concepts, showing task-dependent reliance that can drastically impact performance (-90% to +67%). Code summarization triggers the strongest comment concept activation.


<details>
  <summary>Details</summary>
Motivation: While LLMs rely on code comments for software engineering tasks, the mechanisms and impact of this reliance on model performance remain poorly understood.

Method: The researchers conducted a concept-level interpretability study using Concept Activation Vectors (CAVs) on three SE tasks (code completion, translation, and refinement), systematically activating/deactivating comment subtypes in the embedding space. A controlled experiment measured comment concept activation across 10 SE tasks.

Result: Performance shifts ranged from -90% to +67% when manipulating comment concepts. Code summarization triggered the strongest comment concept activation, while code completion showed the weakest sensitivity.

Conclusion: The study opens new directions for building SE tools and models that reason about and manipulate internal concept representations, moving beyond surface-level input.
...
### [35] [Toward Systematic Counterfactual Fairness Evaluation of Large Language Models: The CAFFE Framework](https://arxiv.org/abs/2512.16816)
*Alessandra Parziale,Gianmario Voria,Valeria Pontillo,Gemma Catolino,Andrea De Lucia,Fabio Palomba*

Main category: cs.SE

TL;DR: The paper presents CAFFE, a new framework for testing counterfactual fairness in LLMs, which outperforms previous metamorphic testing methods in bias coverage and reliability.


<details>
  <summary>Details</summary>
Motivation: Existing methods of testing fairness in Large Language Models (LLMs) may not be sufficient, prompting the need for an improved approach like CAFFE that ensures more comprehensive and reliable fairness testing.

Method: The research introduces CAFFE (Counterfactual Assessment Framework for Fairness Evaluation), a structured framework consisting of test case formalization, targeted data generation, and semantic similarity-based evaluation. CAFFE is designed using principles from non-functional testing

Result: Experiments conducted on three distinct architectural families of LLMs showed that CAFFE provides broader bias coverage and more reliable detection of unfair behavior compared to conventional metamorphic testing methods.

Conclusion: The paper emphasizes the necessity of a robust framework like CAFFE to effectively evaluate fairness in LLMs and recommends its adoption to enhance fairness in AI systems.
