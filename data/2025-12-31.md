...
TL;DR: This paper evaluates monitoring frameworks for AI agents in complex deployments, introducing trade-offs between safety and latency through case studies of three monitoring approaches.


<details>
  <summary>Details</summary>
Motivation: Existing work on AI control monitoring lacks scalability for real-world systems involving parallel agents, latency constraints, incremental attacks, and difficulty distinguishing scheming behavior from harmless actions.

Method: The authors analyze three monitoring approaches (synchronous, semi-synchronous, asynchronous) using a high-level safety case sketch and four case studies to explore their trade-offs.

Result: Identifies three core challenges (oversight, latency, recovery) and demonstrates how different monitoring protocols address these through analysis of four potential future AI deployment scenarios.

Conclusion: The paper provides a framework for analyzing monitoring protocols in real-world AI deployments, highlighting the importance of addressing oversight, latency, and recovery challenges to balance safety and operational efficiency.
...
TL;DR: The paper proposes a statistical risk model for timing leakage in lattice-based post-quantum cryptographic schemes, analyzing distinguishability under various environmental scenarios and leakage models to quantify and compare security risks across Kyber, Saber, and Frodo KEM families.


<details>
  <summary>Details</summary>
Motivation: Timing side-channels in post-quantum cryptography require urgent attention due to scheme-specific vulnerabilities and environmental noise in timing measurements, lacking standardized risk evaluation frameworks.

Method: A scenario-based statistical risk model is developed, synthesizing timing traces for two secret classes under idle, jitter, and loaded scenarios. Leakage is quantified using Welch's t-test, KS distance, Cliff's delta, mutual information, and distribution overlap, combined for consistent risk scoring.

Result: Idle scenarios show highest timing distinguishability, while jitter and loaded conditions reduce it via variance and overlap increases. Cache-index and branch-style leakages yield highest risk signals. Faster schemes (e.g., Kyber) exhibit higher peak risks under similar leakage assumptions.

Conclusion: The model enables reproducible early-stage risk comparisons for lattice-based KEM designs, highlighting the impact of execution environments and leakage models on timing security, with practical implications for mitigating secret-dependent variability in post-quantum implementations.
...
TL;DR: New multi-vulnerability benchmark reveals LLMs' performance plummets under code complexity, with Python/JS posing unique challenges; highlights urgent need for better multi-task code security models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks oversimplify real-world software complexity by focusing on isolated vulnerabilities or function-level analysis. LLMs exhibit unquantified 'count bias' and 'selection bias' in multi-label code security tasks, motivating systematic evaluation across diverse, multi-vulnerability scenarios.

Method: Authors created a multi-vulnerability benchmark with 40,000 files across C/C++/Python/JS, injecting controlled vulnerability counts (1-9) into 7.5k-10k token code samples. Evaluated five advanced LLMs (GPT-4o-mini, Llama-3.3-70B, Qwen-2.5) across varying vulnerability densities.

Result: Performance (F1) degrades sharply with vulnerability density: Llama-3.3-70B drops from 0.97 F1 on single-vulnerability C tasks to 40% lower in high-density settings. Python/JS models undercount severely (recall <0.30) unlike C/C++ models.

Conclusion: The study highlights the limitations of current LLMs in real-world multi-vulnerability detection scenarios, emphasizing the need for improved benchmarks and training strategies to address count/selection biases and language-specific challenges, particularly in dynamically-typed languages like Python and JavaScript.
...
TL;DR: LLA is a novel IP protection method that combines software neuron embedding and hardware locking to secure generative AI models against supply chain threats with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Generative AI models are vulnerable to supply chain IP threats like model theft and information leakage, necessitating robust protection schemes that do not compromise performance.

Method: LLA uses both software-based key embedding in neurons to induce performance degradation and hardware-based secure locking modules on accelerators, along with invariance transformations to protect key values.

Result: LLA demonstrates resistance to oracle-guided key attacks and maintains less than 0.1% overhead even with 7,168 key bits, indicating high security and efficiency.

Conclusion: LLA provides an efficient, secure IP protection framework for generative AI by integrating software and hardware mechanisms, effectively mitigating major supply chain risks with negligible performance impact.
...
TL;DR: Hybrid optical-digital system using NOWA and NSN enables invisible, tamper-evident image authentication with superior forgery resistance.


<details>
  <summary>Details</summary>
Motivation: Modern digital forgeries challenge existing watermarking systems due to sophisticated attacks and visible security markers, necessitating new approaches that embed unforgeable signatures during physical image capture.

Method: A phase mask in the camera aperture embeds a Null-space Optical Watermark (NOWA) invisible to conventional imaging, followed by a Null-Space Network (NSN) that reconstructs protected images while preserving the watermark through measurement-consistent learning.

Result: Experiments show the system achieves comparable image quality to SOTA digital watermarking while outperforming prior methods in tamper localization accuracy, with resistance to compression and forgery without optical/NSN parameters.

Conclusion: The hybrid optical-digital framework establishes structural security through optical watermarking in the camera's null space and neural network-based reconstruction, enabling robust tamper localization while preserving image quality.
...
TL;DR: A new privacy-preserving method for cloud-based AI training called Verifiable Dropout is introduced, which uses zero-knowledge proofs to ensure the integrity of stochastic operations like dropout without exposing sensitive data.


<details>
  <summary>Details</summary>
Motivation: Traditional logging and telemetry in cloud-based AI training struggle to address the non-determinism of deep learning, allowing for potential malicious manipulations to be hidden within stochastic variance, which this proposed method aims to fix.

Method: The method involves binding dropout masks to a deterministic seed that can be cryptographically verified. Zero-knowledge proofs are used to prove the correct execution of the dropout process without revealing the data or internal computation.

Result: Verifiable Dropout enables post-hoc auditing of the integrity of stochastic training steps and ensures that randomness is used correctly without any bias or cherry-picking, which previous methods could not achieve.

Conclusion: The proposed approach effectively closes the integrity gap caused by the non-determinism of deep learning models in cloud environments, allowing for verifiable, privacy-preserving AI training.
...
TL;DR: The paper introduces Raven, a framework that identifies and categorizes smart contract invariants causing transaction reversion on Ethereum, uncovering six new categories and demonstrating its utility in vulnerability detection.


<details>
  <summary>Details</summary>
Motivation: The paper's motivation is to uncover and utilize defensive invariants in Ethereum smart contracts, which have been largely undiscovered and underutilized in security research, despite their value in detecting and preventing vulnerabilities.

Method: Raven uses a fine-tuned BERT model to analyze and embed invariants from Ethereum smart contracts identified in reverted transactions, aligning these invariants to their source code. It then clusters these invariants by semantic intent to discover categories.

Result: Raven successfully identifies 19 semantic clusters of transaction-reverting invariants from 20,000 reverted transactions. Manual review reveals six new invariant categories, including feature toggles and replay prevention. The framework is also demonstrated to be useful in vulnerability detection via a case study involving fuzzing oracles.

Conclusion: Raven is effective in mapping Ethereum's on-chain defenses, leading to new invariant categories that can significantly aid security researchers in creating better data-driven analysis tools for smart contracts.
...
TL;DR: SCyTAG is a framework that efficiently creates minimal cyber twins from attack scenarios in CTI reports to assess cyber threats with reduced resources and improved scalability.


<details>
  <summary>Details</summary>
Motivation: Assessing cyber threats from CTI requires efficient methods balancing manual expertise with automation to prioritize risks without disrupting operations, but current methods are either too manual or lack scalability.

Method: SCyTAG uses attack graphs derived from CTI to construct minimal necessary cyber twins for emulating attack scenarios, thus optimizing resource use.

Result: SCyTAG outperforms previous approaches by reducing network components by up to 85% and resource needs by half while maintaining attack fidelity in tests.

Conclusion: SCyTAG bridges the gap between CTI and practical testing, offering a scalable and efficient solution for cyber threat assessment.
...
TL;DR: This paper reviews vulnerabilities in RSA cryptosystems caused by flawed prime selection in key generation. It analyzes two main attacks: Fermat's factorization and GCD attacks, highlights real-world prevalence of these issues, and suggests mitigations like better entropy and prime validation.


<details>
  <summary>Details</summary>
Motivation: The motivation of the paper is to highlight the ongoing prevalence of RSA vulnerabilities caused by improper prime number selection during key generation, emphasizing the risks to real-world cryptographic systems and the need for improved security practices.

Method: The method involves analyzing two attack vectors—Fermat's factorization and GCD attacks—by drawing on existing research and case studies. The authors examine the technical conditions that allow these attacks, investigate their real-world impact using examples such as Heninger et al.'s and Böck's findings, and assess the root causes such as insufficient entropy in embedded devices.

Result: The results include the identification of weak random number generation in embedded devices as the primary cause of RSA vulnerabilities. The paper documents the persistence of these issues in deployed systems, citing over 64,000 vulnerable TLS hosts found in prior work, and provides empirical validation of how these attacks exploit RSA keys.

Conclusion: The paper concludes that RSA vulnerabilities from poor prime selection are still prevalent, particularly in embedded systems, and emphasizes the importance of robust entropy collection and prime validation in cryptographic implementations to mitigate these risks.
...
TL;DR: This paper proposes a theoretical framework for social bot detection using heterogeneous motifs, improving upon existing topology-based methods by incorporating node-label information and systematic evaluation of motif capabilities.


<details>
  <summary>Details</summary>
Motivation: Current topology-based bot detection methods inadequately address heterogeneity in neighborhood preferences and lack a rigorous theoretical foundation, relying instead on heuristic approaches. This limits their effectiveness in dynamic social media environments.

Method: The authors: (1). Refine homogeneous motifs into heterogeneous motifs by integrating node-label information. (2). Analyze contributions of node pairs within motifs to bot-likelihood. (3). Mathematically quantify each motif's maximum capability using a Naïve Bayes framework. (4). Prioritize high-capability motifs for optimization.

Result: The method outperforms state-of-the-art techniques on four benchmarks across five metrics. Selecting top 20-30Highest-capability motifs retains 95-98 of the performance of using all motifs, demonstrating both effectiveness and efficiency.

Conclusion: This framework establishes a theoretically grounded approach to social bot detection that addresses key limitations of existing methods, enhancing cybersecurity through quantifiable and explainable motif-based analysis.
...
TL;DR: This paper introduces a new framework for sign prediction in signed networks, addressing the limitations of traditional models by explicitly modeling the heterogeneous influence of neighboring nodes. The framework uses two role functions and extends to multiple motifs via two strategies: a generalized multiple motif-based Naïve Bayes (GMNB) and a Feature-driven Generalized Motif-based Naïve Bayes (FGMNB) model that integrates high-dimensional motif features using machine learning methods. FGMNB demonstrates superior performance compared to five state-of-the-art baselines, with the paper highlighting the variability of predictive motif structures across datasets.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the need to enhance sign prediction in signed networks by addressing the limitations of traditional motif-based Naïve Bayes models, which assume equal influence from all neighboring nodes. This approach overlooks the heterogeneity of influences, potentially leading to suboptimal performance. The paper aims to provide a more effective and theoretically grounded solution that enhances the accuracy of sign prediction, particularly for applications involving trust and security in online platforms.

Method: The method involves designing two role functions to quantify the differentiated influence of neighboring nodes. These functions are then extended from a single motif to multiple motifs using two strategies. The first is the generalized multiple motifs-based Naïve Bayes (GMNB) model that linearly combines information from diverse motifs. The second strategy is the FGMNB model, which further integrates high-dimensional motif features using machine learning to enhance predictions.

Result: Extensive experiments on four real-world signed networks show that FGMNB consistently outperforms five state-of-the-art embedding-based baseline methods on three of these networks. The study also reveals that the most predictive motif structures vary across datasets, emphasizing the importance of considering local structural patterns for improved sign prediction.

Conclusion: The conclusion is that the proposed framework offers an effective and theoretically grounded solution for sign prediction in signed networks. Leveraging the heterogeneity of neighboring node influences and the integration of high-dimensional motif features allows the FGMNB model to outperform existing methods. These findings underline the value of local structural patterns and provide actionable insights for improving motif-based feature engineering in sign prediction.
...
TL;DR: This paper introduces Cosmic, an automated framework for detecting privacy violations in web forms concerning user consent, tested on 5,823 websites and 3,598 forms with significant accuracy in identifying violations.


<details>
  <summary>Details</summary>
Motivation: Recent privacy regulations like the GDPR and CCPA require informed and unambiguous user consent for data handling, yet many websites still violate these standards. There is a need for efficient and accurate automated tools to ensure compliance, particularly for diverse and often hard-to-identify web forms.

Method: The paper proposes Cosmic, a novel automated auditing framework designed to analyze and identify consent-related privacy violations in web forms.  Evaluation of the framework is performed on a large-scale dataset encompassing numerous websites and forms, with the aim of detecting violations in compliance with GDPR principles.

Result: Cosmic identifies 3,384 violations in 94.1% of the tested consent forms, achieving 98.6% and 99.1% True Positive Rates (TPR) for consent and violation detection, respectively, across 5,823 websites and 3,598 forms.  It successfully identifies key GDPR violations related to freely given consent, purpose disclosure, and withdrawal options.

Conclusion: The study suggests that Cosmic is a highly effective and accurate tool for automated compliance auditing of web form consent mechanisms,  effectively bridging the gap between legal requirements and real-world implementation by identifying a significant portion of existing violations.
...
TL;DR: This paper introduces a trust-based delegated consensus framework for securing blockchain-enabled IoT networks by integrating FHE and ABAC, while evaluating three reinforcement learning approaches (RL, DRL, and MARL) against various adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of defending against sophisticated adversarial attacks in blockchain-enabled IoT networks, where ensuring secure and private consensus is vital for the effectiveness and adoption of the technology.

Method: The authors introduced a trust-based delegated consensus framework using FHE and ABAC for privacy-preserving policy evaluation. They compared three reinforcement learning approaches (tabular Q-learning (RL), Deep RL with Dueling Double DQN (DRL), and Multi-Agent RL (MARL)) against a set of five different attack types in a simulated 16-node IoT network.

Result: The results show that MARL outperforms RL and DRL in collusive attacks (F1=0.85 vs. 0.50 and 0.68). DRL and MARL achieve perfect detection (F1=1.00) against adaptive attacks, while RL fails (F1=0.50). All agents have perfect defense against Byzantine attacks (F1=1.00), but suffer significantly under Time-Delayed Poisoning attacks, with F1 scores dropping to 0.11-0.16 after sleeper activation.

Conclusion: The study demonstrates that learning-based defense mechanisms, particularly coordinated multi-agent learning, offer significant advantages in defending against sophisticated trust manipulation attacks in blockchain IoT environments, especially against collusion and adaptive threats.
...
TL;DR: The paper discusses the transformative role of foundation-model-based AI in cybersecurity, advocating for agentic cyber resilience as a new paradigm. It outlines a framework for agentic AI workflows and demonstrates case studies.


<details>
  <summary>Details</summary>
Motivation: Current cybersecurity frameworks relying on static rules, perimeters, and human workflows struggle against AI-powered threats. The paper is motivated by the need to transition to a resilience-focused approach that leverages AI agents.

Method: The paper proposes a system-level framework for agentic AI workflows, using a general agentic architecture and coupling attacker/defender processes with game-theoretic formulations for autonomy, information flow, and adaptability.

Result: The proposed framework is illustrated through case studies in penetration testing, remediation, and cyber deception, demonstrating the viability of equilibrium-based design for system-level resiliency.

Conclusion: The paper concludes that agentic cyber resilience is crucial for adapting to AI-driven cybersecurity threats, offering a practical design methodology and validation through real-world applications.
...
TL;DR: Dark patterns manipulate AI agents at 70%+ success rates (far exceeding humans). Bigger models are more vulnerable, and standard defenses fail. This reveals a critical, unaddressed security risk for web agents.


<details>
  <summary>Details</summary>
Motivation: Dark patterns manipulate human users effectively, but their impact on AI agents remained underexplored. This work aims to quantify how these UI manipulation techniques threaten agent robustness, which is critical for ensuring trustworthy AI systems.

Method: The authors created DECEPTICON, an environment with 700 web navigation tasks (600 synthetic, 100 real-world) to evaluate agent robustness against dark patterns. They tested state-of-the-art agents, analyzing trajectory steering rates and correlation with model scale/test-time reasoning.

Result: Dark patterns successfully manipulate agents in 70%+ of tasks (vs. 31% human susceptibility). Larger models and increased reasoning steps correlate with higher vulnerability. Leading defenses (in-context prompting, guardrails) fail to reduce manipulation rates.

Conclusion: The paper concludes that dark patterns represent a significant and unmitigated risk to web agents, especially as model size and reasoning capabilities increase susceptibility. Current countermeasures are ineffective, necessitating urgent development of robust defenses against manipulative design patterns.
...
TL;DR: SecureBank introduces a financial-sector optimized Zero Trust framework that enhances security automation and identity adaptation through domain-specific modeling while maintaining regulatory compliance.


<details>
  <summary>Details</summary>
Motivation: Existing Zero Trust frameworks lack financial domain adaptation, transactional semantics integration, and risk-aware automation, creating vulnerabilities to high-frequency digital threats in banking.

Method: The paper proposes SecureBank, combining Financial Zero Trust, Adaptive Identity Scoring, Contextual Micro Segmentation, and Impact Driven Security Automation. Evaluation uses Monte Carlo simulations measuring TII, ITAL, and SAE metrics.

Result: SecureBank outperforms baseline architectures by improving automated threat handling speed, accelerating identity trust adaptation, while maintaining compliance-regulated transactional integrity standards through simulation validation.

Conclusion: SecureBank addresses limitations in traditional Zero Trust models for financial institutions by integrating financial risk modeling and automated security adaptations, offering a high-assurance architecture aligned with regulatory requirements.
...
TL;DR: This paper addresses the growing security risks in foundation models by analyzing 93 threats and 854 GitHub/Python repositories, identifying unreported vulnerabilities and leveraging a multi-agent RAG system to build a threat graph for adaptive ML-specific security frameworks.


<details>
  <summary>Details</summary>
Motivation: Foundation models in critical sectors are susceptible to various ML-specific attacks (e.g., data poisoning, jailbreaking), yet traditional cybersecurity lacks tailored threat modeling for these systems. Understanding ML security risks is essential for protecting the rapidly expanding ML ecosystem.

Method: The study compiled 93 threats from MITRE ATLAS, AI Incident Database, and literature, and analyzed 854 GitHub/Python repositories. A multi-agent RAG system utilizing ChatGPT-4o mined over 300 articles to generate an ontology-driven threat graph linking TTPs, vulnerabilities, and lifecycle stages.

Result: Unreported threats like LLM API model stealing and preference-guided text-only jailbreaks were identified. Dominant TTPs include MASTERKEY-style jailbreaking and federated poisoning, with vulnerability clusters found in libraries with weak patch propagation, especially affecting pre-training and inference phases.

Conclusion: Adaptive ML-specific frameworks integrating dependency hygiene, threat intelligence, and monitoring are crucial for mitigating supply-chain and inference risks throughout the ML lifecycle, highlighting the need for comprehensive security beyond traditional cybersecurity practices.
...
TL;DR: FedORA enables efficient vertical federated learning unlearning through primal-dual optimization, achieving training-from-scratch quality with reduced overhead.


<details>
  <summary>Details</summary>
Motivation: Vertical federated learning (VFL) unlearning is challenging due to distributed feature architectures and interdependencies, requiring cross-party coordination. Existing methods lack efficiency while maintaining model utility, creating a need for solutions that align with privacy regulations like the 'right to be forgotten.'

Method: The paper proposes FedORA, a primal-dual algorithm framework that formulates unlearning as a constrained optimization problem. Key innovations include a classification-uncertainty promoting loss function, adaptive step size for stability, and asymmetric batch design to handle unlearning and retained data differently.

Result: Theoretical analysis establishes bounded model differences with training-from-scratch, and experiments on tabular/image datasets show FedORA achieves comparable unlearning effectiveness and utility with reduced computational/communication overhead.

Conclusion: FedORA provides an effective and efficient federated unlearning solution for vertical federated learning, achieving unlearning performance comparable to training from scratch while maintaining utility and reducing computational overhead.
...
TL;DR: Equacode is a new jailbreak method for LLMs that uses math and code to bypass security, achieving high success rates


<details>
  <summary>Details</summary>
Motivation: Current jailbreak attacks on LLMs are limited in effectiveness due to reliance on single strategies and natural language attacks.

Method: Equacode transforms malicious intent into a mathematical problem requiring code completion, using cross-domain task complexity to bypass safety constraints.

Result: Equacode achieved 91.19% success on GPT series and 98.65% on three top LLMs with single queries, with ablation studies showing synergy between math and code modules.

Conclusion: Multi-strategy jailbreak approaches like Equacode are more effective than single-strategy methods, indicating the need for improved robustness in LLM security mechanisms.
...
TL;DR: This paper addresses the challenges of secure data storage and access control in the cloud by proposing an improved Attribute-Based Encryption (ABE) method. It uses a Functional Based Stream Cipher (FBSE) and a threshold-based approach, along with techniques like Shamir secret sharing and 2D-Lagrange Interpolation, to ensure minimal storage overhead, secure key reconstruction, and resistance to collision attacks.


<details>
  <summary>Details</summary>
Motivation: The motivation for the paper

Method: The method of the paper

Result: The result of the paper
...
TL;DR: This paper introduces RobustMask, a defense for neural ranking models that combines LM context prediction with randomized masking to provide certified adversarial robustness against character/word/phrase perturbations in retrieval systems.


<details>
  <summary>Details</summary>
Motivation: Neural ranking models are vulnerable to adversarial perturbations that manipulate retrieval results, undermining system integrity. Existing defenses lack generalization or require unrealistic adversarial assumptions.

Method: RobustMask combines the context-prediction capability of pretrained language models with a randomized masking-based smoothing mechanism, leveraging pairwise comparison and probabilistic statistical analysis for certified top-K robustness.

Result: RobustMask certifies over 20% of candidate documents in top-10 rankings against adversarial changes affecting up to 30% of content, demonstrating effective adversarial robustness.

Conclusion: RobustMask marks a significant step toward providing stronger security guarantees for real-world retrieval systems by enhancing adversarial robustness in neural ranking models.
...
TL;DR: Fuzzilicon automates systematic CPU vulnerability discovery using microcode instrumentation, achieving unprecedented coverage and detecting known/unknown microarchitectural flaws without RTL access.


<details>
  <summary>Details</summary>
Motivation: Modern CPUs lack automated frameworks for detecting sophisticated microarchitectural vulnerabilities, which traditionally require manual reverse engineering and remain challenging to systematize post-silicon.

Method: Develops Fuzzilicon, reverse-engineering Intel's microcode interface to enable microcode-level instrumentation. Integrates with hypervisor-based fuzzing to automate feedback-guided input generation without RTL access.

Result: Achieved 16.27% hookable microcode coverage with 31× overhead reduction, identified 5 novel findings (including 2 new speculative execution vulnerabilities), and rediscovered μSpectre class vulnerabilities on Intel Goldmont microarchitecture.

Conclusion: Fuzzilicon establishes a scalable, coverage-guided framework for automated post-silicon CPU vulnerability detection, bridging critical visibility gaps in microarchitectural analysis.
...
TL;DR: The paper proposes an agentic AI system for autonomous software supply chain security that integrates LLM-based reasoning, RL, and multi-agent coordination using LangChain, LangGraph, and blockchain ledger, showing promising results in detection accuracy and mitigation latency.


<details>
  <summary>Details</summary>
Motivation: Conventional post-build integrity mechanisms are inadequate for modern software supply chain attacks that target trusted development and delivery procedures, necessitating proactive and self-defending solutions.

Method: The approach combines large language model (LLM) reasoning for semantic vulnerability analysis, reinforcement learning for adaptive mitigation strategies, and multi-agent coordination via LangChain and LangGraph. It also uses a blockchain security ledger for integrity and the Model Context Protocol to connect with real CI/CD environments.

Result: Testing on simulated and real environments showed better detection accuracy, reduced mitigation latency, and acceptable build-time overhead compared to rule-based, provenance-only, and RL-only baselines. It effectively identified injection attacks, insecure deserialization, access control violations, and configuration errors.

Conclusion: Agentic AI can transform software supply chains from reactive verification systems to proactive, self-defending frameworks by actively identifying and mitigating security threats during development and delivery.
...
TL;DR: A private ICP transfer protocol decouples deposit/retrieval via sealed intermediate nodes, encrypted transport keys, and ephemeral intermediaries with auditable destruction. Implemented as ICPP, it enables confidential, sender-anonymous transactions with verifiable finality and liveness.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for secure and private value transfers on the Internet Computer (ICP), where conventional methods may expose sender identities, content, or lack verifiable liveness. It aims to decouple deposit and retrieval while ensuring confidentiality, forward secrecy, and auditable transaction lifecycle management.

Method: The protocol employs two ephemeral intermediaries, sealed storage, and an RDMPF-based non-interactive encapsulation to generate per-transfer transport keys. A public notice hint ensures retrieval authorization with a decapsulation proof hiding identities, while intermediaries issue certified destruction proofs for auditable transaction finality.

Result: The protocol is implemented as ICPP on ICP, validated through production testing with operational enhancements. It formally proves encapsulation correctness, hint privacy, authorization soundness, and provides timeout reclaim mechanisms, establishing it as a reference for decentralized privacy-preserving transfers.

Conclusion: The paper concludes that the proposed architecture enhances privacy and security in ICP transactions, offering provable confidentiality, sender anonymity, and verifiable finality, making it a robust foundation for future decentralized applications.
...
TL;DR: This paper introduces a multi-agent defense framework with input sanitization, output validation, and provenance tracking to secure agentic AI systems against multimodal prompt injection attacks.


<details>
  <summary>Details</summary>
Motivation: The rise of agentic AI systems (e.g., LangChain, GraphChain) using LLMs/VLMs increases vulnerability to multimodal prompt injection attacks, where hidden/malicious instructions in text/images/metadata/inter-agent messages spread and cause unintended behaviors.

Method: A Cross-Agent Multimodal Provenance-Aware Defense Framework is proposed, consisting of text/visual sanitization agents, an output validator, and a provenance ledger to track metadata and enforce trust boundaries across agent interactions.

Result: Experiments show significant improvements in multimodal injection detection accuracy, minimized cross-agent trust leakage, and stabilized agentic execution pathways via the proposed framework.

Conclusion: The framework enhances secure, understandable, and reliable agentic AI systems by expanding provenance tracking and validation across multi-agent orchestration.
...
TL;DR: WAMM is introduced as an AI-driven framework for web attack detection, improving upon traditional rule-based systems by using machine learning with curated training data to achieve high accuracy and resilience against evasive and polymorphic attacks.


<details>
  <summary>Details</summary>
Motivation: Traditional web application firewalls (WAFs) often miss obfuscated or novel attack patterns because they rely on static rule sets, necessitating the need for a more adaptive and effective detection method.

Method: WAMM is designed as an AI-driven multiclass detection system that employs a multi-phase enhancement pipeline on the SR-BH 2020 dataset. This pipeline includes deduplication, LLM-guided relabeling, attack data augmentation, and LLM-based filtering to create refined datasets. Machine and deep learning models are then evaluated within a unified feature space.

Result: The XGBoost model achieves 99.59% accuracy with microsecond-level inference when using an augmented and filtered dataset on the same technology stack, whereas deep learning models perform worse under noisy augmentation. WAMM outperforms OWASP CRS by achieving true positive block rates of 96–100% with up to 86% improvement.

Conclusion: WAMM demonstrates that efficient machine learning models paired with carefully curated training data can exceed the performance of static rule-based WAFs, offering a scalable and real-time approach to web attack detection suitable for production environments.
...
### [27] [Syntax Is Not Enough: An Empirical Study of Small Transformer Models for Neural Code Repair](https://arxiv.org/abs/2512.22216)
*Shaunak Samant*

Main category: cs.SE

TL;DR: This paper evaluates the effectiveness of a small transformer model for automated Java program repair, finding high syntactic correctness but poor semantic repairs.


<details>
  <summary>Details</summary>
Motivation: Previous studies have shown promise in using neural models for program repair on benchmarks, but real-world application remains limited. This study investigates if a small transformer model can repair real-world Java bugs and if syntactic correctness is a proxy for semantic correctness.

Method: The authors fine-tune CodeT5-small on 52,364 Java bug-fixed pairs and assess token-level performance, syntactic validity via AST parsing, and semantic correctness through exact match.

Result: The model shows high grammatical correctness (94% syntactically valid code) but fails to produce exact-match correct repairs (0% exact matches). In 80% of cases, the model regurgitates the buggy input.

Conclusion: The findings suggest that while small transformer models can generate syntactically correct code, they struggle with semantic correctness in program repair tasks.
...
### [28] [Failure Analysis of Safety Controllers in Autonomous Vehicles Under Object-Based LiDAR Attacks](https://arxiv.org/abs/2512.22244)
*Daniyal Ganiuly,Nurzhau Bolatbek,Assel Smaiyl*

Main category: cs.SE

TL;DR: This paper analyzes the impact of LiDAR-based spoofing attacks on autonomous vehicle safety controllers, finding that even brief attacks can cause unsafe braking and delayed responses, with controller failures more influenced by temporal consistency of spoofed objects than spatial errors alone.


<details>
  <summary>Details</summary>
Motivation: While previous studies have shown LiDAR perception can be manipulated, the specific impact on vehicle safety controllers (e.g., adaptive cruise control) in terms of unsafe behavior and failure modes is not well understood, especially under realistic highway driving conditions.

Method: The authors use a high-fidelity simulation framework that integrates LiDAR perception, object tracking, and closed-loop vehicle control to evaluate how false/dispaced object detections affect perception, planning, and control in various highway scenarios (e.g., cut-in, car following).

Result: The results show that short-duration LiDAR-induced object hallucinations can trigger unsafe braking, delayed responses to real hazards, and unstable control behavior. Cut-in scenarios show increased unsafe deceleration events and time-to-collision violations compared to normal conditions, with controller failures more influenced by temporal consistency of spoofed objects than spatial inaccuracies alone.

Conclusion: The study reveals a critical gap between perception robustness and control-level safety guarantees in autonomous driving systems. It identifies perception-induced controller failure modes and provides practical insights for designing attack-aware safety mechanisms and resilient control strategies for LiDAR-dependent autonomous vehicles.
...
### [29] [Hallucination Detection for LLM-based Text-to-SQL Generation via Two-Stage Metamorphic Testing](https://arxiv.org/abs/2512.22250)
*Bo Yang,Yinfen Xia,Weisong Sun,Yang Liu*

Main category: cs.SE

TL;DR: This paper introduces SQLHD, a hallucination detection method for Text-to-SQL generation using metamorphic testing, which works without ground-truth answers and outperforms existing techniques.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) can produce hallucinations in SQL queries, which are difficult to detect with traditional error detection methods due to the lack of ground-truth data. This necessitates a new approach to identify these errors effectively.

Method: SQLHD uses metamorphic testing and introduces two stages for detection: schema-linking hallucination detection with eight structure-aware MRs, and logical-synthesis hallucination detection with nine logic-aware MRs. It cross-checks outputs against their source through MRs to detect deviations.

Result: SQLHD achieves F1-scores between 69.36% and 82.76% in detecting hallucinations, outperforming existing deep learning-based methods and LLM self-evaluation techniques.

Conclusion: The paper concludes that SQLHD effectively detects hallucinations in Text-to-SQL generation tasks using metamorphic relations, showcasing robust and superior performance without the need for standard answers.
...
### [30] [Agentic Software Issue Resolution with Large Language Models: A Survey](https://arxiv.org/abs/2512.22256)
*Zhonghao Jiang,David Lo,Zhongxin Liu*

Main category: cs.SE

TL;DR: This paper provides a systematic survey of 126 recent studies on LLM-based agentic systems for software issue resolution, outlines their general workflow, introduces a taxonomy, and discusses future research directions.


<details>
  <summary>Details</summary>
Motivation: The paper emphasizes the need for agentic systems in complex software issue resolution tasks and highlights the potential for enhancing software maintenance and validating AI reasoning capabilities.

Method: The authors conducted a survey of recent studies and classified them across benchmarks, techniques, and empirical studies, while also analyzing the impact of agentic reinforcement learning.

Result: The paper demonstrates a comprehensive overview of advancements in agentic software issue resolution, revealing a paradigm shift in AI-grounded software engineering practices.

Conclusion: The work concludes by summarizing major challenges in LLM-based agentic systems for software issue resolution and proposes promising directions for future research.
...
### [31] [AI-Generated Code Is Not Reproducible (Yet): An Empirical Study of Dependency Gaps in LLM-Based Coding Agents](https://arxiv.org/abs/2512.22387)
*Bhanu Prakash Vangala,Ali Adibifar,Tanu Malik,Ashish Gehani*

Main category: cs.SE

TL;DR: This paper investigates the reproducibility of code generated by LLMs (Claude Code, OpenAI Codex, and Gemini) and finds that just 68.3% can be executed successfully, with wide language variances and notable hidden dependencies.


<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used as coding agents, yet their impact on reproducibility of generated code has not been thoroughly examined, prompting the need for such an empirical study.

Method: The authors evaluated three LLMs with 300 projects (100 standardized prompts in Python, JavaScript, and Java) using a three-layer dependency framework: claimed, working, and runtime dependencies.

Result: Only 68.3% of the generated projects executed successfully. Execution rates varied by language: 89.2% in Python and 44.0% in Java. Declared dependencies expanded 13.5 times into actual runtime dependencies, highlighting hidden ones.

Conclusion: LLM-generated code often introduces hidden dependencies, and there are notable reproducibility gaps across different programming languages, indicating a need to better manage and understand dependency expectations.
...
### [32] [Building Software by Rolling the Dice: A Qualitative Study of Vibe Coding](https://arxiv.org/abs/2512.22418)
*Yi-Hung Chou,Boyuan Jiang,Yi Wen Chen,Mingyue Weng,Victoria Jackson,Thomas Zimmermann,James A. Jones*

Main category: cs.SE

TL;DR: This paper explores 'vibe coding' practices enabled by LLMs in software engineering, analyzing 20 videos to uncover developer behaviors, challenges, and mental models around AI-assisted coding.


<details>
  <summary>Details</summary>
Motivation: Despite AI-driven 'vibe coding' being touted as a productivity breakthrough, there is a lack of empirical understanding of how developers actually use this approach in practice.

Method: Grounded theory study of 20 vibe-coding videos (16h live-streamed, 5h opinion, 254 prompts) with analysis of activity durations, prompt intents, and practitioner strategies.

Result: Identifies a spectrum of coding practices (full AI reliance vs. code adaptation), the 'rolling dice'-like debugging process, and divergent mental models influenced by expertise and AI trust levels.

Conclusion: Highlights the need for tools and education addressing AI uncertainty, workflow integration challenges, and diverse developer mental models as LLMs transform software engineering practices.
...
### [33] [GraphLocator: Graph-guided Causal Reasoning for Issue Localization](https://arxiv.org/abs/2512.22469)
*Wei Liu,Chao Peng,Pengfei Gao,Aofan Liu,Wei Zhang,Haiyan Zhao,Zhi Jin*

Main category: cs.SE

TL;DR: GraphLocator uses causal graphs to bridge the semantic gap in issue localization, achieving state-of-the-art results on two common mismatches with 19-28% metric improvements.


<details>
  <summary>Details</summary>
Motivation: The paper addresses two critical challenges in issue localization: symptom-to-cause mismatches (descriptions lack explicit causality) and one-to-many mismatches (single issues affect multiple code entities). Resolving these is essential for improving automated software engineering systems.

Method: GraphLocator introduces a Causal Issue Graph (CIG) to model sub-issues and code dependencies. It operates in two phases: (1) symptom vertex localization identifies initial code locations, and (2) dynamic CIG expansion through iterative neighbor reasoning to resolve interdependencies.

Result: 1) +19.49% function-level recall and +11.89% precision improvements over baselines. 2) +16.44% and +19.18% recall improvements for both mismatch types. 3) CIG achieves 28.74% relative gain on downstream resolving tasks.

Conclusion: GraphLocator effectively addresses the symptom-to-cause and one-to-many mismatches through causal structure discovery and dynamic issue disentangling, achieving significant improvements in issue localization accuracy and downstream task performance.
...
### [34] [Isolating Compiler Faults via Multiple Pairs of Adversarial Compilation Configurations](https://arxiv.org/abs/2512.22538)
*Qingyang Li,Yibiao Yang,Maolin Sun,Jiangchang Wu,Qingkai Shi,Yuming Zhou*

Main category: cs.SE

TL;DR: MultiConf uses adversarial configurations and SBFL to improve compiler fault localization, outperforming existing methods by 35% in identifying bugs.


<details>
  <summary>Details</summary>
Motivation: Modern compiler infrastructures are complex and large-scale, making it extremely challenging to localize faults to specific source files despite their critical role in software development.

Method: MultiConf constructs multiple adversarial compilation configuration pairs (failing vs. passing) with minimal option differences, applies Spectrum-Based Fault Localization (SBFL) to rank suspicious files, and aggregates rankings via a weighted voting scheme.

Result: MultiConf localized 27 out of 60 real-world GCC bugs at the Top-1 file level, achieving 35.0% and 28.6% improvements over leading methods Odfl(20) and Basic(21) respectively.

Conclusion: MultiConf is a novel approach that significantly outperforms existing compiler fault localization techniques in both effectiveness and efficiency, achieving substantial improvements over state-of-the-art methods for real-world GCC compiler bugs.
...
### [35] [Rethinking the Capability of Fine-Tuned Language Models for Automated Vulnerability Repair](https://arxiv.org/abs/2512.22633)
*Woorim Han,Yeongjun Kwak,Miseon Yu,Kyeongmin Kim,Younghan Lee,Hyungon Moon,Yunheung Paek*

Main category: cs.SE

TL;DR: This paper investigates the shortcomings of current learning-based automated vulnerability repair (AVR) models and match-based evaluation metrics, proposing three approaches to better assess their effectiveness and generalization.


<details>
  <summary>Details</summary>
Motivation: Despite the potential of learning-based AVR techniques, it is unclear whether models can reliably repair unseen vulnerabilities due to overfitting and limitations in evaluating patches at the token level.

Method: The study uses three methods: 1) applying semantic-preserving transformations to test sets to check for spurious training, 2) re-splitting training, validation, and test sets to be mutually exclusive for generalization testing, and 3) introducing a new benchmark (L-AVRBench) to evaluate true repair capabilities beyond match-based metrics.

Result: The results indicate that state-of-the-art models have limited generalization on transformed test sets and that current evaluation metrics are inadequate for capturing all valid patching solutions.

Conclusion: To improve learning-based AVR techniques, the paper emphasizes the need for robust pattern learning and more accurate, comprehensive evaluation metrics like the proposed L-AVRBench.
...
### [36] [CFIghter: Automated Control-Flow Integrity Enablement and Evaluation for Legacy C/C++ Systems](https://arxiv.org/abs/2512.22701)
*Sabine Houy,Bruno Kreyssig,Alexandre Bartel*

Main category: cs.SE

TL;DR: CFIghter automates strict type-based CFI enforcement in real-world C software by repairing 95.8% of violations through runtime-guided analysis, eliminating manual code changes.


<details>
  <summary>Details</summary>
Motivation: Compiler-based CFI faces deployment challenges in large C/C++ projects due to visibility mismatches, type inconsistencies, and unintended behavioral failures, requiring solutions to enable strict enforcement.

Method: CFIghter combines whole-program analysis with guided runtime monitoring to iteratively detect, classify, and repair CFI violations using test suites, applying minimal adjustments localized to problematic areas.

Result: CFIghter resolves 95.8% of unintended violations in util-linux, eliminating visibility-related build errors and maintaining strict enforcement at 89% of control-flow sites across four GNU projects without manual code modifications.

Conclusion: Automated compatibility repair with CFIghter enables strict compiler CFI to be practically deployed in mature, modular C software without manual source-code changes.
...
TL;DR: The paper introduces RSA, a pretexting strategy to manipulate LLMs into generating exploits even when they have safety mechanisms. The research shows that non-experts can become attackers by using LLMs, making traditional software security assumptions obsolete. Testing on Odoo with major LLMs achieved a 100% success rate in producing functional exploits. This represents a paradigm shift in software security as threat models need to change in an era where code understanding is no longer essential for exploitation.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the rising use of large language models (LLMs) in software engineering, which can both empower users to create applications and undermine traditional security assumptions. The accessibility of these tools lowers the barrier for exploitation, allowing non-experts to launch effective attacks without needing to understand code. The core issue is that past security principles, which relied on technical expertise as a threat model, are increasingly less relevant given the capabilities of LLMs to abstract technical complexity and enable attackers with just prompt-crafting abilities.

Method: The paper proposes RSA (Role-assignment, Scenario-pretexting, and Action-solicitation), a method designed to socially engineer LLMs into generating functional exploits. This strategy manipulates the LLMs to bypass standard safety measures and produce actionable attack code. Experiments were conducted using Odoo, an ERP platform, to evaluate the efficacy of RSA on five widely recognized LLMs, including GPT-4o, Gemini, Claude, Microsoft Copilot, and DeepSeek. These models were prompted in a series of interaction rounds (typically 3-4) to produce fully working exploits for tested CVEs with a 100% success rate.

Result: The research results demonstrate the effectiveness of the RSA strategy in manipulating LLMs into generating fully functional exploits for tested CVE vulnerabilities. With all five tested LLMs, the success rate was 100%, with the exploits being produced within 3-4 prompting rounds. This indicates that non-technical individuals can easily become capable attackers merely by crafting prompts, without any need for deep technical knowledge of code or vulnerabilities. This challenges the traditional security model assumptions and suggests a need for a paradigm shift in how software security is approached.

Conclusion: The conclusion posits that current software security principles and threat models are insufficient in an era where exploitation can be executed with LLMs by individuals lacking technical expertise. The research highlights the paradigm shift: security measures must be redesigned to account for attackers who can simply craft prompts as a substitute for code understanding. The distinction between technical and non-technical threat actors is becoming obsolete, as LLMs abstract away technical complexity, making traditional defenses ineffective. These findings emphasize the importance of updating security practices to adapt to this new reality.
...
TL;DR: This paper introduces FasterPy, a cost-effective framework leveraging Large Language Models (LLMs) with Retrieval-Augmented Generation (RAG)+Low-Rank Adaptation (LoRA) for Python code optimization, outperforming existing methods on the PIE benchmark.


<details>
  <summary>Details</summary>
Motivation: Traditional code optimization is labor-intensive and limited in scalability; ML approaches require costly program representations and curated datasets, whereas LLMs enable automated, generalizable code optimization through pre-trained language understanding.

Method: FasterPy integrates RAG (using performance code-pair knowledge bases) with LoRA to adapt LLMs for optimization. RAG retrieves relevant code examples, while LoRA fine-tunes the model with minimal parameter updates for efficiency.

Result: Experiments on the PIE benchmark showed superior performance metrics compared to existing models; open-sourced implementations are provided at https://github.com/WuYue22/fasterpy.

Conclusion: FasterPy demonstrates that combining RAG and LoRA with LLMs achieves effective, scalable code optimization with minimal additional cost, outperforming prior rule-based and ML-based approaches.
...
### [39] [Towards the analysis of team members well-being](https://arxiv.org/abs/2512.22845)
*Zan Xu,Sari Nurfauziyyah,Anastasia Romanova,Kaamesh G S,Yiqun Gao,Maria Spichkova*

Main category: cs.SE

TL;DR: This paper investigates how recognition impacts software team well-being, presenting a prototype tool and analysis to improve workplace appreciation and productivity.


<details>
  <summary>Details</summary>
Motivation: The research aims to improve software team well-being, which impacts productivity, performance, and personal health, with a focus on recognition as a critical factor.

Method: The study involved analyzing team well-being factors and creating a prototype tool to address these factors.

Result: The project yielded results from the well-being analysis and a functional prototype to support team well-being.

Conclusion: The paper concludes that addressing the recognition and appreciation of team members can significantly enhance well-being in software development teams, supported by the developed prototype.
...
### [40] [Interpretable Gallbladder Ultrasound Diagnosis: A Lightweight Web-Mobile Software Platform with Real-Time XAI](https://arxiv.org/abs/2512.23033)
*Fuyad Hasan Bhoyan,Prashanta Sarker,Parsia Noor Ethila,Md. Emon Hossain,Md Kaviul Hossain,Md Humaion Kabir Mehedi*

Main category: cs.SE

TL;DR: A new AI diagnostic software uses the MobResTaNet model to classify gallbladder diseases via ultrasound with high accuracy and low parameter count, offering real-time, explainable predictions accessible through mobile and web apps.


<details>
  <summary>Details</summary>
Motivation: Accurate and early detection of gallbladder diseases is vital, but interpreting ultrasound images is complex and challenging. An AI solution is needed to provide reliable and accessible diagnostic support, especially in real-time clinical settings.

Method: The software integrates a hybrid deep learning model called MobResTaNet to classify ten categories of gallbladder conditions directly from ultrasound images. It utilizes Explainable AI (XAI) methods for generating interpretable predictions and is built using technologies such as HTML, CSS, JavaScript, Bootstrap, and Flutter for deployment as web and mobile applications.

Result: The system achieves up to 99.85% accuracy with only 2.24 million parameters, demonstrating high performance and efficiency. It is deployed as accessible mobile and web applications, enabling real-time, explainable predictions for gallbladder disease diagnosis.

Conclusion: The proposed AI-driven diagnostic software offers an innovative solution for the accurate and interpretable classification of gallbladder diseases using ultrasound, with high accuracy and accessibility via mobile and web platforms, potentially enhancing clinical decision-making at the point of care.
...
### [41] [An Automated Grey Literature Extraction Tool for Software Engineering](https://arxiv.org/abs/2512.23066)
*Houcine Abdelkader Cherief,Brahim Mahmoudi,Zacharie Chenail-Larcher,Naouel Moha,Quentin Sti'evenart,Florent Avellaneda*

Main category: cs.SE

TL;DR: This paper introduces GLiSE, a prompt-driven tool for scalable collection and assessment of grey literature in software engineering. GLiSE generates platform-specific queries, gathers results from common software engineering sources, and uses semantic classification for relevance filtering. The paper presents the tool, a curated dataset, and a usability study.


<details>
  <summary>Details</summary>
Motivation: The motivation is the difficulty of collecting and assessing grey literature in software engineering due to heterogeneous sources, formats, and APIs, which hinders reproducible, large-scale synthesis.

Method: The method involves a prompt-driven tool named GLiSE that (i) turns research prompts into platform-specific queries, (ii) gathers results from GitHub, Stack Overflow, and Google Search, (iii) uses semantic classifiers based on embeddings to filter and rank results, and (iv) ensures reproducibility through configuration-based settings and accessible generated queries.

Result: The results include (i) the GLiSE tool, (ii) a curated dataset of classified search results, and (iii) findings from an empirical usability study of the tool.

Conclusion: The conclusion is that GLiSE provides a reproducible and scalable approach for grey literature synthesis in software engineering research, and the empirical study provides insights into its usability.
...
### [42] [An Empirical Study of Generative AI Adoption in Software Engineering](https://arxiv.org/abs/2512.23327)
*Görkem Giray,Onur Demirörs,Marcos Kalinowski,Daniel Mendez*

Main category: cs.SE

TL;DR: This study provides an empirical analysis of GenAI adoption in SE, highlighting its integration, benefits, challenges, and implications for future roles and practices.


<details>
  <summary>Details</summary>
Motivation: While GenAI tools are increasingly adopted in SE, there is a lack of empirical evidence on their practical use, benefits, challenges, and broader implications, motivating this study to fill the knowledge gap.

Method: The study investigates GenAI adoption in SE through empirical analysis, examining its institutionalization, associated benefits and challenges, and long-term impacts on professionals and the community.

Result: GenAI tools are deeply integrated into implementation, verification, and maintenance tasks, yielding benefits like reduced cycle time and quality improvements. Challenges include unreliable outputs, security concerns, and validation overhead. Institutionalization is common but inconsistent, focusing more on tool access than training. Practitioners anticipate role redefinition rather than replacement.

Conclusion: GenAI is widely adopted in software engineering (SE), integrated into daily tasks, and expected to redefine roles rather than replace professionals. Challenges include reliability, security, and varying institutionalization, while benefits focus on productivity and quality improvements.
...
### [43] [Securing the AI Supply Chain: What Can We Learn From Developer-Reported Security Issues and Solutions of AI Projects?](https://arxiv.org/abs/2512.23385)
*The Anh Nguyen,Triet Huynh Minh Le,M. Ali Babar*

Main category: cs.SE

TL;DR: We present a large-scale study on security issues in the AI supply chain by empirical analysis of developer discussions.


<details>
  <summary>Details</summary>
Motivation: Little is known about the common security issues in AI supply chains and how they are addressed in practice.

Method: We analyze a dataset of 312,868 security discussions from Hugging Face and GitHub, sampled 753 posts, and conducted a thematic analysis to identify a taxonomy of 32 security issues and 24 solutions.

Result: We identify four themes, which includes 32 security issues and 24 solutions. We find security issues in Models and Data often lack solutions. The complex dependencies and black-box nature of AI components are identified as root causes for security issues.

Conclusion: Our work can provide guidance for developers to address real-world security threats across the AI supply chain.
...
### [44] [An SLO Driven and Cost-Aware Autoscaling Framework for Kubernetes](https://arxiv.org/abs/2512.23415)
*Vinoth Punniyamoorthy,Bikesh Kumar,Sumit Saha,Lokesh Butra,Mayilsamy Palanigounder,Akash Kumar Agarwal,Kabilan Kannan*

Main category: cs.SE

TL;DR: TLDR; This paper presents a safe and explainable multi-signal autoscaling framework for Kubernetes that improves SLO and cost efficiency by integrating AIOps principles, significantly reducing SLO violations and operational costs while enhancing response time and transparency.


<details>
  <summary>Details</summary>
Motivation: The motivation behind this research is to address the problems in production environments where Kubernetes autoscaling suffers from reactive behavior, limited application-level insights, and opaque logic, leading to frequent SLO violations and inefficiencies in cost.

Method: The method involves a gap-driven analysis of current autoscaling strategies, proposing an AIOps-based multi-signal autoscaling framework that combines SLO-aware and cost-conscious control mechanisms with lightweight demand forecasting for better adaptability and transparency.

Result: Through experimental evaluation using microservice and event-driven workloads, the framework reduced SLO violation duration by 31%, improved scaling response time by 24%, and cut infrastructure costs by 18% compared to standard and optimized Kubernetes autoscaling setups.

Conclusion: The conclusion is that applying AIOps principles with a SLO-first approach to Kubernetes autoscaling significantly enhances reliability, cost efficiency, and operational trust, showcasing the potential of intelligent, multi-signal strategies in cloud-native environments.
...
### [45] [Embedding Quality Assurance in project-based learning](https://arxiv.org/abs/2512.23488)
*Maria Spichkova*

Main category: cs.SE

TL;DR: The paper discusses lessons learned and recommendations for integrating quality assurance topics into Agile/Scrum project-based learning in SE courses over a decade.


<details>
  <summary>Details</summary>
Motivation: The motivation was to analyze and share the experience gained over ten years in teaching software quality aspects in Software Engineering courses with an Agile/Scrum focus, to provide actionable recommendations for embedding quality assurance in project-based learning.

Method: The methods involved teaching software quality topics, primarily in the context of final year software development projects and an SE Project Management course, while focusing on Agile/Scrum settings. The researchers drew lessons learned from this extended educational practice and provided recommendations.

Result: The results are based on over ten years of teaching experience, leading to an in-depth understanding of how to effectively embed quality assurance within Agile/Scrum educational settings. The paper provides specific recommendations for doing so within project-based learning.

Conclusion: The authors conclude that there are valuable lessons that can help guide how quality assurance topics are taught and implemented in software engineering education, especially within project-based learning that utilizes Agile/Scrum methodologies.
...
### [46] [Adaptable Teastore with Energy Consumption Awareness: A Case Study](https://arxiv.org/abs/2512.23498)
*Henrique De Medeiros,Denisse Muñante,Sophie Chabridon,César Perdigão Batista,Denis Conan*

Main category: cs.SE

TL;DR: This paper introduces EnCoMSAS, a monitoring tool for measuring the energy consumption of self-adaptive systems (SAS) in distributed environments such as the Cloud, and evaluates its effectiveness using the Adaptable TeaStore benchmark.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need for effective energy consumption monitoring in self-adaptive systems to enable energy-aware dynamic adaptation, given that data centers are significant contributors to global energy consumption due to the increasing adoption of Cloud-based applications.

Method: The authors designed EnCoMSAS to gather and assess energy consumption data during runtime for SAS. They evaluated it by integrating it into the Adaptable TeaStore system and running experiments with varying workload conditions on the Grid5000 testbed.

Result: The results demonstrate that EnCoMSAS effectively collects energy consumption data, showing a correlation between CPU usage and measured energy. It also indicates that energy consumption is affected by factors beyond algorithmic complexity, including the deployment environment. Furthermore, its impact on the energy use of the SAS ecosystem is modest.

Conclusion: EnCoMSAS is a valid and effective tool for monitoring runtime energy consumption of SAS applications, enabling energy-aware adaptations and revealing insights about environmental influences. Its implementation has a low ecological footprint, making it a practical solution for integrating energy efficiency into self-adaptive systems.
...
### [47] [AdaptiFlow: An Extensible Framework for Event-Driven Autonomy in Cloud Microservices](https://arxiv.org/abs/2512.23499)
*Brice Arléon Zemtsop Ndadji,Simon Bliudze,Clément Quinton*

Main category: cs.SE

TL;DR: AdaptiFlow is a framework that enables microservices to achieve self-adaptive capabilities through autonomous computing principles, decoupling metrics collection and action execution from adaptation logic. It is validated through real-world scenarios and offers tools for resilient distributed systems.


<details>
  <summary>Details</summary>
Motivation: Modern cloud environments require microservices to handle dynamic conditions, but centralized solutions hinder their decentralized architecture. There's a need for a framework that allows decentralized, architecture-preserving adaptability.

Method: AdaptiFlow employs the Monitor and Execute phases of the MAPE-K loop, using standardized interfaces to enable microservices to act autonomously. It comprises Metrics Collectors, Adaptation Actions as declarative actuators, and an event-driven/rule-based mechanism for adaptation logic.

Result: The framework was validated using an enhanced Adaptable TeaStore benchmark, successfully implementing self-healing, self-protection, and self-optimization with minimal code changes. It demonstrates decentralized adaptation and service instrumentation workflows.

Conclusion: AdaptiFlow bridges autonomous computing theory with cloud-native practices, providing a concrete framework for resilient systems. Future research will integrate formal models and AI agents for advanced adaptive capabilities.
...
### [48] [Beyond Correctness: Exposing LLM-generated Logical Flaws in Reasoning via Multi-step Automated Theorem Proving](https://arxiv.org/abs/2512.23511)
*Xinyi Zheng,Ningke Li,Xiaokun Luan,Kailong Wang,Ling Shi,Meng Sun,Haoyu Wang*

Main category: cs.SE

TL;DR: The paper introduces MATP, an evaluation framework for verifying the logical validity of LLM reasoning through multi-step automatic theorem proving. The approach surpasses existing methods in detecting logical flaws and identifies model-level differences.


<details>
  <summary>Details</summary>
Motivation: LLMs are used in high-stakes domains, but their reasoning can have subtle logical errors that current methods fail to detect. There is a need for a more robust solution to verify multi-step reasoning.

Method: MATP translates natural language reasoning into FOL and uses automated theorem provers to systematically verify logical validity, providing fine-grained classification of reasoning correctness.

Result: MATP outperforms prompting-based baselines by over 42 percentage points and demonstrates that reasoning models produce more logically coherent outputs than general models.

Conclusion: MATP's success in identifying hidden logical errors shows its potential to increase the trustworthiness of LLM-generated reasoning in critical domains.
...
### [49] [Model-based Development for Autonomous Driving Software Considering Parallelization](https://arxiv.org/abs/2512.23575)
*Kenshin Obi,Takumi Onozawa,Hiroshi Fujimoto,Takuya Azumi*

Main category: cs.SE

TL;DR: The paper presents a method to parallelize autonomous driving software using MBD, which reduces execution time and improves real-time performance.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles need real-time performance, but existing software struggles with complex environments and functions. There's a need for methods that enable efficient processing.

Method: The authors extend the MBP method in Model-Based Development (MBD) to support parallelization. This allows for the efficient implementation of complex functionalities in autonomous driving software.

Result: The proposed method successfully reduced execution times, indicating improved real-time performance of autonomous driving software.

Conclusion: The paper concludes that the proposed method is suitable for developing autonomous driving software, especially for enhancing real-time capabilities by reducing execution time through parallelization.
...
### [50] [Parallelized Code Generation from Simulink Models for Event-driven and Timer-driven ROS 2 Nodes](https://arxiv.org/abs/2512.23605)
*Kenshin Obi,Ryo Yoshinaka,Hiroshi Fujimoto,Takuya Azumi*

Main category: cs.SE

TL;DR: A new MBD framework for ROS 2-based systems is proposed to address parallelization challenges in multi-input scenarios.


<details>
  <summary>Details</summary>
Motivation: Traditional manual parallelization is error-prone in complex systems like autonomous driving, and current MBD struggles with multi-input integration in ROS 2.

Method: The proposed framework categorizes ROS 2-compatible Simulink models into event-driven and timer-driven types for targeted parallelization, enabling efficient code generation.

Result: All tested patterns showed reduced execution times after applying the new parallelization method.

Conclusion: The framework effectively overcomes MBD limitations in ROS 2, enabling reliable parallelization for multi-input systems.
