...
TL;DR: SAFuzz uses LLM-guided adaptive fuzzing to effectively detect algorithmic vulnerabilities in AI-generated code, improving precision, reducing time cost, and enhancing recall when combined with existing test generation methods.


<details>
  <summary>Details</summary>
Motivation: Current testing frameworks are inefficient at handling the volume and semantic complexity of AI-generated code, with traditional fuzzing methods lacking targeted resource allocation and semantic awareness of algorithmic vulnerabilities.

Method: The method involves an LLM-guided adaptive fuzzing framework called SAFuzz that uses prompt-based behavioral diversification, harness generation with problem-specific oracles, and an LLM-based predictor for adaptive resource allocation and dynamic early stopping.

Result: SAFuzz improves vulnerability discrimination precision from 77.9% to 85.7%, achieves a 1.71x reduction in time cost compared to GreenFuzz, and when combined with unit test generation, increases bug detection recall from 67.3% to 79.5%.

Conclusion: SAFuzz, a hybrid testing framework with LLM-guided adaptive fuzzing, improves vulnerability detection precision and reduces time cost compared to state-of-the-art methods, and offers complementary benefits when combined with existing unit test generation techniques.
...
TL;DR: SWE-MiniSandbox is a lightweight, container-free execution environment for reinforcement learning-based software engineering agents that reduces storage and setup overhead while maintaining task isolation and performance.


<details>
  <summary>Details</summary>
Motivation: Existing reinforcement learning pipelines for software engineering agents rely on per-task containers, which incur high storage costs, slow environment setup, and require elevated privileges, making large-scale training inefficient and inaccessible in resource-constrained settings.

Method: SWE-MiniSandbox uses kernel-level isolation mechanisms to create secure, per-task workspaces without containers, combined with lightweight environment pre-caching to eliminate the need for full container images.
...
Conclusion: SWE-MiniSandbox provides a scalable, low-overhead, and accessible alternative to container-based environments for training RL-driven SWE agents, enabling efficient deployment in resource-limited settings.
...
TL;DR: Patient Medical Digital Twins (PMDTs) represent a transformative approach to chronic care by integrating diverse patient data into dynamic digital models, with early pilots demonstrating feasibility in oncology and federated AI platforms while revealing key technical and governance challenges.


<details>
  <summary>Details</summary>
Motivation: Chronic diseases are the leading cause of death and healthcare costs globally, but current healthcare systems are fragmented and reactive. PMDTs aim to enable proactive, personalized, and continuous care through comprehensive digital representations of patients.

Method: The authors developed PMDTs using ontology-driven modeling and federated analytics, implemented in pilots including the QUALITOP oncology study and a distributed AI platform. They used standards like HL7 FHIR and OMOP, incorporated privacy-preserving governance, and applied automated reasoning and predictive analytics on multimodal data.

Result: Early implementations showed technical feasibility of PMDTs, including automated reasoning over patient data and predictive modeling of outcomes. Key challenges identified include standardization, privacy governance, scalability of federated queries, and usability of clinician interfaces.

Conclusion: PMDTs hold significant potential to transform chronic disease management. The authors recommend software engineering advances such as domain-specific languages (DSLs) and model-driven engineering to build trustworthy, adaptive PMDT-based care ecosystems.
...
TL;DR: Agent-Diff is a benchmarking framework that evaluates agentic LLMs using real-world APIs within a controlled sandbox, employing state-diff contracts to measure task success by environmental state changes, enabling fair, reliable, and ecologically valid comparisons across models.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks face a trade-off between ecological validity and environmental control; Agent-Diff aims to bridge this gap by enabling evaluation of agentic LLMs on real API interfaces while maintaining consistency through sandboxed execution and standardized contracts.

Method: The framework introduces a state-diff contract to assess task success based on environment state changes and employs a sandbox with a standardized scripting layer for executing code across real APIs (e.g., Slack, Box, Google Calendar), allowing consistent evaluation across models.

Result: The framework benchmarks nine LLMs across 224 enterprise workflow tasks and demonstrates robustness through ablation studies, showing the impact of API documentation access on model performance.

Conclusion: Agent-Diff provides a robust and standardized framework for evaluating agentic LLMs on real-world API-driven tasks by combining real service interfaces with a controlled sandbox environment, enabling reliable and ecologically valid benchmarking.
...
TL;DR: Fine-tuning LLMs for code generation with perturbed datasets improves robustness against adversarial inputs at the cost of a slight performance drop.


<details>
  <summary>Details</summary>
Motivation: Ensuring robustness of LLMs in coding tasks is critical due to the risk of incorrect or insecure code from adversarial or varied inputs.

Method: LLMs were fine-tuned on datasets perturbed at character, word, and sentence levels, and their robustness and performance were compared to models fine-tuned on clean data.

Result: Fine-tuning with perturbed data reduced Robustness Degradation (RD) by 4\%-6\% and slightly lowered pass@1 scores by 1\%-3\%, with rare cases of performance improvement.

Conclusion: Perturbed-data fine-tuning enhances LLM robustness for coding tasks, highlighting the need to balance robustness and performance in practical deployment.
...
TL;DR: This paper presents a grounded theory of professional debugging, revealing it as an iterative process centered on updating a mental model of the system. Developers strategically combine navigation, execution, and external resources, adapting their reasoning to context, with implications for tools and training.


<details>
  <summary>Details</summary>
Motivation: Despite the centrality of debugging in software engineering, there is a lack of theoretical understanding of how experienced developers reason about bugs in large, real-world systems. This study aims to build a theory grounded in observational data to explain the cognitive and practical aspects of professional debugging.

Method: A qualitative study using grounded theory methodology was conducted, involving observations of 7 professional developers and 5 live-coding streamers performing 17 debugging tasks in real-world codebases. Data were analyzed to identify recurring patterns in reasoning and information-gathering strategies.

Result: The study identified that developers alternate between navigation and execution strategies, and employ forward and backward tracing reasoning modes. Their approach is adapted based on codebase context, complexity, and familiarity. Experienced developers systematically construct and update a mental model using both code-based and external information sources.

Conclusion: The study concludes that professional debugging can be understood as a structured, iterative diagnostic process guided by the continuous updating of a mental model. This mental model directs how developers gather and interpret information during debugging.
...
TL;DR: This paper addresses contributor retention in open-source software communities by identifying limitations in current retrospective tools and proposing a predictive, practitioner-informed framework. Using a mixed-methods design science approach, the authors develop and evaluate a web-based prototype to help community managers proactively identify and respond to disengagement risks, offering both practical strategies and a foundation for future research on OSS sustainability.


<details>
  <summary>Details</summary>
Motivation: OSS community managers struggle to retain contributors due to a lack of predictive tools, relying instead on retrospective dashboards that do not enable early intervention; this gap motivates the need for proactive retention management solutions.

Method: A mixed-methods approach grounded in Design Science Research was used, including semi-structured interviews, a multi-vocal literature review, surveys, iterative prototype development, and in situ evaluation across two OSS communities.

Result: The research yielded empirical insights into retention challenges, developed actionable strategies for diagnosing risks and guiding engagement, and delivered a validated web-based prototype refined with feedback from over 100 OSS practitioners.

Conclusion: The study concludes that predictive tools are essential for effective contributor retention in open-source software (OSS) communities, and the proposed framework and prototype provide actionable, practitioner-informed support to anticipate disengagement and improve sustainability.
...
TL;DR: SB-QOPS is a search-based quantum program testing framework using commuting Pauli strings to improve fault detection with reduced specification overhead, validated across multiple quantum platforms.


<details>
  <summary>Details</summary>
Motivation: Existing quantum testing methods suffer from reliance on simplistic inputs, statistical oracles, costly specifications, and limited real-hardware validation. SB-QOPS aims to overcome these limitations by leveraging Pauli string commutation for more effective and specification-light testing.

Method: SB-QOPS redefines test cases using Pauli strings and introduces a measurement-centric oracle based on commutation properties. It employs a search-based approach with an expectation-value-based fitness function and evaluates three strategies: Genetic Algorithm, Hill Climbing, and (1+1) Evolutionary Algorithm.

Result: SB-QOPS achieves 100% fault-detection score on quantum circuits up to 29 qubits, outperforms the prior QOPS method, and demonstrates effective performance under both simulated and real noisy conditions across IBM, IQM, and Quantinuum platforms.

Conclusion: SB-QOPS enables more efficient and effective quantum program testing with high fault detection, better resource utilization, and cross-platform portability, advancing practical quantum software testing on real hardware.
...
TL;DR: This paper introduces GUI Agent Autonomy Levels (GAL), a six-level framework to standardize and clarify autonomy in GUI agents, enabling better evaluation and trust in human-software interaction.


<details>
  <summary>Details</summary>
Motivation: The lack of a standardized definition for autonomy in GUI agents leads to ambiguity in capability, responsibility, and risk, which hinders progress and trust in agent-based software interaction.

Method: The authors propose a six-level framework called GUI Agent Autonomy Levels (GAL) to systematically categorize and clarify the varying degrees of autonomy in GUI agents.

Result: The GAL framework provides a structured way to assess and communicate the level of autonomy in GUI agents, facilitating clearer understanding and comparison across systems.

Conclusion: The paper concludes by advocating for the adoption of the GUI Agent Autonomy Levels (GAL) framework to bring conceptual clarity and enable benchmarking of autonomy in GUI agents, promoting trustworthy software interaction.
...
TL;DR: Hydra is a repository-level code generation framework that leverages structure-aware indexing and a dependency-aware retriever to improve code LLM performance in large, cross-file contexts.


<details>
  <summary>Details</summary>
Motivation: Existing RAG methods for code generation rely on NLP-inspired chunking and similarity-based retrieval, which break code coherence and miss critical functional dependencies in repository-level tasks.

Method: Hydra introduces (i) hierarchical structure-aware indexing of code (functions, classes, variables), (ii) a lightweight dependency-aware retriever (DAR) that identifies true code dependencies, and (iii) a hybrid retrieval mechanism combining DAR with similarity-based retrieval.

Result: Hydra achieves state-of-the-art performance on DevEval and RepoExec benchmarks, outperforming the strongest baseline by over 5% in Pass@1 and enabling smaller models to match larger ones.

Conclusion: Treating code as structured rather than natural language significantly enhances repository-level code generation when dependencies and context are critical.
...
TL;DR: This study explores how team dynamics in open-source software (OSS) communities affect project selection, showing that communication quality and motivational alignment (e.g., reputation, networking) shape contributors' preferences, suggesting a need for human-aware recommendation systems.


<details>
  <summary>Details</summary>
Motivation: Current OSS project recommendation systems focus heavily on technical attributes while neglecting collaboration and community aspects that influence contributor engagement and retention; this study aims to bridge that gap by examining the role of team dynamics and motivational factors in project selection.

Method: An online survey was conducted with 198 OSS practitioners, using mixed methods (quantitative and qualitative analyses) to assess contributors' perceptions of team dynamics and how these relate to their motivations for participating in OSS projects.

Result: Communication-related team dynamics—responsiveness, tone, and clarity of replies—are consistently valued across practitioners. However, the importance of specific dynamics varies by motivation: contributors seeking reputation or networking value inclusive communities that support diverse participation.

Conclusion: Aligning team dynamics with contributors' motivations provides key insights into project selection behavior. These findings advocate for human-aware recommendation systems that incorporate social collaboration quality and motivational fit to improve contributor engagement in OSS projects.
...
TL;DR: WebTestPilot is an LLM-based agent for end-to-end web testing that uses symbolic abstraction and inferred pre/post-conditions to build an implicit oracle, enabling reliable bug detection despite LLM hallucinations, and demonstrates state-of-the-art performance in precision, recall, and task completion.


<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the limitations of LLM-based agents in web testing where hallucinations make it difficult to distinguish between actual bugs and false positives, particularly due to the lack of implicit oracles and consistent reasoning mechanisms.

Method: WebTestPilot employs a symbolization layer to convert GUI elements into symbolic variables and translates natural language requirements into step-wise actions with inferred pre- and post-conditions, forming a symbolic oracle that captures data, temporal, and causal dependencies for robust validation.

Result: WebTestPilot achieves a 99% task completion rate, 96% precision, and 96% recall in bug detection, significantly outperforming existing baselines (+70 precision, +27 recall) on a newly created benchmark of bug-injected web applications.

Conclusion: WebTestPilot successfully addresses the challenges of oracle inference and probabilistic reasoning in LLM-based E2E web testing, demonstrating high accuracy, generalization, and effectiveness in detecting real bugs while minimizing hallucination impacts.
...
TL;DR: The paper proposes a reproducible, scalable approach using LLMs in a RAG-pipeline with a two-layer prompting strategy to extract 1,312 high-quality, evidence-based actionable recommendations (ReACTs) from 829 SE papers, linking empirical research to concrete practices for improving OSS sustainability.


<details>
  <summary>Details</summary>
Motivation: While models exist to forecast OSS sustainability, they often rely on high-level, non-actionable socio-technical signals. Meanwhile, a wealth of empirical software engineering research contains concrete, effective practices that remain underutilized. This gap between predictive models and actionable guidance motivates the need for a systematic method to transform research findings into practical recommendations for OSS maintainers.

Method: The authors design a Retrieval-Augmented Generation (RAG) pipeline with a two-layer prompting strategy. In the first layer, they evaluate open LLMs and prompting techniques to extract candidate ReACTs (researched actionables) from 829 ICSE and FSE papers. In the second layer, follow-up prompts filter hallucinations, extract evidence and impact, and assess the soundness and precision of each ReACT. Extracted ReACTs are then categorized into practice-oriented groups and aligned with project signals from tools like APEX.

Result: The pipeline produces 1,922 candidate ReACTs, of which 1,312 meet strict quality criteria. These high-quality ReACTs are structured into actionable, evidence-linked recommendations organized by practice category, enabling direct mapping to observable project signals and providing maintainers with concrete steps to improve sustainability.

Conclusion: The study demonstrates a scalable and reproducible method for transforming fragmented empirical SE research into structured, trustworthy, and actionable guidance for OSS projects. By leveraging LLMs as evidence miners, the approach bridges the gap between sustainability forecasting and practical intervention, empowering maintainers with evidence-based actions to enhance project health and longevity.
...
TL;DR: We propose AmbiBench, the first benchmark for Mobile GUI Agents that evaluates bidirectional intent alignment through a taxonomy of instruction clarity; we also introduce MUSE, an automated multi-dimensional evaluator based on MLLM-as-a-judge, which shows high agreement with human judgment and highlights the importance of active clarification in mobile agents.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for Mobile GUI Agents assume complete and clear user instructions, focusing only on single-turn execution and neglecting the agents' ability to align with ambiguous or evolving user intent. This fails to reflect real-world scenarios where users often provide vague or incomplete directives.

Method: The authors introduce AmbiBench, a benchmark based on a four-level taxonomy of instruction clarity (Detailed, Standard, Incomplete, Ambiguous) grounded in Cognitive Gap theory. They construct a dataset of 240 real-world tasks across 25 apps and develop MUSE, an automated multi-agent evaluation framework using MLLM-as-a-judge to assess Outcome Effectiveness, Execution Quality, and Interaction Quality.

Result: AmbiBench reveals performance limitations of state-of-the-art agents under varying instruction clarity levels, demonstrates measurable gains from active interaction, and shows strong correlation between the proposed MUSE evaluator and human judgment across three assessment dimensions.

Conclusion: AmbiBench redefines evaluation standards for Mobile GUI Agents by introducing bidirectional intent alignment and enabling assessment of agents' ability to handle ambiguous instructions through active clarification, paving the way for next-generation agents with improved user intent understanding.
...
TL;DR: The paper proposes a novel approach to verifiable source code provenance using zero-knowledge virtual machines (zkVMs) to generate cryptographic proofs that a binary was compiled from claimed source code and compiler, demonstrating strong security and practical applicability.


<details>
  <summary>Details</summary>
Motivation: Verifiable source code provenance is essential for security, but existing methods like reproducible builds are difficult due to complex build environment matching; the paper aims to provide a more robust and practical solution.

Method: The authors compile software using a zero-knowledge virtual machine (zkVM), specifically executing a compiler (ChibiCC) within the RISC Zero zkVM, which generates both the compiled binary and a cryptographic proof of correct execution on the claimed source and compiler.

Result: The system is evaluated on 200 synthetic programs, 31 OpenSSL, and 21 libsodium files, showing zk-compilation is feasible for real-world software and effectively prevents compiler substitution, source tampering, output manipulation, and replay attacks.

Conclusion: zkVM-based compilation provides a practical and secure solution for verifiable source code provenance, offering stronger guarantees than reproducible builds without requiring exact build environment replication.
...
TL;DR: This study evaluates the use of large language models (LLMs) like Claude Sonnet 4.5 and GPT-5.2 for co-evolving textual DSL instances with evolving grammars, focusing on correctness and preservation of human-readable elements like layout and comments.


<details>
  <summary>Details</summary>
Motivation: When software languages evolve, existing textual instances may become incompatible with updated grammars. Traditional model-driven approaches for co-evolution are not well-suited for textual DSLs and often discard human-relevant formatting and comments. This work addresses the gap by assessing whether LLMs can effectively adapt instances to grammar changes while preserving such information.

Method: The authors conduct a systematic evaluation using ten case languages, applying grammar changes and using LLMs (Claude Sonnet 4.5 and GPT-5.2) to update corresponding instances. They perform ten runs per case, measuring precision, recall, response time, and the impact of factors like change scale, complexity, and deletion granularity. Human-oriented information preservation is qualitatively assessed.

Result: LLMs achieve high precision and recall (≥94%) for small changes (fewer than 20 modified lines). Performance declines with scale: Claude maintains 85% recall at 40 lines, while GPT fails on the largest instances. Response time increases significantly with instance size. Grammar evolution complexity and deletion granularity have greater impact on performance than change type. Layout and comments are generally well-preserved.

Conclusion: LLM-based co-evolution is effective for small to moderate grammar changes in textual DSLs and preserves human-oriented details well. However, performance degrades with larger changes, especially for GPT, and efficiency remains a concern. The results identify the boundaries of current LLM applicability in this domain.
...
TL;DR: This paper evaluates modern small language models (SLMs) as code correctness judges, showing they outperform the previous RankEF model and rival much larger LLMs in code ranking, offering a cost-effective alternative for code generation.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs underperform on less common or domain-specific languages, making in-house code generators costly to train; smaller models offer a cheaper alternative but their reliability in judging code correctness is not well understood.

Method: The authors train and evaluate state-of-the-art SLMs as code correctness classifiers and rankers, comparing their performance to RankEF and large LLMs, without relying on execution-based feedback.

Result: Modern SLMs surpass RankEF in accuracy and ranking effectiveness, even without execution traces, and achieve competitive performance with LLMs 5-25x larger in code generation tasks.

Conclusion: Modern SLMs can serve as reliable, cost-effective code correctness judges and rankers, enabling organizations to develop high-performance code generators at significantly lower cost.
...
TL;DR: ChatGPT-4 outperforms humans in quantity of code review comments but misses 90% of human-identified issues; however, many of its extra suggestions are valid, indicating a complementary rather than replacement role for AI in code review.


<details>
  <summary>Details</summary>
Motivation: To assess the extent to which DL-based code review tools (using ChatGPT-4 as a representative) can replicate or replace human code review in identifying meaningful quality improvements.

Method: A mining-based study was conducted using 240 PRs and 739 human review comments. Manual classification of human-suggested quality improvements was performed. ChatGPT-4 was used to generate automated reviews on the same PRs, and its recommendations were compared against human ones in terms of coverage and quality.

Result: ChatGPT-4 suggests ~2.4x more code changes than humans but only detects 10% of human-reported quality issues. However, ~40% of its additional suggestions highlight valid, meaningful quality problems not identified by humans.

Conclusion: DL-based code review, exemplified by ChatGPT-4, is not a substitute for human review but can serve as a complementary quality check. Human reviewers must still validate both their own and the model's findings, and the technique does not currently reduce code review effort.
...
TL;DR: Context files like AGENTS.md, whether LLM-generated or human-written, harm coding agent performance by increasing task difficulty and cost—minimal context is recommended.


<details>
  <summary>Details</summary>
Motivation: To investigate whether the widely adopted practice of using context files (e.g., AGENTS.md) actually improves coding agent performance, as recommended by agent developers, despite lacking empirical validation.

Method: Evaluated coding agents' performance on SWE-bench tasks with LLM-generated context files and on real-world issues from repositories with developer-committed context files, comparing success rates and costs.

Result: Context files reduce task success rates compared to no context and increase inference cost by over 20%; both LLM-generated and developer-provided context files lead to broader exploration but do not improve outcomes.

Conclusion: Unnecessary requirements in context files make tasks harder for coding agents; human-written context files should be minimal to avoid reducing task success rates and increasing inference costs.
...
TL;DR: This paper investigates the impact of class imbalance on deep learning-based vulnerability detection through an empirical study on nine datasets and two models, showing that imbalance significantly affects performance and that different mitigation strategies improve different metrics, with no universal solution.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from observed performance variability in deep learning-based vulnerability detection across datasets, which the authors hypothesize is due to the class imbalance problem—where vulnerable code instances are vastly outnumbered by non-vulnerable ones—common in software security datasets.

Method: A comprehensive empirical study was conducted using nine open-source datasets and two state-of-the-art deep learning models to analyze the impact of class imbalance on vulnerability detection; the study evaluates the performance of various imbalance mitigation techniques such as focal loss, mean false error, class-balanced loss, and random over-sampling.

Result: The study confirms that class imbalance is a core factor behind inconsistent model performance; it finds that focal loss improves precision, mean false error and class-balanced loss enhance recall, and random over-sampling boosts F1-measure, but no single method excels across all evaluation metrics.

Conclusion: The imbalance issue significantly affects the performance of deep learning-based vulnerability detection models, and existing imbalance solutions exhibit inconsistent effectiveness across datasets and metrics; thus, there is a need for more robust, universally effective solutions tailored to this domain.
...
TL;DR: ModelWisdom enhances TLA+ model checking with interactive visualization and large language models, offering intuitive navigation, semantic explanations, and automated summarization of counterexamples to improve debugging efficiency and model understanding.


<details>
  <summary>Details</summary>
Motivation: Practitioners struggle with interpreting TLA+ model-checking counterexamples due to the lack of explainability in raw outputs and the absence of scalable, semantic tools for navigating large state-transition graphs, which hampers debugging efficiency and correctness assurance.

Method: ModelWisdom integrates interactive visualization techniques with large language models (LLMs) to enhance TLA+ model checking, featuring colorized violation highlighting, code-to-transition linking, graph folding, tree-based structuring, LLM-based summarization (Model Digest), and structured error extraction for iterative model repair.

Result: ModelWisdom provides an interactive environment that visualizes model-checking results with semantic highlights, optimizes graph navigation through folding and structuring, explains subgraphs using LLMs, and supports targeted model repair, thereby reducing manual debugging effort and improving comprehension of violations.

Conclusion: ModelWisdom significantly improves the interpretability and usability of TLA+ model checking by transforming raw counterexample output into an interactive and explainable debugging workflow, enabling more effective understanding and repair of complex specifications.
...
TL;DR: This paper investigates the impact of performance antipatterns on energy consumption in microservice-based systems, finding that while all antipatterns degrade performance, only a subset significantly increase power consumption, with implications for energy-efficient design.


<details>
  <summary>Details</summary>
Motivation: To determine whether known performance antipatterns in microservices also negatively affect energy consumption, extending performance concerns to energy efficiency.

Method: Implemented ten performance antipatterns as isolated microservices; conducted controlled experiments with 30 runs per antipattern, measuring performance, CPU/DRAM power, and resource utilization under load.

Result: All antipatterns degraded performance, but only a subset showed significant increases in power consumption; some reached CPU saturation (capped power), while others (e.g., Unnecessary Processing, The Ramp) showed clear energy-performance coupling.

Conclusion: Not all performance antipatterns are energy antipatterns; the study identifies which ones are and provides insights for designing more energy-efficient microservices.
...
TL;DR: PPTAM$\eta$ is a GitLab CI-integrated pipeline that automates power and energy measurement for containerized APIs, enabling continuous energy monitoring across commits.


<details>
  <summary>Details</summary>
Motivation: Energy consumption is rarely measured in CI/CD pipelines despite its importance for sustainability and performance, and while code changes affect energy efficiency, there is a lack of tooling to monitor it continuously.

Method: PPTAM$\eta$ is an automated pipeline that orchestrates load generation, container monitoring, and hardware power probing within GitLab CI to collect performance and energy metrics at each commit.

Result: Evaluated on a JWT-authenticated API across four commits, PPTAM$\eta$ collected comparable performance and energy metrics, demonstrating its ability to enable version comparison and trend analysis.

Conclusion: PPTAM$\eta$ successfully integrates energy measurement into CI/CD pipelines for containerized API systems, making energy consumption visible and actionable for developers, test engineers, and researchers.
...
TL;DR: First empirical study of AI-generated code in mobile OSS shows Android has higher AI PR volume and acceptance than iOS, with better outcomes for routine tasks and fluctuating resolution times over time.


<details>
  <summary>Details</summary>
Motivation: Despite growing use of AI coding agents in software development, their empirical impact on mobile app development remains understudied, especially across platforms and task types.

Method: Analyzed 2,901 AI-authored pull requests from 193 open-source Android and iOS GitHub repositories in the AIDev dataset, categorizing by platform, agent type, and task category to assess PR acceptance rates and resolution times.

Result: Android projects received twice as many AI-authored PRs and had higher acceptance rates (71%) compared to iOS (63%), with significant variation across agents. Routine tasks (feature, fix, ui) had higher acceptance and faster resolution than structural tasks (refactor, build). PR resolution time on Android improved until mid-2025 before declining.

Conclusion: This study provides the first empirical characterization of AI coding agents' impact on open-source mobile development, revealing platform, agent, and task-level disparities in PR acceptance and resolution, with implications for designing platform-aware agentic coding systems.
...
TL;DR: This paper investigates few-shot unit test generation using LLMs, showing that human-written examples and combined similarity-based retrieval significantly enhance test quality in terms of correctness, coverage, readability, and maintainability.


<details>
  <summary>Details</summary>
Motivation: Manually writing unit tests is labor-intensive, and while LLMs show promise, their few-shot learning potential for unit test generation remains underexplored despite growing integration in development workflows.

Method: Empirical evaluation using few-shot prompting with test artifacts from human, SBST, and LLM sources on GPT-4o; assessment of retrieval-based methods based on similarity of problem description and code; experiments conducted on HumanEval and ClassEval datasets.

Result: LLMs generate higher-quality unit tests using few-shot prompting, with human-written examples achieving the best correctness and coverage; combining similarity-based retrieval of problem description and code yields the most effective prompts.

Conclusion: Few-shot prompting of LLMs, particularly with human-written test examples, significantly improves the quality of generated unit tests in terms of correctness, coverage, and maintainability, especially in hybrid human-AI development environments.
...
TL;DR: STAB is a novel backdoor attack for code models that achieves high transferability and stealthiness by leveraging flat regions of the loss landscape via sharpness-aware training and context-aware trigger generation using Gumbel-Softmax optimization.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks on code models face a trade-off between transferability (static triggers) and stealthiness (dynamic triggers), with dynamic methods also relying on unrealistic data distribution assumptions. This limits their effectiveness in real-world scenarios.

Method: STAB uses Sharpness-Aware Minimization to train a surrogate model, guiding parameters toward flat loss regions for better cross-dataset transferability. It then generates context-aware, discrete adversarial triggers via Gumbel-Softmax optimization, enabling differentiable search over token sequences without assuming identical data distributions.

Result: STAB achieves a 73.2% average attack success rate after defense, outperforming static triggers that fail under defenses. It surpasses the best dynamic trigger attack by 12.4% in cross-dataset success rate while maintaining clean accuracy.

Conclusion: STAB effectively bridges the gap between transferability and stealthiness in backdoor attacks on code models, demonstrating superior performance across datasets and defenses without requiring full knowledge of victim training data.
...
TL;DR: Yaksha-Prashna is a system that enables cloud operators and eBPF developers to verify the functional correctness and dependencies of third-party eBPF bytecode without requiring source code disclosure, using scalable program analysis and a domain-specific language to express properties with significant performance gains.


<details>
  <summary>Details</summary>
Motivation: Third-party eBPF-based network functions are widely used in cloud infrastructure for security, observability, and load balancing, but are distributed in bytecode form, limiting operators' ability to assess their correctness and interactions. Developers also need to prove correctness without revealing source code. This creates a need for a trustworthy, efficient verification mechanism.

Method: The authors design Yaksha-Prashna, which uses domain-specific models and scalable program analysis to extract and model eBPF programs from bytecode. They introduce the Yaksha-Prashna language to formally express behavioral and dependency properties, enabling verification of conformance to specifications without source code.

Result: The system successfully expresses 24 properties across standard and non-standard eBPF network functions, achieving 200 to 1000 times faster analysis compared to existing state-of-the-art approaches.

Conclusion: Yaksha-Prashna provides an efficient, scalable, and privacy-preserving solution for verifying third-party eBPF bytecode, empowering operators to ensure safe deployment and developers to assert correctness without compromising intellectual property.
...
TL;DR: This paper introduces peak + accumulation scoring, a novel method to detect multi-turn prompt injection attacks by aggregating per-turn risks using peak value, persistence, and diversity, outperforming weighted averages and achieving high detection performance without invoking an LLM.


<details>
  <summary>Details</summary>
Motivation: Existing detection methods fail to properly aggregate per-turn risk scores into a conversation-level assessment, leading to poor detection of multi-turn prompt injection attacks that spread malicious intent across turns; no effective formula has been published for proxy-layer, LLM-free detection.

Method: The paper proposes a novel scoring formula—peak + accumulation—that combines peak single-turn risk, persistence ratio, and category diversity, inspired by change-point detection (CUSUM), Bayesian belief updating, and security alerting models, without requiring additional LLM calls.

Result: The proposed formula achieves 90.8% recall at a 1.20% false positive rate (F1 = 85.9%) on 10,654 conversations (588 attacks from WildJailbreak, 10,066 benign from WildChat); sensitivity analysis reveals a phase transition in recall at persistence parameter rho ~ 0.4.

Conclusion: The proposed peak + accumulation scoring formula effectively detects multi-turn prompt injection attacks by addressing the limitations of weighted-average approaches, demonstrating high recall and low false positive rates, and is released as open-source for broader adoption.
...
TL;DR: This paper introduces CryptoAnalystBench, a benchmark for evaluating LLM-based analyst agents in high-data-density crypto/DeFi tasks, revealing persistent higher-order failure modes not captured by current evaluation methods. Using human annotation, the authors develop an error taxonomy and refine an LLM-as-judge evaluator that, while not perfectly scoring, reliably identifies critical flaws. The work highlights evaluation challenges in multi-tool, long-context agent systems and releases tools and frameworks to support future research.


<details>
  <summary>Details</summary>
Motivation: The motivation is to study LLM failure modes in complex analyst agent settings where large volumes of dynamic, structured, and unstructured data from multiple tools must be integrated—specifically in high-data-density domains like crypto and DeFi—where current evaluation methods focusing on factuality or simple tool use are insufficient.

Method: The authors introduce CryptoAnalystBench, a benchmark of 198 crypto/DeFi queries, and an agentic evaluation harness with crypto-specific tools. They generate responses using multiple frontier LLMs, evaluate them using a multi-dimensional rubric (relevance, temporal relevance, depth, data consistency), and perform human annotation to identify failure modes. They refine an LLM-as-judge evaluator based on a human-derived error taxonomy.

Result: The authors identify a taxonomy of seven higher-order error types not captured by standard factuality or quality checks. They find these errors persist even in state-of-the-art systems and can impact high-stakes decisions. The LLM-as-judge shows low alignment with humans on fine-grained scoring but reliably detects critical failures. The evaluation pipeline, benchmark, tools, rubrics, and error taxonomy are released.

Conclusion: The study concludes that existing LLM evaluation methods fail to capture critical higher-order error types in complex, multi-tool, data-dense analyst agent workflows, especially in high-stakes domains like crypto and DeFi. While LLM-based judges do not perfectly align with human scores, they effectively identify critical failure modes, enabling scalable agent evaluation. The authors advocate for improved evaluation frameworks and provide open resources to support future research.
...
TL;DR: The paper introduces a category-theoretic framework for modeling trust in trusted computing and remote attestation, formalizing trust components and processes categorically, using Heyting algebras for nuanced trust levels, and applying exponentiation to measure attestation expressiveness, with examples and extensions to complex systems.


<details>
  <summary>Details</summary>
Motivation: To provide a rigorous, formal foundation for trust in trusted computation systems, addressing ambiguities in terms like 'trustworthiness' and 'justification', and enabling precise modeling of attestation processes beyond binary trust outcomes.

Method: Models elements, claims, results, and decisions as objects in a category, with attestation, verification, and decision-making as morphisms; uses Heyting algebra to structure the trust decision space; employs categorical exponentiation to compose attestation operations and measure expressivity of attestation environments.

Result: A formal framework that enables nuanced trust modeling, supports compositional analysis of attestation protocols, provides semantics for trust terms, and applies to real-world scenarios like boot sequences and Evil Maid attacks, with demonstrated scalability to dynamic, complex systems.

Conclusion: Category theory provides a powerful and expressive foundation for modeling trust in computation systems, enabling precise, compositional, and semantically rich analysis of remote attestation and trust decision-making.
...
TL;DR: This paper proposes a multi-layered DDoS mitigation system tailored for containerized cloud environments, effectively defending against low-rate DDoS attacks by combining WAF, rate limiting, dynamic blacklisting, header analysis, and zero trust principles.


<details>
  <summary>Details</summary>
Motivation: Low-rate DDoS attacks are hard to detect due to minimal traffic anomalies but can severely impact containerized applications; existing defenses are often insufficient in such dynamic, lightweight infrastructures.

Method: The system employs a multi-layered approach: integrates a Web Application Firewall (WAF), enforces rate limiting, applies dynamic blacklisting, analyzes TCP/UDP headers, and adopts zero trust principles to inspect every packet; it is deployed within Docker-orchestrated environments for seamless management.

Result: The proposed system effectively detects and blocks low-rate DDoS attacks at various stages of the attack lifecycle, enhances security through continuous packet inspection, and demonstrates improved resilience and ease of deployment in containerized setups.

Conclusion: Combining multiple defense mechanisms with zero trust and container orchestration support provides a robust, scalable solution for mitigating low-rate DDoS attacks in modern cloud infrastructures.
...
TL;DR: This paper introduces autonomy metrics to evaluate AI agents under system-level defenses for prompt injection, and proposes a security-aware agent design that enhances human-in-the-loop interactions and joint planning for task and policy compliance, achieving higher autonomy without compromising security or utility.


<details>
  <summary>Details</summary>
Motivation: Indirect prompt injection attacks pose a serious threat to AI agents performing consequential actions, necessitating robust system-level defenses. While such defenses ensure security through confidentiality and integrity policies, they often reduce task efficiency and increase reliance on human oversight. The authors aim to reduce this reliance by quantifying agent autonomy and improving it through better agent design.

Method: The authors design a security-aware agent that enhances human-in-the-loop (HITL) interactions and jointly plans for task progress and policy compliance. This agent is implemented within an information-flow control framework and evaluated using the AgentDojo and WASP benchmarks.

Result: The proposed approach increases agent autonomy—measured as the fraction of actions executed without human approval—while maintaining security and task completion performance. Experiments on AgentDojo and WASP show improved autonomy without sacrificing utility, compared to prior system-level defenses.

Conclusion: The proposed security-aware agent design, which incorporates richer human-in-the-loop interactions and explicit planning for policy compliance, enables higher autonomy in AI agents without compromising task utility, when implemented with system-level defenses against indirect prompt injection.
...
TL;DR: Google and Intel conducted a security assessment of Intel TDX in 2025, resulting in the discovery of one critical vulnerability allowing VMM to compromise a TD and four vulnerabilities enabling memory leaks from the TDX Module, highlighting the need for defense-in-depth in confidential computing.


<details>
  <summary>Details</summary>
Motivation: To evaluate the security of Intel TDX following major updates such as Live Migration and TD Partitioning, building on prior assessments and ensuring robustness of confidential computing features.

Method: Collaborative assessment with access to TDX-enabled hardware, development of a live testing toolkit, analysis using Gemini and NotebookLM, and review of updated TDX 1.5 source code and documentation.

Result: Identified one critical vulnerability (VMM can fully compromise a TD), four vulnerabilities allowing confidential memory leakage from the TDX Module, and several security weaknesses with limited impact.

Conclusion: Confidential computing technologies like TDX require continuous security refinement and layered defenses (defense-in-depth) to address evolving threats and implementation flaws.
...
TL;DR: This paper introduces address space layout randomization (ASLR) into the OSv unikernel to reduce layout predictability and improve security against exploitation, with minimal performance overhead.


<details>
  <summary>Details</summary>
Motivation: Unikernels like OSv lack standard security mitigations such as ASLR, making them vulnerable to predictable memory layouts that facilitate reliable exploitation.

Method: The authors implement ASLR-style randomization in OSv by modifying core memory-management and loading routines to randomize application base addresses and thread stack regions.

Result: The implementation shows comparable boot time, runtime performance, and memory usage to the baseline, while achieving uniformly distributed address layouts.

Conclusion: Layout randomization can be efficiently integrated into unikernels like OSv without sacrificing performance, significantly improving resistance to exploitation.
...
TL;DR: Cachemir introduces a KV cache-accelerated homomorphic encrypted LLM inference framework that overcomes the inefficiency of FHE-based LLMs in autoregressive decoding by enabling effective KV cache integration, achieving significant speedups over prior works.


<details>
  <summary>Details</summary>
Motivation: The widespread use of LLMs raises privacy concerns due to untrusted environments handling sensitive user inputs. Fully homomorphic encryption (FHE) offers secure inference but suffers from high latency, especially in autoregressive generation, because existing frameworks cannot effectively utilize the key-value (KV) cache, leading to redundant computations.

Method: Cachemir proposes three key techniques: (1) novel homomorphic encryption (HE) packing algorithms tailored to exploit KV cache reuse, (2) an interleaved replicated packing method to optimize vector-matrix multiplication in Transformer layers with cached keys and values, and (3) an augmented bootstrapping placement strategy that considers KV cache state to reduce bootstrapping frequency and cost.

Result: Cachemir achieves 48.83x speedup over MOAI (ICML'25) and 67.16x over THOR (CCS'25) on CPU, and generates each output token in under 100 seconds on GPU for Llama-3-8B, demonstrating efficient FHE-based autoregressive LLM inference with KV cache support.

Conclusion: By enabling effective integration of the KV cache in fully homomorphic encrypted LLM inference, Cachemir significantly reduces latency and makes private LLM inference practical, setting a new standard for secure and efficient LLM deployment.
...
TL;DR: This paper proposes a Unified Smart Safety and Security Architecture for intelligent mining systems, integrating multimodal perception, secure federated learning, reinforcement learning, delay-tolerant networking, and energy-aware sensing to ensure operational reliability under harsh and adversarial conditions.


<details>
  <summary>Details</summary>
Motivation: Modern mining environments face significant challenges including poor illumination, GPS denial, intermittent connectivity, and increasing cyber-physical threats, which collectively degrade perception, situational awareness, and trust in AI-driven systems.

Method: The authors design a unified architecture with five core modules: Miner Finder for localization, Multimodal Situational Awareness for hazard detection, Backdoor Attack Monitor for intrusion detection, TrustFed LFD for robust federated learning with defense against poisoned updates, and IoT-driven Equipment Health Monitoring for predictive maintenance, leveraging reinforcement learning, secure FL, and DTN-based communication.

Result: The framework enables resilient miner localization, continuous hazard awareness, secure collaborative learning despite malicious inputs, and reliable equipment monitoring under energy and connectivity constraints.

Conclusion: The proposed architecture provides a comprehensive, end-to-end solution for building trustworthy, adaptive, and secure AI-driven mining systems capable of maintaining safety and operational continuity in adversarial and infrastructure-limited environments.
...
TL;DR: DEL enables privacy-preserving and communication-efficient LLM inference through differentially private quantization and server-side soft prompts, eliminating the need for local models while maintaining utility.


<details>
  <summary>Details</summary>
Motivation: The high computational demands of LLMs prevent local deployment, leading to reliance on cloud-based inference, which raises privacy concerns. Existing privacy-preserving methods suffer from high communication or computational costs, necessitating a more efficient and practical solution.

Method: DEL introduces an embedding projection module and a differentially private stochastic quantization mechanism to reduce communication overhead while preserving privacy. It eliminates local models by using server-side soft prompts to recover utility lost due to privacy-preserving transformations.

Result: Extensive experiments show that DEL effectively reduces communication overhead and maintains strong performance on text generation and natural language understanding tasks, outperforming existing approaches in the privacy-utility trade-off.

Conclusion: DEL is a novel framework that addresses both privacy and communication efficiency in LLM split inference by combining differentially private quantization, embedding projection, and server-side soft prompts, achieving an improved privacy-utility trade-off.
...
TL;DR: The paper proposes TRACE-RPS, a unified defense framework combining fine-grained text anonymization (TRACE) and inference-preventing optimization (RPS) to mitigate privacy breaches in large language models by reducing attribute inference accuracy from ~50% to <5%.


<details>
  <summary>Details</summary>
Motivation: Large language models can infer sensitive user attributes from online text, posing significant privacy risks. Existing anonymization methods are coarse and ineffective at preventing inference based on model reasoning, necessitating a more precise and robust defense.

Method: The framework consists of two components: (1) TRACE, which uses attention mechanisms and inference chain generation to identify and anonymize privacy-leaking words at a fine-grained level; and (2) RPS, which applies a lightweight two-stage optimization to train models to reject attribute inference queries. The combination aims to both mask sensitive cues and disrupt inference mechanisms.

Result: TRACE-RPS reduces attribute inference accuracy from approximately 50% to below 5% across various open-source LLMs. It demonstrates strong generalization across models, robustness to prompt variations, and favorable utility-privacy tradeoffs.

Conclusion: TRACE-RPS provides an effective, generalized, and robust defense against privacy leakage in LLMs by jointly addressing textual and reasoning-based vulnerabilities, setting a new standard for privacy-preserving user interaction with language models.
...
TL;DR: QDBFT is a quantum-secured dynamic consensus algorithm that combines consistent hash-based node rotation and Quantum Key Distribution (QKD) to achieve information-theoretic security and resilience against quantum attacks while maintaining performance comparable to traditional PBFT.


<details>
  <summary>Details</summary>
Motivation: Classical blockchain security relies on cryptographic assumptions vulnerable to quantum computing, particularly via Shor's algorithm. Additionally, existing PBFT protocols lack efficiency in dynamic node environments and offer no quantum resistance.

Method: The authors propose QDBFT, which introduces (1) a primary node automatic rotation mechanism using a consistent hash ring for dynamic membership management and equitable authority distribution, and (2) integration of QKD networks to secure inter-node communication with information-theoretic security.

Result: Experiments show QDBFT achieves comparable performance to traditional PBFT in terms of throughput and latency, while providing robust security against quantum adversaries and supporting efficient dynamic node reconfiguration.

Conclusion: QDBFT offers a practical and quantum-secure alternative to conventional consensus protocols, enabling future blockchain systems to withstand quantum threats and operate efficiently in dynamic environments.
...
TL;DR: DMind-3 is a sovereign Edge-Local-Cloud intelligence stack for Web3 that ensures secure, low-latency financial execution via a three-tier architecture and novel training methods, outperforming baselines in success rate and reasoning accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing Web3 assistants either compromise privacy (cloud-centric) or lack ecosystem-wide context (local-only), making them vulnerable to adversarial risks and latency issues. DMind-3 aims to unify privacy, global awareness, and real-time performance for secure financial execution.

Method: DMind-3 employs a three-layer architecture: an edge-based intent firewall, a local reasoning engine, and a cloud-based context synthesizer. It uses policy-driven selective offloading, Hierarchical Predictive Synthesis (HPS) for macro-signal fusion, and Contrastive Chain-of-Correction SFT (C^3-SFT) for improved local verification.

Result: DMind-3 achieves a 93.7% multi-turn success rate in protocol-constrained tasks and demonstrates superior domain-specific reasoning over general-purpose models, with effective balance between local security and global context utilization.

Conclusion: DMind-3 provides a scalable, sovereign intelligence framework for Web3 by tightly binding safety to edge execution while enabling efficient, privacy-preserving use of cloud resources, thus ensuring secure and low-latency financial decision-making.
...
TL;DR: The paper proposes a continuous learning framework for edge-based malware detection using lightweight transformers and LoRA adapters, enabling local adaptation and global knowledge sharing without exchanging raw data, achieving significant accuracy improvements under evolving threats with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Edge devices face growing malware threats, but existing solutions are hindered by resource constraints, model staleness, and lack of cross-device generalization. Centralized or static models fail to adapt to evolving and heterogeneous edge environments, necessitating an efficient, adaptive, and privacy-preserving approach.

Method: The authors deploy parameter-efficient fine-tuning via LoRA (Low-Rank Adaptation) on lightweight transformer models (DistilBERT, DistilGPT-2, TinyT5) at the edge. Each device performs local incremental learning on its traffic; only LoRA adapter weights are sent to a coordinator for aggregation and redistribution, enabling knowledge transfer without sharing raw data. This supports continuous learning across devices under resource constraints.

Result: Evaluated on Edge-IIoTset and TON-IoT datasets, the LoRA-based approach improves accuracy by 20–25% over isolated models when detecting cross-domain unseen attacks. The method maintains stable loss and F1 scores across learning rounds, with LoRA adding less than 1% to model size (0.6–1.8 MB), making it feasible for edge deployment.

Conclusion: The proposed architecture effectively enables continuous, collaborative malware detection on edge devices by combining local adaptability with global knowledge transfer through lightweight LoRA updates, offering strong performance gains with minimal resource overhead and preserving data privacy.
...
TL;DR: This paper challenges the 'stronger-is-better' paradigm in LLM watermarking by showing that strong watermarks reduce token entropy, harming multi-layer detection; instead, it proposes a framework using weaker watermarks to preserve entropy, improving overall detectability and robustness.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the observation that existing watermark ensemble methods, which aim to maximize watermark strength at each layer, inadvertently reduce token distribution entropy, thereby degrading detectability in deeper layers due to signal decay.

Method: The authors propose a general watermarking framework that employs weaker single-layer watermarks to maintain higher entropy in token distributions. They analyze the theoretical relationship between watermark strength, entropy, and detectability, and validate their approach through empirical evaluation across multiple layers.

Result: Experiments show that the proposed method mitigates entropy loss and green-list ratio decay across layers, resulting in enhanced detectability and robustness compared to strong watermarking baselines, despite using weaker individual watermarks.

Conclusion: The proposed framework of using weaker single-layer watermarks enables more effective multi-layer ensembling by preserving token distribution entropy, leading to improved detectability and robustness compared to conventional strong watermarking methods.
...
TL;DR: This paper proposes a novel, dynamically reconfigurable Collaborative Intrusion Detection System (CIDS) framework for distributed environments, optimized for edge devices. It enables resource-aware, adaptive detection with minimal overhead and is evaluated on diverse datasets, including a new public dataset simulating a cyberattack on a drone targeting critical infrastructure.


<details>
  <summary>Details</summary>
Motivation: The increasing dynamism and heterogeneity of distributed critical infrastructures, such as civil and military drone systems, demand flexible and adaptive CIDS architectures capable of evolving with changing operational conditions without incurring high computational costs.

Method: A novel CIDS framework was developed with dynamic detector allocation based on node resources and data types. It was informed by a literature review and real-world use cases, and evaluated using multiple distributed datasets, including a newly introduced public dataset simulating a realistic cyberattack on a ground drone.

Result: The framework demonstrated effective adaptive intrusion detection across diverse network topologies and attack chains, achieving optimal detector configurations with minimal computational overhead, validated through experiments on edge devices.

Conclusion: The proposed CIDS framework enables adaptive, efficient, and lightweight intrusion detection in dynamic distributed environments, particularly suitable for resource-constrained edge devices in critical infrastructures such as drone systems.
...
TL;DR: This paper proposes a shift in AI-driven cybersecurity from model-centric pipelines to agentic, multi-agent cognitive systems governed by meta-cognitive judgement, enabling accountable and context-aware decision-making under adversarial uncertainty.


<details>
  <summary>Details</summary>
Motivation: Current AI cybersecurity systems focus on task-level performance but lack accountability, justification, and governance under uncertain, adversarial conditions, limiting their suitability for real-world operational and regulatory environments.

Method: The authors introduce a conceptual architectural framework based on distributed cognition and multi-agent systems, where specialized AI agents handle detection, interpretation, explanation, and governance, coordinated by a meta-cognitive judgement function that regulates autonomy and decision readiness.

Result: The framework enables dynamic calibration of system autonomy in response to incomplete or conflicting evidence, aligning AI-driven responses with organizational and regulatory requirements, and exposing the implicit cognitive structure in existing security operations.

Conclusion: Embedding meta-cognitive judgement as a first-class function in cybersecurity architectures enables accountable, governed, and context-sensitive AI autonomy, advancing the design of next-generation cyber defense systems.
...
### [50] [PAC to the Future: Zero-Knowledge Proofs of PAC Private Systems](https://arxiv.org/abs/2602.11954)
*Guilhem Repetto,Nojan Sheybani,Gabrielle De Micheli,Farinaz Koushanfar*

Main category: cs.CR

TL;DR: This paper proposes a verifiable privacy framework by combining PAC Privacy with zero-knowledge proofs, enabling users to validate privacy and correctness guarantees in cloud-based machine learning without trusting the computation provider.


<details>
  <summary>Details</summary>
Motivation: Traditional privacy-preserving techniques lack verifiability in untrusted environments, making it difficult for users to confirm that privacy mechanisms like noise addition are properly applied during outsourced computations.

Method: The authors propose a novel framework that integrates PAC Privacy with non-interactive zero-knowledge proofs (ZKPs) to allow verification of correct noise application and computation correctness without revealing sensitive or proprietary information.

Result: The framework demonstrates feasibility in achieving verifiable PAC privacy, with efficient proof generation and verification that ensures both privacy and integrity in cloud-based systems.

Conclusion: The combination of PAC Privacy and zero-knowledge proofs enables verifiable privacy guarantees in trustless environments, offering a practical solution for ensuring both privacy and computational integrity in outsourced machine learning and database systems.
...
### [51] [Evaluation of Security-Induced Latency on 5G RAN Interfaces and User Plane Communication](https://arxiv.org/abs/2602.12059)
*Sotiris Michaelides,Jakub Lapawa,Daniel Eguiguren Chavez,Martin Henze*

Main category: cs.CR

TL;DR: This paper evaluates the latency overhead of enabling optional 5G security controls in disaggregated RAN and user plane architectures, showing that while disaggregation maintains a latency advantage, achieving sub-1 ms round-trip times is challenging due to cryptographic overhead.


<details>
  <summary>Details</summary>
Motivation: The motivation is to assess how optional 5G security mechanisms impact latency in disaggregated network architectures, particularly for mission-critical applications requiring ultra-reliable low-latency communication (URLLC).

Method: The authors deploy the first testbed implementing a disaggregated RAN with standardized optional security mechanisms, guided by input from a network operator and a 5G manufacturer, to measure latency overhead across internal RAN interfaces and the user plane.

Result: Disaggregated RAN deployments retain a latency advantage over monolithic designs even with security enabled, but cryptographic overhead from security controls can exceed the 1 ms round-trip latency target.

Conclusion: While disaggregation supports lower latency in 5G networks, achieving sub-1 ms round-trip times is difficult when optional security controls are enabled, indicating a trade-off between security and latency performance in 5G URLLC deployments.
...
### [52] [MedExChain: Enabling Secure and Efffcient PHR Sharing Across Heterogeneous Blockchains](https://arxiv.org/abs/2602.12106)
*Yongyang Lv,Xiaohong Li,Kui Chen,Zhe Hou,Guangdong Bai,Ruitao Feng*

Main category: cs.CR

TL;DR: This paper proposes MedExChain, a secure cross-chain scheme for sharing Personal Health Records across heterogeneous blockchains in IoMT environments, leveraging CRF and audit mechanisms to ensure security and efficiency under device constraints.


<details>
  <summary>Details</summary>
Motivation: The lack of interoperability and security in cross-chain data sharing within blockchain-based healthcare systems limits the efficient and secure exchange of Personal Health Records (PHRs) generated by IoMT devices.

Method: The authors propose MedExChain, a cross-chain data sharing scheme that integrates Cryptographic Reverse Firewall (CRF) and a blockchain audit mechanism. Security is verified using BAN logic, Scyther tool, Chosen Plaintext Attack (CPA), and Algorithm Substitution Attack (ASA) analyses, while performance is evaluated in terms of computation and communication overhead.

Result: MedExChain demonstrates significantly reduced computation and communication overhead, is resilient against internal and external threats, and maintains security under CPA and ASA models, as validated by formal verification and extensive evaluation.

Conclusion: MedExChain enables secure and efficient cross-chain sharing of Personal Health Records (PHRs) across heterogeneous blockchain systems, making it robust, scalable, and suitable for resource-constrained IoMT devices.
...
### [53] [BlackCATT: Black-box Collusion Aware Traitor Tracing in Federated Learning](https://arxiv.org/abs/2602.12138)
*Elena Rodríguez-Lois,Fabio Brau,Maura Pintor,Battista Biggio,Fernando Pérez-González*

Main category: cs.CR

TL;DR: This paper introduces BlackCATT, the first general collusion-resistant black-box traitor tracing method in federated learning, with iterative trigger optimization and a collusion-aware loss, extended by BlackCATT+FR to handle batch normalization via functional regularization.


<details>
  <summary>Details</summary>
Motivation: To address the under-explored threat of collusion in black-box federated learning watermarking, where multiple malicious participants collude to remove or obscure watermarks, undermining existing traitor tracing mechanisms.

Method: Proposes BlackCATT, featuring a collusion-aware embedding loss and iteratively optimized triggers for black-box traitor tracing; extends it with BlackCATT+FR using functional regularization via auxiliary examples at the aggregator to ensure feature space consistency, especially in models with batch normalization.

Result: Demonstrates strong traitor tracing performance under collusion attacks across various architectures and datasets; shows that BlackCATT+FR mitigates update incompatibility issues in models with batch normalization, preserving both main task accuracy and watermark detectability.

Conclusion: BlackCATT and BlackCATT+FR provide effective collusion-resistant traitor tracing in black-box federated learning, enabling robust watermarking across diverse architectures and datasets while maintaining model utility.
...
### [54] [Unknown Attack Detection in IoT Networks using Large Language Models: A Robust, Data-efficient Approach](https://arxiv.org/abs/2602.12183)
*Shan Ali,Feifei Niu,Paria Shirani,Lionel C. Briand*

Main category: cs.CR

TL;DR: SiamXBERT is a data-efficient, transformer-based Siamese meta-learning framework for detecting unknown cyberattacks in IoT networks. By combining flow- and packet-level features and using meta-learning, it enables robust detection of zero-day threats under data scarcity and encrypted traffic, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing intrusion detection systems struggle with zero-day attacks due to reliance on large labeled datasets, payload inspection, and closed-set classification, which are impractical in IoT environments with encrypted traffic, data scarcity, and evolving threats.

Method: The authors propose SiamXBERT, a Siamese meta-learning framework using a transformer-based language model. It integrates flow-level and packet-level features (dual-modality) to model network behavior and leverages meta-learning for fast adaptation to new attacks with few labeled samples, without relying on payload inspection.

Result: SiamXBERT achieves state-of-the-art performance on IoT intrusion datasets, showing consistent superiority in both within-dataset and cross-dataset evaluations. It achieves up to a 78.8% improvement in unknown F1-score while requiring significantly less labeled training data.

Conclusion: SiamXBERT demonstrates strong practicality for robust and data-efficient unknown attack detection in real-world IoT environments, outperforming existing methods under limited labeled data and cross-dataset scenarios.
...
TL;DR: This paper presents the first systematic study of malicious tool code implementations in LLM-agent systems, introducing a taxonomy of threats and MalTool, a framework that uses coding LLMs to generate functionally correct and structurally diverse malicious tools, revealing the inadequacy of existing detection mechanisms.


<details>
  <summary>Details</summary>
Motivation: While prior work focuses on social engineering aspects of malicious tool attacks (e.g., tool names and descriptions), the implementation of malicious code within tools remains underexplored, creating a critical gap in understanding and defending against such threats in LLM-agent settings.

Method: The authors propose a taxonomy of malicious behaviors based on the CIA triad, develop MalTool—a coding-LLM-based framework that generates malicious tools with verified functionality and structural diversity—and construct two datasets of malicious tools for evaluation.

Result: MalTool successfully generates 1,200 standalone malicious tools and 5,287 real-world tools with embedded malicious behaviors, demonstrating high effectiveness even with safety-aligned coding LLMs; evaluation shows that current detection methods, including VirusTotal and LLM-agent-specific approaches, perform poorly against these tools.

Conclusion: Existing detection methods are insufficient for identifying malicious tools generated by coding LLMs, indicating an urgent need for more effective defenses in LLM-agent environments.
...
TL;DR: This paper introduces a novel communication-game-based technique to establish the first unconditional space lower bounds for user-level differential privacy, proving that private algorithms require significantly more memory than non-private ones, with an exponential separation shown for distinct elements estimation.


<details>
  <summary>Details</summary>
Motivation: Understanding the inherent memory cost of differential privacy, particularly at the user level, is crucial for designing efficient private algorithms. While accuracy-privacy trade-offs are well studied, memory efficiency remains underexplored, motivating the need for lower bounds that reveal the fundamental limitations of private computation in limited space.

Method: The authors introduce a multi-player communication game that captures the hardness of low-memory private algorithms. They establish a connection between the necessity of 'contribution capping' in differentially private algorithms and the communication complexity of this game. Winning the game requires transmitting information proportional to the number of over-active users, which implies a space lower bound. This framework is applied to problems like distinct elements estimation, private medians, quantiles, and max-select.

Result: The paper proves an almost $\widetilde\Omega(T^{1/3})$ space lower bound for user-level differentially private distinct elements estimation, resolving an open problem. This shows an exponential separation from the $\widetilde{O}(1)$ space used by non-private algorithms. The method generalizes to yield space lower bounds for other fundamental tasks including private medians, quantiles, and max-select.

Conclusion: The paper establishes that user-level differential privacy inherently requires large memory due to the need for contribution capping, demonstrated via a novel communication-theoretic framework. It provides the first unconditional space lower bounds for private algorithms and reveals an exponential gap in space complexity between private and non-private computation for natural problems.
...
TL;DR: This paper introduces a Scope × Authority framework to systematize emergency interventions in decentralized protocols, revealing key tradeoffs between speed, precision, and centralization; empirical analysis of 705 exploits shows containment effectiveness depends on authority structure, losses are heavy-tailed, and community trust modulates operational cost.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the tension between the need for emergency interventions in decentralized systems (e.g., to respond to exploits) and the risk of undermining trust through perceived illegitimate centralization; despite high financial stakes (~$10 billion), current designs are ad hoc and lack systematic analysis.

Method: The authors develop a Scope × Authority taxonomy to analyze emergency mechanisms in decentralized protocols, formalize tradeoffs using a stochastic cost-minimization model, and validate predictions using empirical data from 705 exploit incidents combined with community sentiment analysis.

Result: Key results include: (1) containment time systematically varies by authority type, (2) exploit losses follow a heavy-tailed distribution (α≈1.33), indicating risk concentration in rare catastrophic events, and (3) community sentiment significantly influences the perceived cost of maintaining intervention capabilities.

Conclusion: The paper concludes that emergency governance in decentralized protocols can be advanced from ideological debates to quantitative engineering through a structured Scope × Authority taxonomy, enabling more effective and trusted intervention mechanisms.
