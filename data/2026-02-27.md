...
TL;DR: This paper proposes a dynamic, multiagent resource-allocation approach to blockchain configuration, optimizing ephemeral chain formation and pricing each epoch to improve scalability and adaptability under shifting demand and capacity.


<details>
  <summary>Details</summary>
Motivation: Existing blockchain systems face scalability limitations and lack dynamic adaptability; multichain designs are mostly static, unable to respond efficiently to changing application demands and operator capacities.

Method: The authors model blockchain configuration as a multiagent resource-allocation problem where applications and operators declare their demands, capacities, and price bounds. An off-chain optimizer groups them into ephemeral chains per epoch and sets clearing prices to maximize a governance-weighted combination of normalized utilities, with solutions verifiable on-chain.

Result: The approach enables dynamic, modular blockchain configuration that supports capability compatibility, application diversity, and epoch stability. Simulations demonstrate trade-offs between throughput, decentralization, operator yield, and service stability.

Conclusion: The proposed model provides a flexible, efficient, and governance-aware framework for scalable and adaptive blockchain systems, balancing the interests of applications, operators, and the overall network.
...
TL;DR: TT-SEAL is a selective-encryption framework for tensor-train compressed neural networks that securely minimizes encryption overhead by identifying and protecting only the most sensitive TT cores, enabling low-latency, secure edge AI.


<details>
  <summary>Details</summary>
Motivation: Existing selective-encryption methods assume dense model weights and lack applicability to tensor-train decomposed models; TT-SEAL addresses security under model compression for edge AI.

Method: Proposes TT-SEAL, a selective-encryption framework that ranks TT cores via sensitivity-based importance, sets a one-time robustness threshold, and uses a value-DP optimizer to encrypt only critical cores with AES.

Result: TT-SEAL encrypts only 4.89–15.92% of parameters while matching full encryption robustness, reduces AES decryption latency share to as low as 2.76% (from 58%) on ResNet-18, and is validated under TTD-aware, transfer-based threats on an FPGA-prototyped edge processor.

Conclusion: TT-SEAL enables secure and efficient edge AI by achieving strong robustness with minimal encryption overhead in TT-compressed models.
...
TL;DR: This paper evaluates prompt-injection and jailbreak vulnerabilities in open-source LLMs using a large curated dataset, finding that while lightweight inference-time defenses help against simple attacks, they fail against complex, reasoning-heavy prompts, with notable differences in model behavior due to internal safety mechanisms.


<details>
  <summary>Details</summary>
Motivation: Given the widespread deployment of LLMs and their susceptibility to prompt-based attacks, there is a critical need to evaluate security risks, especially for resource-constrained organizations relying on prompt engineering, to ensure safe and reliable model deployment.

Method: The authors conducted an empirical evaluation using a large, manually curated dataset to assess the vulnerability of multiple open-source LLMs (Phi, Mistral, DeepSeek-R1, Llama 3.2, Qwen, Gemma variants) to prompt-injection and jailbreak attacks, and tested several lightweight, inference-time defense filters without retraining or fine-tuning.

Result: Significant behavioral variation across models was observed, including refusals and silent non-responsiveness due to internal safety mechanisms. Lightweight defenses were effective against simple attacks but consistently bypassed by long, reasoning-heavy prompts.

Conclusion: The study concludes that while lightweight, inference-time defenses can mitigate simple prompt-injection and jailbreak attacks on LLMs, they are ineffective against more complex, reasoning-heavy prompts, highlighting the need for more robust and adaptive defense mechanisms.
...
TL;DR: This paper introduces DiSP, a self-purification defense framework for Multimodal Diffusion Language Models (MDLMs) that effectively mitigates backdoor attacks by selectively masking vision tokens and refining the model on self-purified data, achieving strong defense without auxiliary models or clean data.


<details>
  <summary>Details</summary>
Motivation: Despite the growing competitiveness of Multimodal Diffusion Language Models (MDLMs), their vulnerability to backdoor attacks via data poisoning is understudied, and effective defense mechanisms are currently lacking.

Method: DiSP leverages selective vision token masking during inference to identify and neutralize trigger-induced behaviors, followed by self-purification of the poisoned dataset and fine-tuning on the cleaned data.

Result: DiSP reduces the attack success rate from over 90% to under 5% across extensive experiments while preserving model performance on clean, benign inputs.

Conclusion: DiSP provides an effective and self-contained defense mechanism against backdoor attacks in MDLMs, enabling robust recovery of model integrity without relying on external models or clean data.
...
TL;DR: This paper presents a lightweight, privacy-preserving deception detection system using knowledge distillation from MobileBERT to a compact BiLSTM with multi-head attention, achieving near-state-of-the-art performance with significantly reduced size and latency for real-time email and endpoint security.


<details>
  <summary>Details</summary>
Motivation: To detect sophisticated LLM-generated social-engineering content that evades traditional text screening, while meeting the performance, privacy, and deployment requirements of enterprise communication systems.

Method: A distillation approach from a fine-tuned MobileBERT teacher to a BiLSTM model with multi-head attention, trained on a hybrid dataset of human-written and LLM-generated paraphrased messages using masking and personalization techniques.

Result: The distilled BiLSTM model achieves competitive weighted-F1 scores (within 1–2.5 points of strong transformer baselines), 80–95% faster inference, and 95–99% smaller size, with effective performance across human-only, LLM-only, cross-distribution, and production-like mixed traffic tests.

Conclusion: The proposed system enables real-time, privacy-protective deception detection in communication platforms with high accuracy, low latency, and compact model size, making it suitable for deployment on resource-constrained devices without acceleration hardware.
...
TL;DR: Training data poisoning via minimal label flipping causes undetectable, high-success attacks in imbalanced audio classification; the paper proves accuracy monitoring is insufficient and proposes a cryptographic, trust-minimized defense using Merkle trees and post-quantum signatures.


<details>
  <summary>Details</summary>
Motivation: The motivation is to demonstrate the stealth and effectiveness of training-data poisoning attacks in real-world, class-imbalanced scenarios (e.g., urban vehicle audio classification), where small label corruptions can cause high attack success rates without detectable accuracy drops, thereby exposing the inadequacy of current monitoring practices.

Method: The authors analyze label-flipping and backdoor trigger attacks on a compact 2D CNN trained on log-mel spectrograms using the MELAUDIS dataset. They mathematically bound the maximum accuracy drop from targeted attacks by the minority class fraction and introduce a formalized ML pipeline attack surface. A defense mechanism is proposed using content-addressed hashing, Merkle trees, and post-quantum digital signatures (CRYSTALS-Dilithium3) for data integrity.

Result: The CNN achieves a 95.7% Attack Success Rate (ASR) in a Truck-to-Car label-flipping attack with only 0.5% label corruption (48 samples), with no detectable change in overall accuracy (87.6% baseline, 95% CI: 88-100%). Theoretical analysis shows the maximum accuracy drop is bounded by the minority class fraction (β≈3%), rendering attacks undetectable via accuracy. A backdoor trigger attack collapses to pure label flipping due to trigger-dominance in minority classes.

Conclusion: The paper concludes that training-data poisoning attacks can induce stealthy, targeted failures in deep neural networks, particularly in imbalanced real-world datasets, and that aggregate accuracy is an insufficient defense metric. The authors propose a trust-minimized, cryptographic defense based on verifiable data provenance to secure the machine learning pipeline.
...
TL;DR: This paper proposes Public-moment-guided Truncation (PMT), a method that uses public second-moment information to transform private data and enable effective differential privacy with better conditioning, leading to improved accuracy and stability in DP linear and generalized linear models.


<details>
  <summary>Details</summary>
Motivation: Differential privacy (DP) typically requires bounded data distributions, limiting its applicability; this work addresses that constraint by using a small set of public data to guide data transformation and enable effective DP learning under unbounded or poorly conditioned distributions.

Method: PMT uses a public second-moment matrix to transform private data, applies truncation based on data dimension and sample size, and integrates with penalized and generalized linear regressions via newly designed loss functions and algorithms that preserve invertibility and mapping to the original domain.

Result: Theoretical analysis shows improved error bounds, robustness, and convergence for DP estimation under PMT; experiments on synthetic and real datasets demonstrate significant gains in accuracy and stability of DP models.

Conclusion: Public-moment-guided Truncation (PMT) effectively improves the accuracy and stability of differentially private models by leveraging public second-moment information to enable principled truncation, resulting in well-conditioned data transformations that robustly resist DP noise.
...
TL;DR: Hubscan is an open-source security scanner for Retrieval-Augmented Generation systems that detects harmful 'hub' items in vector indices using a multi-faceted detection approach, demonstrating high efficacy across benchmarks and real-world data.


<details>
  <summary>Details</summary>
Motivation: RAG systems are vulnerable to hubness, where certain items disproportionately appear in top-k results, enabling security exploits such as injection of harmful content and manipulation of search rankings. This necessitates a robust tool for detecting such vulnerabilities.

Method: Hubscan employs a multi-detector architecture including statistical hubness detection (median/MAD z-scores), cluster spread analysis, stability testing under query perturbations, and domain- and modality-aware detection. It supports multiple vector databases and retrieval methods.

Result: Hubscan achieves 90% recall at a 0.2% alert budget and 100% recall at 0.4% on adversarial benchmarks, with adversarial hubs ranked above the 99.8th percentile. Domain-scoped scanning detects 100% of targeted attacks evading global detection. Evaluation on 1M real MS MARCO documents shows clear separation between clean and adversarial content.

Conclusion: Hubscan provides a practical and extensible framework for detecting hubness-based security threats in production RAG systems, effectively identifying adversarial hubs across multiple datasets and real-world deployments.
...
TL;DR: This thesis proposes a transformer-based method to predict software vulnerabilities from natural-language cyberattack descriptions by leveraging semantic similarity. Using models like MMPNet, it successfully links MITRE ATT&CK techniques to CVEs, uncovering previously undocumented relationships and enabling proactive threat intelligence through the VULDAT tool.


<details>
  <summary>Details</summary>
Motivation: The incomplete coverage of attack-vulnerability relationships in existing Cyber Threat Intelligence resources (e.g., MITRE ATT&CK and CVE) creates a critical gap, especially when attack information emerges before vulnerabilities are formally documented. This necessitates automated methods to infer likely vulnerabilities from natural-language attack descriptions to improve threat intelligence and response.

Method: The research employs transformer-based sentence embedding models to encode textual descriptions of cyberattacks and vulnerabilities into semantic vectors. Fourteen state-of-the-art models are evaluated across four types of attack descriptions (Tactic, Technique, Procedure, Attack Pattern) using similarity-based ranking. The best-performing model is integrated into the VULDAT tool for automated linking.

Result: Technique-level descriptions in MITRE ATT&CK yielded the strongest predictive performance. The multi-qa-mpnet-base-dot-v1 (MMPNet) model outperformed others due to its hybrid pre-training and semantic similarity optimization. The VULDAT tool successfully identified previously undocumented attack-vulnerability links and demonstrated strong generalization on real-world, unseen cyberattack reports.

Conclusion: The study concludes that transformer-based models, particularly MMPNet, are effective in predicting known vulnerabilities from natural-language attack descriptions, with Technique-level descriptions in MITRE ATT&CK being the most informative. The successful implementation in VULDAT demonstrates the practical utility of the approach for uncovering undocumented attack-vulnerability links and enhancing proactive cyber defense.
...
TL;DR: A differentially private method is introduced to protect user data in Markov chain models by privatizing simplex-valued queries, with analytical and empirical guarantees on utility preservation.


<details>
  <summary>Details</summary>
Motivation: Accurate Markov chain modeling requires large amounts of user data, which may contain sensitive information; sharing non-private models risks exposing this data, necessitating privacy-preserving methods that maintain model utility.

Method: The authors propose a method to privatize database queries that output elements of the unit simplex (e.g., probability distributions), proving it satisfies differential privacy; this is extended to stochastic matrices (Markov chains), and they derive bounds on the KL divergence, stationary distribution perturbation, and convergence rate change between original and private models.

Result: The method achieves differential privacy for Markov chain models with bounded expected KL divergence; analytical bounds are provided on the impact to stationary distributions and convergence rates; simulations show less than 2% error in the stationary distribution under typical privacy parameters.

Conclusion: The proposed approach effectively balances privacy and utility in data-driven Markov chain models, enabling accurate and differentially private representation of user behavior.
...
TL;DR: Agentic LLMs are vulnerable to silent data exfiltration via implicit prompt injection in URL previews; malicious metadata can trigger undetectable outbound requests, with sharded exfiltration further evading detection—highlighting the need for system- and network-level security controls beyond prompt hardening.


<details>
  <summary>Details</summary>
Motivation: As agentic LLMs increasingly retrieve URLs and invoke external tools autonomously, they become vulnerable to implicit prompt injection through maliciously crafted web content; this introduces system-level threats such as undetectable data exfiltration (silent egress), which existing output-based safety mechanisms fail to catch.

Method: The authors designed a fully local, reproducible testbed to evaluate implicit prompt injection attacks via malicious URL previews on a qwen2.5:7b-based agentic LLM system, measuring success rates of silent egress and evaluating detection evasion, including sharded exfiltration; ablation studies assessed the effectiveness of prompt-, system-, and network-layer defenses.

Result: In 480 experiments, the attack achieved an 89% success rate in inducing unauthorized outbound requests, with 95% of successful exfiltrations bypassing output-based detection; sharded exfiltration reduced single-request leakage by 73% (Leak@1) and evaded basic data loss prevention; prompt-layer defenses were largely ineffective, whereas system- and network-layer controls (e.g., domain allowlisting, redirect-chain analysis) showed stronger mitigation potential.

Conclusion: Network egress must be treated as a critical security outcome in agentic LLM systems, necessitating system- and network-level controls such as domain allowlisting, provenance tracking, and capability isolation to mitigate risks like silent egress and sharded exfiltration.
...
TL;DR: This study evaluates seven pre-trained CNN architectures for multi-class IoT DDoS detection using image-represented network traffic and the CICDDoS2019 dataset, emphasizing performance, reliability, and interpretability.


<details>
  <summary>Details</summary>
Motivation: DDoS attacks threaten IoT availability, especially under resource constraints; while transfer learning shows promise in detection accuracy, its reliability, computational feasibility, and interpretability in real-world settings are not well understood.

Method: The authors employ image-based traffic representation and evaluate seven pre-trained CNNs (e.g., DenseNet, MobileNet) using the CICDDoS2019 dataset. Evaluation includes performance metrics (accuracy, F1), reliability measures (MCC, Youden Index, confidence intervals), latency and training cost, and interpretability via Grad-CAM and SHAP.

Result: DenseNet and MobileNet-based models achieve high detection performance, with DenseNet169 showing best reliability-interpretability alignment and MobileNetV3 offering optimal latency-accuracy trade-off for fog environments. Attribution maps are compact and class-consistent, supporting model trustworthiness.

Conclusion: Selecting deep learning models for IoT DDoS detection should balance performance, reliability, and explainability; DenseNet169 and MobileNetV3 are recommended for high-reliability and edge-deployable scenarios, respectively.
...
### [15] [Systems-Level Attack Surface of Edge Agent Deployments on IoT](https://arxiv.org/abs/2602.22525)
*Zhonghao Zhan,Krinos Li,Yefan Zhang,Hamed Haddadi*

Main category: cs.CR

TL;DR: Edge deployment of LLM agents in IoT introduces critical security vulnerabilities; this study identifies key attack surfaces and shows that architecture choice directly impacts security properties like data egress, sovereignty, and provenance integrity.


<details>
  <summary>Details</summary>
Motivation: To understand and quantify emerging security risks in edge-deployed LLM agents for IoT, which introduce new attack surfaces not present in cloud-hosted systems.

Method: Empirical security analysis of three architectures (cloud-hosted, edge-local swarm, hybrid) using a multi-device home-automation testbed with local MQTT and an Android smartphone as an edge node; assessment based on measurable systems metrics.

Result: Identified five systems-level attack surfaces, including coordination-state divergence and induced trust erosion; found that edge-local deployments prevent cloud data egress but risk silent sovereignty degradation during fallbacks; provenance chains lack cryptographic enforcement; failover windows create exploitable blind spots.

Conclusion: Deployment architecture is a primary factor in determining security risk in LLM-agent-controlled IoT systems, significantly influencing systems-level vulnerabilities beyond model or prompt design.
...
### [16] [Layer-Targeted Multilingual Knowledge Erasure in Large Language Models](https://arxiv.org/abs/2602.22562)
*Taoran Li,Varun Chandrasekaran,Zhiyuan Yu*

Main category: cs.CR

TL;DR: This paper identifies intervention depth as the key factor in multilingual unlearning failure in LLMs and proposes MUTE, a framework that targets language-agnostic intermediate layers using CKA and LRDS to achieve robust, cross-lingual knowledge erasure without harming utility.


<details>
  <summary>Details</summary>
Motivation: Machine unlearning in LLMs often fails to generalize across languages, with erased knowledge remaining accessible in other languages; the root cause and a systematic solution for this issue were previously unknown.

Method: MUTE leverages Centered Kernel Alignment (CKA) and Linguistic Regions Development Score (LRDS) to identify intermediate, language-agnostic layers where cross-lingual representations converge, then applies unlearning updates only in these layers to balance erasure and multilingual utility.

Result: MUTE achieves effective multilingual knowledge removal across three LLM architectures and three unlearning algorithms, with logit lens probing confirming actual erasure (not output masking) and minimal impact on multilingual performance.

Conclusion: The choice of intervention depth is critical for successful multilingual unlearning in LLMs; MUTE enables effective and robust multilingual knowledge erasure by targeting language-agnostic intermediate layers identified through CKA and LRDS.
...
### [17] [DPSQL+: A Differentially Private SQL Library with a Minimum Frequency Rule](https://arxiv.org/abs/2602.22699)
*Tomoya Matsumoto,Shokichi Takakura,Shun Takagi,Satoshi Hasegawa*

Main category: cs.CR

TL;DR: DPSQL+ is a modular, privacy-preserving SQL library that enforces both differential privacy and minimum frequency constraints, enabling compliant, accurate, and scalable data analysis across multiple database systems.


<details>
  <summary>Details</summary>
Motivation: While differential privacy (DP) provides strong privacy guarantees, real-world data governance policies often require additional constraints like the minimum frequency rule (k-anonymity per cell). Prior systems fail to jointly enforce both DP and such rules, creating a gap between theoretical privacy and regulatory compliance.

Method: DPSQL+ implements a modular architecture with three components: a Validator to restrict SQL queries to a DP-safe subset, an Accountant to track cumulative privacy loss across queries, and a Backend to interface with various database engines, ensuring extensibility and portability.

Result: Experiments on the TPC-H benchmark show that DPSQL+ maintains practical accuracy across diverse analytical queries—including aggregates, quadratic statistics, and joins—and supports significantly more queries under a fixed privacy budget compared to existing systems.

Conclusion: DPSQL+ is a privacy-preserving SQL library that effectively combines user-level differential privacy with the minimum frequency rule, offering a practical, modular, and portable solution for secure data analysis while supporting a broad range of SQL queries under strict privacy governance.
...
### [18] [IMMACULATE: A Practical LLM Auditing Framework via Verifiable Computation](https://arxiv.org/abs/2602.22700)
*Yanpei Guo,Wenjie Qu,Linyu Wu,Shengfang Zhai,Lionel Z. Wang,Ming Xu,Yue Liu,Binhang Yuan,Dawn Song,Jiaheng Zhang*

Main category: cs.CR

TL;DR: IMMACULATE is a practical auditing framework for black-box LLM APIs that detects model substitution, quantization abuse, and token overbilling using selective verifiable computation, achieving strong security with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: The lack of transparency in commercial LLM APIs creates risks of model substitution, quantization abuse, and token overbilling, undermining user trust and fairness; there is a need for lightweight, practical auditing that does not rely on trusted hardware or internal model access.

Method: IMMACULATE uses verifiable computation to selectively audit a small subset of API requests, leveraging cryptographic methods to detect anomalies while minimizing performance overhead.

Result: Experiments on both dense and Mixture-of-Experts (MoE) models demonstrate that IMMACULATE achieves high detection accuracy for malicious behavior with less than 1% throughput overhead.

Conclusion: IMMACULATE provides a practical and efficient solution for auditing black-box commercial LLM APIs, enabling clients to detect economically motivated misconduct without requiring trusted hardware or model access.
...
### [19] [AgentSentry: Mitigating Indirect Prompt Injection in LLM Agents via Temporal Causal Diagnostics and Context Purification](https://arxiv.org/abs/2602.22724)
...
TL;DR: AgentSentry is a novel inference-time framework that detects and mitigates indirect prompt injection in LLM agents by identifying causal takeover points through counterfactual re-execution and enabling safe execution via context purification, significantly improving security and utility under attack.


<details>
  <summary>Details</summary>
Motivation: The increasing use of external tools and retrieval systems by LLM agents introduces vulnerability to indirect prompt injection (IPI) attacks, which are hard to detect in multi-turn interactions and are inadequately addressed by existing heuristic-based defenses that often disrupt legitimate workflows.

Method: AgentSentry uses controlled counterfactual re-executions at tool-return boundaries to localize takeover points and applies causally guided context purification to remove attack-induced deviations while preserving legitimate task context.

Result: Evaluated on the AgentDojo benchmark across multiple attack families and LLMs, AgentSentry eliminates successful IPI attacks and achieves an average Utility Under Attack (UA) of 74.55%, outperforming the best baselines by 20.8 to 33.6 percentage points without affecting performance on benign tasks.

Conclusion: AgentSentry is the first inference-time defense that effectively mitigates multi-turn indirect prompt injection in tool-augmented LLM agents by modeling the attack as a temporal causal takeover, enabling secure and continuous agent operation without compromising task performance.
...
### [20] [Automated Vulnerability Detection in Source Code Using Deep Representation Learning](https://arxiv.org/abs/2602.23121)
*C. Seas,G. Fitzpatrick,J. A. Hamilton,M. C. Carlisle*

Main category: cs.CR

TL;DR: This paper proposes a CNN model for bug detection in C code, trained on machine- and human-labeled datasets, achieving superior recall at high precision and demonstrating effectiveness in finding real bugs in complex code.


<details>
  <summary>Details</summary>
Motivation: Software vulnerabilities in C pose significant security risks, and existing detection methods have limitations in precision and recall. This work aims to improve bug detection in C code by leveraging deep learning with specialized optimization for the language.

Method: A CNN-based model was developed using tokenized C code (91 token categories encoded as binary vectors). The architecture includes two convolution and pooling layers followed by two fully connected layers, trained on a machine-labeled dataset (from Draper Labs static analyzers) and the human-labeled NIST SATE Juliet dataset, with duplicates removed.

Result: The model achieves higher recall than Russell et al.'s approach at high precision levels on the same datasets. It successfully identifies real vulnerabilities in a custom Linux kernel dataset with low false positives.

Conclusion: The proposed convolutional neural network model effectively identifies software vulnerabilities in C code with high recall and precision, outperforming prior work, and demonstrates practical applicability by detecting real vulnerabilities in complex systems like the Linux kernel with a low false-positive rate.
...
### [21] [SettleFL: Trustless and Scalable Reward Settlement Protocol for Federated Learning on Permissionless Blockchains (Extended version)](https://arxiv.org/abs/2602.23167)
*Shuang Liang,Yang Hua,Linshan Jiang,Peishen Yan,Tao Song,Bin Yao,Haibing Guan*

Main category: cs.CR

TL;DR: SettleFL is a scalable, trustless reward settlement protocol for decentralized federated learning that reduces economic friction using two blockchain-efficient, interoperable strategies: optimistic execution with challenge-based arbitration and validity-proof-based instant finality.


<details>
  <summary>Details</summary>
Motivation: Decentralized reward settlement in open FL is hindered by high blockchain costs and scalability issues; existing solutions compromise decentralization or suffer from linear on-chain expenses.

Method: Proposes SettleFL, a dual-strategy protocol using a shared domain-specific circuit: (1) Commit-and-Challenge for optimistic execution with dispute resolution, and (2) Commit-with-Proof using validity proofs for instant finality.

Result: Experiments show SettleFL scales to 800 participants with significantly lower gas costs while supporting flexible latency and cost trade-offs.

Conclusion: SettleFL enables trustless and scalable reward settlement in decentralized federated learning, maintaining practical efficiency and economic viability at scale.
...
### [22] [Strengthening security and noise resistance in one-way quantum key distribution protocols through hypercube-based quantum walks](https://arxiv.org/abs/2602.23261)
*David Polzoni,Tommaso Bianchi,Mauro Conti*

Main category: cs.CR

TL;DR: This paper presents a novel one-way Quantum Key Distribution protocol using quantum walks on a hypercube topology, which offers superior security and robustness against noise compared to circular topologies. The authors also release an open-source Qiskit-based simulation framework for evaluating such protocols under realistic conditions.


<details>
  <summary>Details</summary>
Motivation: Classical QKD protocols like BB84 have limited eavesdropping resistance and poor performance under noise; existing quantum walk-based schemes primarily use less secure circular topologies, prompting the need for topologically optimized, noise-resilient alternatives.

Method: A one-way QKD protocol is designed using discrete-time quantum walks on a hypercube topology. An open-source, noise-aware simulation framework is implemented using Qiskit to evaluate and compare performance under realistic noise conditions.

Result: The hypercube-based QKD protocol demonstrates significantly improved security and noise tolerance over the circular topology under identical parameters, validated through simulations with realistic noise models.

Conclusion: The proposed QKD protocol based on quantum walks over a hypercube topology provides enhanced security and noise resistance compared to the state-of-the-art circular topology, establishing a foundation for topology-aware QKD protocols.
...
### [23] [EmpiRE-Compass: A Neuro-Symbolic Dashboard for Sustainable and Dynamic Knowledge Exploration, Synthesis, and Reuse](https://arxiv.org/abs/2602.22276)
*Oliver Karras,Amirreza Alasti,Lena John,Sushant Aggarwal,Yücel Celik*

Main category: cs.SE

TL;DR: EmpiRE-Compass is a neuro-symbolic dashboard that enhances sustainability, transparency, and reusability of literature reviews in software and requirements engineering by integrating research knowledge graphs and large language models.


<details>
  <summary>Details</summary>
Motivation: The growing volume of literature reviews and the rise of generative AI have led to concerns about quality, reproducibility, and lack of shared data; secondary studies often omit artifact transparency, hindering replication and reuse.

Method: EmpiRE-Compass combines research knowledge graphs (RKGs) with LLMs in a modular, open-source dashboard, supporting exploratory visual analytics, neuro-symbolic synthesis for custom queries, and full openness of data and results, demonstrated through two RE use cases.

Result: The dashboard enables dynamic access, replication, and reuse of literature review data with transparent workflows; it is publicly available with a usage cap for cost management, offering three core capabilities: visual analytics, custom question synthesis, and open sharing of all artifacts.

Conclusion: EmpiRE-Compass advances sustainable, transparent, and collaborative literature reviews in RE, SE, and related fields by unifying symbolic knowledge representation and generative AI in a reusable, open framework.
...
### [24] [The Ethos of the PEERfect REVIEWer: Scientific Care and Collegial Welfare](https://arxiv.org/abs/2602.22292)
*Oliver Karras*

Main category: cs.SE

TL;DR: This experience report introduces the ethos of the PEERfect REVIEWer, advocating for a peer review culture that balances scientific care and collegial welfare through 16 practical guidelines, emphasizing that rigor and empathy are complementary in impactful reviewing.


<details>
  <summary>Details</summary>
Motivation: Peer review often prioritizes scientific rigor at the expense of empathy, leading to diminished well-being and collaboration among scholars; this paper aims to reform review culture by integrating care and respect for all participants.

Method: Drawing on ten years of academic experience, professional discussions, literature review, and analysis of personal and received reviews, the author develops the PEERfect REVIEWer ethos and a 16-item guideline to promote constructive and empathetic peer review.

Result: A structured framework (the PEERfect REVIEWer) with 16 actionable recommendations that support both high scientific standards and positive interpersonal dynamics in peer review.

Conclusion: Scientific rigor and empathy are not mutually exclusive but essential partners in effective peer review; reviewers should act as supportive collaborators rather than mere gatekeepers to advance collective academic progress and well-being.
...
### [25] [EyeLayer: Integrating Human Attention Patterns into LLM-Based Code Summarization](https://arxiv.org/abs/2602.22368)
*Jiahao Zhang,Yifan Zhang,Kevin Leach,Yu Huang*

Main category: cs.SE

TL;DR: EyeLayer leverages human eye-gaze data as a proxy for code-reading expertise, enhancing LLM-based code summarization by integrating learned attention priors into the model's token representations, resulting in consistent and significant performance gains across architectures.


<details>
  <summary>Details</summary>
Motivation: To investigate whether human expertise in code understanding, captured through eye-gaze patterns, can guide and improve large language models in code summarization beyond standard fine-tuning approaches.

Method: EyeLayer, a lightweight attention-augmentation module that integrates human eye-gaze patterns into LLMs via a Multimodal Gaussian Mixture model to redistribute token embeddings based on learned attention priors (μ_i, σ_i^2), without altering the base model's representations.

Result: EyeLayer consistently outperforms strong fine-tuning baselines across multiple LLM families (LLaMA-3.2, Qwen3, CodeBERT), achieving up to a 13.17% improvement in BLEU-4 score, demonstrating effective transfer of human attention priors.

Conclusion: Human gaze patterns provide complementary attention signals that enhance the semantic focus of LLMs in code summarization and can be effectively transferred across diverse model architectures using the proposed EyeLayer module.
...
### [26] [Contextual Memory Virtualisation: DAG-Based State Management and Structurally Lossless Trimming for LLM Agents](https://arxiv.org/abs/2602.22402)
*Cosmo Santoni*

Main category: cs.SE

TL;DR: Contextual Memory Virtualisation (CMV) preserves large language model reasoning state across sessions using version-controlled, DAG-structured context management and a structurally lossless trimming algorithm that reduces token usage by up to 86% while retaining all user and assistant messages.


<details>
  <summary>Details</summary>
Motivation: Extended reasoning tasks in LLMs accumulate valuable context that is typically lost when sessions hit context window limits or undergo lossy compaction; CMV addresses this by treating context as persistent, reusable state.

Method: CMV models session history as a Directed Acyclic Graph (DAG) with snapshot, branch, and trim operations; it implements a three-pass trimming algorithm that removes non-essential content like tool outputs and metadata without altering user or assistant messages.

Result: The trimming algorithm achieves a mean 20% token reduction, up to 86% in high-overhead sessions, with 39% average reduction in mixed tool-use cases; session reuse remains economically viable under prompt caching, reaching break-even within 10 turns.

Conclusion: CMV enables efficient, lossless context management for long-running LLM applications by virtualizing memory akin to operating systems, facilitating context reuse, scalability, and cost-effective reasoning over extended sessions.
...
### [27] [XMENTOR: A Rank-Aware Aggregation Approach for Human-Centered Explainable AI in Just-in-Time Software Defect Prediction](https://arxiv.org/abs/2602.22403)
*Saumendu Roy,Banani Roy,Chanchal Roy,Richard Bassey*

Main category: cs.SE

TL;DR: XMENTOR is a human-centered, rank-aware aggregation method implemented as a VS Code plugin that unifies conflicting post-hoc explanations from XAI methods (e.g., LIME, SHAP, BreakDown) into a single, coherent view to improve interpretability and usability of ML-based defect prediction models for developers.


<details>
  <summary>Details</summary>
Motivation: The opacity of ML-based defect prediction models poses a challenge to developer trust and usability. While XAI methods aim to increase transparency, their use in combination often leads to conflicting explanations, increasing confusion, frustration, and cognitive load for developers.

Method: The authors propose XMENTOR, which aggregates multiple post-hoc explanations using adaptive thresholding, rank and sign agreement, and fallback strategies to generate a unified and interpretable explanation. It is implemented as a VS Code plugin and integrated into the developer workflow. A user study was conducted to evaluate its effectiveness.

Result: In the user study, nearly 90% of participants preferred the aggregated explanations provided by XMENTOR, reporting reduced confusion and better support for debugging and code review tasks. The approach effectively reduced cognitive load and improved usability.

Conclusion: Aggregating multiple XAI explanations in a human-centered, rank-aware manner enhances interpretability, usability, and trust in ML-based defect prediction models. Integrating such explanations into development environments supports practical adoption and improves software quality.
...
### [28] [Automating the Detection of Requirement Dependencies Using Large Language Models](https://arxiv.org/abs/2602.22456)
...
### [29] [RepoMod-Bench: A Benchmark for Code Repository Modernization via Implementation-Agnostic Testing](https://arxiv.org/abs/2602.22518)
*Xuefeng Li,Nir Ben-Israel,Yotam Raz,Belal Ahmed,Doron Serebro,Antoine Raux*

Main category: cs.SE

TL;DR: RepoMod-Bench is a large-scale, implementation-agnostic benchmark for evaluating AI coding agents in repository-level code modernization, featuring 21 real-world repositories across 8 languages with 1.6M LOC and hidden test suites to prevent overfitting; evaluation reveals a sharp decline in agent performance on larger projects, highlighting the challenge of scalable code modernization.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for AI coding agents are limited by small scale, language-specific tests, and exposed test suites that enable test-driven overfitting, making it difficult to assess true generalization in repository-level code translation; there is a need for a more rigorous, scalable, and fair evaluation framework.

Method: Introduce RepoMod-Bench, a benchmark comprising 21 real-world repositories (1.6M LOC, 8 languages) with standardized interfaces and an implementation-agnostic test suite; use black-box evaluation with hidden tests to verify functional equivalence across source and target implementations, preventing agents from exploiting test visibility.

Result: Evaluation of four state-of-the-art AI coding agents shows a dramatic 'scaling collapse': average pass rates drop from 91.3% on projects <10K LOC to 15.3% on projects >50K LOC, indicating severe performance degradation at scale.

Conclusion: Autonomous, large-scale code modernization remains a significant open challenge; RepoMod-Bench provides a more rigorous and scalable benchmarking framework that enables fair, cross-language evaluation of code translation agents while avoiding test-driven overfitting.
...
### [30] [RandSet: Randomized Corpus Reduction for Fuzzing Seed Scheduling](https://arxiv.org/abs/2602.22729)
...
### [31] [Evaluating and Improving Automated Repository-Level Rust Issue Resolution with LLM-based Agents](https://arxiv.org/abs/2602.22764)
*Jiahong Xiang,Wenxiao He,Xihua Wang,Hongliang Tian,Yuqun Zhang*

Main category: cs.SE

TL;DR: This paper presents Rust-SWE-bench, a new large-scale benchmark for evaluating LLM-based code agents on real-world Rust software tasks, and proposes RUSTFORGER, an agent that uses automated environment setup and dynamic tracing via metaprogramming to achieve superior performance, resolving 28.6% of issues and outperforming baselines by 34.9%.


<details>
  <summary>Details</summary>
Motivation: The steep learning curve and strict semantics of Rust create significant barriers to entry; automating issue resolution using LLM agents can aid adoption, but existing benchmarks and methods lack sufficient scale and effectiveness for real-world Rust development.

Method: The authors introduce Rust-SWE-bench, a repository-level benchmark with 500 real-world tasks from 34 Rust repositories, and evaluate four LLM-powered agents using four state-of-the-art LLMs. Based on identified limitations, they propose RUSTFORGER, which integrates automated test environment setup and a Rust metaprogramming-driven dynamic tracing mechanism to improve issue reproduction and resolution.

Result: RUSTFORGER achieves a 28.6% task resolution rate on Rust-SWE-bench using Claude-Sonnet-3.7, representing a 34.9% improvement over the strongest baseline. It uniquely solves 46 tasks unresolved by any other agent across all tested LLMs. The study also identifies key challenges: understanding repository-wide code structure and adhering to Rust's type and trait system.

Conclusion: RUSTFORGER introduces an effective agentic approach for addressing Rust software engineering tasks by combining automated test environment setup with dynamic tracing via Rust metaprogramming, significantly outperforming existing methods and highlighting the importance of issue reproduction and deep semantic understanding in the Rust ecosystem.
...
### [32] [Productivity and Collaboration in Hybrid Agile Teams: An Interview Study](https://arxiv.org/abs/2602.22835)
*Elisabeth Mo,Jefferson Seide Molléri,Asle Fagerstrøm*

Main category: cs.SE

TL;DR: Hybrid work impacts Agile teams by reducing informal interactions, causing uneven participation, and increasing digital tool reliance, with trust, communication, and tool support being key mediators of effectiveness.


<details>
  <summary>Details</summary>
Motivation: To understand how hybrid work environments post-pandemic affect Agile team productivity and collaboration.

Method: Conducted nine interviews across three Norwegian Agile teams to explore the effects of hybrid work on Agile practices.

Result: Hybrid work reduces informal interaction, leads to uneven participation, increases dependence on digital tools, and makes Agile ceremonies critical for alignment; trust, communication, and tool support are essential for team effectiveness.

Conclusion: Hybrid Agile work requires tailored structures to enhance inclusion, team cohesion, and sustainable performance as it continues to evolve.
...
### [33] [Managing Uncertainty in LLM-based Multi-Agent System Operation](https://arxiv.org/abs/2602.23005)
*Man Zhang,Tao Yue,Yihua He*

Main category: cs.SE

TL;DR: This paper presents a lifecycle-based uncertainty management framework for LLM-based multi-agent software systems in safety-critical domains, addressing system-level and runtime uncertainties through representation, identification, evolution, and adaptation, and demonstrates improved reliability in a clinical echocardiography application.


<details>
  <summary>Details</summary>
Motivation: Existing approaches focus on model-level uncertainty, neglecting system-level and runtime uncertainties in safety-critical domains like echocardiography; a structured software engineering approach is needed to handle uncertainty propagation across agents, data, human interaction, and control logic.

Method: The paper introduces a framework with four mechanisms—representation, identification, evolution, and adaptation—to manage uncertainty across the system lifecycle, differentiating between epistemic and ontological uncertainty in LLM-based multi-agent systems.

Result: The framework was validated in a real-world LLM-based multi-agent echocardiographic system, demonstrating improved reliability and diagnosability in diagnostic reasoning.

Conclusion: The proposed lifecycle-based uncertainty management framework enables principled runtime governance and adaptation in LLM-based multi-agent software systems, enhancing reliability and diagnosability in safety-critical applications beyond model-centric approaches.
...
### [34] [CL4SE: A Context Learning Benchmark For Software Engineering Tasks](https://arxiv.org/abs/2602.23047)
*Haichuan Hu,Ye Shang,Guoqing Xie,Congqing He,Quanjun Zhang*

Main category: cs.SE

TL;DR: CL4SE introduces a fine-grained taxonomy and benchmark for context learning in software engineering, evaluates four context types across core SE tasks using 13,000+ samples from open-source projects, and demonstrates significant performance improvements in LLMs, establishing a standardized framework for reproducible research.


<details>
  <summary>Details</summary>
Motivation: Despite the success of context engineering in enhancing LLM performance for software engineering tasks, there is a lack of systematic taxonomy and dedicated benchmarks to understand and quantify the impact of different context types across SE workflows.

Method: The authors propose CL4SE, a comprehensive benchmark with a fine-grained taxonomy of four SE-specific context types—interpretable examples, project-specific context, procedural decision-making context, and positive & negative context—each aligned with a core SE task. They construct a dataset of over 13,000 samples from more than 30 open-source projects and evaluate five mainstream LLMs across nine metrics.

Result: Context learning yields an average 24.7% performance improvement across tasks. Procedural context improves code review by up to 33% (Qwen3-Max), mixed positive-negative context boosts patch assessment by 30% (DeepSeek-V3), project-specific context increases code summarization BLEU by 14.78% (GPT-Oss-120B), and interpretable examples enhance code generation PASS@1 by 5.72% (DeepSeek-V3).

Conclusion: CL4SE establishes the first standardized evaluation framework for context learning in software engineering, demonstrating that tailored context types significantly improve LLM performance across SE tasks and providing a foundation for reproducible, task-specific context design.
...
### [35] [LLM-Powered Silent Bug Fuzzing in Deep Learning Libraries via Versatile and Controlled Bug Transfer](https://arxiv.org/abs/2602.23065)
...
### [36] [Utilizing LLMs for Industrial Process Automation](https://arxiv.org/abs/2602.23331)
*Salim Fares*

Main category: cs.SE

TL;DR: This paper explores the application of Large Language Models (LLMs) in the industrial process automation domain, focusing on specialized programming languages used in proprietary environments, and aims to accelerate manufacturing software development by solving real-world tasks like robotic arm programming.


<details>
  <summary>Details</summary>
Motivation: While LLMs have shown promise in software engineering for mainstream languages like Python, their effectiveness in industrial automation—where specialized, proprietary languages dominate—is underexplored. There is a need to adapt LLMs to these niche contexts to improve development efficiency.

Method: The research investigates the integration of LLMs into industrial software development workflows by tailoring them to domain-specific languages and evaluating their performance on practical tasks such as generating control routines for robotic systems.

Result: The study demonstrates that LLMs can be effectively adapted to support programming in specialized industrial automation languages, enabling faster development cycles and accurate generation of functional code for real-world manufacturing tasks.

Conclusion: LLMs have significant potential to enhance software development in the industrial automation sector, provided they are properly fine-tuned or conditioned for domain-specific languages and use cases, paving the way for broader adoption in manufacturing environments.
