...
TL;DR: SGAgent is a suggestion-guided multi-agent framework for repository-level software repair that introduces a localize-suggest-fix paradigm and a Knowledge Graph-enhanced reasoning process, achieving state-of-the-art performance and strong generalization in bug and vulnerability repair.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based software repair agents often rely on a direct localize-then-fix approach, which skips intermediate reasoning steps and limits repair accuracy; SGAgent aims to close this gap by introducing an explicit suggestion phase to improve contextual understanding and repair quality.

Method: SGAgent follows a localize-suggest-fix paradigm with three specialized sub-agents: a localizer to identify buggy files, a suggester that retrieves contextual information via a repository-derived Knowledge Graph and generates actionable repair suggestions, and a fixer that produces patches based on these suggestions; collaboration among agents is facilitated through a KG-based toolkit for enhanced global reasoning.

Result: SGAgent achieves 51.3% repair accuracy on SWE-Bench, outperforming all baseline methods using the same base model (Claude-3.5), with 81.2% file-level and 52.4% function-level localization accuracy at an average cost of $1.48 per instance; it also reaches 48% accuracy on vulnerability repair tasks across VUL4J and VJBench, demonstrating strong cross-task and cross-language generalization.

Conclusion: SGAgent introduces a novel suggestion-guided multi-agent framework that bridges the reasoning gap in existing software repair approaches by incorporating a dedicated suggestion phase, leading to improved performance and generalization in repository-level software repair tasks.
...
TL;DR: PoCo enhances coverage-based seed selection by removing obstacle conditionals for deeper and more efficient testing.


<details>
  <summary>Details</summary>
Motivation: Traditional coverage-based seed selection methods can be hindered by conditional statements that block deeper exploration; PoCo addresses this limitation to improve testing effectiveness.

Method: PoCo improves coverage-based seed selection (e.g., afl-cmin) by incrementally removing obstacle conditional statements to facilitate deeper and more efficient seed selection.

Result: PoCo achieves better coverage and more effective seed selection by reducing the impact of obstructive conditional branches during program analysis.

Conclusion: PoCo enhances modern coverage-based seed selection techniques by gradually eliminating obstacle conditional statements and enabling more effective deep seed selection.
...
TL;DR: Low-experience AI-assisted developers (vibe coders) produce larger code changes but receive significantly more review comments, have lower PR acceptance rates, and take longer to resolve, indicating higher verification burden on reviewers.


<details>
  <summary>Details</summary>
Motivation: To investigate whether developer experience remains important in AI-assisted software development and if less experienced developers can effectively substitute expert developers using AI coding agents.

Method: Analyzed 22,953 Pull Requests from 1,719 vibe coders in the AIDev dataset, categorizing them into low-experience (Exp_Low) and high-experience (Exp_High) groups; compared PR volume, review comments, acceptance rates, and resolution time across groups.

Result: Exp_Low submitted 2.15× more commits and 1.47× more files changed, received 4.52× more review comments, had 31% lower acceptance rates, and PRs remained open 5.16× longer than Exp_High.

Conclusion: Developer experience still matters in AI-assisted development; replacing experts with low-experience vibe coders increases reviewer burden, suggesting the need for targeted novice training and adaptive review processes.
...
TL;DR: This paper evaluates and improves methods for identifying flaky tests in large-scale industrial settings, specifically SAP HANA, using source code identifiers. It replicates prior work, assesses TF-IDF and TF-IDFC-RF for feature extraction, and tests CodeBERT and XGBoost for classification, achieving high F1-scores but finds limited practical actionability.


<details>
  <summary>Details</summary>
Motivation: Flaky tests undermine automated testing by producing inconsistent results without code changes. While prior research used source code identifiers to detect them, their effectiveness in large industrial systems remains unverified. This work aims to evaluate and extend these approaches in a real-world context.

Method: The study replicates Pinto et al.'s approach on SAP HANA data, compares TF-IDF and TF-IDFC-RF for feature extraction, and evaluates CodeBERT and XGBoost as classifiers. Evaluation uses the original dataset and two SAP HANA datasets for robust comparison.

Result: The replication achieves F1-scores of 0.94 (original) and 0.92 (SAP HANA). Proposed extensions reach 0.96 and 0.99, respectively. Reliance on external data sources is identified as a key cause of flakiness in SAP HANA.

Conclusion: While term categories (e.g., remote dependencies) align with prior findings, vocabulary differs slightly in industrial contexts. Despite high F1-scores, both identifier-based detection and black-box models offer limited practical utility due to lack of actionable insights for developers.
...
TL;DR: LogiDroid is a two-stage approach for generating functional test cases in mobile apps by extracting business logic and adapting it to real-time GUI contexts, significantly outperforming current methods in test coverage and effectiveness.


<details>
  <summary>Details</summary>
Motivation: Functional testing of mobile apps is crucial but hampered by difficulties in capturing business logic from unstructured requirements and adapting it to varied GUIs; automated, accurate test generation is needed to reduce manual effort and improve coverage.

Method: LogiDroid employs a two-stage approach: (1) Knowledge Retrieval and Fusion to extract business logic from relevant cases, and (2) Context-Aware Test Generation that jointly analyzes the extracted logic and real-time GUI to produce test cases with verification assertions.

Result: LogiDroid achieved 40% functional requirement coverage on the FrUITeR dataset (48% improvement) and 65% on the Lin dataset (55% improvement) over existing methods, successfully generating complete and context-aware test cases.

Conclusion: LogiDroid effectively addresses the challenges in functional testing of mobile applications by bridging the gap between unstructured requirements and diverse GUI environments, demonstrating significant improvements over state-of-the-art approaches in generating functional test cases.
...
TL;DR: This paper introduces a unified architecture for securing AI-cloud systems in critical infrastructure by integrating AI governance and cloud security standards, demonstrating through a power grid case study that coordinated, automated defenses can meet multiple compliance and safety requirements simultaneously.


<details>
  <summary>Details</summary>
Motivation: The intersection of AI inference pipelines and cloud infrastructure creates a dual attack surface with fragmented enforcement across AI governance, cloud security, and industrial control standards, enabling cross-layer attacks on safety-critical systems.

Method: Synthesis of AI governance and cloud security frameworks into a lifecycle-staged threat taxonomy; design of a Unified Reference Architecture with Secure Data Factory, hardened model supply chain, and runtime governance layer; validation via a Grid-Guard case study mapping controls to NIST AI RMF, MITRE ATLAS, OWASP AI, CSA MAESTRO, and NERC CIP.

Result: The proposed Unified Reference Architecture successfully mitigates a multi-tier physical-financial manipulation campaign in a hybrid Transmission System Operator environment without human intervention, with controls validated across five major frameworks and NERC CIP standards.

Conclusion: A single cloud-native architecture can simultaneously satisfy AI governance, adversarial robustness, agentic safety, and industrial regulatory compliance by unifying security and AI governance controls across frameworks.
...
TL;DR: Vul2Safe is a secure code generation framework that uses LLM self-reflection to create high-quality repair pairs and diverse prompts (PrimeVul+ dataset), while SRCode introduces token-level reinforcement learning rewards to precisely optimize fine-grained security patterns, reducing vulnerabilities and improving code quality.


<details>
  <summary>Details</summary>
Motivation: Large language models (LLMs) are effective at code generation but often introduce security vulnerabilities. Existing methods are limited by insufficient high-quality security training data and coarse-grained reward signals in reinforcement learning, which hinder effective vulnerability mitigation.

Method: The authors propose two key components: (1) Vul2Safe, which uses LLM self-reflection to generate high-confidence repair pairs from real-world vulnerabilities and creates diverse implicit prompts to build the PrimeVul+ dataset; (2) SRCode, a training framework that introduces token-level rewards in reinforcement learning, enabling fine-grained optimization by focusing on critical security-relevant tokens during training.

Result: Experiments show that PrimeVul+ enhances the quality of secure code training data, and SRCode significantly reduces security vulnerabilities in generated code. The approach outperforms traditional instance-level reward methods across multiple benchmarks, with measurable improvements in both security and general code quality.

Conclusion: By combining high-quality data generation via self-reflection and fine-grained token-level reinforcement learning, Vul2Safe and SRCode offer a more effective approach to secure code generation, addressing key limitations in data quality and reward granularity in current LLM-based methods.
...
TL;DR: This paper presents a testbed to evaluate Wi-Fi resilience against deauthentication attacks, showing that WPA2 with PMF and WPA3 prevent such attacks, while older or unprotected configurations remain vulnerable.


<details>
  <summary>Details</summary>
Motivation: Wi-Fi deauthentication attacks exploit unprotected management frames to cause denial-of-service, posing a persistent threat to wireless networks. This work aims to empirically assess the resilience of common Wi-Fi security configurations and evaluate the effectiveness of existing countermeasures like PMF and WPA3.

Method: A software-defined testbed was developed to systematically evaluate Wi-Fi resilience against deauthentication attacks. Five security configurations were tested: open networks, WPA1, WPA2 without PMF, WPA2 with PMF, and WPA3. Controlled experiments measured client disconnection rates, packet injection volume, and time-to-disruption, with packet-level analysis performed using standard wireless auditing tools.

Result: Open networks, WPA1, and WPA2 without PMF showed complete vulnerability to deauthentication attacks, resulting in immediate client disconnections. In contrast, WPA2 with PMF and WPA3 exhibited full resistance—no successful deauthentication was observed under the tested conditions, confirming the protective role of secured management frames.

Conclusion: Management-frame protection mechanisms such as PMF in WPA2 and the security enhancements in WPA3 effectively mitigate Wi-Fi deauthentication attacks, whereas open networks, WPA1, and WPA2 without PMF remain highly vulnerable, emphasizing the need to phase out legacy or improperly configured systems.
...
TL;DR: This paper introduces Lap2, a method enabling L2 clipping for Laplace mechanism in DP-SGD via a novel multivariate privacy accounting approach, achieving state-of-the-art utility under strong differential privacy in large-scale models.


<details>
  <summary>Details</summary>
Motivation: The Laplace mechanism in DP-SGD is limited by L1 norm clipping, which causes the noise scale to grow with model dimensionality, degrading utility; Lap2 aims to overcome this barrier by enabling L2 clipping for Laplace without sacrificing privacy.

Method: The paper proposes Lap2, which uses coordinate-wise moment bounds and majorization theory with Schur-convex functions to construct a tight, data-independent multivariate privacy accountant that supports L2 clipping for the Laplace mechanism.

Result: Lap2 significantly improves Laplace DP-SGD performance, achieving 87.88% accuracy on RoBERTa-base fine-tuned on SST-2 at =0.54, outperforming Gaussian (87.16%) and standard Laplace (48.97%) under the same privacy budget.

Conclusion: Lap2 enables effective use of the Laplace mechanism with L2 clipping in DP-SGD, achieving strong privacy and superior utility compared to both standard Laplace and Gaussian mechanisms, especially in high-dimensional settings.
...
TL;DR: A practical intersection attack on Tor's introduction circuits can identify hops toward an onion service by analyzing time-constrained message patterns, requiring only partial network observation.


<details>
  <summary>Details</summary>
Motivation: Tor's onion services use fixed introduction circuits that maintain predictable routing patterns, creating a vulnerability to traffic correlation even without full network visibility.

Method: The attack leverages repeated probing and intersects observed destination IP sets during precise time windows between INTRODUCE1 and RENDEZVOUS2 messages, using only one observed relay per stage without payload inspection.

Result: Experiments show the attack reliably converges on circuit hops in real-world conditions, with success rates influenced by relay consensus weight and background traffic levels.

Conclusion: The study demonstrates that even localized adversaries can compromise Tor's anonymity through timing-based intersection attacks, highlighting risks from relay geographic concentration and calling for protocol-level mitigation.
...
TL;DR: CLOAQ is a quantum circuit obfuscation method that secures quantum algorithms by jointly hiding gate logic and phase angles, offering stronger protection against IP theft in untrusted quantum compilers.


<details>
  <summary>Details</summary>
Motivation: Untrusted quantum compilers pose a threat to intellectual property by potentially stealing quantum circuit designs, necessitating robust obfuscation techniques to protect sensitive quantum algorithms.

Method: CLOAQ employs a quantum circuit obfuscation technique that simultaneously hides the logic structure and phase angles of selected gates; evaluation is performed by uniformly sampling input states from the full Hilbert space, improving assessment accuracy over previous methods using only |0> inputs.

Result: CLOAQ shows increased functional disruption under incorrect keys and improved resistance to attacks due to the synergistic effect of dual-level obfuscation, outperforming existing methods that protect only logic or phase.

Conclusion: CLOAQ provides enhanced security for quantum circuits by combining logic and phase obfuscation, demonstrating superior resilience against attacks compared to prior single-perspective quantum circuit obfuscation methods.
...
TL;DR: This paper proposes a lightweight, PUF-based fingerprinting framework for knowledge distillation that enables traceability of stolen or cloned models by embedding device-specific signatures into teacher logits, offering strong security with minimal impact on model performance.


<details>
  <summary>Details</summary>
Motivation: To address security risks in knowledge distillation, particularly model theft, by enabling post-theft accountability and traceability without the high resource costs or architectural constraints of existing methods like watermarking or secure enclaves.

Method: A novel fingerprinting framework is introduced that embeds device-specific Physical Unclonable Function (PUF) signatures into teacher logits during knowledge distillation. Signature recovery involves a two-stage process: a neural network-based decoder followed by a Hamming distance decoder. A bit compression scheme is also proposed to scale to a large number of devices.

Result: The framework achieves a high key recovery rate with negligible accuracy loss on distilled models, demonstrates robustness against reverse engineering and tampering, and allows a tunable trade-off between recovery performance and model accuracy.

Conclusion: The proposed fingerprinting framework using PUF-based signatures offers a practical, lightweight, and robust solution for post-theft accountability and traceability of knowledge-distilled models, without compromising accuracy or requiring architectural modifications.
...
TL;DR: This paper proposes a privacy-preserving protocol for local energy markets (LEMs) that integrates network fees and physical constraints using a double-auction mechanism, secure multiparty computation, and Schnorr's identification with multiparty verification, achieving secure, efficient, and authenticated market clearing within 4.17 minutes for 5,000 participants.


<details>
  <summary>Details</summary>
Motivation: Local energy markets (LEMs) require both privacy protection for participant data and adherence to physical grid constraints and network costs, but existing approaches address these aspects separately, creating a need for an integrated solution.

Method: The authors design a double-auction-based market protocol enhanced with secure multiparty computation (MPC) to protect data privacy, incorporate network fees to reflect physical constraints, and employ Schnorr's identification protocol with multiparty verification for authenticated yet private participant involvement. The protocol is optimized to reduce communication and round complexity.

Result: The proposed protocol securely integrates privacy preservation and grid-aware market clearing, with experimental validation showing it can clear a market of 5,000 participants in 4.17 minutes while meeting security, authenticity, and efficiency requirements.

Conclusion: The protocol effectively bridges the gap between privacy protection and grid-aware operation in LEMs, offering a scalable, secure, and efficient solution for real-world deployment.
...
TL;DR: This paper proposes a physical-layer authentication method for drone Remote Identification (RID) that uses wireless sensing and motion consistency to detect spoofing, combining a yaw-augmented CA-EKF with an LSTM-based motion estimator for robust authentication under dynamic flight conditions.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the vulnerability of broadcast-based drone Remote Identification (RID) systems to spoofing and replay attacks due to lack of cryptographic protection, necessitating robust, non-cryptographic physical-layer authentication mechanisms.

Method: The method involves developing a RID-aware sensing and decoding module to extract wireless sensing parameters (e.g., angle-of-arrival, Doppler shift, channel gain, number of transmit antennas) and decode identity and motion data. These are fed into a yaw-augmented constant-acceleration extended Kalman filter (CA-EKF) combined with a data-driven LSTM-based motion estimator using an error-aware fusion strategy. Authentication is achieved by verifying consistency in transmit antenna count, motion estimates, and no-fly-zone compliance.

Result: Simulation results show the proposed algorithm significantly improves authentication reliability and robustness under realistic wireless impairments and complex, maneuverable flight scenarios, outperforming existing RF feature-based and motion model-based physical-layer authentication schemes.

Conclusion: The proposed consistency verification-based physical-layer authentication algorithm effectively enhances the security and reliability of drone Remote Identification (RID) systems by leveraging physical-layer features and motion consistency, showing superior performance over existing methods.
...
TL;DR: This paper presents Tilewise Domain-Separated Selective Encryption (TDS-SE) to prevent structural leakage in remote sensing image encryption by using domain-separated key derivation per tile and frame. Evaluated with linear and CNN-based distinguishers, it shows improved security under realistic attacks while preserving efficiency and functionality.


<details>
  <summary>Details</summary>
Motivation: Existing selective image encryption schemes in remote sensing are vulnerable to structural leakage under chosen-plaintext attacks due to reuse of secret-dependent transformations across spatial positions. This leakage compromises security even when only parts of the image are encrypted. The work aims to eliminate such leakage while preserving computational efficiency and selective encryption benefits.

Method: The paper proposes TDS-SE, which uses HKDF with explicit domain separation to derive per-tile (and optionally per-frame) keys from a master secret. ROI masks are treated as external side information. Security is evaluated using two reconstruction-based distinguishers—a linear model and a lightweight CNN—across multiple attack settings, including ablation studies, transferability, generalization, and asymmetry in ROI knowledge.

Result: Experiments on RESISC45 and SEN12MS datasets show that per-tile domain separation reduces position-conditioned transfer in the linear probe, while adding frame freshness enhances robustness against imperfect ROI assumptions in the CNN probe. Cross-sample generalization results are mixed, reflecting realistic attacker limitations. The selective encryption functionality remains intact under consistent tiling and ROI policies.

Conclusion: Tilewise Domain-Separated Selective Encryption (TDS-SE) improves resistance to cross-tile structural leakage in remote sensing image encryption by introducing per-tile and optional per-frame domain separation via HKDF, and treating ROI masks as external side information. The evaluation framework using reconstruction-based distinguishers demonstrates enhanced security, particularly under realistic attacker assumptions, while maintaining selective encryption functionality.
...
TL;DR: MI$^2$DAS is a multi-layer intrusion detection framework for IIoT that combines hierarchical anomaly detection, open-set recognition, and incremental learning to effectively detect and adapt to both known and unknown cyberattacks with high accuracy and scalability.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the increased security vulnerabilities in IIoT systems due to device heterogeneity and evolving cyberattacks, along with limitations in traditional intrusion detection systems that depend heavily on labeled data and cannot effectively detect novel threats.

Method: The proposed MI$^2$DAS framework employs a multi-layer approach: (1) anomaly-based hierarchical traffic pooling using GMM for normal-attack discrimination, (2) open-set recognition using GMM and LOF to identify known and unknown attacks, and (3) fine-grained classification using Random Forest, combined with incremental learning to adapt to new attack types with minimal labeled data.

Result: Experiments on the Edge-IIoTset dataset show strong performance: GMM achieves 0.953 accuracy and 1.000 TPR in first-layer normal-attack detection; GMM obtains 0.813 recall for known attacks and LOF achieves 0.882 recall for unknown attacks in open-set recognition; Random Forest reaches a macro-F1 of 0.941 in fine-grained classification; incremental learning maintains a macro-F1 of 0.8995 when adapting to new attacks.

Conclusion: MI$^2$DAS is concluded to be an effective, scalable, and adaptive framework for enhancing security in IIoT environments by efficiently detecting both known and unknown cyberattacks through hierarchical detection, open-set recognition, and incremental learning.
...
TL;DR: This study evaluates the transferability of flow-based feature sets (Argus, Zeek, CICFlowMeter) across IoT and IIoT datasets, showing significant performance degradation in cross-domain intrusion detection due to distribution shifts, and provides guidelines for improving robustness through feature engineering and algorithm selection.


<details>
  <summary>Details</summary>
Motivation: Cross-domain intrusion detection is challenging due to variability in network traffic and feature distributions; this work aims to understand how well intrusion detection models and feature sets generalize across diverse IoT and Industrial IoT environments.

Method: The authors evaluate three flow-based feature sets (Argus, Zeek, CICFlowMeter) across four widely used IoT/IIoT datasets, using multiple classification models. They assess both in-domain and cross-domain performance and employ SHAP (SHapley Additive exPlanations) to analyze feature importance and transferability.

Result: Models trained on one domain exhibit significant performance degradation when applied to another, indicating sensitivity to domain shifts. The choice of both feature set and classification algorithm strongly affects transferability, with some features showing low generalization across domains.

Conclusion: Effective cross-domain intrusion detection requires not only high in-domain accuracy but also resilience to distribution shifts, achieved through thoughtful feature space design, proper algorithm selection, and adaptive strategies.
...
TL;DR: JAILBREAK FOUNDRY (JBF) is a system that automates the reproduction and standardized evaluation of LLM jailbreak attacks using a multi-agent workflow, achieving high fidelity and code reuse while enabling living benchmarks for reliable security assessment.


<details>
  <summary>Details</summary>
Motivation: Jailbreak techniques evolve rapidly, outpacing static benchmarks and causing inconsistencies in evaluation due to divergent datasets, tools, and judging methods, which undermines reliable robustness assessment.

Method: JBF employs a multi-agent system (JBF-FORGE) to translate jailbreak papers into executable modules, integrated via shared infrastructure (JBF-LIB) and evaluated uniformly (JBF-EVAL) using a consistent GPT-4o judge across 30 attacks and 10 victim models.

Result: JBF reproduces 30 jailbreak attacks with high fidelity (mean ASR deviation of +0.26 pp), reduces implementation code by nearly 50%, achieves 82.5% code reuse, and enables standardized evaluation across models and attacks.

Conclusion: JBF enables standardized, scalable, and automated evaluation of jailbreak attacks on LLMs, supporting living benchmarks that keep pace with fast-evolving threats.
...
TL;DR: SAILOR is a modular, ultra-lightweight RISC-V core family designed for IoT, offering scalability across 1- to 32-bit data-paths, integrating cryptography with minimal overhead, achieving significant improvements in energy efficiency and area over state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Existing RISC-V cores for IoT often prioritize either energy efficiency or area, but lack integration of cryptographic features without compromising compactness; this creates security and performance trade-offs in constrained environments.

Method: The authors design SAILOR, a modular and scalable RISC-V core family with serialized execution data-paths (1-, 2-, 4-, 8-, 16-, 32-bit), optimized for minimal area and efficient integration of RISC-V cryptographic extensions. They perform comprehensive evaluation of area, energy, and efficiency trade-offs.

Result: SAILOR achieves up to 13x higher performance and energy efficiency and reduces area by up to 59% compared to state-of-the-art solutions, demonstrating low overhead for cryptographic integration.

Conclusion: Lightweight cryptographic extensions can be efficiently integrated into ultra-lightweight RISC-V cores without sacrificing performance or area, enabling secure, energy-efficient IoT designs.
...
TL;DR: Anansi is the first scalable system to measure and analyze job-based smishing scams using LLMs, automated browsers, and infrastructure fingerprinting, revealing widespread fraud tactics and millions in crypto losses.


<details>
  <summary>Details</summary>
Motivation: Job-based smishing scams are a growing but understudied form of online fraud that exploits remote job seekers; existing methods lack scalability and systematic analysis capabilities.

Method: Anansi combines large language models (LLMs), automated browser agents, and infrastructure fingerprinting to engage with scammers, collect scam messages, and extract behavioral, financial, and infrastructural signals at scale.

Result: Collected over 29,000 scam messages, interacted with 1,900+ scammers, uncovered widespread reuse of templates, domains, and crypto wallets, and identified techniques like domain fronting and brand impersonation leading to millions in cryptocurrency losses.

Conclusion: Anansi demonstrates that automated, large-scale engagement with scammers is feasible and effective, providing a new methodological foundation for studying and combating online fraud ecosystems.
