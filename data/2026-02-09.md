...
TL;DR: The paper proposes a probabilistic framework to understand how coding agents interact with execution environments, formalizing selection heuristics and backprompting, proving theoretical advantages of fuzzy similarity over exact matching, and introducing a new benchmark for improved task descriptions.


<details>
  <summary>Details</summary>
Motivation: The theoretical underpinnings of how coding agents use execution feedback are poorly understood, despite their widespread use in test-driven development; this work aims to formalize and analyze dominant strategies like code selection and backprompting.

Method: The authors model code selection as environment-aware estimation of correctness, proving signal-to-noise advantages of fuzzy functional similarity; they interpret backprompting as in-context Thompson sampling and derive a regret bound for reward functions with unobservable components; they validate findings on three code benchmarks using open-weight models.

Result: Fuzzy similarity-based estimators theoretically dominate equivalence-based ones in signal quality; backprompting's effectiveness is limited by task ambiguity, formalized as irreducible regret; empirical results across benchmarks confirm the theoretical analysis; improved task descriptions lead to the creation of QiskitHumanEvalSimX.

Conclusion: The proposed probabilistic framework provides theoretical justification for common practices in coding agents and identifies limits of current feedback mechanisms; it also offers actionable insights for improving task specifications and evaluation benchmarks.
...
TL;DR: This paper introduces an automated mobile chaos testing system that combines LLM-based testing (DragonCrawl) with backend fault injection (uHavoc) to enable scalable resilience validation across Uber's mobile apps, identifying critical failures and reducing debugging effort through AI-driven execution and root cause analysis.


<details>
  <summary>Details</summary>
Motivation: Traditional chaos engineering does not scale for mobile testing due to the combinatorial complexity of user flows, geographic locations, and failure modes, necessitating an automated approach to validate mobile application resilience in large-scale distributed systems.

Method: The authors integrate DragonCrawl, an LLM-based mobile testing platform, with uHavoc, a service-level fault injection system, to automate chaos testing across diverse user flows, locations, and failure scenarios. Adaptive AI-driven execution navigates mobile apps under backend degradation, while automated root cause analysis identifies fault sources with high precision.

Result: The system executed over 180,000 automated chaos tests across 47 critical flows in Uber's apps, saving ~39,000 manual testing hours. It identified 23 resilience risks, including 70% architectural dependency violations, 12 severe functional issues, and 2 crashes only detectable via mobile testing. Root cause analysis achieved 88% precision@5 and reduced debugging time from hours to minutes, with 99% test reliability under fault injection.

Conclusion: The presented automated mobile chaos testing system enables continuous, large-scale resilience validation in production environments by combining AI-driven test execution with service-level fault injection, demonstrating that manual testing limitations can be overcome with adaptive, intelligent systems.
...
TL;DR: This vision paper redefines AI agents as collaborative members of software engineering teams and emphasizes objective trustworthiness through technical, ethical, and societal dimensions, advocating for ethics-by-design in AI software engineering systems.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the increasing integration of AI coding agents into software engineering practices, prompting a re-evaluation of the role of software engineers and the need to ensure that AI agents are trustworthy participants in collaborative development environments.

Method: The paper uses a conceptual and reflective methodology, grounded in established definitions of software engineering and recent research on agentic AI, to define AI software engineers within human-AI teams and identify core dimensions of their trustworthiness.

Result: The paper identifies key dimensions of trustworthiness in AI software engineers—technical quality, transparency and accountability, epistemic humility, and societal and ethical alignment—and highlights a trust measurement gap where critical aspects of trust cannot be easily quantified.

Conclusion: The paper concludes that trustworthiness in AI software engineers should be treated as an essential, objective property rather than a subjective human perception, and calls for an ethics-by-design approach in the development, evaluation, and governance of AI software engineering systems to foster appropriate trust in human-AI teams.
...
TL;DR: This paper introduces AgentStepper, the first interactive debugger for LLM-based software development agents, enabling stepwise control, inspection, and modification of agent behavior through high-level, structured trajectory visualization, resulting in substantially improved debugging performance and reduced cognitive load.


<details>
  <summary>Details</summary>
Motivation: Debugging LLM-based software development agents is difficult due to their complex, dynamic behaviors and the lack of transparent, high-level representations of their execution trajectories, which hinders effective analysis and troubleshooting.

Method: The authors designed AgentStepper, an interactive debugger that models agent executions as structured conversations among the LLM, agent program, and tools, enabling breakpoints, stepwise execution, live editing of prompts and tool calls, and visualization of code changes at the repository level.

Result: AgentStepper was integrated into three state-of-the-art agents with minimal code changes (39–42 lines) and evaluated in a user study with twelve participants, showing improved trajectory interpretation (64% vs. 67% mean performance), higher bug identification success (17% vs. 60%), and reduced perceived workload and frustration (from 5.4 to 2.4 on a 7-point scale).

Conclusion: AgentStepper significantly improves developers' ability to understand, debug, and interact with LLM-based software development agents by providing an interactive, high-level debugging interface that captures and visualizes agent trajectories effectively.
...
TL;DR: AST(NIT) is an AST serialization method for code summarization that enhances LLM input efficiency by preserving lexical and structural information, yielding comparable performance with reduced input size and training time.


<details>
  <summary>Details</summary>
Motivation: To fully exploit the structural information of Abstract Syntax Trees (ASTs) in LLM-based code summarization, addressing the limitation of current methods that use raw code or partial AST signals.

Method: AST(NIT), an AST augmentation and serialization technique that preserves lexical information and encodes structural details into sequences compatible with large language models.

Result: Using LLaMA-3.1-8B on the CodeXGLUE Python dataset, AST(NIT) reduces input length and training time while achieving summarization quality on par with existing methods.

Conclusion: The proposed AST(NIT) method effectively leverages complete AST representations for LLM-based code summarization, demonstrating efficiency and competitiveness in performance.
...
TL;DR: This study evaluates an LLM-based system for automating CI/CD pipeline failure management in SAP HANA. By leveraging domain knowledge—especially historical failure data—the system achieves high accuracy in error localization (97.4%) and exact solution generation (92.1%), demonstrating the effectiveness of LLMs in industrial software engineering automation.


<details>
  <summary>Details</summary>
Motivation: Manual CI/CD pipeline failure management is labor-intensive and inefficient. Traditional automation approaches struggle with unstructured data, motivating the use of LLMs capable of processing such information to improve automation accuracy and efficiency.

Method: An LLM-based system was developed and evaluated to automate CI/CD pipeline failure management. The system was provided with various types of domain knowledge—pipeline information, failure management instructions, and historical failure data. An ablation study was conducted to assess the impact of each knowledge type on performance.

Result: The LLM-based system achieved 97.4% accuracy in identifying error locations when domain knowledge was provided, compared to 84.2% without it. Exact solution generation reached 92.1% accuracy, with historical failure data proving to be the most impactful knowledge source.

Conclusion: LLMs, when augmented with historical failure data, show strong potential for automating CI/CD pipeline failure management in large-scale industrial projects like SAP HANA, significantly improving accuracy in error localization and solution generation.
...
TL;DR: This paper proposes an interpretable, threshold-based fault prediction method for embedded firmware using static analysis metrics, validated on three industrial C projects, enabling cross-project fault detection without retraining and supporting integration into safety-critical software development workflows.


<details>
  <summary>Details</summary>
Motivation: The lack of interpretability in machine learning-based fault prediction models hinders their adoption in safety-critical embedded systems, where compliance with standards like ISO 26262 demands transparent and actionable fault detection mechanisms.

Method: A structured process was developed to derive context-specific software metric thresholds from static analysis data (using Coverity and Understand tools) across three real-world C-embedded firmware projects. Discriminative metrics were identified through statistical analysis and hypothesis testing, and thresholds were validated via experimental evaluation for cross-project fault prediction without retraining.

Result: Empirical thresholds were successfully derived and validated, demonstrating high precision in identifying fault-prone functions. The method enabled effective cross-project fault prediction without domain-specific tuning or model retraining, supporting reuse across similar firmware systems.

Conclusion: The proposed approach provides an interpretable, threshold-based method for fault prediction in embedded firmware that aligns with industrial software quality assurance practices and functional safety standards, offering a practical alternative to black-box AI models.
...
TL;DR: TraceCoder is a multi-agent framework that improves code repair in LLMs by using runtime tracing, causal analysis, and learning from past failures, achieving up to 34.43% better Pass@1 accuracy than state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: LLMs frequently produce buggy code for complex tasks, and existing repair methods rely on coarse pass/fail feedback, limiting error localization and leading to repetitive, inefficient repair attempts due to lack of memory or learning from prior failures.

Method: TraceCoder employs a collaborative multi-agent system that (1) instruments code with diagnostic probes to collect fine-grained runtime traces, (2) performs causal analysis on traces to pinpoint root causes, (3) uses a Historical Lesson Learning Mechanism (HLLM) to distill knowledge from past failures, and (4) applies a Rollback Mechanism to ensure only beneficial changes are retained during iterative repair.

Result: TraceCoder achieves up to 34.43% relative improvement in Pass@1 accuracy across multiple benchmarks. Ablation studies show the iterative repair process contributes a 65.61% relative accuracy gain, and the system outperforms leading methods in both accuracy and cost-efficiency.

Conclusion: By integrating runtime observation, causal diagnosis, and memory-driven learning, TraceCoder effectively enhances code repair in LLMs, enabling precise error correction and efficient convergence while avoiding recurring mistakes.
...
### [10] [Multi-Agent-Driven Cognitive Secure Communications in Satellite-Terrestrial Networks](https://arxiv.org/abs/2602.06048)
*Yujie Ling,Zan Li,Lei Guan,Zheng Zhang,Shengyu Zhang,Tony Q. S. Quek*

Main category: cs.CR

TL;DR: A cognitive secure communication framework using multi-agent coordination and generative adversarial networks is proposed to enhance secrecy probability and reduce power overhead in satellite-terrestrial networks.


<details>
  <summary>Details</summary>
Motivation: Secure communication in satellite-terrestrial networks is threatened by intelligent, non-cooperative eavesdroppers that can compromise private information, necessitating proactive and adaptive defense mechanisms.

Method: A two-layer coordinated defense system is proposed: 1) a foundation layer using multi-agent coordination for spectrum scheduling (satellite operation and frequency slot matrices) to reduce congestion and improve reliability; 2) a protection layer using generative adversarial networks to create adversarial matrices and learning-aided power control to optimize real and adversarial signal powers, degrading eavesdropper inference capabilities. An optimization problem is formulated to maximize secrecy probability under reliability constraints.

Result: Simulation results show the proposed method outperforms benchmarks in enhancing security (higher secrecy probability) and reducing power overhead while maintaining reliable transmission in cognitive satellite-terrestrial networks.

Conclusion: The integration of multi-agent coordination and adversarial learning enables robust, proactive defense against intelligent eavesdroppers, making the proposed framework effective for secure and efficient cognitive communication in satellite-terrestrial networks.
...
### [11] [Know Your Scientist: KYC as Biosecurity Infrastructure](https://arxiv.org/abs/2602.06172)
*Jonathan Feldman,Tal Feldman,Annie I Anton*

Main category: cs.CR

TL;DR: This paper proposes a Know Your Customer (KYC) framework for biological AI, using institutional verification and behavioral monitoring to mitigate dual-use risks in protein design and structure prediction, offering a feasible, immediate governance solution without new regulations.


<details>
  <summary>Details</summary>
Motivation: Rapid advances in biological AI tools for protein design and structure prediction pose significant dual-use risks that current content-based safeguards are unable to effectively address due to the complexity and unpredictability of biological functions.

Method: The authors propose a three-tier Know Your Customer (KYC) framework adapted from anti-money laundering practices, with Tier I relying on institutional vetting, Tier II employing sequence homology and functional annotation for output screening, and Tier III implementing behavioral monitoring to detect anomalous usage patterns.

Result: The KYC framework enables effective governance by shifting focus from unreliable content inspection to verifiable user identity and behavior, enhancing traceability and accountability while maintaining accessibility for legitimate scientific work.

Conclusion: The proposed three-tier KYC framework offers a practical and immediate solution for governing biological AI tools by emphasizing user verification and institutional accountability, thereby mitigating dual-use risks without restricting legitimate research.
...
### [12] [Identifying Adversary Tactics and Techniques in Malware Binaries with an LLM Agent](https://arxiv.org/abs/2602.06325)
*Zhou Xuan,Xiangzhe Xu,Mingwei Zheng,Louis Zheng-Hua Tan,Jinyao Guo,Tiantai Zhang,Le Yu,Chengpeng Wang,Xiangyu Zhang*

Main category: cs.CR

TL;DR: TTPDetect is the first LLM agent designed to identify Tactics, Techniques, and Procedures (TTPs) in stripped malware binaries by combining dense and neural retrieval to pinpoint analysis entry points, using a context-aware function-level agent with a Context Explorer and TTP-specific reasoning, achieving high precision and recall on both labeled data and real-world samples.


<details>
  <summary>Details</summary>
Motivation: Identifying TTPs in real-world malware is difficult due to stripped symbols, numerous functions, and distributed malicious logic; existing LLMs struggle with entry point identification, partial observability, and misalignment with TTP reasoning.

Method: TTPDetect uses hybrid retrieval (dense + LLM-based) to identify entry points, employs a function-level agent with a Context Explorer for incremental context gathering, and introduces a TTP-Specific Reasoning Guideline for inference-time alignment; evaluation is performed on a new dataset with function-level TTP annotations across diverse malware.

Result: TTPDetect achieves 93.25% precision and 93.81% recall on function-level TTP recognition, outperforming baselines by 10.38% and 18.78%; on real malware, it reaches 87.37% precision, recovers 85.7% of expert-reported TTPs, and discovers an average of 10.5 new TTPs per sample.

Conclusion: TTPDetect effectively bridges the gap between LLMs and practical malware analysis by enabling accurate, context-aware TTP attribution in stripped binaries, demonstrating strong performance and utility in real-world threat intelligence.
...
### [13] [AdFL: In-Browser Federated Learning for Online Advertisement](https://arxiv.org/abs/2602.06336)
*Ahmad Alemari,Pritam Sen,Cristian Borcea*

Main category: cs.CR

TL;DR: AdFL is a browser-based federated learning framework enabling privacy-preserving targeted advertising by training on user behavior locally and aggregating model updates server-side, achieving high ad viewability prediction accuracy while complying with privacy regulations.


<details>
  <summary>Details</summary>
Motivation: The need to comply with online privacy regulations like GDPR while maintaining effective targeted advertising motivates the development of privacy-preserving ad systems that avoid direct collection of user data.

Method: AdFL employs federated learning to train models directly in users' browsers using behavioral features (e.g., ad click-through, dwell time, page content) without sharing raw data. A central server at the publisher orchestrates model aggregation and distribution. The system uses standard web APIs and integrates differential privacy to protect model updates.

Result: AdFL successfully captures training data in milliseconds per user. The ad viewability prediction model achieves up to 92.59% AUC. With differential privacy, performance remains high with only minor degradation compared to the non-DP version.

Conclusion: AdFL is a feasible and practical framework for targeted advertising that balances user privacy and revenue generation by leveraging federated learning in browsers, with strong performance even when incorporating differential privacy.
...
### [14] [Zero-Trust Runtime Verification for Agentic Payment Protocols: Mitigating Replay and Context-Binding Failures in AP2](https://arxiv.org/abs/2602.06345)
*Qianlong Lan,Anuj Kaul,Shaun Jones,Stephanie Westrum*

Main category: cs.CR

TL;DR: This paper analyzes security gaps in the Agent Payments Protocol (AP2) during runtime in autonomous agent systems and proposes a zero-trust framework using time-bound nonces to enforce context binding and single-use mandates, preventing replay and context-redirect attacks with low overhead and high throughput.


<details>
  <summary>Details</summary>
Motivation: Runtime behaviors in autonomous agent systems—such as retries, concurrency, and orchestration—introduce security risks that challenge the implicit assumptions of mandate-based payment protocols like AP2, necessitating runtime enforcement beyond static cryptographic guarantees.

Method: A zero-trust runtime verification framework is proposed, utilizing dynamically generated, time-bound nonces to enforce explicit context binding and consume-once mandate semantics. Evaluation is conducted via simulation under high concurrency to assess security and performance.

Result: The framework effectively mitigates replay and context-redirect attacks, with stable verification latency of ~3.8 ms at up to 10,000 transactions per second. Runtime state is bounded by peak concurrency, not transaction history, ensuring scalability and efficiency.

Conclusion: Robust runtime security for agentic payment execution can be achieved with minimal and predictable overhead by enforcing context-aware binding and consume-once semantics using a zero-trust verification framework.
...
### [15] [VENOMREC: Cross-Modal Interactive Poisoning for Targeted Promotion in Multimodal LLM Recommender Systems](https://arxiv.org/abs/2602.06409)
*Guowei Guan,Yurong Hao,Jiaming Zhang,Tiantong Wu,Fuyao Zhang,Tianxiang Chen,Longtao Huang,Cyril Leung,Wei Yang Bryan Lim*

Main category: cs.CR

TL;DR: This paper identifies a new security threat in multimodal recommender systems where synchronized text-image poisoning can exploit cross-modal fusion; it proposes VENOMREC, a novel attack method using exposure-aligned, attention-guided perturbations that effectively manipulates recommendations with minimal detection risk.


<details>
  <summary>Details</summary>
Motivation: While cross-modal fusion in multimodal recommender systems improves robustness against single-modality attacks, it introduces a new vulnerability to synchronized multimodal poisoning that can subtly and consistently manipulate model behavior through aligned perturbations.

Method: VENOMREC employs Exposure Alignment to detect high-exposure regions in the joint embedding space and uses Cross-modal Interactive Perturbation to generate attention-guided, coupled token-patch modifications in text and image modalities for coordinated poisoning.

Result: VENOMREC significantly outperforms strong baselines across three real-world multimodal datasets, achieving a mean ER@20 of 0.73 and an average improvement of +0.52 ER points over the best baseline, while maintaining comparable recommendation performance.

Conclusion: The proposed VENOMREC framework effectively exploits the vulnerability of cross-modal consensus in multimodal recommender systems, demonstrating that synchronized multimodal poisoning can reliably manipulate fused representations during fine-tuning while preserving recommendation utility.
...
TL;DR: CPRVul is a context-aware vulnerability detection framework that improves upon function-level methods by using context profiling, selection, and structured reasoning to incorporate relevant inter-procedural information, achieving state-of-the-art performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Vulnerabilities often depend on inter-procedural context, but existing methods either ignore context or degrade performance when using raw, noisy context. There is a need for selective and structured use of context in vulnerability detection.

Method: CPRVul builds a code property graph to extract candidate context, uses an LLM to profile and score context relevance, selects high-impact context within the model's window, and employs structured reasoning traces (target function + selected context + metadata) to fine-tune LLMs for detection.

Result: CPRVul outperforms function-only baselines across PrimeVul, TitanVul, and CleanVul datasets, achieving 64.94%–73.76% accuracy vs. 56.65%–63.68% for UniXcoder. On PrimeVul, it reaches 67.78% accuracy (22.9% improvement over prior SOTA). Ablations show gains require both processed context and structured reasoning.

Conclusion: Effective vulnerability detection requires not just more context, but intelligently selected and structured context combined with reasoning-aware training; CPRVul demonstrates that this approach significantly boosts performance over existing methods.
...
### [17] [The Avatar Cache: Enabling On-Demand Security with Morphable Cache Architecture](https://arxiv.org/abs/2602.06433)
*Anubhav Bhatla,Navneet Navneet,Moinuddin Qureshi,Biswabandan Panda*

Main category: cs.CR

TL;DR: Avatar is a morphable last-level cache that dynamically switches between secure and non-secure modes, providing strong protection against conflict- and occupancy-based side-channel attacks with minimal overhead, enabling practical and efficient security in modern processors.


<details>
  <summary>Details</summary>
Motivation: Modern LLCs are vulnerable to cross-core attacks; existing secure designs suffer from high overheads or complexity, limiting industrial adoption. There is a need for a secure LLC that minimizes changes to conventional designs and enables on-demand security.

Method: Design and evaluation of Avatar, a morphable LLC supporting three operational modes (non-secure, randomized secure, and partitioned secure), leveraging invalid entries and high associativity for security with minimal performance and area impact.

Result: Avatar-R provides strong conflict-based attack resistance with 1.5% storage overhead, 2.7% increase in static power, and 0.2% performance slowdown. Avatar-P reduces performance overhead to 3%, significantly better than prior partitioned designs. Security can be disabled (Avatar-N) for optimal efficiency.

Conclusion: Avatar enables strong LLC security with minimal modifications to conventional set-associative caches, offering dynamic switching between secure and non-secure modes, low overhead, and improved practicality for industrial adoption.
...
### [18] [TrajAD: Trajectory Anomaly Detection for Trustworthy LLM Agents](https://arxiv.org/abs/2602.06443)
*Yibing Liu,Chong Zhang,Zhongyi Han,Hansong Liu,Yong Wang,Yang Yu,Xiaoyan Wang,Yilong Yin*

Main category: cs.CR

TL;DR: This paper introduces Trajectory Anomaly Detection (TrajAD) for identifying and localizing runtime errors in LLM agents' execution processes, proposing a new benchmark (TrajBench) and a specialized verifier trained with fine-grained supervision to improve agent reliability.


<details>
  <summary>Details</summary>
Motivation: Current LLM safety mechanisms rely on static input/output filtering, which cannot ensure reliability during dynamic execution; thus, there is a need to audit intermediate steps for trustworthy agent behavior.

Method: The authors formulate trajectory anomaly detection with error localization, create TrajBench using a perturb-and-complete strategy to generate procedural anomalies, and propose TrajAD—a specialized verifier trained with fine-grained process supervision—to detect and localize anomalies.

Result: Experiments show that general-purpose LLMs fail at zero-shot anomaly localization; TrajAD significantly outperforms baselines on TrajBench, demonstrating the necessity of specialized training for process reliability.

Conclusion: Specialized process supervision via fine-grained annotations is essential for enabling reliable and trustworthy LLM agents, surpassing general-purpose models in runtime anomaly detection and localization.
...
### [19] [Subgraph Reconstruction Attacks on Graph RAG Deployments with Practical Defenses](https://arxiv.org/abs/2602.06495)
*Minkyoo Song,Jaehan Kim,Myungchul Kang,Hanna Kim,Seungwon Shin,Sooel Son*

Main category: cs.CR

TL;DR: This paper presents GRASP, a stealthy, multi-turn attack that effectively reconstructs subgraphs from Graph RAG systems despite safety prompts, by using context-aware querying, output grounding, and query diversification; it achieves high reconstruction accuracy and proposes effective lightweight countermeasures.


<details>
  <summary>Details</summary>
Motivation: While Graph RAG enhances relational reasoning in LLMs by leveraging structured knowledge, it poses a privacy risk: adversarial subgraph reconstruction can lead to leakage of sensitive or proprietary knowledge. Existing attacks are ineffective due to explicit exfiltration patterns that trigger prompt-based safeguards, necessitating a more stealthy and robust approach.

Method: The authors introduce GRASP, a closed-box, multi-turn attack method that reframes extraction as a context-processing task. Key technical components include format-compliant, instance-grounded outputs using per-record identifiers to minimize hallucinations, and a momentum-aware scheduler to diversify queries and operate within limited query budgets.

Result: GRASP achieves up to 82.9 F1 score in type-faithful subgraph reconstruction across two real-world knowledge graphs, four safety-aligned LLMs, and multiple Graph RAG frameworks—outperforming prior methods where they fail. Evaluation of proposed defenses shows they significantly reduce attack success without degrading system performance.

Conclusion: GRASP represents a significant advancement in subgraph reconstruction attacks on Graph RAG systems, demonstrating high efficacy even under realistic safety constraints. The paper concludes by proposing two lightweight defenses that reduce reconstruction fidelity without impairing system utility.
...
### [20] [Sequential Auditing for f-Differential Privacy](https://arxiv.org/abs/2602.06518)
*Tim Kutta,Martin Dunsche,Yu Wei,Vassilis Zikas*

Main category: cs.CR

TL;DR: This paper introduces adaptive auditors for f-Differential Privacy that efficiently detect privacy violations using minimal samples without user input on sample size, improving practicality for costly training processes like DP-SGD.


<details>
  <summary>Details</summary>
Motivation: Existing DP auditors are often limited to batch processing and (ε,δ)-DP, requiring large, user-specified sample sizes; this work addresses the need for more expressive, efficient, and automated auditing for the richer f-DP framework.

Method: The authors develop adaptive, sample-efficient auditors for f-Differential Privacy that operate by analyzing output samples of an algorithm; these auditors work in both whitebox and blackbox settings and support single-run execution, using statistical methods to determine a near-optimal number of samples for decision-making.

Result: The proposed auditors achieve statistically sound privacy validation across the full f-DP spectrum with significantly reduced sample complexity, demonstrated through theoretical analysis and simulations.

Conclusion: The proposed auditors for f-DP offer adaptive, statistically significant detection of privacy violations across the entire privacy spectrum without requiring a pre-specified sample size, making them efficient and practical for real-world applications such as DP-SGD.
...
### [21] [Dependable Artificial Intelligence with Reliability and Security (DAIReS): A Unified Syndrome Decoding Approach for Hallucination and Backdoor Trigger Detection](https://arxiv.org/abs/2602.06532)
*Hema Karnam Surendrababu,Nithin Nagaraj*

Main category: cs.CR

TL;DR: This paper introduces DAIReS, a unified framework using Syndrome Decoding adapted to NLP sentence embeddings to detect both backdoor data-poisoning attacks and hallucinations in LLMs, enhancing the security and reliability of AI systems.


<details>
  <summary>Details</summary>
Motivation: Machine learning models, especially LLMs, face critical security threats like backdoor data-poisoning and reliability issues such as hallucinations, necessitating a unified detection mechanism to ensure dependable AI.

Method: The authors adapt syndrome decoding—a technique from coding theory—to the NLP sentence-embedding space to identify anomalies; this method detects poisoned samples in training data and hallucinated outputs in LLMs by analyzing deviations in semantic representations.

Result: The proposed approach effectively distinguishes between poisoned and clean training samples and detects hallucinations in LLM outputs, demonstrating high accuracy in identifying both security and reliability violations.

Conclusion: Syndrome decoding can be successfully applied to NLP embedding spaces, providing a robust, unified solution for enhancing the dependability of AI systems by simultaneously addressing security and reliability challenges.
...
### [22] [AlertBERT: A noise-robust alert grouping framework for simultaneous cyber attacks](https://arxiv.org/abs/2602.06534)
*Lukas Karner,Max Landauer,Markus Wurzenberger,Florian Skopik*

Main category: cs.CR

TL;DR: AlertBERT is a self-supervised framework using masked language models and density-based clustering to improve alert grouping in cybersecurity, reducing analyst workload and outperforming time-based methods in noisy, large-scale environments.


<details>
  <summary>Details</summary>
Motivation: High alert volumes from intrusion detection systems cause alert fatigue in SOCs, leading to slow responses and errors; existing time-based grouping fails in complex, noisy, and concurrent attack scenarios.

Method: Proposes AlertBERT, combining masked language models (for semantic alert representation) and density-based clustering; introduces a data augmentation method to simulate noise and concurrent attacks for evaluation.

Result: AlertBERT consistently outperforms conventional time-based grouping techniques in accuracy for correct alert grouping under high noise and concurrent attack conditions.

Conclusion: AlertBERT effectively addresses limitations of traditional alert grouping, enabling robust, scalable alert correlation in real-world, noisy network environments for both real-time and forensic analysis.
...
### [23] [Malicious Agent Skills in the Wild: A Large-Scale Security Empirical Study](https://arxiv.org/abs/2602.06547)
*Yi Liu,Zhihao Chen,Yanjun Zhang,Gelei Deng,Yuekang Li,Jianting Ning,Leo Yu Zhang*

Main category: cs.CR

TL;DR: This paper presents the first large-scale behavioral analysis of third-party LLM agent skills, identifying 157 malicious skills with 632 vulnerabilities across 98,380 samples. It reveals intentional, sophisticated attacks—categorized as Data Thieves and Agent Hijackers—with widespread use of supply chain attacks and instruction manipulation, and shows that advanced threats exploit undocumented 'shadow features' and platform APIs. The study underscores critical security risks in agent ecosystems and releases a dataset to support future research.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the increasing use of third-party agent skills in LLM-based systems, which execute with user privileges but are distributed with minimal vetting. The lack of a ground-truth dataset to understand the nature and scope of security threats posed by these skills necessitated a systematic investigation into their malicious potential.

Method: The authors constructed the first labeled dataset of malicious agent skills by behaviorally analyzing 98,380 skills from two community registries. They verified malicious behavior through dynamic analysis and manual inspection, identifying vulnerabilities and categorizing attacks based on kill chain phases and attack patterns. They also examined shadow features and platform-level exploits, and conducted responsible disclosure to measure takedown efficacy.

Result: The analysis confirmed 157 malicious skills containing 632 vulnerabilities, averaging 4.03 vulnerabilities per skill across a median of three kill chain phases. Two attack archetypes were identified: Data Thieves and Agent Hijackers. A single actor was responsible for 54.1% of confirmed cases via brand impersonation. Shadow features were present in 100% of advanced attacks but 0% of basic ones, with some skills exploiting platform mechanisms like hook systems and permission flags. Responsible disclosure resulted in 93.6% of malicious skills being removed within 30 days.

Conclusion: The study concludes that the ecosystem of third-party agent skills poses significant security risks, with malicious skills exhibiting multiple vulnerabilities and operating across distinct attack archetypes: Data Thieves and Agent Hijackers. The research highlights the prevalence of sophisticated, intentional attacks—often from a single actor using templated brand impersonation—and underscores the importance of behavioral verification and responsible disclosure. The release of the dataset and analysis pipeline aims to advance research in agent skill security.
...
### [24] [A Survey of Security Threats and Trust Management in Vehicular Ad Hoc Networks](https://arxiv.org/abs/2602.06608)
*Rezvi Shahariar,Chris Phillips*

Main category: cs.CR

TL;DR: This paper surveys and classifies state-of-the-art trust models in VANETs, highlighting two main classes—receiver-side and sender-side trust evaluation—and advocates for further research in sender-side models due to their efficiency and support for real-time decision-making.


<details>
  <summary>Details</summary>
Motivation: Traditional security mechanisms fail to address malicious insider attacks in VANETs; thus, trust management is essential. Existing trust models vary in design and scope, necessitating a comprehensive survey to understand their strengths, limitations, and deployment challenges.

Method: The authors conduct a literature survey, classify trust models based on evaluation point (receiver-side vs. sender-side), summarize key approaches, and compare them. A sequence diagram is used to contrast the two evaluation paradigms.

Result: Two broad classes of trust models are identified: receiver-side (computationally heavy on receivers) and sender-side (computation at sender, enabling fast decisions). Only one known sender-side model exists for VANETs, with most works favoring receiver-side evaluation. The analysis shows sender-side evaluation reduces receiver load and enhances response time.

Conclusion: Sender-side trust evaluation is promising for real-time, efficient trust management in VANETs. The paper calls for future research in this direction and emphasizes challenges like real-time constraints and computational efficiency in deployment.
...
### [25] [HYDRA: Unearthing "Black Swan" Vulnerabilities in LEO Satellite Networks](https://arxiv.org/abs/2602.06612)
*Bintao Yuan,Mingsheng Tang,Binbin Ge,Hongbin Luo,Zijie Yan*

Main category: cs.CR

TL;DR: HYDRA is a hypergraph-based framework for dynamic risk analysis in LEO satellite constellations, introducing Hyper-Bridge Centrality (HBC) to identify critical 'Black Swan' nodes at ground-space interfaces, which are topologically peripheral but structurally crucial; simulations on Starlink-like networks show HBC outperforms traditional centrality metrics in revealing high-impact vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: Traditional topology-centric security analysis fails to capture systemic risks in LEO constellations caused by dynamic load imbalances and high-order dependencies, necessitating a new approach to identify critical failure points that can trigger network-wide cascades.

Method: Proposes HYDRA, a hypergraph-based framework with Hyper-Bridge Centrality (HBC) as a novel metric that quantifies node criticality using a load-to-redundancy ratio within dependency structures. Uses realistic simulations based on Starlink TLE data and a population-based gravity model to evaluate vulnerabilities.

Result: HBC outperforms traditional centrality metrics (e.g., betweenness) in identifying critical nodes. The most vulnerable points are found at ground-space interfaces—'Black Swan' nodes—that are topologically marginal but cause disproportionate structural damage when compromised.

Conclusion: Security in LEO constellations should shift from connectivity-based models to structural stress analysis. Protecting edge interfaces is essential, requiring a rethinking of redundancy and resilience strategies in satellite network design.
...
### [26] [Confundo: Learning to Generate Robust Poison for Practical RAG Systems](https://arxiv.org/abs/2602.06616)
*Haoyang Hu,Zhejun Jiang,Yueming Lyu,Junyuan Zhang,Yi Liu,Ka-Ho Chow*

Main category: cs.CR

TL;DR: Confundo is a learning-to-poison framework that improves the effectiveness, robustness, and stealthiness of poisoning attacks on real-world retrieval-augmented generation systems by training a language model to generate adaptive poisoned content, exposing key vulnerabilities while also enabling protection against unauthorized data use.


<details>
  <summary>Details</summary>
Motivation: Existing poisoning attacks on retrieval-augmented generation (RAG) systems show limited effectiveness in real-world settings due to overlooked factors like content preprocessing and query variation, leading to underestimation of security risks; this work aims to close the gap by evaluating and enhancing attack practicality.

Method: Confundo employs a learning-to-poison framework that fine-tunes a large language model to generate optimized poison content, addressing preprocessing-induced fragmentation and query variability to enhance attack effectiveness, robustness, and stealthiness across multiple objectives such as factual manipulation, bias induction, and hallucination triggering.

Result: Confundo significantly outperforms existing purpose-built attacks across various datasets and RAG configurations, maintaining high success rates even under defensive measures, and demonstrates a novel defensive use case for preventing unauthorized web content scraping in RAG systems.

Conclusion: Confundo successfully exposes critical vulnerabilities in practical RAG systems by achieving highly effective, robust, and stealthy poisoning attacks, while also enabling a defensive application to protect web content from unauthorized use without affecting user experience.
...
### [27] [TrapSuffix: Proactive Defense Against Adversarial Suffixes in Jailbreaking](https://arxiv.org/abs/2602.06630)
*Mengyao Du,Han Fang,Haokai Ma,Gang Yang,Quanjun Yin,Shouling Ji,Ee-Chien Chang*

Main category: cs.CR

TL;DR: TrapSuffix is a proactive, lightweight defense that uses fine-tuning to make suffix-based jailbreak attacks fail or leave detectable traces, achieving near-zero attack success with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Existing defenses against suffix-based jailbreak attacks are mostly passive and fail to leverage the defender's asymmetric advantage in controlling the model. The open-ended nature of adversarial suffixes necessitates a proactive, controllability-oriented defense.

Method: TrapSuffix is a lightweight fine-tuning method that injects trap-aligned behaviors into the base model, proactively reshaping its response landscape to adversarial suffixes. It creates optimization traps and traceable fingerprints without altering the inference pipeline.

Result: TrapSuffix reduces the average attack success rate to below 0.01% and achieves an 87.9% average tracing success rate across various jailbreak settings. It adds only 15.87 MB of memory overhead on average and introduces no inference-time cost, significantly outperforming LLM-based detection methods in efficiency.

Conclusion: TrapSuffix provides a lightweight, effective, and traceable defense against suffix-based jailbreak attacks by reshaping the model's response landscape to channel attacks into either failure or detectable patterns, without introducing inference overhead and while complementing existing defenses.
...
### [28] [Jamming Attacks on the Random Access Channel in 5G and B5G Networks](https://arxiv.org/abs/2602.06634)
...
TL;DR: Ethereum's large validator set ensures decentralization and economic security but results in slow finality due to inefficient signature aggregation. Existing protocols also have exploitable weaknesses. This paper presents Wonderboom, the first million-scale aggregation protocol that finalizes blocks 32x faster while enhancing security. A new simulator validates its performance, showing it can handle over 2 million signatures per slot.


<details>
  <summary>Details</summary>
Motivation: The slow block finalization in Ethereum (around 15 minutes) due to costly signature aggregation across nearly one million validators hinders real-world applicability and introduces security vulnerabilities that can be exploited by adversaries to skew stake distribution.

Method: Wonderboom introduces a novel aggregation protocol designed for million-scale validator sets, leveraging efficient cryptographic techniques and a new simulation framework to validate performance at scale.

Result: Wonderboom achieves signature aggregation and verification of over 2 million signatures within a single Ethereum slot (x32 faster than current protocols) even under worst-case conditions, as demonstrated by a newly developed large-scale simulation tool.

Conclusion: Wonderboom provides a scalable and secure solution for signature aggregation in Ethereum, enabling faster block finalization and improved resistance to adversarial manipulation compared to the current protocol.
...
### [30] [Evaluating and Enhancing the Vulnerability Reasoning Capabilities of Large Language Models](https://arxiv.org/abs/2602.06687)
*Li Lu,Yanjie Zhao,Hongzhou Rao,Kechi Zhang,Haoyu Wang*

Main category: cs.CR

TL;DR: The paper introduces DAGVul, a DAG-based framework with reinforcement learning and verifiable rewards for reliable vulnerability reasoning in code. It addresses hallucinated logic in LLMs by enforcing causal structure, achieving significant gains in reasoning accuracy—even outperforming much larger models.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs often produce correct vulnerability detection results using flawed or hallucinated reasoning, which remains undetected due to coarse evaluation metrics. There is a need for benchmarks with causal ground truth and methods that ensure logically consistent and robust reasoning.

Method: The authors propose DAGVul, a framework that models vulnerability reasoning as a Directed Acyclic Graph (DAG) generation task to capture causal dependencies, combined with Reinforcement Learning with Verifiable Rewards (RLVR) to align the model's reasoning traces with program-intrinsic logic. They evaluate using a benchmark with expert-curated causal reasoning and semantically equivalent code variants.

Result: DAGVul improves reasoning F1-score by an average of 18.9% over baselines. An 8B-parameter model outperforms larger reasoning-specialized models like Qwen3-30B-Reasoning and GPT-OSS-20B-High, and achieves performance close to Claude-Sonnet-4.5 (75.47% vs 76.11%). The study identifies 12 systematic failure patterns in current models' reasoning.

Conclusion: DAGVul establishes new efficiency in vulnerability reasoning by outperforming larger and specialized models, demonstrating that structured reasoning with verifiable rewards significantly improves reliability and scalability in LLM-based vulnerability detection.
...
### [31] [Taipan: A Query-free Transfer-based Multiple Sensitive Attribute Inference Attack Solely from Publicly Released Graphs](https://arxiv.org/abs/2602.06700)
*Ying Song,Balaji Palanisamy*

Main category: cs.CR

TL;DR: Taipan is the first query-free, transfer-based attack framework that exploits intrinsic leakage of multiple sensitive attributes from publicly released graphs. By leveraging hierarchical knowledge routing and prompt-guided refinement, it effectively infers sensitive information without model queries, exposing critical privacy risks even under differential privacy, and calls for improved multi-attribute graph privacy methods.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the limitations of existing attribute inference attacks that rely on repeated model queries, which are impractical due to regulatory, budgetary, and detectability constraints. The authors aim to expose a critical blind spot: intrinsic leakage of multiple sensitive attributes from publicly released graph structures without any model querying.

Method: The proposed method, Taipan, introduces a query-free transfer-based attack framework for Graph-based Multiple Sensitive Attribute Inference Attacks (G-MSAIAs). It employs Hierarchical Attack Knowledge Routing to model inter-attribute dependencies and Prompt-guided Attack Prototype Refinement to reduce negative transfer. A systematic evaluation framework is also designed to assess performance across diverse distribution settings.

Result: Taipan achieves strong attack performance across various real-world graph datasets, demonstrating effectiveness in same-distribution, heterogeneous, similar- and out-of-distribution settings—even with mismatched feature dimensionalities. Notably, it remains effective under rigorous differential privacy mechanisms, highlighting inherent vulnerabilities in current privacy-preserving graph publishing techniques.

Conclusion: The paper concludes that intrinsic multiple sensitive information leakage in publicly released graphs poses a significant privacy threat, necessitating the development of more robust privacy-preserving graph publishing and data-sharing methods to defend against query-free, transfer-based attacks like Taipan.
...
### [32] [GhostCite: A Large-Scale Analysis of Citation Validity in the Age of Large Language Models](https://arxiv.org/abs/2602.06718)
...
TL;DR: CiteVerifier is introduced as an open-source framework to assess citation validity in academic writing amid the rise of LLMs; a comprehensive study reveals widespread citation hallucination by LLMs (14.23% to 94.93%) and detects 1.07% of papers in top AI/security venues containing fabricated citations, with an 80.9% increase in 2025, exposing a critical verification gap in research practices.


<details>
  <summary>Details</summary>
Motivation: The increasing use of LLMs in academic writing raises concerns about citation hallucinations ('ghost citations'), which undermine scientific integrity; this work aims to quantify the extent of citation invalidity across LLM-generated and real-world scholarly literature and to identify systemic weaknesses in human and institutional verification processes.

Method: The authors develop CiteVerifier, an open-source framework for large-scale citation verification, and conduct three experiments: (1) benchmarking 13 state-of-the-art LLMs on citation generation across 40 research domains, (2) analyzing 2.2 million citations from 56,381 papers published in top AI/ML and Security venues from 2020 to 2025, and (3) surveying 97 researchers (94 valid responses) on citation practices and attitudes toward citation verification.

Result: All 13 LLMs hallucinate citations, with rates ranging from 14.23% to 94.93%, varying significantly across domains; 1.07% of real papers (604 out of 56,381) contain invalid or fabricated citations, showing an 80.9% increase in 2025 alone; the survey reveals that 41.5% of researchers copy-paste BibTeX without verification, 44.4% take no action when encountering suspicious references, 76.7% of reviewers do not thoroughly check citations, and 80.0% never suspect fake citations.

Conclusion: The convergence of unreliable LLM-generated citations and inadequate human and peer-review scrutiny creates an accelerating threat to scientific integrity; the study calls for coordinated interventions targeting researchers, academic venues, and tool developers to improve citation verification and preserve trust in the scholarly record.
...
### [33] [A Unified Framework for LLM Watermarks](https://arxiv.org/abs/2602.06754)
...
### [34] [$f$-Differential Privacy Filters: Validity and Approximate Solutions](https://arxiv.org/abs/2602.06756)
*Long Tran,Antti Koskela,Ossi Räisä,Antti Honkela*

Main category: cs.CR

TL;DR: The paper demonstrates that the natural approach to defining an f-DP privacy filter under fully adaptive composition is fundamentally invalid, characterizes the conditions for its validity, and constructs an approximate Gaussian DP filter for subsampled Gaussian mechanisms with improved privacy bounds over Rényi DP at extreme sampling rates.


<details>
  <summary>Details</summary>
Motivation: Differential privacy under fully adaptive composition is challenging because the choice of mechanisms and their privacy parameters can depend on prior outputs. While privacy filters are used to enforce global privacy budgets, it is unclear whether f-DP, a powerful framework using trade-off functions, supports valid privacy filters in this adaptive setting.

Method: The authors analyze the failure of the natural f-DP filter, which composes trade-off functions and stops when a threshold is crossed. They derive necessary and sufficient conditions for its validity. They also prove a fully adaptive central limit theorem for f-DP and construct an approximate Gaussian DP filter for subsampled Gaussian mechanisms, particularly at small (q < 0.2) and large (q > 0.8) sampling rates.

Result: The natural f-DP filter is shown to be invalid in general, with failure analyzed through violations of stopping rule constraints. Conditions ensuring validity are established. The proposed approximate Gaussian DP filter yields tighter privacy bounds than Rényi DP-based filters at both low and high sampling rates, especially in adaptive compositions.

Conclusion: While the natural f-DP filter fails under full adaptivity, the paper provides theoretical conditions under which it can be valid. The constructed approximate filter using Gaussian DP and a central limit theorem approach offers improved privacy accounting for subsampled Gaussian mechanisms in practical, adaptive settings.
...
### [35] ["Tab, Tab, Bug'': Security Pitfalls of Next Edit Suggestions in AI-Integrated IDEs](https://arxiv.org/abs/2602.06759)
*Yunlong Lyu,Yixuan Tang,Peng Chen,Tian Dong,Xinyu Wang,Zhiqiang Dong,Hao Chen*

Main category: cs.CR

TL;DR: This paper presents the first systematic security study of Next Edit Suggestions (NES) in AI-integrated IDEs, uncovering new attack surfaces from expanded context retrieval, demonstrating vulnerabilities like context poisoning in lab evaluations, and revealing low security awareness among developers through a large-scale survey.


<details>
  <summary>Details</summary>
Motivation: Existing research overlooks the security implications of Next Edit Suggestions (NES), which extend beyond traditional code completion by leveraging rich contextual data, thereby introducing new attack vectors that remain unexplored.

Method: The study combines a technical dissection of NES mechanisms, an in-lab security evaluation focusing on context poisoning and sensitivity to edits/interactions, and a large-scale survey of over 200 professional developers.

Result: The analysis reveals that NES systems are vulnerable to context poisoning and are highly sensitive to transactional edits and user-IDE interactions; the survey shows most developers are unaware of these risks.

Conclusion: NES systems in AI-integrated IDEs introduce significant security risks due to expanded context retrieval and complex interactions, yet developer awareness is low, indicating an urgent need for improved security measures and education.
...
### [36] [Next-generation cyberattack detection with large language models: anomaly analysis across heterogeneous logs](https://arxiv.org/abs/2602.06777)
...
### [37] [Plato's Form: Toward Backdoor Defense-as-a-Service for LLMs with Prototype Representations](https://arxiv.org/abs/2602.06887)
*Chen Chen,Yuchen Sun,Jiaxin Gao,Yanwen Jia,Xueluan Gong,Qian Wang,Kwok-Yan Lam*

Main category: cs.CR

TL;DR: This paper proposes PROTOPURIFY, a reusable and minimal-assumption backdoor purification method for large language models that identifies and removes backdoor influences via prototype-based parameter editing, achieving high efficacy and low utility loss across diverse attacks and models.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor defenses are impractical for real-world deployment in BDaaS due to reliance on unrealistic side information (e.g., clean data, known triggers, or task-specific knowledge) and lack of scalability and reusability; PROTOPURIFY addresses these limitations by operating under minimal assumptions.

Method: PROTOPURIFY constructs a backdoor vector pool from clean and backdoored model pairs, aggregates these into candidate prototypes, and selects the most similar prototype to the target model via similarity matching. It identifies the boundary layer using layer-wise prototype alignment and performs targeted purification by suppressing prototype-aligned parameters in affected layers.

Result: PROTOPURIFY reduces attack success rates (ASR) to below 10%, as low as 1.6% in some cases, across single-trigger, multi-trigger, and triggerless backdoor attacks, while maintaining clean accuracy with less than a 3% drop. It outperforms six state-of-the-art defenses and shows robustness against adaptive attacks and stability on clean models.

Conclusion: The paper concludes that PROTOPURIFY is an effective, reusable, and scalable backdoor purification framework suitable for deployment as a Backdoor Defense-as-a-Service (BDaaS), offering strong performance across diverse LLMs and attack types with minimal impact on clean model utility.
...
### [38] [TamperBench: Systematically Stress-Testing LLM Safety Under Fine-Tuning and Tampering](https://arxiv.org/abs/2602.06911)
