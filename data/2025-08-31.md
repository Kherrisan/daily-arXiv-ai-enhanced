...
<details>
  <summary>Details</summary>
Motivation: Existing LLM watermarking methods like SynthID-Text are vulnerable to attacks that preserve semantic meaning (e.g., paraphrasing, back-translation), undermining watermark detectability. This limitation motivates the need for more robust solutions.

Method: The authors propose SynGuard, a hybrid framework integrating Semantic Information Retrieval (SIR) for semantic alignment with SynthID-Text's probabilistic watermarking. This dual-level (lexical + semantic) embedding enhances robustness against meaning-preserving attacks.

Result: SynGuard achieves an average 11.1% improvement in F1 score for watermark recovery across multiple attack scenarios compared to SynthID-Text, validating its effectiveness against real-world tampering.

Conclusion: The paper concludes that semantic-aware watermarking, as implemented in SynGuard, effectively resists real-world tampering by combining lexical and semantic-level watermark embedding, demonstrating significant improvements in watermark recovery.
...
<details>
  <summary>Details</summary>
Motivation: WRAs, deployed locally for privacy and legal purposes, exhibit distinguishable network behaviors (70-140 domain visits with timed correlations), creating vulnerabilities to passive adversary attacks that contradict their intended use.

Method: The study builds a WRA trace dataset using real and synthetic queries, introduces a behavioral metric (OBELS) to evaluate similarity between original and inferred prompts, and tests attacks under partial observability conditions. Mitigation strategies focus on domain diversity constraints and trace obfuscation.

Result: The attack recovers 73% of functional/domain knowledge from prompts (via OBELS), recovers 19/32 latent traits with high accuracy, remains effective under noise, and mitigations reduce attack success by 29% with negligible utility impact.

Conclusion: The paper demonstrates vulnerabilities in Web and Research Agents (WRAs) against inference attacks using network metadata, showing that deployed WRAs can be fingerprinted and user information can be leaked. Mitigations reduce attack effectiveness with minimal utility loss.
...
<details>
  <summary>Details</summary>
Motivation: The shift in cyber attack objectives from traditional goals to manipulating AI outputs, such as degrading model accuracy or flooding systems with false positives, necessitates a reevaluation of cybersecurity strategies in AI-integrated systems.

Method: The paper explores operational cybersecurity and supply chain risks throughout the AI lifecycle, highlighting past exploitations and offering insights derived from practical experience in the field.

Result: The study identifies novel cyber threats in AI systems and provides actionable insights to improve security measures, helping organizations safeguard AI infrastructure against evolving risks.

Conclusion: The paper emphasizes the necessity for tailored security frameworks to address emerging threats in AI-driven systems, urging organizations to enhance protection and ensure reliability through awareness of operational and supply chain risks.
...
<details>
  <summary>Details</summary>
Motivation: Medical AI development faces significant barriers due to strict patient privacy regulations and legal restrictions on data sharing. Centralized training on large-scale datasets is hindered by these constraints, limiting model scalability and real-time updates. This motivates the exploration of privacy-preserving frameworks like Federated Learning to address these challenges in a decentralized manner.

Method: The study employs a survey methodology, analyzing FL implementations across the medical imaging pipeline. FL is applied in two stages: (1) enabling joint training on diverse datasets for robust imaging reconstruction while preserving privacy, and (2) facilitating local fine-tuning for downstream clinical tasks without centralizing sensitive data. Innovations focus on communication efficiency, heterogeneous data alignment, and secure parameter aggregation.

Result: The analysis highlights FL's effectiveness in alleviating data scarcity and enabling collaborative training in medical imaging. Key results include successful applications of FL in CT/MRI reconstruction networks and downstream diagnostic tasks, with improvements in communication efficiency, data heterogeneity handling, and secure aggregation. The study also identifies gaps and future research directions.

Conclusion: Federated Learning (FL) presents a viable solution to the challenges of data privacy and centralization in medical AI, enabling collaborative model development across decentralized datasets. The paper emphasizes FL's role in both upstream imaging reconstruction and downstream clinical tasks, suggesting it serves as a strategic reference for advancing secure, scalable medical AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Approximate caching optimizes diffusion model computation but compromises user isolation, creating security risks. This work systematically evaluates these vulnerabilities, demonstrating real attacks.

Method: The paper employs three attack scenarios: (1) a remote covert channel via cache-injected keywords, (2) prompt stealing via cache hit detection, and (3) poisoning attacks embedding malicious logos into cached prompts. All attacks exploit cache isolation weaknesses.

Result: The presented attacks successfully exfiltrated hidden information, stole cached prompts, and injected malicious content into the cache, proving remote exploitability of isolation violations in approximate caching.

Conclusion: Approximate caching in diffusion models introduces severe security vulnerabilities, necessitating design reevaluation or isolation mechanisms to mitigate risks like remote covert channels, prompt stealing, and poisoning attacks.
...
<details>
  <summary>Details</summary>
Motivation: This work addresses the gap in understanding how malicious actors could weaponize large language models for autonomous, adaptive, and persistent ransomware attacks without traditional code dependencies.

Method: The authors utilize automated reasoning, contextual decision-making, and code synthesis via LLMs to create a ransomware prototype that dynamically generates polymorphic malicious code from natural language prompts. Attack efficacy is validated through phase-centric evaluation across diverse environments.

Result: Open-source LLMs successfully produce functional ransomware components and maintain closed-loop execution across personal, enterprise, and embedded systems. Behavioral analysis provides telemetry for defense strategy development.

Conclusion: The paper concludes that Ransomware 3.0 demonstrates the feasibility of LLM-driven autonomous ransomware, compelling the cybersecurity field to develop advanced defenses and policies against AI-enabled threats.
...
<details>
  <summary>Details</summary>
Motivation: The Internet of Energy (IoE) faces sophisticated cyber threats with significant public safety risks, necessitating resilient solutions. Traditional safeguards are insufficient to address adversarial attacks targeting interconnected critical infrastructure, motivating the need for robust, evolving security frameworks.

Method: The paper proposes a Graph Structure Learning (GSL)-based safeguards framework that jointly optimizes graph topology and node representations. This approach is designed to inherently resist adversarial network model manipulation through a conceptual overview, architectural discussion, and case study on a security dataset.

Result: The case study demonstrates the GSL framework's superior robustness compared to representative methods in securing IoE networks. This provides practitioners with an effective strategy to defend against adversarial attacks and maintain system reliability.

Conclusion: This work highlights the potential of Graph Structure Learning (GSL) to enhance the resilience and reliability of future Internet of Energy (IoE) networks. It also identifies key open challenges and proposes future research directions in this emerging field, offering practitioners a viable approach to secure critical infrastructure against evolving cyber threats.
...
<details>
  <summary>Details</summary>
Motivation: TEEs are critical for confidential computing, but their containers face usability and reliability challenges, necessitating accurate analysis of isolation strategies to prevent security risks.

Method: The authors designed an automated analyzer to identify and evaluate the isolation boundaries of TEE containers, focusing on their security mechanisms.

Result: Existing TEE containers exhibit critical flaws like information leakage, rollback attacks, and denial-of-service vulnerabilities, undermining their security guarantees.

Conclusion: The study provides key lessons for secure TEE container development and discusses emerging design trends to enhance system security.
...
<details>
  <summary>Details</summary>
Motivation: Cross-chain bridges are vulnerable due to design flaws and the inadequacy of existing detection methods that fail to address cross-chain semantics.

Method: Proposes BridgeShield, a detection framework using heterogeneous graph attention networks with intra-meta-path and inter-meta-path attention to model cross-chain behaviors.

Result: Achieved an average F1-score of 92.58% on 51 real-world attacks, a 24.39% improvement over state-of-the-art baselines.

Conclusion: BridgeShield effectively secures cross-chain bridges by capturing cross-chain semantics through heterogeneous graph attention networks, enhancing multi-chain ecosystem resilience.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of establishing a trusted, tamper-evident monetary standard between Earth and Mars given high-latency, intermittently connected interplanetary communication channels.

Method: The authors propose Proof-of-Transit Timestamping (PoTT), integrating Delay/Disruption-Tolerant Networking (DTN), optical LEO mesh constellations, header-first replication, and federated/blind-merge-mined commit chains to address communication constraints and enable secure cross-planetary transactions.

Result: PoTT formalization demonstrates improved reliability and accountability in Bitcoin data propagation across planetary networks, with feasible deployment strategies ranging from near-term federations to long-term blind-merge-mined commit chains while maintaining the Earth L1 monetary base.

Conclusion: The proposed architecture enables Bitcoin's deployment in interplanetary contexts with minimal alterations to its core protocol, ensuring monetary continuity and security through PoTT and supporting technologies like DTNs and federated sidechains.
...
<details>
  <summary>Details</summary>
Motivation: Current cybersecurity research focuses heavily on offensive LLM applications (e.g., red-team operations), while defensive uses like forensics and incident response remain underexplored. The paper aims to address this gap by systematically studying LLM-agent design for forensic investigations.

Method: The authors propose CyberSleuth, an autonomous agent that analyzes packet-level traces and application logs to identify targeted services, exploited vulnerabilities (CVEs), and attack outcomes. They evaluate four agent architectures and six LLM backends across 2025 incident scenarios, comparing performance and interpretability.

Result: CyberSleuth achieves 80% accuracy in identifying exact CVEs across 2025 scenarios and is rated highly by 22 forensic experts for report quality and completeness. It outperforms other agent architectures and shows a slight preference for the open-source DeepSeek R1 model.

Conclusion: The study concludes that CyberSleuth is a robust and effective LLM agent for forensic investigations in cybersecurity. It demonstrates that open-source models like DeepSeek R1 can perform competitively, and the release of the benchmark and platform promotes reproducibility and further research in defensive LLM applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluation methods for detecting successful jailbreak attempts are inconsistent, subjective, and misaligned with human perception, leading to unreliable assessments of model responses.

Method: JADES employs a framework that automatically decomposes input harmful questions into weighted sub-questions, scores individual sub-answers, and aggregates scores using a weighted method. It also includes an optional fact-checking module to identify hallucinations in responses.

Result: JADES achieved 98.5% agreement with human evaluators in binary success/failure settings, outperforming previous methods by over 9%. Re-evaluations revealed significant overestimations in attack success rates (e.g., 93% → 69% for LAA on GPT-3.5-Turbo).

Conclusion: JADES is a reliable and accurate framework for evaluating jailbreak attempts, offering a robust and interpretable solution to a previously unresolved challenge in automated detection.
...
<details>
  <summary>Details</summary>
Motivation: LLMs are increasingly used in scientific peer-review, but their susceptibility to manipulation via hidden adversarial prompts in PDFs could compromise review reliability and fairness.

Method: The researchers formalized three threat models, designed adversarial prompts imperceptible to humans, evaluated their effectiveness across multiple LLM systems and papers, and tested countermeasures to reduce prompt detectability.

Result: Adversarial prompts reliably influenced LLM reviews, sometimes harming "honest-but-lazy" reviewers. Countermeasures showed mixed success in reducing adversarial prompt detectability.

Conclusion: The study highlights the vulnerabilities in LLM-based peer-review systems to adversarial prompts and emphasizes the need for countermeasures to detect or mitigate such attacks, ensuring the integrity of automated review processes.
...
<details>
  <summary>Details</summary>
Motivation: Traditional vulnerability detection methods face scalability and accuracy challenges, while AI approaches are hindered by inadequate training data. This paper aims to overcome these limitations by generating realistic, category-specific vulnerabilities in codebases for improved dataset creation.

Method: The framework utilizes a coordinated system of multiple AI agents, function agents, and code analysis tools. It integrates Retrieval-Augmented Generation for context-aware vulnerability injection and employs Low-Rank approximation for efficient model fine-tuning, enabling realistic and scalable dataset generation.

Result: The framework achieved 89%-95% success rates in injecting vulnerabilities across 116 code samples from three benchmarks, demonstrating superior dataset accuracy compared to existing techniques.

Conclusion: The proposed framework addresses the limitations of traditional methods by generating high-quality datasets through AI agents and advanced techniques, thereby enhancing the effectiveness of AI-driven vulnerability detection systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmark datasets inadequately capture the evolving nature of prompt injection attacks (paraphrased inputs, obfuscation, multi-task strategies), and current defense mechanisms fail to generalize against these advanced threats, creating a critical need for more comprehensive evaluation frameworks and robust solutions.

Method: The authors (1) constructed a new benchmark by extending prior datasets with novel manipulation techniques and multi-task scenarios, and (2) developed PromptSleuth, a semantic-oriented defense framework that detects injections by analyzing task-level intent through semantic reasoning rather than relying on superficial features.

Result: The new benchmark reveals significant weaknesses in existing defenses that appear effective on older datasets. PromptSleuth consistently outperforms state-of-the-art methods across multiple benchmarks while maintaining runtime and cost efficiency, demonstrating the effectiveness of intent-based semantic analysis.

Conclusion: The study concludes that intent-based semantic reasoning through PromptSleuth provides a robust, efficient, and generalizable defense strategy against evolving prompt injection threats, highlighting the importance of addressing the adversary's invariant intent rather than surface-level attack variations.
...
<details>
  <summary>Details</summary>
Motivation: LLMs are expected to generate real-world microservice code, but their current capabilities have limits, especially in complexity and integration.

Method: The researchers created a microservice specification template with a difficulty metric, a framework to test LLM-synthesized code using unit testing, then evaluated LLMs on tasks at varying difficulty levels.

Result: Strong LLMs like GPT-3o-mini achieved good results on medium tasks but failed in high difficulty cases, due to challenges like complex business logic and external service integration.

Conclusion: LLMs currently struggle with hard microservice code generation. By identifying specific LLM weaknesses in these tasks, the paper proposes future research areas to enhance code generation for real-world problems.
...
<details>
  <summary>Details</summary>
Motivation: Code generation models currently struggle with runtime efficiency limitations, hindering their practical application in performance-critical scenarios despite advances in code generation capabilities.

Method: An efficiency-oriented reinforcement learning framework incorporating three key techniques: (1) dynamic exploration for efficient implementations, (2) error-insensitive reinforcement learning with high-contrast efficiency signals, and (3) online exploration starting from high-correctness baselines. A two-stage tuning method integrates these approaches.

Result: Achieved 10.18% improvement in code correctness and 7.75% improvement in runtime efficiency on a 7B model, matching performance levels typically requiring much larger models.

Conclusion: The proposed method effectively balances code correctness and runtime efficiency, making it more practical for performance-sensitive applications with results comparable to larger models.
...
<details>
  <summary>Details</summary>
Motivation: The integration of generative AI into agile software development faces significant challenges including tool fragmentation, governance issues, data quality concerns, and critical skills gaps in AI literacy and prompt engineering.

Method: The analysis was conducted through a full-day interdisciplinary workshop featuring structured interactive breakout sessions with over 30 academic and industry participants, enabling collective identification and analysis of challenges/opportunities.

Result: Participants identified concrete pain points and underlying causes, culminating in a prioritized research roadmap with implementable actions and visionary directions for GenAI-agile integration.

Conclusion: The workshop's collaborative creation of a multi-thematic research roadmap provides a cohesive agenda to guide future investigations and promote responsible, human-centered integration of GenAI into agile software development practices.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM applications exhibiting non-determinism, dynamism, and context dependence outgrow traditional software testing paradigms, requiring quality assurance approaches that reconcile software engineering and AI safety analysis methods.

Method: Decomposed LLM applications into three layers (System Shell, Prompt Orchestration, LLM Inference Core); evaluated traditional software testing methods in each layer; compared software engineering testing with AI safety analysis; proposed four collaborative strategies (Retain, Translate, Integrate, Runtime) and a closed-loop QA framework with AICL protocol.

Result: Identified four fundamental differences between testing approaches leading to six core challenges; developed collaborative strategies addressing these gaps; implemented AICL protocol with test-oriented features for inter-agent communication.

Conclusion: This paper proposes collaborative strategies and a closed-loop framework to standardize LLM application testing, bridging the gap between software engineering practices and AI safety analysis through three-layer architectural decomposition and the AICL communication protocol.
...
<details>
  <summary>Details</summary>
Motivation: Legal compliance artifact creation is manual, labor-intensive, and error-prone; GenAI offers automation to translate technology-neutral legal texts into developer-friendly behavioral specifications.

Method: A quasi-experimental study with 10 participants evaluating 60 Gherkin specifications (from food-safety regulations via Claude and Llama LLMs) using five criteria (Relevance, Clarity, Completeness, Singularity, Time Savings) with 120 total assessments.

Result: 75-90% of specifications scored highest across metrics (Clarity: 90%), no lowest ratings. Llama outperformed Claude in Clarity/Completeness/Time Savings; Claude in Singularity. Feedback highlighted utility despite hallucinations/omissions.
...
<details>
  <summary>Details</summary>
Motivation: Architects lack structured guidance to address sustainability in software design phases, despite its growing importance as an emergent quality property.

Method: Formulated a sustainability perspective using snowballing on seminal literature and expert feedback from a focus group, ensuring alignment with industrial needs.

Result: Evidence confirmed the relevance of perspective elements in practice, providing actionable insights for developing sustainability-focused architectural knowledge artifacts.

Conclusion: The paper contributes a vision for integrating sustainability into software architecture through a structured perspective, validated expert insights, and literature synthesis, offering practical implications for industry adoption.
...
<details>
  <summary>Details</summary>
Motivation: CPS simulation-based testing is costly, time-consuming, and prone to flaky outcomes, necessitating interpretable, execution-free test oracles to reduce cost and improve reliability.

Method: The paper proposes assertion-based test oracles as logical predicates, generated via two methods: (1) Genetic Programming (GP) using SBFL rankings (Ochiai, Tarantula, Naish) as fitness functions, and (2) Decision Trees (DT) and Decision Rules (DR).

Result: GP-generated oracles using Ochiai SBFL achieved significantly higher accuracy than Tarantula/Naish-based GP, DT, or DR, with 4% average accuracy variation despite flaky system behavior in aerospace, networking, and autonomous driving domains.

Conclusion: The study concludes that GP with Ochiai-based assertion oracles is the most accurate and robust method for test oracle generation in flaky CPS simulators, outperforming other GP-based approaches and DT/DR methods across various domains.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for concurrency bug detection suffer from lack of datasets, insufficient concurrency semantics representation, and inability to localize bugs in source code.

Method: A dedicated concurrency bug dataset was constructed, combined with a pre-trained model and heterogeneous graph neural network (GNN) that incorporates a Concurrency-Aware Code Property Graph (CCPG). SubgraphX interpretability is employed for precise bug localization.

Result: Achieved 10% improvement in accuracy/precision and 26% improvement in recall compared to prior methods across diverse evaluation settings.

Conclusion: The proposed method effectively detects and localizes concurrency bugs with substantial improvements in accuracy, precision, and recall over state-of-the-art approaches, while providing interpretable debug information.
...
<details>
  <summary>Details</summary>
Motivation: Modern configurable systems face misconfigurations and latent bugs due to complex configuration spaces, and existing methods lack sufficient log information for effective diagnosis.

Method: ConfLogger combines configuration-aware static taint analysis to identify sensitive code segments and LLM-based log generation to analyze configuration contexts.

Result: ConfLogger achieves 100% error localization accuracy in 30 silent misconfigurations, 80% direct resolution with configuration info, 74% log coverage surpassing baselines by 12-30%, and a 251.4% improvement in troubleshooting accuracy via user studies.

Conclusion: ConfLogger demonstrates superior effectiveness in enhancing configuration diagnosability through targeted logging, achieving higher accuracy, coverage, and user efficiency compared to existing methods.
...
<details>
  <summary>Details</summary>
Motivation: The paper aims to address gender biases in software engineering by examining its origins, professionalism, and authorship patterns.

Method: The study profiles historical leaders in software engineering, surveys the field's attention to gender issues, and quantitatively analyzes authorship data from 1976–2010.

Result: Analysis identified 12 years of statistically significant gender exclusion in authorship at the International Conference of Software Engineering.

Conclusion: The paper suggests policy dimensions to address gender bias in computing and highlights historical periods of exclusion.
