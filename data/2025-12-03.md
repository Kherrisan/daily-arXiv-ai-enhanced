...
TL;DR: The paper introduces the Superpixel Attack, a new attack method for generating black-box adversarial attacks. It uses superpixels to achieve an improved attack success rate.


<details>
  <summary>Details</summary>
Motivation: Current black-box adversarial attacks use simple rectangular regions to change perturbations in a single iteration. These shapes are not effective as they may include irrelevant content with respect to the target class. The paper proposes to use more effective superpixels as image regions for adversarial perturbation changes to improve attack success rates.

Method: The paper uses superpixels to replace the rectangular regions used in existing attacks. Superpixel regions are better balanced in terms of color variance and compactness, which allows more relevant features to be focused on. The authors also propose a new search method, versatile search, and a novel attack method, Superpixel Attack, which applies superpixels and performs versatile search.

Result: The Superpixel Attack improves attack success rates by an average of 2.10% compared to existing attacks. The results are significant for black-box adversarial attacks. The code is available at https://github.com/oe1307/SuperpixelAttack.git.

Conclusion: The proposed Superpixel Attack achieves a significant improvement in attack success rates compared to existing methods. The use of superpixels and the new versatile search method provides a more effective way of generating black-box adversarial attacks, which can help to develop more effective defenses. The code is publicly available for use and further research.
...
TL;DR: LLMBugScanner uses fine-tuning and ensemble learning to enhance LLMs for robust smart contract vulnerability detection.


<details>
  <summary>Details</summary>
Motivation: Smart contract auditing faces challenges where individual LLMs exhibit inconsistent performance across vulnerability types and contract structures, even after fine-tuning.

Method: The framework combines domain knowledge adaptation through parameter-efficient fine-tuning on complementary datasets with ensemble learning that leverages multiple LLMs and a consensus-based conflict resolution strategy.

Result: Extensive experiments show LLMBugScanner achieves consistent accuracy improvements and stronger generalization compared to both pretrained and fine-tuned individual models across multiple LLMs.

Conclusion: LLMBugScanner provides a principled, cost-effective, and extensible framework for smart contract vulnerability detection, demonstrating consistent accuracy improvements and strong generalization capabilities.
...
TL;DR: This paper proposes three Ascon-based DRBG constructions for embedded systems, demonstrating improved efficiency and reduced memory footprint compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Security and performance are critical for DRBGs in embedded systems, which have limited computational and memory resources. Existing solutions may not meet these requirements, necessitating optimized lightweight cryptographic alternatives like those enabled by the NIST-backed Ascon standard.

Method: The paper revises the DRBG standard and proposes three Ascon-based constructions: Hash-Based DRBG, HMAC DRBG, and CTR DRBG. These were implemented and tested on a Raspberry Pi platform.

Result: Experimental results on Raspberry Pi showed the proposed methods outperformed existing DRBG implementations in computational efficiency and memory usage while maintaining security.

Conclusion: The proposed Ascon-driven DRBG constructions offer higher computational efficiency and lower memory usage compared to existing methods, making them suitable for resource-constrained embedded systems.
...
TL;DR: This survey examines the Hidden Subgroup Problem’s role in cryptography, comparing quantum solutions for abelian and unsolved non-abelian cases with implications for post-quantum security.


<details>
  <summary>Details</summary>
Motivation: Understanding HSP is crucial for assessing the security of public-key cryptosystems against quantum attacks.

Method: The paper employs mathematical analysis and surveys existing approaches, focusing on Fourier sampling and black-box techniques for non-abelian HSP.

Result: The paper provides a comprehensive overview of abelian and non-abelian HSP, linking these problems to cryptographic challenges like shortest vector and graph isomorphism.

Conclusion: The paper emphasizes the importance of solving the non-abelian HSP for advancing quantum-safe cryptography, highlighting gaps in current methodologies.
...
TL;DR: The paper introduces control protocols based on factored cognition for improving safety with untrusted AI. Key findings include 41%-63% safety gains in monitoring and no increase in backdoor vulnerability, though usefulness remains low in current tasks.


<details>
  <summary>Details</summary>
Motivation: Existing AI control protocols using weak monitors fail on complex tasks. Need for robust safeguards against intentionally subversive AI systems.

Method: Developed factored cognition protocols where trusted models decompose tasks, untrusted models solve subtasks, and solutions are reassembled. Tested in APPS coding environment with adversarial GPT-4.1 Nano attacks.

Result: 1) 22pp safety improvement via factored cognition. 2) Monitor performance enhanced. 3 No added difficulty in writing backdoors. Protocols show low APPS usefulness but potential for complex tasks.

Conclusion: Factored cognition protocols increase monitoring effectiveness without enabling new attacks, suggesting practical safety gains for complex AI systems despite current task limitations.
...
TL;DR: PhishSnap uses perceptual hashing for on-device phishing detection, achieving 0.79 accuracy with privacy-preserving local comparison. Dataset size and phishing page unavailability posed challenges.


<details>
  <summary>Details</summary>
Motivation: Existing URL/HTML-based systems fail against obfuscated, visually-similar phishing sites. Phishing page take-downs reduce dataset diversity.

Method: Browser extension captures webpage screenshots, computes perceptual hashes, and compares to legitimate templates locally. Dataset: 10,000 URLs (70-20-10 split) from PhishTank/Netcraft, with limited phishing page availability.

Result: 0.79 accuracy, 0.76 precision, 0.78 recall. Local inference ensures privacy and low latency despite dataset completeness issues.

Conclusion: Visual similarity detection remains viable for phishing defense when implemented with privacy focus, though dataset limitations require attention for robustness.
...
TL;DR: The paper analyzes the Apache ecosystem's security vulnerabilities, examining recurring issues, disclosure timelines, and remediation practices to improve security measures.


<details>
  <summary>Details</summary>
Motivation: The Apache Software Foundation is central to modern software infrastructure, yet its projects face persistent security vulnerabilities. This study aims to understand the patterns and improve secure practices.

Method: The authors compiled a dataset of CVEs and CWEs in the Apache ecosystem, analyzed trends in exploit recurrence, and examined disclosure and remediation timelines using four research questions as a framework.

Result: The study found recurring CWEs in Apache libraries, long persistence times for CVEs, delays in disclosure, and varied remediation times, providing a curated dataset with insights into vulnerability lifecycles.

Conclusion: The research highlights the need for improved secure coding, monitoring, and remediation strategies in the Apache ecosystem, offering actionable recommendations for developers.
...
TL;DR: A method is developed for falsifying safety properties in robotic vehicles by using property-guided reduction and surrogate models, which simplifies testing and enhances scalability.


<details>
  <summary>Details</summary>
Motivation: The motivation lies in the need for scalable and efficient methods to verify safety properties in complex robotic systems, where traditional approaches can be resource-intensive.

Method: The method involves isolating relevant control logic and physical dynamics via property-guided reduction to construct surrogate models. These models allow for efficient falsification through trace analysis and temporal logic oracles.

Result: The approach was tested on a drone control system with a known flaw, where the surrogate models effectively replicated failure conditions at lower simulation costs and efficiently identified violations.

Conclusion: The conclusion is that combining controller reduction with logic-aware test generation offers a practical, scalable solution for semantic verification in cyber-physical systems.
...
TL;DR: Belobog is the first type-aware fuzzer for Move smart contracts that overcomes type system limitations, achieving comprehensive vulnerability detection through type graphs and concolic execution, validated against real-world exploits.


<details>
  <summary>Details</summary>
Motivation: Existing smart contract fuzzers are ineffective for Move due to its strong type system, necessitating a new framework to validate Move smart contracts and uncover vulnerabilities beyond type system scope.

Method: Belobog constructs a type graph based on Move's type system to generate well-typed transactions and employs a concolic executor to handle Move's complex checks during fuzzing.

Result: Belobog detected 100% of critical and 79% of major vulnerabilities in 109 real-world projects, successfully reproducing two notorious exploits (Cetus and Nemo) without prior knowledge.

Conclusion: Belobog significantly enhances the analysis of Move smart contracts by providing a type-aware fuzzing framework that addresses the limitations of existing tools, enabling comprehensive vulnerability detection with proven effectiveness in real-world scenarios.
...
### [10] [HOT Protocol](https://arxiv.org/abs/2512.02287)
*Peter Volnov,Georgii Kuksa,Andrey Zhevlakov*

Main category: cs.CR

TL;DR: HOT Protocol uses MPC and TEEs to let smart contracts securely manage private keys across multiple blockchains with improved security and lower costs.


<details>
  <summary>Details</summary>
Motivation: Current blockchain systems lack secure and economically feasible methods for smart contracts to own and manage private keys. The HOT Protocol addresses this by combining MPC and TEEs to mitigate vulnerabilities while reducing costs.

Method: The protocol employs a Multi-Party Computation (MPC) Network managed via a Trusted Execution Environment (TEE) to secure signing keys. It integrates with the NEAR Protocol’s decentralized state layer and is designed for cross-chain compatibility with Stellar, TON, Solana, and EVM-compatible networks.

Result: The protocol achieves stronger security guarantees through TEE integration and MPC, ensures cross-chain operability, and lowers participation costs. It enables secure, decentralized key management for smart contracts on diverse blockchain ecosystems.

Conclusion: The HOT Protocol enhances the security and accessibility of private key management for smart contracts across multiple blockchain networks, offering a robust solution with reduced economic burdens.
...
### [11] [Quantum Vanguard: Server Optimized Privacy Fortified Federated Intelligence for Future Vehicles](https://arxiv.org/abs/2512.02301)
*Dev Gurung,Shiva Raj Pokhrel*

Main category: cs.CR

TL;DR: TL;DR: This paper introduces vQFL, a quantum federated learning framework for autonomous vehicles that enhances privacy and security against classical and quantum threats without compromising model accuracy.


<details>
  <summary>Details</summary>
Motivation: Motivation: With the increasing data volume in autonomous networks and the threat of quantum computing to current security mechanisms, there's a need for secure, efficient, and privacy-preserving learning systems.

Method: Method: vQFL combines quantum federated learning, differential privacy, and quantum key distribution (QKD). ft-VQFL, a server-side fine-tuning method, is introduced for resilience. The framework is implemented with VQC, QCNN, and SamplerQNN.

Result: Results: Experiments on KITTI, Waymo, and nuScenes show vQFL matches standard QFL accuracy while improving privacy guarantees and security, with minimal performance overhead.

Conclusion: Conclusion: vQFL's modular design provides a crucial foundation for quantum-resistant autonomous vehicles, suitable for post-quantum infrastructure while handling high data volumes.
...
### [12] [COGNITION: From Evaluation to Defense against Multimodal LLM CAPTCHA Solvers](https://arxiv.org/abs/2512.02318)
*Junyu Wang,Changjia Zhu,Yuanbo Zhou,Lingyao Li,Xu He,Junjie Xiong*

Main category: cs.CR

TL;DR: LLMs can efficiently solve many CAPTCHA types at low cost, but spatially complex tasks remain resistant. Their insights provide guidelines for secure CAPTCHA design.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs pose a growing threat to CAPTCHA security by enabling cost-effective, automated CAPTCHA solving. Understanding their attack potential and limitations is critical for designing robust CAPTCHA defenses.

Method: Evaluated 7 MLLMs on 18 CAPTCHA types, measuring accuracy, latency, cost, and the effectiveness of prompt engineering. Analyzed reasoning traces to identify success/failure factors and derive defense strategies.

Result: LLMs achieved human-like performance on recognition-oriented and low-interaction CAPTCHA tasks at low cost and latency, but struggled with tasks requiring fine-grained localization, spatial reasoning, or cross-frame consistency. Model behaviors were analyzed to link capabilities to task structures.

Conclusion: The study highlights that while MLLMs can compromise visual CAPTCHA security in recognition and low-interaction tasks, challenges remain in tasks requiring fine-grained localization or multi-step reasoning, offering actionable guidelines for CAPTCHA design and deployment.
...
### [13] [LeechHijack: Covert Computational Resource Exploitation in Intelligent Agent Systems](https://arxiv.org/abs/2512.02321)
*Yuanhe Zhang,Weiliu Wang,Zhenhong Zhou,Kun Wang,Jie Zhang,Li Sun,Yang Liu,Sen Su*

Main category: cs.CR

TL;DR: The paper introduces LeechHijack, a new attack vector in LLM-based agent systems using the Model Context Protocol (MCP), where a malicious tool covertly exploits the agent's computational resources by embedding a backdoor, highlighting the need for better security mechanisms in open ecosystems.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the growing use and openness of MCP in LLM-based agents, which introduce security risks due to implicit trust in third-party tools. The research addresses the need to uncover and mitigate vulnerabilities arising from this trust model to ensure the safety of computational resources in these systems.

Method: The authors propose LeechHijack, a two-stage attack method: an implantation stage that embeds a backdoor in a tool, and an exploitation stage that triggers the backdoor to establish a command-and-control channel, allowing unauthorized tasks to be executed using the agent's resources.

Result: The results demonstrate that LeechHijack is effective across four major LLM families, achieving an average success rate of 77.25% with an 18.62% resource overhead, thus validating the feasibility and efficiency of the attack against current MCP-based systems.

Conclusion: The study concludes that the identified security vulnerabilities in the MCP framework underscore the necessity for computational provenance and resource attestation mechanisms to protect the developing ecosystem, emphasizing the importance of proactive security measures in open and composable systems.
...
### [14] [AtomGraph: Tackling Atomicity Violation in Smart Contracts using Multimodal GCNs](https://arxiv.org/abs/2512.02399)
*Xiaoqi Li,Zongwei Li,Wenkai Li,Zeng Zhang,Lei Xie*

Main category: cs.CR

TL;DR: AtomGraph is a new framework that uses GCN for smart contract atomicity violation detection through multimodal feature learning and fusion.


<details>
  <summary>Details</summary>
Motivation: Atomicity violations in smart contracts pose a significant security risk, and current tools lack the necessary precision to detect these issues effectively.

Method: AtomGraph employs a collaborative learning mechanism with GCN to integrate structural topological features from the contract's Control Flow Graph (CFG) and deep semantics from its opcode sequence, using an adaptive weighted fusion method to combine them efficiently.

Result: Experiments show AtomGraph achieves 96.88% accuracy and 96.97% F1 score, outperforming existing tools by 6.4% in F1 score when compared to concatenation fusion models.

Conclusion: AtomGraph demonstrates high effectiveness in detecting atomicity violations in smart contracts, offering a significant improvement over current tools through its adaptive multimodal fusion and GCN-based approach.
...
### [15] [Characterizing Cyber Attacks against Space Infrastructures with Missing Data: Framework and Case Study](https://arxiv.org/abs/2512.02414)
...
### [16] [Leveraging Large Language Models to Bridge On-chain and Off-chain Transparency in Stablecoins](https://arxiv.org/abs/2512.02418)
*Yuexin Xiang,Yuchen Lei,SM Mahir Shazeed Rish,Yuanzhe Zhang,Qin Wang,Tsz Hon Yuen,Jiangshan Yu*

Main category: cs.CR

TL;DR: This paper proposes an LLM-based framework to unify on-chain and off-chain data for stablecoin transparency analysis, revealing systematic discrepancies between disclosed and verifiable data through cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Stablecoin transparency is fragmented between verifiable on-chain data and unstructured off-chain disclosures, hindering auditable stability verification.

Method: The framework combines LLM-driven document parsing, semantic alignment of financial indicators, and a Model Context Protocol (MCP), enabling cross-modal analysis of multi-chain issuance records and disclosure narratives.

Result: Quantified discrepancies between reported and observed stablecoin circulation across chains, demonstrating LLMs enhance cross-modal transparency and automate DeFi auditing.

Conclusion: LLM-powered integration of heterogeneous stablecoin data sources establishes a standardized approach for systematic transparency analysis and automated cross-chain auditing in DeFi.
...
### [17] [Detection of Crowdsourcing Cryptocurrency Laundering via Multi-Task Collaboration](https://arxiv.org/abs/2512.02534)
*Guang Li,Litong Sun,Jieying Zhou,Weigang Wu*

Main category: cs.CR

TL;DR: This paper addresses the challenge of detecting crowdsourcing laundering in stablecoins (e.g., USDT) through a novel framework called MCCLD, which leverages transaction groups and multi-task graph neural networks. The framework achieves enhanced detection performance by jointly optimizing laundering transaction and group detection tasks.


<details>
  <summary>Details</summary>
Motivation: The emerging crowdsourcing laundering method, characterized by decentralized transaction patterns and refined labor division, exploits stablecoin anonymity and stability. Existing detection methods struggle to address its diverse patterns and polycentric structure.

Method: The proposed MCCLD framework uses transaction groups as auxiliary information, employing an end-to-end graph neural network. It collaborates laundering transaction detection and group detection tasks via a shared classifier and feature encoder that fuses multi-level embeddings (e.g., transaction semantics and group information).

Result: Extensive experiments demonstrate MCCLD's effectiveness in detecting both crowdsourcing and general laundering scenarios, achieving strong performance and generalization. It is the first work specifically targeting crowdsourcing laundering detection.

Conclusion: MCCLD provides a robust solution for detecting complex crowdsourcing laundering patterns in stablecoin transactions through multi-task collaborative learning. The framework's end-to-end graph neural network approach advances the state of the art in decentralized financial crime detection.
...
TL;DR: A paper presents a new public-key crypto algorithm using subset element counting in a multiset, claiming speed, device compatibility, and quantum resistance.


<details>
  <summary>Details</summary>
Motivation: Traditional public-key cryptography becomes computationally heavy with advancements in the internet and quantum computing, necessitating more efficient and quantum-resistant algorithms.

Method: The algorithm creates ciphertexts by counting elements in subsets randomly drawn from a multiset, which is then refined using satisfiability problems for cryptographic strength.

Result: The algorithm is significantly faster, works on any device without needing large numbers, and allows a single public key to work with multiple private keys.

Conclusion: The proposed algorithm offers a promising alternative to existing public-key methods by being fast, versatile across devices, and resistant to quantum computing threats.
...
### [19] [S3C2 SICP Summit 2025-06: Vulnerability Response Summit](https://arxiv.org/abs/2512.02600)
...
TL;DR: A Vulnerability Response Summit summarized in this paper gathers industry practitioners to address software supply chain security challenges and foster collaboration through panel discussions on vulnerability reporting, tools, regulatory compliance, and bug bounties.


<details>
  <summary>Details</summary>
Motivation: Increased cyber attacks on software supply chains necessitate enhanced security measures, prompting government and industry collaboration to address vulnerabilities and regulatory compliance (CRA/NIS2).

Method: Conducted a summit with 9 practitioners from diverse companies, using five panel discussions based on open-ended questions about vulnerability management practices, tools, and organizational strategies.

Result: Identified common challenges in supply chain security (e.g., vulnerability response, compliance) and shared practical experiences, paving the way for new industry collaborations.

Conclusion: The summit highlighted critical challenges in software supply chain security and underscored the importance of cross-industry knowledge sharing and collaboration to strengthen resilience against cyber threats.
...
### [20] [Semigroup action based on skew polynomial evaluation with applications to Cryptography](https://arxiv.org/abs/2512.02603)
*Daniel Camazón-Portela,Juan Antonio López-Ramos*

Main category: cs.CR

TL;DR: The paper introduces an action of the skew polynomial ring over a finite field, using polynomial valuation and left skew product, to construct a public key exchange protocol secure in the Canetti and Krawczyk model.


<details>
  <summary>Details</summary>
Motivation: The need for secure cryptographic protocols that are resistant to attacks and can be based on non-commutative algebraic structures is the motivation behind this research.

Method: The method involves defining an action of the skew polynomial ring over a finite field using polynomial valuation and the left skew product of functions. A subset of the ring is constructed to manage the non-commutativity, which is then used to build a public key exchange protocol.

Result: The result is a public key exchange protocol that is secure within the Canetti and Krawczyk model, derived from the non-commutative structure of the skew polynomial ring.

Conclusion: The paper demonstrates that the non-commutative properties of skew polynomial rings can be effectively utilized to create secure cryptographic protocols, specifically a public key exchange protocol that meets the security requirements of the Canetti and Krawczyk model.
...
### [21] [CryptoQA: A Large-scale Question-answering Dataset for AI-assisted Cryptography](https://arxiv.org/abs/2512.02625)
*Mayar Elfares,Pascal Reisert,Tilman Dietz,Manpa Barman,Ahmed Zaki,Ralf Küsters,Andreas Bulling*

Main category: cs.CR

TL;DR: TLDR; A new QA dataset, named CryptoQA, was created to evaluate and enhance LLMs cryptographic reasoning abilities.


<details>
  <summary>Details</summary>
Motivation: The motivation stemmed from inadequate understanding of LLMs in cryptography, due to lack of proper data for assessment and training.

Method: The method involved creating CryptoQA, a dataset with over 2 million QA pairs from academic sources, and benchmarking 15 LLMs on it.

Result: The result revealed poor LLM performance in cryptographic tasks, and demonstrated improvement through dataset-based fine-tuning.

Conclusion: The conclusion emphasizes the necessity of specialized LLM assistants for cryptography, as evidenced by the dataset's findings.
...
### [22] [Bin2Vec: Interpretable and Auditable Multi-View Binary Analysis for Code Plagiarism Detection](https://arxiv.org/abs/2512.02197)
*Moussa Moussaoui,Tarik Houichime,Abdelalim Sadiq*

Main category: cs.SE

TL;DR: Bin2Vec merges static program structure and runtime behavior into interpretable features, enabling reliable similarity comparisons with ML integration through visualized multi-view analysis.


<details>
  <summary>Details</summary>
Motivation: Existing tools analyze binaries using isolated, single-type features (static or dynamic), missing the holistic context needed for accurate program similarity assessment in security and reverse-engineering tasks.

Method: Bin2Vec integrates static features (built-in functions, imports/exports) with dynamic features (execution traces, memory patterns) into separate 'views' visualized via charts. These are fused into a unified similarity score using ML-compatible feature representations.

Result: Experiments on PuTTY and 7-Zip showed Bin2Vec effectively distinguishes version-specific traits (e.g., PuTTY's complex behavior vs. 7-Zip's performance-driven patterns), validating its ability to generate optimal, visualization-friendly representations.

Conclusion: Bin2Vec offers a reliable and explainable framework for software comparison by combining static and dynamic analysis, enabling applications in cybersecurity and reverse-engineering due to its modularity and extensibility.
...
### [23] [Towards autonomous normative multi-agent systems for Human-AI software engineering teams](https://arxiv.org/abs/2512.02329)
*Hoa Khanh Dam,Geeta Mahala,Rashina Hoda,Xi Zheng,Cristina Conati*

Main category: cs.SE

TL;DR: This paper proposes a future where AI agents, powered by Large Language Models, take on primary roles in software engineering by performing core development tasks such as design, implementation, testing, and deployment with enhanced speed, reliability, and adaptability. These agents work alongside humans and are governed by norms to ensure proper interactions and compliance.


<details>
  <summary>Details</summary>
Motivation: The authors aim to address the limitations of current software development processes by leveraging the full potential of AI through autonomous agents, envisioning a more efficient and scalable collaborative system for the future.

Method: The paper introduces a class of software engineering agents equipped with beliefs, desires, intentions, and memory, enabling human-like reasoning. These agents collaborate using deontic modalities as regulatory norms to manage interactions and ensure compliance.

Result: The proposed framework demonstrates the potential to redefine software engineering practices by achieving a higher level of speed, reliability, and adaptability compared to traditional methods.

Conclusion: The innovations presented establish a scalable, transparent, and trustworthy framework for future Human-AI teams in software engineering, suggesting significant advancements in the field by utilizing AI in this autonomous capacity.
...
### [24] [Process-Centric Analysis of Agentic Software Systems](https://arxiv.org/abs/2512.02393)
*Shuyang Liu,Yang Chen,Rahul Krishna,Saurabh Sinha,Jatin Ganhotra,Reyhan Jabbarvand*

Main category: cs.SE

TL;DR: Graphectory is introduced to evaluate agentic systems by encoding their temporal and semantic relations, enabling process-centric metrics. Analyzing 4,000 trajectories from SWE-agent and OpenHands reveals how problem-solving efficiency and strategies are influenced by prompt richness and LLM strength.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of agentic systems focus solely on final outcomes, neglecting insights into their problem-solving processes. This approach fails to understand how agents reason, plan, or adapt over time. The authors are motivated to provide a more comprehensive method for analyzing agentic workflows by capturing both temporal and semantic relations.

Method: The authors introduce Graphectory, a systematic method for encoding the temporal and semantic relations of agentic systems as graphical representations. Using Graphectory, they design process-centric metrics and conduct automated analysis on 4,000 trajectories from dominant agentic programming workflows, SWE-agent and OpenHands, employing four LLM backbones to solve SWE-bench Verified issues.

Result: The analysis shows that the structure of Graphectory reveals agents' depth of exploration and validation strategies. Richer prompts and stronger LLMs produce more complex Graphectory, while problem difficulty and LLM influence strategy coherence. Finally, the results highlight inefficiencies in some successful agentic workflows, leading to prolonged processes.

Conclusion: Graphectory provides a structured, process-centric evaluation framework for agentic systems, uncovering hidden patterns in their problem-solving behaviors. This provides a valuable tool for both understanding and improving such systems, particularly through optimizing strategies to reduce redundancy and enhance efficiency.
...
### [25] [Feedback Loops and Code Perturbations in LLM-based Software Engineering: A Case Study on a C-to-Rust Translation System](https://arxiv.org/abs/2512.02567)
*Martin Weiss,Jesko Hecking-Harbusch,Jochen Quante,Matthias Woehrle*

Main category: cs.SE

TL;DR: LLMs can be effectively used for C-to-Rust translation with a generate-and-check approach, especially when feedback loops and code perturbations are incorporated to improve success rates and robustness.


<details>
  <summary>Details</summary>
Motivation: The increasing adoption of generative AI in software engineering calls for reliable automated approaches, such as code translation, in industrial settings. This paper investigates factors that influence the success and robustness of such systems by focusing on C-to-Rust translation as a relevant use case.

Method: The study evaluates a generate-and-check system that translates C code to Rust using a Large Language Model (LLM). The LLM-generated code is checked for compilability and behavioral equivalence with the original C. If the checks fail, a feedback loop prompts the model to correct its output. Experiments compare the effects of model choice, feedback loops, and code perturbations on translation success.

Result: LLM selection significantly impacts translation performance without feedback loops, but the performance differences across models reduce when feedback is used. Feedback loops also improve system robustness. Code perturbations enhance the system's adaptability and overall performance.

Conclusion: Feedback loops and code perturbations are vital for improving the reliability and effectiveness of LLM-based code translation. These strategies mitigate model variability and create more robust translation pipelines for real-world applications.
...
### [26] [Empirical Assessment of the Perception of Software Product Line Engineering by an SME before Migrating its Code Base](https://arxiv.org/abs/2512.02707)
*Thomas Georges,Marianne Huchard,Mélanie König,Clémentine Nebut,Chouki Tibermacine*

Main category: cs.SE

TL;DR: This paper discusses the migration of software variants into a product line within an SME, highlighting benefits, risks, and strategies for stakeholder engagement to ensure a smooth transition.


<details>
  <summary>Details</summary>
Motivation: The paper is motivated by a collaboration with an SME aiming to migrate its existing code base into an SPL, emphasizing the need to understand potential challenges and benefits.

Method: The method involved an in-depth evaluation of current development processes, anticipated benefits and risks, and conducting interviews with key stakeholders to gather insights.

Result: Participants identified personal benefits of migration in their roles; effective risk mitigation includes stakeholder engagement, preserving good practices, and active involvement.

Conclusion: To minimize migration challenges, companies should involve stakeholders, maintain effective practices, and ensure transparency and engagement throughout the process.
...
### [27] [Integrative Analysis of Risk Management Methodologies in Data Science Projects](https://arxiv.org/abs/2512.02728)
*Sabrina Delmondes da Costa Feitosa*

Main category: cs.SE

TL;DR: The paper reviews and compares risk management methodologies for data science projects, highlighting gaps in traditional approaches and advocating for hybrid frameworks that integrate ethical and organizational aspects.


<details>
  <summary>Details</summary>
Motivation: High failure rates in data science projects motivate the need to evaluate risk management methodologies for their effectiveness in addressing technical, organizational, ethical, and sociotechnical risks.

Method: An integrative literature review using indexed databases and a structured protocol to analyze and compare selected risk management standards and data science-specific frameworks.

Result: Traditional risk management approaches show limited coverage of emerging data-related risks, while newer models support ethical oversight and continuous monitoring through multidimensional risk management.

Conclusion: The paper recommends developing hybrid frameworks that blend technical, organizational, and ethical risk management elements to improve data science project outcomes and outlines research gaps for future work.
...
### [28] ["Can you feel the vibes?": An exploration of novice programmer engagement with vibe coding](https://arxiv.org/abs/2512.02750)
*Kiev Gama,Filipe Calegario,Victoria Jackson,Alexander Nolte,Luiz Augusto Morais,Vinicius Garcia*

Main category: cs.SE

TL;DR: The study explores the educational potential of 'vibe coding' through a one-day hackathon with 31 undergraduate participants.


<details>
  <summary>Details</summary>
Motivation: Vibe coding, using natural language prompts to create software, may democratize software development, but its educational effects have not been thoroughly examined.

Method: An inclusive one-day educational hackathon was conducted at a Brazilian public university with 9 teams of participants from computing and non-computing disciplines. Data was gathered via observations, an exit survey, and interviews to assess creative processes, tool usage, collaboration, and learning outcomes.

Result: Vibe coding supported quick prototyping and collaboration, but had issues with premature ideas, code quality, and engagement with key software practices. Participants learned prompt engineering but needed scaffolding and guidance for better learning.

Conclusion: Vibe coding hackathons can be useful for low-pressure learning when structured with supports for creative thinking, checking AI results, and realistic quality expectations.
...
### [29] [Towards Observation Lakehouses: Living, Interactive Archives of Software Behavior](https://arxiv.org/abs/2512.02795)
*Marcus Kessel*

Main category: cs.SE

TL;DR: The paper introduces the concept of an observation lakehouse for capturing continuous behavior data of code, using a database of runtime observations to improve the training and evaluation of code-generating LLMs.


<details>
  <summary>Details</summary>
Motivation: Current large language models generate code using static artifacts, making them prone to internalize bugs. Dynamic observation of behaviors provides the only practical way to establish ground-truth functionality.

Method: The method involves constructing an observation lakehouse based on Sequence Sheets, Stimulus-Response Matrices (SRMs), and Stimulus-Response Cubes (SRCs), which store observations as an append-only table. This is implemented using Apache technologies and SQL queries to materialize specific slices of data for analysis.

Result: The observation lakehouse was tested on a 509-problem benchmark, achieving efficient data ingestion and reconstruction of views and clusters with performance under 100ms on a regular laptop, indicating the approach's practicality.

Conclusion: The observation lakehouse presents a practical infrastructure for managing continuous behavioral data, enabling behavior-aware training and evaluation of LLMs, and it's publicly available as an open source project.
...
### [30] [Model-Based Diagnosis with Multiple Observations: A Unified Approach for C Software and Boolean Circuits](https://arxiv.org/abs/2512.02898)
*Pedro Orvalho,Marta Kwiatkowska,Mikoláš Janota,Vasco Manquinho*

Main category: cs.SE

TL;DR: TL;DR: The paper introduces CFaults, a new fault localization tool for C software and Boolean circuits that improves efficiency and reduces redundancy in diagnoses by using a MaxSAT formula approach. It outperforms existing tools in speed on C benchmarks and provides subset-minimal diagnoses, unlike other methods.


<details>
  <summary>Details</summary>
Motivation: Existing formula-based fault localization methods do not ensure a consistent set of diagnoses across all failing test cases and often produce redundant, non-subset-minimal diagnoses, especially when dealing with multiple faults. This is problematic for both software and circuit design where accuracy and efficiency are crucial.

Method: CFaults uses Model-Based Diagnosis (MBD) with multiple observations and aggregates failing test cases into a single MaxSAT formula. This allows for a unified approach to fault localization, ensuring consistency and minimizing redundant computations.

Result: On C benchmarks like TCAS and C-Pack-IPAs, CFaults is faster than tools such as BugAssist, SNIPER, and HSD. It also provides only subset-minimal diagnoses, whereas others produce redundant ones. On the Boolean circuit benchmark ISCAS85, while slightly slower, it still localizes faults in 94% of circuits.

Conclusion: CFaults effectively addresses the issues of redundancy and inconsistency in fault localization across multiple failing tests. It is a competitive tool for debugging, especially in scenarios with multiple faults, and offers a promising approach to unify and optimize the localization process with MaxSAT.
...
### [31] [The Evolutionary Ecology of Software: Constraints, Innovation, and the AI Disruption](https://arxiv.org/abs/2512.02953)
*Sergi Valverde,Blai Vidiella,Salva Duran-Nebreda*

Main category: cs.SE

TL;DR: This paper explores software evolution through evolutionary ecology, examining how constraints, innovation, and AI-driven tools shape software ecosystems and societal dynamics, using agent-based models, network analysis, and case studies.


<details>
  <summary>Details</summary>
Motivation: The study aims to understand the interplay between software evolution, technological innovation, and societal change, particularly addressing how AI tools like LLMs may reshape developer practices and cultural diversity in software ecosystems.

Method: The approach combines agent-based modeling, case studies of programming language evolution, complex network analysis, and evolutionary theory to analyze socio-technological systems.

Result: The analysis reveals how programming languages co-evolve with societal norms, and AI tools introduce evolutionary pressures that risk cultural stagnation, paralleling historical declines in software diversity.

Conclusion: The research underscores the need to monitor AI-mediated software production as a cultural and technological force, offering insights to guide future innovation and mitigate stagnation risks in software ecosystems.
