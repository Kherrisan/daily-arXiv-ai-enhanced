...
TL;DR: EmpiRE-Compass is an open-access, neuro-symbolic dashboard that enhances the sustainability of literature reviews by combining research knowledge graphs and large language models to enable transparent, reproducible, and reusable secondary studies in software and requirements engineering.


<details>
  <summary>Details</summary>
Motivation: The growing volume of literature reviews in software and requirements engineering, exacerbated by low-quality generative AI outputs and poor data sharing practices, hinders replication and reuse. There is a need for tools that improve transparency, rigor, and sustainability in secondary studies.

Method: The authors designed and implemented EmpiRE-Compass, a modular neuro-symbolic dashboard that combines research knowledge graphs (RKGs) with large language models (LLMs) to support exploratory analytics, custom query synthesis, and open reuse. The system was developed using two RE use cases and includes workflows for both curated and user-defined competency questions.

Result: EmpiRE-Compass provides three core capabilities: (1) exploratory visual analytics for predefined questions, (2) neuro-symbolic synthesis for custom queries, and (3) fully reusable knowledge with open access to all queries, analyses, and results. The tool is publicly available as open-source, with usage limits to manage LLM costs.

Conclusion: EmpiRE-Compass advances sustainable literature reviews in requirements engineering, software engineering, and related fields by integrating research knowledge graphs and large language models into a transparent, reproducible, and collaborative neuro-symbolic framework.
...
TL;DR: EyeLayer integrates human eye-gaze patterns as attention priors into LLMs for code summarization, using a lightweight module that enhances model performance without altering existing representations, showing consistent improvements across diverse models.


<details>
  <summary>Details</summary>
Motivation: Despite advances in LLMs for code summarization, there is a gap in leveraging human expertise in code understanding; eye-gaze data serves as a proxy for such expertise to guide model attention.

Method: EyeLayer uses a Multimodal Gaussian Mixture to model human eye-gaze patterns during code reading, deriving attention priors (μ_i, σ_i^2) that adjust token embeddings in LLMs without modifying their original representations.

Result: EyeLayer consistently improves performance across multiple LLMs (LLaMA-3.2, Qwen3, CodeBERT), achieving up to a 13.17% gain in BLEU-4 compared to strong fine-tuning baselines.

Conclusion: Human gaze patterns provide complementary attention signals that can effectively enhance the performance of LLMs in code summarization, and EyeLayer enables this enhancement in a lightweight, representation-preserving manner.
...
TL;DR: CMV introduces version-controlled context management for LLMs using a DAG-based model and lossless trimming, reducing token usage significantly while preserving full dialogue integrity and enabling cross-session reuse.


<details>
  <summary>Details</summary>
Motivation: Extended LLM reasoning sessions accumulate valuable context that is typically lost or degraded when hitting context limits; CMV addresses this by preventing information loss through systematic, reusable context management.

Method: CMV models session history as a Directed Acyclic Graph (DAG) with snapshot, branch, and trim primitives; employs a three-pass structurally lossless trimming algorithm to remove non-essential content like tool outputs and metadata while preserving user and assistant messages verbatim.

Result: The trimming algorithm reduces token counts by a mean of 20% and up to 86% in high-overhead sessions; mixed tool-use sessions show a 39% average reduction, achieving cost break-even within 10 turns under prompt caching.

Conclusion: Contextual Memory Virtualisation (CMV) enables efficient, structured management of LLM context by treating it as version-controlled state, allowing lossless reuse across sessions and improving economic viability in extended reasoning tasks.
...
TL;DR: XMENTOR is a human-centered, rank-aware explanation aggregation tool for ML-based software defect prediction, reducing developer confusion by unifying conflicting XAI outputs (LIME, SHAP, BreakDown) via adaptive thresholding and rank-sign consensus, and integrating them into VS Code; user study shows high preference and improved task support.


<details>
  <summary>Details</summary>
Motivation: Conflicting explanations from multiple XAI methods (e.g., LIME, SHAP, BreakDown) increase confusion, cognitive load, and distrust among developers, hindering the adoption of ML-based defect prediction models.

Method: XMENTOR uses adaptive thresholding, rank and sign agreement, and fallback strategies to aggregate explanations from LIME, SHAP, and BreakDown into a unified, rank-aware visualization, implemented as a VS Code plugin.

Result: In a user study, nearly 90% of participants preferred XMENTOR's aggregated explanations due to reduced confusion and improved support for debugging and code review tasks.

Conclusion: XMENTOR enhances interpretability, usability, and trust in ML-based defect prediction by integrating multiple XAI methods into a coherent, human-centered explanation system embedded in developer workflows.
...
TL;DR: LEREDD is an LLM-based method using RAG and ICL to automatically detect requirement dependencies from natural language text, achieving high accuracy and outperforming baselines, especially in identifying fine-grained dependency types.


<details>
  <summary>Details</summary>
Motivation: Requirement dependencies are crucial for software development decisions but are difficult to detect due to the volume, complexity, ambiguity in natural language, and frequent changes in requirements. Current practices often overlook or manually perform this task, necessitating automated, accurate solutions.

Method: LEREDD combines Retrieval-Augmented Generation (RAG) and In-Context Learning (ICL) with Large Language Models (LLMs) to detect diverse types of dependencies directly from natural language requirements. It was evaluated against two state-of-the-art baselines using an annotated dataset of 813 requirement pairs across three systems.

Result: LEREDD achieves an accuracy of 0.93 and an F1 score of 0.84 overall, with an average F1 of 0.96 for non-dependent cases. It shows significant improvement over baselines, with average relative F1 gains of 94.87% and 105.41% for the 'Requires' dependency type.

Conclusion: LEREDD is an effective LLM-based approach for automated requirement dependency detection, outperforming existing baselines, especially in fine-grained dependency classification, and demonstrates the potential of LLMs enhanced with RAG and ICL in requirements engineering.
...
TL;DR: RepoMod-Bench is a large-scale, implementation-agnostic benchmark for evaluating AI agents in repository-level code modernization, revealing significant performance degradation in larger codebases.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for AI coding agents are limited by small scale, language-specific tests, and test leakage, making evaluation unreliable. Code modernization offers a clearer evaluation paradigm due to fixed source-ground truth, but lacks scalable, rigorous benchmarks.

Method: The authors introduce RepoMod-Bench, a benchmark of 21 real-world repositories (1.6M LOC, 8 languages), with standardized interfaces and hidden, implementation-agnostic test suites. Evaluation is performed via black-box functional equivalence testing, preventing test-driven overfitting. Four state-of-the-art AI agent configurations are evaluated across repository sizes.

Result: A sharp 'scaling collapse' is observed: agent pass rates drop from 91.3% on repositories under 10K LOC to 15.3% on those over 50K LOC, indicating poor scalability in real-world settings.

Conclusion: Autonomous, large-scale code modernization remains a major open challenge. RepoMod-Bench provides a rigorous, scalable, and language-agnostic evaluation framework to advance AI coding agent development.
...
TL;DR: RandSet proposes a randomized corpus reduction technique for fuzzing that addresses seed explosion by computing small, diverse seed subsets via a randomized set cover approach, significantly improving coverage and bug detection with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Seed explosion in fuzzing leads to inefficient seed scheduling due to large corpus sizes; existing prioritization methods do not reduce corpus size, and current reduction techniques sacrifice diversity or incur high overhead, necessitating a more efficient and diverse approach.

Method: RandSet formulates corpus reduction as a randomized set cover problem to compute a small, diverse subset of seeds that covers all features of the full corpus, and schedules fuzzing seeds from this subset to reduce overhead and improve diversity.

Result: RandSet achieves average subset ratios of 4.03% and 5.99% on standalone and FuzzBench programs, provides up to 16.58% coverage gain in AFL++, detects up to 7 more bugs on Magma than state-of-the-art methods, and introduces only 1.17%-3.93% overhead.

Conclusion: RandSet effectively mitigates the seed explosion problem in fuzzing by enabling corpus reduction with high seed diversity and low runtime overhead, outperforming existing techniques in coverage and bug detection when integrated into popular fuzzers.
...
TL;DR: This paper introduces Rust-SWE-bench, a benchmark of 500 real-world Rust software engineering tasks, and evaluates current LLM agents, revealing key challenges in code structure understanding and type compliance. To address these, the authors propose RUSTFORGER, a new agent that uses automated test setup and Rust metaprogramming-based dynamic tracing, achieving a 28.6% task resolution rate—34.9% better than the best baseline.


<details>
  <summary>Details</summary>
Motivation: The steep learning curve and complexity of Rust hinder automation in issue resolution; existing LLM-based code agents lack evaluation on large-scale, real-world Rust tasks due to the absence of a suitable benchmark.

Method: The authors introduce Rust-SWE-bench, a repository-level benchmark with 500 real-world Rust tasks, and evaluate four LLM-powered agents on it. Based on identified limitations, they propose RUSTFORGER, which integrates automated test environment setup and a dynamic tracing strategy using Rust metaprogramming to improve issue reproduction and code understanding.

Result: RUSTFORGER achieves a 28.6% solve rate on Rust-SWE-bench, a 34.9% improvement over the strongest baseline, and uniquely solves 46 tasks unsolved by any other agent. The study reveals that ReAct-style agents achieve up to 21.2% solve rate but are limited by poor comprehension of code structure and Rust's type/traits system.

Conclusion: RUSTFORGER significantly outperforms existing agents on the Rust-SWE-bench benchmark by improving issue reproduction and dynamic analysis through automated test setup and Rust metaprogramming-driven tracing, demonstrating the importance of understanding repository-wide structure and Rust semantics for effective automated issue resolution.
...
TL;DR: This study explores the impact of hybrid work on Agile teams through interviews with three Norwegian teams, revealing reduced informal interactions, uneven participation, and increased digital tool reliance, with Agile ceremonies and trust playing crucial roles in maintaining effectiveness.


<details>
  <summary>Details</summary>
Motivation: To understand how hybrid work settings, which became prevalent post-pandemic, influence Agile teams' ability to deliver value, collaborate, and adapt.
...
Result: Hybrid work reduces informal interaction, creates uneven participation, and increases reliance on digital tools. Agile ceremonies serve as alignment anchors, while trust, communication, and tool support are key mediators of team effectiveness.

Conclusion: Hybrid Agile work is an evolving field that requires tailored structures to support inclusion, team cohesion, and sustainable performance.
...
TL;DR: This paper proposes a lifecycle-based uncertainty management framework for LLM-based multi-agent software systems, addressing system-level and runtime uncertainties through representation, identification, evolution, and adaptation mechanisms, validated in a clinical echocardiography setting.


<details>
  <summary>Details</summary>
Motivation: Current approaches to uncertainty in LLM-based multi-agent systems focus primarily on model-level accuracy and fail to address system-level and runtime uncertainties arising from agent coordination, data pipelines, human interaction, and control logic, especially in safety-critical domains like echocardiography.

Method: The paper introduces a framework with four mechanisms  (representation, identification, evolution, and adaptation), applied to LLM-based multi-agent echocardiographic software, with validation in a clinical setting.

Result: The framework demonstrated improved reliability and diagnosability in diagnostic reasoning within a real-world clinical LLM-based multi-agent echocardiographic system, showing feasibility and generalizability to other safety-critical AI applications.

Conclusion: The proposed lifecycle-based uncertainty management framework provides a structured approach to handling uncertainty in LLM-based multi-agent software systems, enabling improved reliability and diagnosability in safety-critical applications like echocardiography, and generalizes beyond model-centric methods to support principled runtime assurance.
...
TL;DR: CL4SE introduces a systematic benchmark for context learning in Software Engineering, proposing a fine-grained taxonomy of four SE-specific context types and evaluating their impact across four core tasks using over 13,000 samples from open-source projects, showing average performance gains of 24.7% across five LLMs.


<details>
  <summary>Details</summary>
Motivation: Despite the success of context engineering in improving LLM performance for Software Engineering tasks, there is a lack of systematic taxonomy for SE-specific context types and a dedicated benchmark to evaluate the impact of different contexts across core SE workflows.

Method: The authors propose CL4SE, a comprehensive benchmark with a fine-grained taxonomy of four SE-specific context types—interpretable examples, project-specific context, procedural decision-making context, and positive & negative context—each mapped to a representative SE task. They construct a dataset of over 13,000 samples from more than 30 open-source projects and evaluate five mainstream LLMs across nine metrics.

Result: Context learning yields an average performance improvement of 24.7% across all tasks. Procedural context improves code review by up to 33% (Qwen3-Max), mixed positive-negative context enhances patch assessment by 30% (DeepSeek-V3), project-specific context increases code summarization BLEU by 14.78% (GPT-Oss-120B), and interpretable examples boost code generation PASS@1 by 5.72% (DeepSeek-V3).

Conclusion: CL4SE establishes the first standardized evaluation framework for context learning in Software Engineering, providing actionable insights into task-specific context design and releasing a large-scale dataset to support reproducible research.
...
TL;DR: TransFuzz uses LLMs to extract and transfer silent bug patterns from historical reports to similar APIs in deep learning libraries, enabling effective oracle-aware fuzzing and discovering 79 new bugs, including 12 CVEs.


<details>
  <summary>Details</summary>
Motivation: Silent bugs in deep learning libraries can have severe consequences in critical applications, but existing fuzzing techniques fail to detect them due to lack of effective oracles and test programs; thus, a new approach is needed to proactively identify such bugs.

Method: TransFuzz leverages large language models to extract context-aware bug patterns from historical bug reports, matches functionally similar APIs using embeddings, synthesizes test cases with customized oracles, and employs an LLM-powered self-validation module to ensure transferred bug validity for silent bug detection in DL libraries.

Result: TransFuzz discovered 79 previously unknown silent bugs across PyTorch, TensorFlow, and MindSpore, with 12 confirmed as CVEs, covering 10 distinct bug types, demonstrating high effectiveness and broad applicability.

Conclusion: TransFuzz effectively transfers silent bug detection capabilities across APIs in deep learning libraries using LLM-driven context-aware bug pattern extraction and self-validation, demonstrating strong generalizability and practical impact in uncovering critical vulnerabilities.
...
TL;DR: This paper investigates the use of Large Language Models in industrial process automation, where specialized, proprietary languages are common. It demonstrates that LLMs can be leveraged to perform practical programming tasks—such as generating robotic arm movements—thereby accelerating development in manufacturing, despite limited training data in this domain.


<details>
  <summary>Details</summary>
Motivation: Most LLM research in software engineering focuses on general-purpose languages like Python, leaving a gap in understanding their effectiveness in niche, proprietary domains such as industrial automation with specialized languages.

Method: The study explores the application of LLMs to real-world industrial programming tasks by adapting models to domain-specific languages and evaluating their performance on practical use cases like robotic arm routine generation.

Result: LLMs can be effectively adapted to support real-life programming tasks in industrial automation, demonstrating feasibility in accelerating development cycles despite limited exposure to domain-specific languages.

Conclusion: The integration of LLMs into industrial process automation holds potential for improving development efficiency, despite challenges posed by specialized programming languages and limited training data.
...
TL;DR: Alpha-Root is a cybersecurity dataset built via one-shot community detection on the Common Crawl graph, using 20 seed domains to find high-quality sites without iterative content scoring.


<details>
  <summary>Details</summary>
Motivation: To develop a more efficient and scalable method for discovering high-quality cybersecurity-related domains by leveraging web graph structure instead of content analysis.

Method: The authors use community detection on the Common Crawl web graph, starting from 20 trusted seed domains, to identify quality domains in a single pass rather than through iterative content-based scoring.

Result: Alpha-Root successfully identifies relevant domains from a single crawl using graph-based community detection, bypassing the need for repeated content evaluation.

Conclusion: Alpha-Root demonstrates that high-quality, cybersecurity-relevant domains can be effectively identified directly from the web graph using community detection, without requiring iterative content scoring.
...
TL;DR: This paper proposes a dynamic, optimizer-driven multichain architecture that allocates blockchain resources via multiagent declarations, maximizing governance-weighted utilities across applications, operators, and the system, with off-chain computation and on-chain verification.


<details>
  <summary>Details</summary>
Motivation: Existing blockchain systems and multichain designs lack scalability and adaptability to dynamic demand and capacity changes, necessitating a flexible, incentive-aware, and decentralized configuration mechanism.

Method: Blockchains are modeled as a multiagent resource-allocation system where applications and operators submit demands and capacities; an off-chain optimizer assigns them to ephemeral chains per epoch using a governance-weighted utility function, with on-chain verifiable outcomes.

Result: Simulations demonstrate trade-offs between throughput, decentralization, operator yield, and service stability, while the model supports fairness, incentive alignment, and compatibility constraints.

Conclusion: The proposed model offers a dynamic and modular approach to blockchain configuration that improves scalability and adaptability by treating it as a multiagent resource-allocation problem, enabling efficient and verifiable chain formation and pricing.
...
TL;DR: This paper identifies cryptographic hashing as a critical bottleneck in disaster recovery for distributed storage and proposes a metadata-driven, deterministic identification scheme that assigns unique block IDs at ingestion, enabling fast, hash-free synchronization during failover and failback.


<details>
  <summary>Details</summary>
Motivation: Cryptographic hashing, while effective for deduplication, creates a bottleneck in disaster recovery when hash indexes are stale or lost, leading to costly re-hashing and jeopardizing RTOs.

Method: The paper introduces a framework that assigns globally unique composite identifiers to data blocks at ingestion time, decoupling data identification from content-based hashing and enabling instantaneous delta computation during failover and failback.

Result: The approach removes cryptographic overhead during DR, allows immediate synchronization, and avoids full or partial re-hashing, significantly reducing recovery time.

Conclusion: The proposed metadata-driven, deterministic identification framework eliminates the need for cryptographic re-hashing during disaster recovery, enabling faster DR operations and improving RTO compliance in distributed storage systems.
...
TL;DR: TT-SEAL is a selective-encryption framework for tensor-train compressed neural networks that securely and efficiently protects only the most sensitive components, drastically reducing encryption overhead while maintaining robustness on edge devices.


<details>
  <summary>Details</summary>
Motivation: Existing selective-encryption methods assume dense model weights and are not optimized for tensor-train decomposed models; TT-SEAL addresses the security-compression gap in compressed on-device AI.

Method: TT-SEAL introduces a sensitivity-based importance metric to rank TT cores, applies a one-time robustness threshold, and uses a value-DP optimizer to selectively encrypt critical cores with AES under TTD-compressed models.

Result: TT-SEAL encrypts only 4.89-15.92% of parameters across ResNet-18, MobileNetV2, and VGG-16 while matching full encryption robustness, and reduces AES decryption latency share from 58% to as low as 2.76% on ResNet-18 under TTD-aware threat models.

Conclusion: TT-SEAL enables secure, low-latency edge AI by efficiently minimizing both the number of encrypted parameters and decryption latency, achieving strong robustness with minimal overhead.
...
TL;DR: This paper evaluates prompt-injection and jailbreak vulnerabilities in multiple open-source LLMs using a large curated dataset, finding that while lightweight inference-time defenses help against simple attacks, they fail against complex, reasoning-based prompts, revealing critical security gaps.


<details>
  <summary>Details</summary>
Motivation: Given the widespread deployment of LLMs and the growing reliance on prompt engineering, especially by resource-scarce organizations, it is crucial to understand and address the security risks posed by prompt-based attacks such as prompt injection and jailbreaking.
...
Result: Significant behavioral differences were observed across models, including varied refusal rates and silent failures due to internal safety mechanisms. Lightweight defenses were effective against basic attacks but consistently bypassed by longer, complex, reasoning-heavy prompts.

Conclusion: The study concludes that while lightweight, inference-time defense mechanisms can mitigate simple prompt-injection and jailbreak attacks on open-source LLMs, they are ineffective against more sophisticated, reasoning-heavy prompts, highlighting the need for more robust and adaptive security measures.
...
TL;DR: We propose DiSP, a backdoor defense framework for Multimodal Diffusion Language Models that uses selective vision token masking and self-purification to remove backdoors without clean data or auxiliary models.


<details>
  <summary>Details</summary>
Motivation: Multimodal Diffusion Language Models (MDLMs) are vulnerable to data-poisoning-based backdoor attacks, but effective defenses are lacking. This work addresses the gap in backdoor defense for MDLMs.

Method: DiSP exploits the observation that masking specific vision tokens at inference can suppress trigger-induced behaviors. It uses the compromised model to purify the poisoned training data via self-purification, then fine-tunes the model on the cleaned dataset to eliminate the backdoor.

Result: DiSP reduces the attack success rate from over 90% to below 5% across multiple benchmarks, while preserving performance on clean inputs.

Conclusion: DiSP provides an effective, standalone defense against backdoor attacks in MDLMs, requiring no external models or clean data, making it practical for real-world deployment.
...
TL;DR: Training-data poisoning attacks can stealthily corrupt deep neural networks by flipping a small fraction of labels, demonstrated on acoustic vehicle classification with high attack success and no detectable accuracy drop; a cryptographically verifiable data provenance framework is proposed for defense.


<details>
  <summary>Details</summary>
Motivation: To demonstrate the severity and stealth of training-data poisoning attacks in real-world deep learning systems, particularly under class imbalance, and to show that conventional accuracy monitoring fails to detect such attacks.

Method: A compact 2D CNN is trained on log-mel spectrograms from the MELAUDIS dataset to evaluate label-flipping and backdoor trigger attacks. Theoretical analysis bounds the maximum accuracy drop from targeted attacks by the minority class fraction. A cryptographic defense is proposed using artifact hashing, Merkle trees, and post-quantum digital signatures for data provenance.

Result: Achieved 95.7% attack success rate (ASR) in a Truck-to-Car label-flipping attack with only 0.5% label corruption (48 samples), with no detectable change in overall accuracy (87.6% baseline, 95% CI: 88-100%). Proved that accuracy drop is bounded by minority class fraction (beta), making detection via accuracy impossible under realistic class imbalance. Backdoor trigger attacks degenerate to pure label flipping when the target is a minority class, due to trigger-dominance collapse.

Conclusion: Label-flipping poisoning attacks are structurally undetectable under class imbalance, rendering standard accuracy monitoring ineffective. Backdoor triggers lose efficacy when targeting minority classes. A trust-minimized, cryptographic data provenance system is required to ensure integrity in machine learning pipelines.
...
TL;DR: Public-moment-guided Truncation (PMT) leverages public second-moment information to improve differential privacy in linear models by enabling principled data transformation and truncation, leading to better-conditioned data and enhanced DP estimation accuracy.


<details>
  <summary>Details</summary>
Motivation: Differential privacy (DP) typically requires bounded data distributions, limiting its applicability to real-world data with unbounded or poorly conditioned distributions. The authors aim to overcome this limitation by utilizing a small amount of public data to guide privatization effectively.

Method: The proposed method, PMT, uses the second-moment matrix estimated from public data to transform private data into a well-conditioned space. A principled truncation is then applied based on data dimension and sample size, independent of private information. The transformation enables stable inversion of the second-moment matrix and reduces sensitivity to DP noise. New loss functions and algorithms are designed for penalized and generalized linear regression to ensure that solutions in the transformed space map back to valid solutions in the original space.

Result: PMT achieves tighter theoretical error bounds, improved robustness, and convergence guarantees in DP estimation. Empirical results on synthetic and real datasets show significant improvements in accuracy and stability of DP models, especially under high privacy regimes.

Conclusion: By incorporating public second-moment information, PMT effectively addresses key limitations of DP in unbounded settings, offering a practical and theoretically sound approach to enhancing DP performance in regression tasks without compromising privacy.
...
TL;DR: Hubscan is an open-source security scanner that detects harmful hubs in RAG systems using a multi-faceted detection approach, demonstrating high recall and robustness across benchmarks and real-world data.


<details>
  <summary>Details</summary>
Motivation: Retrieval-Augmented Generation (RAG) systems are vulnerable to hubness, where certain items dominate retrieval results across unrelated queries, enabling security exploits such as injection of harmful content and manipulation of search rankings. This work addresses the critical need for systematic detection of such hubs.

Method: Hubscan employs a multi-detector architecture featuring statistical hubness detection (median/MAD z-scores), cluster spread analysis, stability testing under query perturbations, and domain- or modality-aware detection. It supports multiple vector databases and retrieval techniques, including hybrid and lexical search with reranking.

Result: Hubscan achieves 90% recall at a 0.2% alert budget and 100% recall at 0.4% on adversarial hubness benchmarks (Food-101, MS-COCO, FiQA), with adversarial hubs ranked above the 99.8th percentile. Domain-scoped scanning detects 100% of targeted attacks evading global detection. Evaluation on 1M real documents (MS MARCO) shows clear score separation between clean and adversarial content.

Conclusion: Hubscan provides a practical and extensible framework for detecting hubness-based security threats in production RAG systems, enabling robust, multi-faceted detection across diverse vector databases and retrieval methods.
...
TL;DR: This work develops transformer-based models to predict software vulnerabilities from natural-language cyberattack descriptions, showing that Technique-level attack data and the MMPNet model achieve top performance, with the VULDAT tool enabling discovery of undocumented attack-vulnerability links.


<details>
  <summary>Details</summary>
Motivation: Cyber Threat Intelligence resources like MITRE ATT&CK and CVE lack complete mappings between attacks and vulnerabilities. Attack descriptions often precede formal vulnerability documentation, creating a gap that requires automated methods to infer potential vulnerabilities from textual attack data to improve threat response and preparedness.

Method: The research employs transformer-based sentence embedding models to encode cyberattack and vulnerability descriptions into semantic vectors. Fourteen state-of-the-art models are evaluated across four types of attack descriptions (Tactic, Technique, Procedure, Attack Pattern), with similarity-based ranking used to predict vulnerability associations. The best-performing model is integrated into the VULDAT tool.

Result: Technique-level descriptions in MITRE ATT&CK yielded the strongest predictive performance. The multi-qa-mpnet-base-dot-v1 (MMPNet) model outperformed others due to its hybrid pre-training and semantic similarity optimization. VULDAT, the implemented tool, successfully identified previously undocumented attack-vulnerability relationships and demonstrated generalization to real-world, unseen attack reports.

Conclusion: The study concludes that transformer-based models, particularly MMPNet, are effective in predicting known vulnerabilities from natural-language attack descriptions, with Technique-level descriptions in MITRE ATT&CK offering the most reliable input. The integration into the VULDAT tool enables the discovery of undocumented attack-vulnerability links and supports proactive cyber defense.
...
TL;DR: The paper proposes a differentially private method for generating Markov chain models from user data, ensuring privacy while maintaining model accuracy by bounding KL divergence, stationary distribution shifts, and convergence rate changes.


<details>
  <summary>Details</summary>
Motivation: Generating accurate Markov chain models requires large user datasets, but sharing such models risks exposing sensitive individual data; thus, privacy-preserving methods are essential.

Method: The authors develop a differentially private mechanism for queries returning simplex-valued outputs (e.g., probability distributions), then extend it to privatize stochastic matrices used in Markov chains. They analyze the impact on model accuracy by deriving bounds on the KL divergence, changes in stationary distribution, and convergence rate.

Result: The method ensures differential privacy with bounded expected KL divergence. Analytical bounds show minimal impact on the stationary distribution and convergence rate. Simulations demonstrate less than 2% error in the stationary distribution under standard privacy settings.

Conclusion: The proposed method effectively preserves user privacy in data-driven Markov chain models while maintaining high fidelity to the original system's behavioral dynamics.
...
TL;DR: Malicious URL previews can trigger silent data exfiltration in agentic LLMs via implicit prompt injection; the attack evades output monitoring and is mitigated more effectively by network- and system-level controls than by prompt hardening.


<details>
  <summary>Details</summary>
Motivation: As agentic LLMs increasingly interact with external tools and retrieve web content autonomously, they become vulnerable to implicit prompt injection through seemingly benign URL previews, posing undetected data exfiltration risks that bypass traditional output-focused safety mechanisms.

Method: The authors designed a reproducible, fully local testbed to evaluate implicit prompt injection attacks via URL previews on a qwen2.5:7b-based agentic LLM. They measured the success rate of silent egress attacks, evaluated detection evasion (including sharded exfiltration), and conducted ablation studies on prompt-layer vs. system- and network-layer defenses.

Result: The attack achieved an 89% success rate (P=0.89) in inducing unintended egress across 480 trials; 95% of successful attacks evaded output-based safety checks. Sharded exfiltration reduced single-request leakage by 73% (Leak@1) and bypassed basic DLP tools. Prompt-layer defenses showed limited efficacy, whereas system- and network-level controls (e.g., domain allowlisting, redirect-chain analysis) were significantly more effective.

Conclusion: Network egress must be treated as a critical security concern in agentic LLM systems, requiring system- and network-layer defenses such as domain allowlisting, provenance tracking, and capability isolation to effectively mitigate risks like silent egress and sharded exfiltration.
...
TL;DR: This study evaluates seven pre-trained CNN models for multi-class IoT DDoS detection using image-represented network traffic, focusing on performance, reliability, and interpretability; DenseNet and MobileNet variants perform best, with DenseNet169 excelling in reliability and interpretability, while MobileNetV3 suits low-latency fog environments.


<details>
  <summary>Details</summary>
Motivation: DDoS attacks threaten IoT availability, especially under resource constraints; while transfer learning shows promise in detection, its reliability, computational feasibility, and interpretability in real-world settings need deeper investigation.

Method: The authors use image-based network traffic representation from the CICDDoS2019 dataset and apply seven pre-trained CNNs; they evaluate performance (accuracy, F1), reliability (MCC, Youden Index, confidence intervals), computational cost (latency, training time), and interpretability using Grad-CAM and SHAP.

Result: DenseNet and MobileNet architectures achieve high detection accuracy and reliability; DenseNet169 shows the best reliability-interpretability alignment with consistent attribution patterns, while MobileNetV3 offers optimal latency-accuracy trade-off for edge/fog deployment.

Conclusion: Selecting deep learning models for IoT DDoS detection should balance performance, reliability, and interpretability; DenseNet169 and MobileNetV3 are recommended for high-trust and resource-constrained settings, respectively.
...
TL;DR: MUTE identifies intervention depth as the key to multilingual unlearning in LLMs and proposes a method to target language-agnostic intermediate layers using CKA and LRDS, enabling effective cross-lingual knowledge erasure with minimal source language data.


<details>
  <summary>Details</summary>
Motivation: Machine unlearning in LLMs often fails to generalize across languages, with erased knowledge remaining accessible in non-source languages. The cause—intervention depth—has not been systematically studied, and there lacks a principled method to achieve multilingual unlearning.

Method: MUTE uses Centered Kernel Alignment (CKA) and Linguistic Regions Development Score (LRDS) to identify intermediate, language-agnostic layers where cross-lingual representations converge, and applies unlearning updates only within these layers. The approach is evaluated across multiple LLM architectures and unlearning algorithms, with Logit Lens probing for mechanistic validation.

Result: MUTE achieves robust multilingual knowledge erasure while preserving model utility, outperforming shallow and deep intervention strategies. Experiments across three LLMs and three unlearning methods confirm effectiveness, with Logit Lens analysis verifying actual knowledge removal rather than superficial output suppression.

Conclusion: The choice of intervention depth is critical for successful multilingual unlearning in LLMs, and MUTE provides a principled solution by targeting intermediate, language-agnostic layers identified via CKA and LRDS, enabling effective and robust cross-lingual knowledge erasure.
...
TL;DR: IMMACULATE is a practical auditing framework that detects provider-side dishonesty in black-box LLM APIs—such as model substitution and token overbilling—using lightweight verifiable computation, achieving high detection accuracy with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address trust issues in commercial LLM APIs, where users must rely on providers to run correct models, use honest token counting, and avoid cost-cutting abuses that degrade service quality or inflate costs.

Method: IMMACULATE uses verifiable computation to selectively audit a small fraction of API requests, leveraging cryptographic methods to detect deviations such as model substitution, improper quantization, and token overbilling while minimizing overhead.

Result: Experiments demonstrate that IMMACULATE effectively distinguishes between benign and malicious model executions on both dense and Mixture-of-Experts (MoE) models with less than 1% throughput overhead.

Conclusion: IMMACULATE provides a practical and efficient auditing framework for black-box LLM APIs that enables detection of economically motivated provider misbehavior without requiring trusted hardware or model access.
...
TL;DR: AgentSentry is a novel inference-time defense that detects and mitigates indirect prompt injection in LLM agents by identifying causal takeover points through counterfactual analysis and enabling secure task continuation via context purification, significantly improving robustness and utility under attack.


<details>
  <summary>Details</summary>
Motivation: The increasing use of external tools and retrieval systems in LLM agents introduces vulnerability to indirect prompt injection (IPI) attacks, which manipulate agent behavior through malicious content in tool outputs across multiple turns, evading traditional detection methods.
...
Result: Evaluated on the AgentDojo benchmark across multiple attack families and LLMs, AgentSentry eliminates successful IPI attacks and achieves an average Utility Under Attack (UA) of 74.55%, outperforming strong baselines by 20.8 to 33.6 percentage points without harming performance on benign inputs.

Conclusion: AgentSentry represents the first inference-time defense that effectively mitigates multi-turn indirect prompt injection in tool-augmented LLM agents by modeling attacks as temporal causal takeovers, enabling both precise detection and safe execution continuation.
...
TL;DR: This paper presents a CNN-based model for detecting bugs in C code, trained on machine- and human-labeled datasets, achieving high recall and precision and demonstrating effectiveness on real-world code like the Linux kernel.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the detection of software vulnerabilities in C code, which is prone to bugs and exploitation, by leveraging deep learning techniques tailored specifically for C, unlike prior general-purpose approaches.

Method: The method involves training a CNN model on two datasets: a machine-labeled dataset generated by Draper Labs using static analyzers and the human-labeled NIST SATE Juliet dataset. The input C code is tokenized into 91 categories, converted into binary vectors, and processed through two convolution and pooling layers, followed by two fully connected layers to classify code into CWE categories or as 'clean'.

Result: The model achieves higher recall than Russell et al.'s approach on the same datasets under high-precision requirements and successfully identifies real vulnerabilities in a custom Linux kernel dataset with a low false-positive rate.

Conclusion: The proposed convolutional neural network model effectively identifies software vulnerabilities in C code with high recall and precision, outperforming prior work, and demonstrates practical applicability by detecting real vulnerabilities in complex code such as the Linux kernel with low false positives.
...
TL;DR: SettleFL enables scalable, fair reward settlement in decentralized federated learning using two interoperable blockchain protocols that minimize cost and latency while ensuring trustless operation.


<details>
  <summary>Details</summary>
Motivation: In open, decentralized federated learning environments, fair collaboration requires trustless reward settlement, but existing approaches face high blockchain costs or scalability limitations, motivating the need for an efficient, fully decentralized solution.

Method: SettleFL introduces two interoperable protocols—a Commit-and-Challenge variant for optimistic execution with dispute resolution and a Commit-with-Proof variant using validity proofs for instant finality—both built on a shared domain-specific circuit architecture to minimize on-chain costs and ensure rational robustness.

Result: Experiments with real FL workloads and simulations show SettleFL scales to 800 participants with significantly lower gas costs, remaining practical under high participant counts and varying network conditions.

Conclusion: SettleFL provides a scalable, trustless reward settlement solution for decentralized federated learning, enabling fair collaboration without relying on a central authority while maintaining practical efficiency at scale.
...
TL;DR: This paper proposes a quantum key distribution protocol using quantum walks on a hypercube topology, showing enhanced security and noise tolerance over circular topologies, supported by an open-source Qiskit-based simulation framework.


<details>
  <summary>Details</summary>
Motivation: To enhance the security and robustness of QKD protocols against eavesdropping and noise, moving beyond limitations of classical protocols like BB84 and existing QW-based schemes with circular topologies.

Method: A novel QKD protocol is designed using discrete-time quantum walks on a hypercube topology; an open-source, noise-aware simulation framework is developed using Qiskit to evaluate and compare performance under realistic conditions.

Result: The hypercube-based QW protocol demonstrates significantly improved security and noise resistance over the circular topology under identical parameters, validated through simulations with realistic noise models.

Conclusion: The proposed one-way QKD protocol based on quantum walks over a hypercube topology offers superior security and noise resistance compared to circular topologies, establishing a foundation for topology-aware QKD protocols.
