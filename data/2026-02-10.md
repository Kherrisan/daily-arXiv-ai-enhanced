...
### [1] [TraceCoder: A Trace-Driven Multi-Agent Framework for Automated Debugging of LLM-Generated Code](https://arxiv.org/abs/2602.06875)
...
### [2] [Zero-Trust Runtime Verification for Agentic Payment Protocols: Mitigating Replay and Context-Binding Failures in AP2](https://arxiv.org/abs/2602.06345)
*Qianlong Lan,Anuj Kaul,Shaun Jones,Stephanie Westrum*

Main category: cs.CR

TL;DR: This paper analyzes security gaps in the runtime execution of Agent Payments Protocol (AP2) mandates due to autonomous agent behaviors, and proposes a zero-trust verification framework using time-bound nonces to enforce context binding and single use of mandates. The approach effectively mitigates replay and context-redirect attacks with low latency and bounded state, enabling secure, scalable agentic commerce.


<details>
  <summary>Details</summary>
Motivation: Runtime behaviors in autonomous AI agents—such as retries, concurrency, and orchestration—introduce security vulnerabilities that challenge the static assumptions of mandate-based payment protocols like AP2, necessitating stronger runtime enforcement mechanisms.

Method: A zero-trust runtime verification framework is designed using dynamically generated, time-bound nonces to enforce explicit context binding and consume-once mandate semantics; its effectiveness is evaluated through high-concurrency simulations.

Result: The framework prevents replay and context-redirect attacks under high concurrency, achieves stable verification latency (~3.8 ms), supports up to 10,000 transactions per second, and maintains bounded runtime state tied to peak concurrency rather than transaction history.

Conclusion: The proposed zero-trust runtime verification framework effectively enforces secure mandate usage in agent-based payment systems by ensuring context-aware binding and consume-once semantics, closing critical enforcement gaps in AP2 during runtime.
...
TL;DR: This paper identifies a new security vulnerability in multimodal recommender systems—cross-modal interactive poisoning—and proposes VENOMREC, a method that aligns exposures and perturbs both text and image modalities in an attention-guided way, achieving high attack effectiveness with minimal impact on recommendation performance.


<details>
  <summary>Details</summary>
Motivation: While cross-modal consensus in MLLMs can defend against traditional poisoning attacks, it creates a new attack surface via synchronized multimodal poisoning, which can manipulate fused representations during fine-tuning in a controlled manner.

Method: VENOMREC employs Exposure Alignment to detect high-exposure regions in the joint embedding space and uses Cross-modal Interactive Perturbation to generate attention-guided, coupled token-patch modifications for synchronized multimodal poisoning.

Result: VENOMREC achieves a mean ER@20 of 0.73, surpassing the strongest baseline by +0.52 absolute ER points on average across three real-world multimodal datasets, while preserving recommendation utility.

Conclusion: Cross-modal interactive poisoning poses a significant threat to multimodal recommender systems, and VENOMREC provides an effective framework to exploit this vulnerability through coordinated perturbations across modalities.
...
TL;DR: This paper proposes Avatar, a morphable secure last-level cache that dynamically switches between non-secure, randomized secure, and partitioned secure modes, offering strong security with minimal overhead and enabling practical deployment in modern processors.


<details>
  <summary>Details</summary>
Motivation: Modern processors remain vulnerable to cross-core cache attacks due to the shared last-level cache, and existing secure LLC designs face significant overheads in area, performance, or complexity, hindering industrial adoption. The paper aims to achieve strong security with minimal changes and overhead.

Method: Avatar introduces a morphable LLC with three operational modes—non-secure (Avatar-N), randomized secure (Avatar-R), and partitioned secure (Avatar-P)—and uses extra invalid entries and high associativity in Avatar-R, while employing efficient way partitioning in Avatar-P, all with minimal modifications to a conventional set-associative LLC.

Result: Avatar-R incurs only 1.5% storage overhead, 2.7% increase in static power, and 0.2% performance slowdown, providing strong security against conflict-based attacks. Avatar-P achieves only 3% performance overhead while mitigating both conflict- and occupancy-based attacks, outperforming prior partitioned designs. Security can be disabled via Avatar-N for optimal performance.

Conclusion: Avatar provides a practical and secure LLC design that enables dynamic switching between non-secure and secure modes, achieving strong security with minimal overhead, thus facilitating industrial adoption.
...
### [5] [TrajAD: Trajectory Anomaly Detection for Trustworthy LLM Agents](https://arxiv.org/abs/2602.06443)
*Yibing Liu,Chong Zhang,Zhongyi Han,Hansong Liu,Yong Wang,Yang Yu,Xiaoyan Wang,Yilong Yin*

Main category: cs.CR

TL;DR: This paper introduces Trajectory Anomaly Detection for LLM agents, emphasizing the need to monitor intermediate execution steps. The authors present TrajBench, a synthetic dataset, and TrajAD, a specialized verifier trained with process supervision, which outperforms general LLMs in detecting and localizing procedural errors, highlighting that trustworthiness requires dedicated training beyond general capabilities.


<details>
  <summary>Details</summary>
Motivation: To enhance the trustworthiness of LLM agents by moving beyond static input/output filtering and instead auditing the intermediate execution process for runtime anomalies, enabling reliable rollback-and-retry mechanisms.

Method: The authors propose TrajAD, a specialized verifier trained with fine-grained process supervision, evaluated on TrajBench—a dataset created via a perturb-and-complete strategy to benchmark trajectory anomaly detection in LLM agent executions.

Result: General-purpose LLMs perform poorly in zero-shot trajectory anomaly detection and localization; in contrast, TrajAD achieves superior performance, demonstrating the necessity of specialized training for process-level reliability.

Conclusion: Specialized supervision, as implemented in TrajAD, is essential for effective runtime trajectory anomaly detection and precise error localization in LLM agents, surpassing general-purpose models that lack such training.
...
### [6] [Subgraph Reconstruction Attacks on Graph RAG Deployments with Practical Defenses](https://arxiv.org/abs/2602.06495)
*Minkyoo Song,Jaehan Kim,Myungchul Kang,Hanna Kim,Seungwon Shin,Sooel Son*

Main category: cs.CR

TL;DR: GRASP is a novel closed-box, multi-turn attack that effectively reconstructs subgraphs from Graph RAG systems despite safety prompts, achieving up to 82.9 F1 score, and introducing practical challenges in preserving privacy while maintaining utility.


<details>
  <summary>Details</summary>
Motivation: Graph RAG systems, while improving relational reasoning in LLMs, expose privacy risks through potential subgraph reconstruction; however, existing attacks are ineffective due to explicit exfiltration intent being caught by safety mechanisms, necessitating a more stealthy and robust approach.

Method: GRASP reframes subgraph extraction as a context-processing task, uses per-record identifiers to ensure format compliance and reduce hallucinations, and employs a momentum-aware scheduler to diversify queries under strict query budgets in a closed-box, multi-turn setup.

Result: GRASP achieves state-of-the-art, type-faithful subgraph reconstruction with up to 82.9 F1 across multiple knowledge graphs, LLMs, and Graph RAG frameworks, outperforming prior methods that fail under basic safeguards; evaluated defenses show that lightweight mitigations can reduce reconstruction fidelity without harming utility.

Conclusion: Subgraph reconstruction in Graph RAG is feasible even under realistic safeguards using stealthy, structured attacks like GRASP, highlighting the need for stronger privacy-preserving designs in knowledge-augmented LLM systems.
...
### [7] [Sequential Auditing for f-Differential Privacy](https://arxiv.org/abs/2602.06518)
...
### [8] [Dependable Artificial Intelligence with Reliability and Security (DAIReS): A Unified Syndrome Decoding Approach for Hallucination and Backdoor Trigger Detection](https://arxiv.org/abs/2602.06532)
*Hema Karnam Surendrababu,Nithin Nagaraj*

Main category: cs.CR

TL;DR: This paper proposes a unified approach using Syndrome Decoding to detect both security (e.g., backdoor data-poisoning) and reliability (e.g., hallucinations in LLMs) violations in machine learning systems, particularly adapting the method to NLP sentence-embedding spaces.


<details>
  <summary>Details</summary>
Motivation: ML models, especially LLMs, face critical security threats like data-poisoning and reliability issues such as hallucinations, which compromise system dependability and pose significant risks to users.

Method: The authors adapt syndrome decoding—a technique from error-correcting codes—to the NLP sentence-embedding space to detect anomalies. This method identifies poisoned samples in training data and hallucinated outputs in LLMs by leveraging structural inconsistencies in embeddings.

Result: The proposed approach effectively detects both backdoor data-poisoning attacks in training datasets and hallucinated content in LLM outputs, demonstrating versatility across security and reliability threats.

Conclusion: Syndrome decoding provides a promising unified framework for enhancing the dependability of AI systems by simultaneously addressing security and reliability challenges in learning-based models.
...
### [9] [AlertBERT: A noise-robust alert grouping framework for simultaneous cyber attacks](https://arxiv.org/abs/2602.06534)
*Lukas Karner,Max Landauer,Markus Wurzenberger,Florian Skopik*

Main category: cs.CR

TL;DR: AlertBERT is a self-supervised, language model-based alert grouping framework that improves detection accuracy in noisy, large-scale networks by semantically clustering security alerts, outperforming traditional time-based methods.


<details>
  <summary>Details</summary>
Motivation: The high volume of security alerts, including many false positives, causes alert fatigue in SOCs, reducing analyst efficiency. Existing time-based grouping methods fail in complex, large-scale networks with simultaneous attacks and noise, necessitating a more robust and intelligent grouping approach.

Method: AlertBERT uses a self-supervised framework combining masked language models (MLMs) and density-based clustering to group security alerts based on semantic similarity rather than time proximity, enabling effective grouping in environments with high false positives and concurrent attacks.

Result: AlertBERT demonstrates superior accuracy in alert grouping compared to conventional time-based techniques, validated using a novel data augmentation method that simulates varying noise levels and concurrent attack scenarios.

Conclusion: AlertBERT provides a more accurate and scalable solution for alert grouping in large-scale, noisy network environments, outperforming conventional time-based methods and supporting both real-time and forensic analysis.
...
### [10] [Malicious Agent Skills in the Wild: A Large-Scale Security Empirical Study](https://arxiv.org/abs/2602.06547)
*Yi Liu,Zhihao Chen,Yanjun Zhang,Gelei Deng,Yuekang Li,Jianting Ning,Leo Yu Zhang*

Main category: cs.CR

TL;DR: This paper presents the first large-scale behavioral analysis of third-party LLM agent skills, revealing 157 malicious skills with 632 vulnerabilities across two main attack types—data theft and agent hijacking—showing that threats are systematic and sophisticated, often using undocumented 'shadow features' and platform exploits, with most malicious skills removed after responsible disclosure.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand and characterize the security threats posed by third-party agent skills, which execute with user privileges and are distributed with minimal vetting, leading to potential malicious exploitation.

Method: The authors constructed the first labeled dataset of malicious agent skills by behaviorally analyzing 98,380 skills from two community registries, verifying vulnerabilities through dynamic analysis and behavioral confirmation.

Result: The analysis confirmed 157 malicious skills containing 632 vulnerabilities, with malicious skills averaging 4.03 vulnerabilities across multiple kill chain phases; two main attack archetypes were identified—Data Thieves and Agent Hijackers—shadow features were found exclusively in advanced attacks, and responsible disclosure led to 93.6% removal within 30 days.

Conclusion: The study concludes that malicious third-party agent skills pose significant and intentional security threats, with a clear division into Data Thieves and Agent Hijackers, and highlights the importance of behavioral verification and transparency in securing agent ecosystems.
...
### [11] [A Survey of Security Threats and Trust Management in Vehicular Ad Hoc Networks](https://arxiv.org/abs/2602.06608)
*Rezvi Shahariar,Chris Phillips*

Main category: cs.CR

TL;DR: This survey classifies and analyzes trust models in VANETs, highlighting a dichotomy between receiver-side and sender-side trust evaluation, with the latter offering efficiency and faster decisions, and calls for more research on sender-side approaches despite deployment challenges.


<details>
  <summary>Details</summary>
Motivation: Traditional security mechanisms fail to address malicious insider attacks in VANETs; thus, trust management is essential. The motivation is to provide a structured overview of current trust models and highlight the underexplored potential of sender-side trust evaluation.

Method: The paper conducts a comprehensive literature survey, classifying and analyzing existing trust models in VANETs based on their trust evaluation point (receiver-side vs. sender-side), and uses a sequence diagram to compare the two approaches.

Result: The survey identifies two main classes of trust models in VANETs: receiver-side and sender-side. It reveals that most models use receiver-side evaluation, while only one exists for sender-side (with RSU arbitration in disputes), and demonstrates that sender-side models offload computation from receivers and support faster response.

Conclusion: The paper concludes by advocating for future research on sender-side trust evaluation in VANETs, highlighting its advantages in reducing receiver-side computational load and enabling faster decision-making, while emphasizing deployment challenges such as real-time constraints and efficiency.
...
### [12] [HYDRA: Unearthing "Black Swan" Vulnerabilities in LEO Satellite Networks](https://arxiv.org/abs/2602.06612)
*Bintao Yuan,Mingsheng Tang,Binbin Ge,Hongbin Luo,Zijie Yan*

Main category: cs.CR

TL;DR: HYDRA is a hypergraph-based framework for dynamic risk analysis in LEO satellite constellations that introduces Hyper-Bridge Centrality (HBC) to identify critical 'Black Swan' nodes at ground-space interfaces, which are topologically peripheral but structurally crucial, outperforming traditional centrality metrics in predicting cascading failures.


<details>
  <summary>Details</summary>
Motivation: Traditional topology-centric security analysis fails to capture systemic risks in LEO constellations caused by dynamic load imbalances and high-order dependencies, leading to underestimation of cascading failure risks, especially at marginal but critical ground-space interface nodes.

Method: Proposes HYDRA, a hypergraph-based framework with a novel metric Hyper-Bridge Centrality (HBC) that quantifies node criticality through a load-to-redundancy ratio within dependency structures; uses realistic StarLink TLE data and a population-based gravity model for simulation and validation.

Result: HBC outperforms traditional metrics like betweenness centrality in identifying critical nodes; simulations show that failures at ground-space interface nodes—despite being topologically peripheral—cause greater structural damage and are more likely to trigger network-wide cascades.

Conclusion: The study shifts the security paradigm from connectivity-centric to structural stress-centric analysis, emphasizing that the resilience of LEO constellations depends on securing edge interfaces, necessitating a rethinking of redundancy and defense strategies.
...
### [13] [Confundo: Learning to Generate Robust Poison for Practical RAG Systems](https://arxiv.org/abs/2602.06616)
...
### [14] [TrapSuffix: Proactive Defense Against Adversarial Suffixes in Jailbreaking](https://arxiv.org/abs/2602.06630)
*Mengyao Du,Han Fang,Haokai Ma,Gang Yang,Quanjun Yin,Shouling Ji,Ee-Chien Chang*

Main category: cs.CR

TL;DR: TrapSuffix is a lightweight, proactive defense against suffix-based jailbreak attacks on LLMs that forces attackers into detectable failure or traceable success, reducing attack success rates to below 0.01% while enabling 87.9% tracing accuracy with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Existing defenses against adversarial suffix attacks rely on passive detection and fail to exploit the defender's asymmetric advantage in controlling model behavior. The vast diversity of possible adversarial suffixes makes passive detection infeasible, prompting the need for a proactive, controllability-oriented defense.

Method: TrapSuffix uses lightweight fine-tuning to inject trap-aligned behaviors into the model, reshaping its response landscape to adversarial suffixes. It creates optimization traps that either cause attackers to fail or generate suffixes with unique, identifiable fingerprints, without altering inference or requiring additional detection modules.

Result: TrapSuffix reduces average attack success rates to below 0.01% across diverse suffix-based jailbreak methods and achieves an average tracing success rate of 87.9%. It adds no inference-time overhead and only 15.87 MB of memory cost, significantly less than LLM-based detection methods (~1e4 MB).

Conclusion: TrapSuffix provides a highly effective, low-cost defense by proactively manipulating model controllability, offering both strong resistance to jailbreaks and reliable attacker traceability while seamlessly integrating with existing defenses.
...
### [15] [Jamming Attacks on the Random Access Channel in 5G and B5G Networks](https://arxiv.org/abs/2602.06634)
*Wilfrid Azariah,Yi-Quan Chen,Zhong-Xin You,Ray-Guang Cheng,Shiann-Tsong Sheu,Binbin Chen*

Main category: cs.CR

TL;DR: This paper proposes and validates an analytical model showing that low-power Msg1 jamming can severely disrupt 5G/B5G random access, posing a critical security threat.


<details>
  <summary>Details</summary>
Motivation: To assess the security threat posed by RACH jamming in 5G and beyond networks by analyzing the effectiveness of Msg1 jamming attacks.

Method: An analytical model was developed to predict the impact of Msg1 jamming on RACH performance, and the model was validated using over-the-air experiments with OpenAirInterface (OAI)-based UE.

Result: Experimental results confirm that even low-power, stealthy Msg1 jamming significantly degrades RACH performance, effectively blocking legitimate user equipment from accessing the network.

Conclusion: Low-power and stealthy Msg1 jamming can effectively disrupt legitimate UE access in 5G/B5G networks, highlighting a significant security vulnerability.
...
### [16] [Wonderboom -- Efficient, and Censorship-Resilient Signature Aggregation for Million Scale Consensus](https://arxiv.org/abs/2602.06655)
*Zeta Avarikioti,Ray Neiheiser,Krzysztof Pietrzak,Michelle X. Yeo*

Main category: cs.CR

TL;DR: This paper presents Wonderboom, the first million-scale signature aggregation protocol for Ethereum, which enables finalization within a single slot (up to 32x faster) while improving security and scalability. It includes a new simulation tool and demonstrates robust performance with over 2M signatures per slot.


<details>
  <summary>Details</summary>
Motivation: The growing size of Ethereum's validator set (~1M) has led to slow block finalization (around 15 minutes) due to inefficiencies in signature aggregation and dissemination. This delay hinders real-world applicability and introduces exploitable security weaknesses in the current protocol.

Method: Wonderboom introduces a novel aggregation protocol designed for million-scale validator sets, leveraging efficient cryptographic techniques and optimized dissemination strategies. The authors also develop a large-scale simulation framework to empirically evaluate its performance and security.

Result: Wonderboom achieves signature aggregation and verification of over 2 million signatures within a single Ethereum slot, achieving up to 32x speedup compared to the current protocol, even under worst-case conditions. It also provides stronger security guarantees by mitigating stake-shifting attacks.

Conclusion: Wonderboom provides a scalable, secure, and efficient solution for signature aggregation in Ethereum, enabling block finalization within a single slot even with millions of validators, thereby addressing critical performance and security limitations of the current protocol.
...
### [17] [Evaluating and Enhancing the Vulnerability Reasoning Capabilities of Large Language Models](https://arxiv.org/abs/2602.06687)
*Li Lu,Yanjie Zhao,Hongzhou Rao,Kechi Zhang,Haoyu Wang*

Main category: cs.CR

TL;DR: This paper introduces DAGVul, a framework that improves the reliability of vulnerability detection in code by modeling reasoning as a Directed Acyclic Graph (DAG) and using Reinforcement Learning with Verifiable Rewards (RLVR) to align model reasoning with program logic, achieving state-of-the-art performance even with smaller models.


<details>
  <summary>Details</summary>
Motivation: Existing LLMs often produce correct vulnerability detection results based on hallucinated or superficial reasoning, which is not detectable using current coarse-grained benchmarks. There is a need for fine-grained evaluation of reasoning processes and better alignment with actual causal logic in code vulnerabilities.

Method: The authors construct a new benchmark with two components: (1) real-world vulnerabilities with expert-annotated causal reasoning as ground truth, and (2) semantically equivalent code perturbations to test reasoning robustness. They then propose DAGVul, which models vulnerability reasoning as a DAG generation task to capture causal dependencies, replacing standard chain-of-thought. Reinforcement Learning with Verifiable Rewards (RLVR) is used to train the model to align its internal reasoning with program semantics.

Result: Empirical evaluation reveals 12 systematic failure patterns in current LLMs. DAGVul achieves an average 18.9% improvement in reasoning F1-score over baselines. An 8B-parameter model outperforms larger and specialized reasoning models like Qwen3-30B-Reasoning and GPT-OSS-20B-High, and approaches the performance of Claude-Sonnet-4.5 (75.47% vs 76.11%).

Conclusion: By structuring reasoning as a DAG and using verifiable rewards, DAGVul significantly enhances the correctness and reliability of vulnerability detection reasoning in LLMs. The results set a new standard for efficient and logically consistent vulnerability analysis across model scales.
...
### [18] [Taipan: A Query-free Transfer-based Multiple Sensitive Attribute Inference Attack Solely from Publicly Released Graphs](https://arxiv.org/abs/2602.06700)
...
### [19] [GhostCite: A Large-Scale Analysis of Citation Validity in the Age of Large Language Models](https://arxiv.org/abs/2602.06718)
...
TL;DR: This paper introduces CiteVerifier, an open-source framework for large-scale citation verification, to assess the prevalence of hallucinated (ghost) citations in both LLM-generated text and real scientific literature. The study evaluates 13 LLMs across 40 research domains, revealing high citation hallucination rates (14.23% to 94.93%), and analyzes over 2.2 million citations from recent AI/ML and Security papers, finding 1.07% contain fabricated references, with a sharp increase in 2025. A researcher survey uncovers a 'verification gap' due to poor checking practices. The paper calls for systemic interventions to safeguard scientific integrity.


<details>
  <summary>Details</summary>
Motivation: Citations underpin scientific credibility, but LLMs' tendency to generate fake citations threatens research integrity. This risk is compounded by insufficient human verification and peer review practices. The study aims to quantify this growing problem across AI-generated content and actual publications to inform mitigation strategies.

Method: The authors develop CiteVerifier, an open-source citation verification framework, and conduct three main experiments: (1) Benchmarking 13 state-of-the-art LLMs on citation generation across 40 research domains; (2) Analyzing 2.2 million citations from 56,381 papers published at top AI/ML and Security venues between 2020 and 2025; (3) Surveying 97 researchers (analyzing 94 valid responses) on citation behaviors and attitudes toward suspicious references.

Result: All 13 LLMs hallucinate citations, with rates ranging from 14.23% to 94.93%, varying significantly by domain. Among real publications, 1.07% of papers (604 total) contain invalid or fabricated citations, with an 80.9% increase observed in 2025 alone. The survey reveals that 41.5% of researchers copy-paste BibTeX without verification, 44.4% take no action when encountering suspicious citations, 76.7% of reviewers do not thoroughly check references, and 80.0% never suspect fake citations.

Conclusion: The convergence of unreliable LLMs and inadequate human oversight creates a growing crisis in citation integrity, allowing fabricated references to infiltrate the scientific record. The paper calls for coordinated interventions: researchers must verify citations rigorously, venues should enforce stricter scrutiny during peer review, and tool developers should integrate verification mechanisms like CiteVerifier into writing and publishing workflows.
...
### [20] [Beyond Function-Level Analysis: Context-Aware Reasoning for Inter-Procedural Vulnerability Detection](https://arxiv.org/abs/2602.06751)
...
TL;DR: CPRVul is a context-aware vulnerability detection framework that improves detection accuracy by intelligently selecting relevant inter-procedural context via LLM-generated security profiles and enhancing LLM reasoning through structured trace fine-tuning, achieving state-of-the-art results across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing vulnerability detection methods operate at the function level without sufficient inter-procedural context, while simply adding raw context often harms performance due to length, redundancy, and noise. There is a need for context-aware yet selective and structured approaches.

Method: CPRVul uses a code property graph to extract candidate context, employs an LLM to generate security-focused profiles and score context relevance, selects high-impact context fitting within the model's context window, and integrates the target function, selected context, and metadata to produce reasoning traces for fine-tuning LLMs in vulnerability detection.

Result: CPRVul achieves accuracies between 64.94% and 73.76% across PrimeVul, TitanVul, and CleanVul datasets, outperforming UniXcoder (56.65%-63.68%) and prior state-of-the-art on PrimeVul (improving from 55.17% to 67.78%, a 22.9% increase). Ablation studies confirm that performance gains require both processed context and structured reasoning.

Conclusion: CPRVul demonstrates that combining context profiling, selective contextual element inclusion, and structured reasoning significantly improves vulnerability detection performance over function-level and raw context-based approaches, establishing a new state-of-the-art on multiple benchmarks.
...
### [21] [A Unified Framework for LLM Watermarks](https://arxiv.org/abs/2602.06754)
...
### [22] [$f$-Differential Privacy Filters: Validity and Approximate Solutions](https://arxiv.org/abs/2602.06756)
*Long Tran,Antti Koskela,Ossi Räisä,Antti Honkela*

Main category: cs.CR

TL;DR: This paper investigates privacy filters in the context of fully adaptive composition in differential privacy, particularly focusing on $f$-DP. It shows that the natural approach to constructing $f$-DP filters by composing trade-off functions is invalid in general, characterizes the conditions for validity, and proposes an approximate Gaussian DP filter for subsampled Gaussian mechanisms with improved performance over Rényi DP-based filters at extreme sampling rates.


<details>
  <summary>Details</summary>
Motivation: In differential privacy, especially under fully adaptive composition where mechanisms and parameters depend on prior outputs, ensuring that the global privacy budget is not exceeded is critical. Privacy filters serve as stopping rules for compositions, but it is unclear whether optimal $f$-DP-based frameworks support valid filters in such adaptive settings. This work addresses that gap.

Method: The authors analyze the natural approach of composing individual trade-off functions to form an $f$-DP filter, proving its invalidity in general. They derive necessary and sufficient conditions for validity and develop a fully adaptive central limit theorem for $f$-DP. Using this, they construct an approximate Gaussian DP filter for subsampled Gaussian mechanisms, applicable at both small ($q < 0.2$) and large ($q > 0.8$) sampling rates.

Result: The study demonstrates that directly composing trade-off functions to form an $f$-DP filter fails under full adaptivity, and characterizes precisely when such a filter is valid. The proposed approximate Gaussian DP filter provides tighter privacy bounds than existing Rényi DP-based methods in the specified sampling regimes.

Conclusion: The paper establishes theoretical limits on the validity of natural $f$-DP filters under full adaptivity and provides a practical, improved alternative via an approximate Gaussian DP filter for common subsampling scenarios, advancing the understanding and application of adaptive composition in differential privacy.
...
### [23] ["Tab, Tab, Bug'': Security Pitfalls of Next Edit Suggestions in AI-Integrated IDEs](https://arxiv.org/abs/2602.06759)
*Yunlong Lyu,Yixuan Tang,Peng Chen,Tian Dong,Xinyu Wang,Zhiqiang Dong,Hao Chen*

Main category: cs.CR

TL;DR: This paper presents the first systematic security study of Next Edit Suggestions (NES) in AI-integrated IDEs, uncovering new threat vectors such as context poisoning through expanded context retrieval. Through technical analysis, lab evaluation, and a developer survey, it demonstrates that NES is vulnerable and that developers are largely unaware of these risks.


<details>
  <summary>Details</summary>
Motivation: While existing research has focused on the security of LLM-based code generation in isolation, the emerging paradigm of Next Edit Suggestions (NES) introduces novel and understudied attack surfaces due to its dynamic context retrieval from user actions and the global codebase. This work is motivated by the need to systematically investigate the security implications of NES, which are currently not well understood.

Method: The study employs a three-pronged approach: (1) a technical dissection of NES mechanisms to identify new threat vectors, (2) an in-lab evaluation assessing NES's susceptibility to context poisoning and sensitivity to user interactions, and (3) a large-scale online survey of over 200 professional developers to gauge real-world risk perception.

Result: The analysis reveals that NES systems retrieve broader and potentially vulnerable contexts, making them susceptible to context poisoning attacks. Lab evaluations confirm sensitivity to transactional edits and human-IDE interactions. The developer survey shows a widespread lack of awareness about NES-related security risks, indicating a critical gap in both tooling and education.

Conclusion: This paper concludes that Next Edit Suggestions (NES) in modern AI-integrated IDEs introduce significant security risks due to expanded context retrieval and complex interaction patterns, with developers largely unaware of these threats. It calls for improved security mechanisms and greater awareness to mitigate context poisoning and other vulnerabilities.
...
### [24] [Next-generation cyberattack detection with large language models: anomaly analysis across heterogeneous logs](https://arxiv.org/abs/2602.06777)
*Yassine Chagna,Antal Goldschmidt*

Main category: cs.CR

TL;DR: The paper introduces LogAtlas, a framework for anomaly detection in heterogeneous log sources using large language models, featuring novel datasets, revised evaluation metrics, and an efficient two-phase training approach.


<details>
  <summary>Details</summary>
Motivation: Traditional intrusion detection systems face challenges including high false positives, inability to understand log semantics, and lack of high-quality, annotated datasets due to data sensitivity and scarcity. This work aims to overcome these limitations using LLMs.

Method: The authors propose: (1) two new datasets—LogAtlas-Foundation-Sessions and LogAtlas-Defense-Set—with balanced, privacy-preserving, and attack-annotated logs; (2) a critique of traditional evaluation metrics like F1 and accuracy in security contexts; and (3) a two-phase framework where a 3B-parameter model (Base-AMAN) learns log semantics, followed by knowledge distillation into a lightweight 0.5B-parameter model (AMAN) for real-time detection.

Result: The framework achieves inference times of 0.3–0.5 seconds per session with operational costs under 50 USD per day, demonstrating efficiency and scalability. Benchmarking reveals flaws in standard metrics and shows improved detection performance.

Conclusion: The proposed LLM-based approach with specialized datasets and evaluation practices offers a practical and effective solution for anomaly detection in real-world, heterogeneous log environments.
...
### [25] [Plato's Form: Toward Backdoor Defense-as-a-Service for LLMs with Prototype Representations](https://arxiv.org/abs/2602.06887)
*Chen Chen,Yuchen Sun,Jiaxin Gao,Yanwen Jia,Xueluan Gong,Qian Wang,Kwok-Yan Lam*

Main category: cs.CR

TL;DR: PROTOPURIFY is a reusable, minimal-assumption backdoor purification framework for LLMs that detects and removes backdoors via prototype-based parameter editing, enabling practical Backdoor Defense-as-a-Service.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor defenses are hard to deploy in real-world services due to stringent requirements on clean data, known triggers, or domain knowledge, and lack reusability and scalability for Backdoor Defense-as-a-Service (BDaaS).

Method: PROTOPURIFY constructs a backdoor vector pool from clean and backdoored model pairs, forms candidate prototypes, selects the most similar prototype for the target model via similarity matching, identifies the boundary layer through layer-wise alignment, and performs targeted purification by suppressing prototype-aligned components in affected layers.

Result: PROTOPURIFY reduces attack success rate (ASR) to below 10% (as low as 1.6%) across various LLMs and tasks, with less than 3% drop in clean accuracy. It outperforms six existing defenses against six diverse backdoor attacks and shows robustness to adaptive variants and stability on clean models.

Conclusion: PROTOPURIFY is a practical, BDaaS-ready backdoor purification framework that enables reusable, scalable, and fine-grained defense with minimal benign utility loss.
...
### [26] [TamperBench: Systematically Stress-Testing LLM Safety Under Fine-Tuning and Tampering](https://arxiv.org/abs/2602.06911)
...
TL;DR: TamperBench is the first unified framework for systematic evaluation of tamper resistance in open-weight LLMs, integrating diverse attacks, standardized metrics, and hyperparameter sweeps to assess safety and utility across 21 models, revealing key insights on vulnerabilities and defenses.


<details>
  <summary>Details</summary>
Motivation: The lack of standardized evaluation methods for tamper resistance in open-weight LLMs hinders comparison of safety, utility, and robustness under various tampering threats, necessitating a unified and reproducible benchmark.

Method: The authors introduce TamperBench, a unified evaluation framework that integrates state-of-the-art weight-space and latent-space attacks, performs systematic hyperparameter sweeps for realistic adversarial evaluation, and assesses both safety and utility across model variants with standardized metrics.

Result: Evaluation of 21 open-weight LLMs across nine tampering threats reveals that post-training affects tamper resistance, jailbreak-tuning is the most severe attack, and Triplet is a top-performing alignment-stage defense; the framework ensures minimal setup cost and full reproducibility.

Conclusion: TamperBench provides a standardized, reproducible framework for evaluating LLM tamper resistance, enabling comparative analysis across models and defenses, with findings highlighting the effectiveness of Triplet as a leading defense and the significant risk posed by jailbreak-tuning.
