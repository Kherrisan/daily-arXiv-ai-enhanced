...
TL;DR: This paper presents zk-MCP, a framework for agent communication that maintains privacy while enabling verifiable audits using zero-knowledge proofs, ensuring data authenticity without compromising confidentiality.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of achieving auditability in agent communication without sacrificing privacy, which is crucial for applications involving billing, compliance, and accountability in regulated domains.

Method: The framework combines zero-knowledge proofs with the Model Context Protocol (MCP) to verify messages without disclosing their contents. It introduces asynchronous audit verification compatible with MCP, enabling mutual audits of message formats, types, and quality without revealing specifics.

Result: The authors demonstrate the framework's implementation, including zero-knowledge proof generation via Circom and integration with MCP's bidirectional channels. zk-MCP achieves efficient verification with minimal latency, providing mutual auditing capabilities while preserving privacy.

Conclusion: zk-MCP is the first privacy-preserving audit system for agent communications, offering verifiable mutual auditing without exposing message content, setting a new standard for secure, auditable agent interactions in privacy-sensitive environments.
...
TL;DR: The paper addresses backdoor attacks on large language models (LLMs), focusing on how these attacks persist through continual fine-tuning. It introduces the P-Trojan algorithm, which ensures over 99% persistence of backdoors while maintaining accuracy. The work highlights the need for more robust defenses in model adaptation pipelines.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor attacks against LLMs are not resilient to continual fine-tuning after deployment, as empirical evidence shows their effectiveness diminishes. This undermines their threat potential in real-world scenarios where models are continuously adapted, thus necessitating research into persistent backdoor strategies.

Method: The authors propose P-Trojan, an attack algorithm designed for multi-stage post-deployment continual fine-tuning. The method aligns poisoned gradients with clean-task gradients on token embeddings, aiming to prevent the backdoor from being suppressed during subsequent updates. Theoretical analysis backs up the method's feasibility, supported by experiments on models like Qwen2.5 and LLaMA3.

Result: Experiments showed that P-Trojan achieves more than 99% backdoor persistence under diverse continual fine-tuning conditions while accurately maintaining clean-task performance. This confirms the algorithm's effectiveness in making backdoors durable through real-world model adaptations.

Conclusion: The paper's findings emphasize potential security threats posed by persistent backdoors in post-deployment settings. Researchers and practitioners should consider persistence-aware evaluations and enhance defensive mechanisms in the model adaptation pipelines to address these risks.
...
TL;DR: O-RAN cybersecurity framework with quantum computing for intrusion detection in RANs.


<details>
  <summary>Details</summary>
Motivation: O-RAN's modularity increases attack surface, so cybersecurity solutions are needed to protect control, user, and management planes.

Method: Three-layer hierarchical defense with anomaly detection, intrusion confirmation, and classification, using quantum computing and ML with specific encodings and classifiers.

Result: Benchmarking on synthetic/real telemetry shows high accuracy, recall, and separability; robustness confirmed by analysis of decision boundaries and latent space geometry.

Conclusion: Framework offers scalable deployment in O-RAN with robust, interpretable diagnostics and readiness for slice-aware applications.
...
TL;DR: Untrusted decomposition models in Factor(U,T) cannot be reliably monitored through decompositions alone - implementation monitoring of child tasks achieves robust safety detection (AUROC 0.96 vs decomposition-only 0.52).


<details>
  <summary>Details</summary>
Motivation: Previous trust-based decomposition limits usefulness for complex tasks where decomposition itself is difficult. The work addresses the gap in detecting malicious activity when using powerful but potentially untrusted decomposition models.

Method: The research introduces Factor(U,T), which uses an untrusted decomposer model for complex task decomposition, then evaluates security through monitors analyzing either decompositions or complete implementations. Evaluation is conducted on BigCodeBench using AUROC metrics to measure monitor effectiveness.

Result: Monitors analyzing decompositions (AUROC 0.52) perform poorly compared to those analyzing implementations (AUROC 0.96). Factor(D,U) achieves 96% AUROC and 1.2% attack success rate, demonstrating that implementation monitoring effectively prevents malicious behavior.

Conclusion: Monitoring the actual implementation of child tasks, rather than relying solely on decomposition checks, proves significantly more effective in detecting malicious activity. The study concludes that implementation-context monitoring should be prioritized for safety in protocols using untrusted decomposers.
...
TL;DR: BlindSpot enables real-time bystander privacy control via gesture, VLC, and UWB signals, validated through smartphone-based testing that confirms their feasibility and performance trade-offs.


<details>
  <summary>Details</summary>
Motivation: Current camera-equipped mobile devices lack real-time mechanisms for bystanders to control their visibility in photos/videos, creating significant privacy risks.

Method: The study designs and evaluates three signaling modalities (hand gestures, improved VLC protocol, and novel UWB protocol) with geometric consistency checks to validate signals and defend against impersonation attacks.

Result: Implementation and evaluation on a smartphone confirm the feasibility of all three modalities, with detailed analysis of accuracy, latency, and robustness across distances, lighting conditions, and user movements.

Conclusion: BlindSpot demonstrates the feasibility of novel bystander signaling techniques using three modalities, highlighting their performance trade-offs and effectiveness in managing real-time privacy controls.
...
TL;DR: The paper introduces a copula-based analysis method to understand and quantify the interactions between safety and security in CAVs, using dynamic risk functions and Weibull distributions to model threats and hardware failures, with a joint failure model for coupled system analysis.


<details>
  <summary>Details</summary>
Motivation: Current research lacks a theoretical framework to analyze the interaction mechanisms between safety and security in connected and automated vehicles (CAVs), making co-design of these domains challenging.

Method: The authors develop a copula-based joint safety-security analysis method, combining time-varying cyberattack modeling with hardware failure modeling (Weibull distribution), and propose a joint failure model with elliptical and Archimedean copulas to assess system-level interactions.

Result: The study identifies key insights such as monotonic relationships between joint failure probability and dependence, the role of defensive responses in mitigation, and quantifies dynamic coupling strengths. Simulations test sensitivity to parameters and deployment timing.

Conclusion: This approach offers a quantifiable theoretical foundation for co-design of safety and security in CAVs, highlighting how cyberattacks and functional faults mutually affect each other under dependence structures.
...
TL;DR: This paper addresses the inheritability of vulnerabilities in the pretrain-to-finetune paradigm of large language models and proposes a new attack method (PGP) with strong transfer success.


<details>
  <summary>Details</summary>
Motivation: This paper is motivated by the increasing reliance on finetuned large language models (LLMs) and the lack of understanding regarding whether these models inherit vulnerabilities from their pretrained counterparts, specifically in terms of jailbreak vulnerabilities and transferability of adversarial prompts.

Method: The authors investigate a pretrain-to-finetune threat model where the attacker has white-box access to the pretrained model but only black-box access to the finetuned ones, using empirical analysis and representation-level probing. The Probe-Guided Projection (PGP) attack is introduced, which leverages the transferability of prompts in pretrained representations.

Result: The study shows that adversarial prompts are highly transferable from the pretrained to the finetuned LLMs. The PGP attack demonstrates strong transfer success across multiple LLM families and various finetuned tasks, confirming the inheritability of vulnerabilities.

Conclusion: The paper concludes that finetuned LLMs inherit vulnerabilities from their pretrained versions, highlighting significant security risks. The PGP method is presented as a concerning demonstration of this vulnerability, urging the community to address the security challenges inherent in the pretrain-to-finetune paradigm.
...
TL;DR: This paper addresses the vulnerability of code-watermarking techniques to comment removal attacks by introducing a Cue List-based approach to enhance watermark detectability and usability.


<details>
  <summary>Details</summary>
Motivation: Existing code-watermarking methods fail against comment removal attacks and struggle with code's low entropy. This limits their effectiveness in protecting intellectual property in LLM-generated code.

Method: The authors leverage a Cue List to identify high/low-entropy code regions and inject watermarks strategically, avoiding areas vulnerable to attacks while maintaining code functionality.

Result: Evaluation on HumanEval shows the proposed method outperforms three state-of-the-art approaches in watermark detection and usability under comment removal attacks.

Conclusion: The Cue List-guided method provides a robust solution for code watermarking by effectively balancing detectability, attack resistance, and code usability.
...
TL;DR: This paper introduces a secure, model-free method to calculate feature contributions in vertical federated learning using encrypted set intersections and Shapley-CMI. The system allows parties to fairly assess data value without sharing raw data, enabling secure and scalable cross-party collaboration.


<details>
  <summary>Details</summary>
Motivation: In VFL scenarios, parties hold complementary features for shared users and need to assess data contributions before collaborative modeling. Existing model-free methods like Shapley-CMI lack practical implementations that preserve privacy during valuation, creating a critical gap in secure, pre-training feature evaluation.

Method: The authors propose a system using a private set intersection (PSI) server to compute encrypted intersection sizes across discretized and encrypted ID groups. This allows parties to calculate Shapley-CMI values for feature contributions without raw data exchange, relying on information-theoretic principles and secure computation techniques.

Result: Experiments validate the correctness and privacy guarantees of the proposed system. The approach securely computes feature contributions in VFL settings, maintains data confidentiality, scales across multiple participants, and avoids raw data exchange or model training.

Conclusion: The paper successfully develops a privacy-preserving implementation of the Shapley-CMI method for Vertical Federated Learning (VFL), demonstrating its effectiveness in fairly and securely estimating feature contributions without raw data sharing or model training. The system leverages a PSI server to compute encrypted intersections, ensuring data confidentiality while enabling scalable, secure feature valuation.
...
TL;DR: This paper identifies the shifting skill requirements for Philippine cybersecurity graduates, advocating for a blend of technical, soft, and ethical skills over traditional technical expertise alone to meet industry demands.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the need to understand evolving skill requirements for entry-level cybersecurity roles in the Philippines, highlighting gaps between traditional education and the industry's growing emphasis on interdisciplinary competencies like ethics, adaptability, and communication.

Method: A descriptive cross-sectional research design was employed, analyzing Philippine job listings and surveys of students, teachers, and cybersecurity professionals to identify required skills and competencies.

Result: The study found consensus on the importance of communication, critical thinking, problem-solving, and adaptability, though these were prioritized differently. Business acumen was deemed less critical for entry-level roles, with a clear push toward technical-ethical balance and continuous learning.

Conclusion: The study concludes that new cybersecurity professionals should develop a diverse skillset combining technical knowledge, soft skills (e.g., communication, critical thinking), and ethical competencies. Traditional technical skills alone are insufficient, emphasizing adaptability, continuous learning, and alignment with evolving industry demands.
...
TL;DR: The paper introduces MALCDF, a framework of four LLM agents for real-time cyber defense, using secure, ontology-aligned messaging. It outperforms ML baselines and single-LLM systems in accuracy and consistency.


<details>
  <summary>Details</summary>
Motivation: The motivation is driven by the limitations of traditional, centralized security tools in addressing adaptive and multi-vector attacks.

Method: The method involves four LLM agents (Detection, Intelligence, Response, Analysis) working in real time, using a Secure Communication Layer with encrypted, ontology-aligned messages, and evaluated on a 50-record live-stream dataset from CICIDS2017.

Result: The results show MALCDF achieves 90.0% detection accuracy, 85.7% F1-score, and 9.1% false-positive rate with 6.8s average latency, outperforming a lightweight ML-IDS baseline and a single-LLM setup.

Conclusion: The conclusion suggests that coordinating LLM agents with secure messaging can enhance real-time cyber defense, demonstrating practical improvements over traditional and single-agent approaches.
...
TL;DR: The paper proposes an AI-Augmented Security Operations Center (AISOC) on AWS using cloud-native instrumentation and ML-based detection. It demonstrates effective threat triage with high F1 scores using a fusion of two classifiers. The approach is suitable for cost-sensitive cloud environments.


<details>
  <summary>Details</summary>
Motivation: Cloud Security Operations Centers (SOCs) face challenges in triaging large volumes of heterogeneous telemetry from dynamic cloud environments within budget constraints. There is a need for efficient and cost-effective methods to enhance threat detection and triage in such settings.

Method: The authors implemented an AISOC on AWS with three EC2 instances (Attacker, Defender, Monitoring). They simulated a reverse-shell intrusion using Metasploit and leveraged Filebeat, Elasticsearch, and Kibana for log analysis. The solution uses two classifiers: a malware detector trained on a public dataset and a log-anomaly detector trained on synthetically augmented logs. The scores from these classifiers are calibrated and fused to triage threats into three categories. 

Result: The fusion model achieved a macro-F1 score of up to 1.00 in controlled environments, indicating strong performance. However, performance may vary in more complex and noisy real-world scenarios. The results suggest that calibrated fusion models can improve threat triage in cost-sensitive cloud environments.

Conclusion: The paper concludes that the simple yet effective fusion of calibrated classifiers can enhance the capabilities of cloud SOCs in detecting and triaging threats within budget constraints. It highlights the potential of AI-augmented approaches for improving security operations in cloud environments.
...
TL;DR: This paper proposes a machine learning-based intrusion detection system for the Internet of Vehicles (IoV) to identify malicious CAN bus traffic, focusing on attacks like DoS and spoofing. Experiments using the CiCIoV2024 dataset confirm clear structural differences between attack types and benign data, enabling effective classification.


<details>
  <summary>Details</summary>
Motivation: The IoV's enhanced connectivity introduces vulnerabilities to cyber-attacks (e.g., DoS, message spoofing), threatening vehicle safety and critical systems. Existing mitigation strategies lack robust classification of diverse attack vectors targeting parameters like gas pedal and steering.

Method: The study employs machine learning to classify CAN bus traffic, analyzing attack patterns such as DoS,Spoofing GAS,RPM,Speed, and Steering attacks in the CiCIoV2024 dataset. Differences in traffic patterns between benign and malicious activities are exploited for multi-class classification.

Result: Clear structural differences were observed between attack types (e.g., spoofing) and benign data, confirming the feasibility of using machine learning for multi-class intrusion detection in IoV systems.

Conclusion: The findings establish a robust foundation for developing machine learning models to detect and classify malicious CAN bus traffic, addressing IoV security challenges through attack-specific pattern analysis.
...
TL;DR: SEBERTIS is a framework that trains classifiers to detect security-related GitHub issues by fine-tuning transformer models on semantically equivalent words (Semantic Surrogates), outperforming ML and LLM baselines by significant margins in detection accuracy.


<details>
  <summary>Details</summary>
Motivation: Accurate prioritization of security issues in software maintenance is crucial. Existing techniques rely on lexical cues and fail for complex submissions, leading to poor detection rates.

Method: SEBERTIS trains DNNs by fine-tuning BERT-like transformers as MLMs on a series of semantically equivalent words (Semantic Surrogates) that replace tokens during training. This trains the model to focus on semantic understanding rather than lexical patterns.

Result: SEBERTIS achieves 0.9880 F1-score on 10,000 GitHub reports, with improvements of 14-97%, 15-93%, and 15-95% over ML baselines for precision, recall, and F1 respectively. It also outperforms LLM baselines by 23-64%, 37-86%, and 39-75% improvements in those metrics.

Conclusion: SEBERTIS offers a promising approach for detecting security-related issues in issue reports by learning semantic representations rather than lexical patterns, achieving state-of-the-art performance that meets practical application requirements.
...
TL;DR: The paper presents ScamSweeper, a framework for detecting web3 scams on Ethereum by leveraging dynamic transaction graph analysis with a structure temporal random walk and variational Transformer.


<details>
  <summary>Details</summary>
Motivation: Web3 scams are increasingly sophisticated and previous methods fail to address their unique features. Current phishing detection tools struggle with the scale and temporal aspects of Ethereum transaction networks which follow a power-law distribution. This necessitates a more effective solution for web3 scam detection.

Method: ScamSweeper is introduced to detect web3 scams by emphasizing the dynamic evolution of transaction graphs. The method uses structure temporal random walk for network sampling, followed by a directed graph encoder to extract features from subgraphs based on temporal intervals. A variational Transformer is then used to extract dynamic evolution from the sequence. It also uses a comprehensive dataset of Ethereum transactions spanning 18 million blocks to analyze web3, phishing, and normal accounts in terms of nodes, edges, and degree distribution.

Result: Experiments show that ScamSweeper outperforms existing detection frameworks. For web3 scam detection, it achieves a minimum 17.29% improvement in weighted F1-score over the next best baseline (SIEGE with 0.59 baseline). In phishing detection, there is at least 17.5% improvement in F1-score over BERT4ETH, starting from 0.80.

Conclusion: The paper concludes that ScamSweeper is an effective novel framework for detecting web3 scams. It introduces a structure-aware temporal sampling method, and the use of a variational Transformer for subgraph sequence analysis leads to significant improvements in detection performance when compared to existing approaches. Thus, it is a suitable solution to handle the unique challenges presented by large-scale, time-evolving Ethereum transaction networks where scams are prevalent and difficult to detect.
...
TL;DR: The paper presents RELIC-GNN, a graph neural network method for identifying state registers in gate-level netlists, showing improved performance over previous methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods for state register identification in netlists are inefficient for large-scale designs, necessitating a more efficient approach.

Method: RELIC-GNN models register paths as graphs and uses a GNN with node attributes and structural features to identify state registers efficiently.

Result: RELIC-GNN achieved 100% recall, 30.49% precision, and 88.37% accuracy on average across different designs, outperforming previous methods.

Conclusion: The proposed RELIC-GNN method effectively addresses the inefficiency of previous approaches and improves state register identification in large-scale gate-level netlists.
...
TL;DR: The paper introduces APT-ClaritySet, a standardized dataset for APT research that addresses data scarcity, inconsistent actor aliases, and redundant samples to improve reproducibility and quantitative analysis.


<details>
  <summary>Details</summary>
Motivation: Large-scale, standardized datasets for APT research are scarce, and inconsistent actor aliases and redundant samples hinder reproducibility.

Method: The authors present APT-ClaritySet, which includes a full dataset, a deduplicated version (APT-ClaritySet-Unique), and function-reuse clusters (APT-ClaritySet-FuncReuse). They use alias normalization to reconcile inconsistent names and apply graph-feature deduplication to reduce redundancy while preserving distinct behavioral variants.

Result: APT-ClaritySet-Full contains 34,363 malware samples, APT-ClaritySet-Unique includes 25,923 unique samples after deduplication, and APT-ClaritySet-FuncReuse comprises 324,538 function-reuse clusters. The deduplication process reduced the executable dataset by 47.55% while retaining distinct variants, and alias normalization resolved 11.22% of inconsistencies.

Conclusion: The paper provides a high-fidelity, reproducible foundation for quantitative APT studies by releasing APT-ClaritySet and its construction pipeline, enabling research on APT patterns, evolution, and attribution.
...
TL;DR: The paper introduces a decision-oriented framework to guide the deployment of safeguards for LLM-based systems, using financial risk metrics to enable cost-effective security decisions. It quantifies the effectiveness of ABAC, NER redaction, and NeMo Guardrails in mitigating attacks on a RAG service, showing ABAC as the most effective with an RoC of 9.83.


<details>
  <summary>Details</summary>
Motivation: Practitioners need quantitative guidance to determine which LLM safeguards are worth deploying, especially in security-critical environments. This framework allows for monetary comparisons of different defense strategies.

Method: The paper uses a RAG service with the DeepSeek-R1 model over a synthetic PII corpus, subjected to automated attacks across five vulnerability classes. Attack success probabilities are estimated using Laplace's Rule of Succession and Monte Carlo simulations involving loss triangle distributions to estimate financial risks and RoC metrics for different controls.

Result: ABAC showed the highest RoC (9.83), significantly reducing total expected losses. NER redaction achieved a good RoC (5.97), while NeMo Guardrails provided minimal benefit. The baseline RAG had very high attack success rates (>=0.98) and a simulated expected loss of $313k.

Conclusion: Quantifiable financial metrics and risk assessment can help practitioners make cost-effective security decisions for LLM-based systems. ABAC is highly effective for PII and prompt-related attacks, NER redaction for PII, but NeMo Guardrails offers limited benefits in the tested context.
...
TL;DR: MCPZoo is a large-scale accessible dataset of 90,146 MCP servers to support research on security analysis.


<details>
  <summary>Details</summary>
Motivation: Empirical research into Model Context Protocol (MCP) is limited by the absence of large-scale accessible datasets.

Method: The study aggregated servers from multiple public sources and verified runnability and interactivity through deployment, with unified metadata and interfaces provided.

Result: MCPZoo includes over 90,000 servers with thousands verified as deployable, interactable, and ready for experimental use.

Conclusion: MCPZoo offers an open and comprehensive resource to facilitate and enhance further empirical investigations in MCP-based security research.
...
TL;DR: A paper proposing a framework combining Transformer models and AlphaZero-style MCTS for automated ATT&CK kill-chain reconstruction, showing improved semantic fidelity over baselines.


<details>
  <summary>Details</summary>
Motivation: Current attack reports lack details on kill chain phases and dependencies, making automated reconstruction challenging. Threat analysts need better frameworks to infer complete kill chains from sparse data.

Method: The method couples phase-conditioned semantic priors from Transformer models with a symbolic Markov Decision Process and AlphaZero-style Monte Carlo Tree Search guided by a Policy-Value Network to infer seven-phase kill chains. A multi-objective reward function ensures semantic relevance, phase cohesion, and transition plausibility.

Result: The framework outperforms Transformer baselines in semantic fidelity and operational coherence for FIN6, APT24, and UNC1549 intrusions, aligning with expert TTPs.

Conclusion: The paper concludes that combining contextual embeddings with search-based decision-making is a practical approach for automated, interpretable kill-chain reconstruction, advancing cyber defense capabilities.
...
TL;DR: This paper presents SCALM, an LLM-powered framework for detecting bad practices in Ethereum smart contracts. It introduces a hybrid architecture and multi-layer verification system, showing superior performance in extensive experiments.


<details>
  <summary>Details</summary>
Motivation: With Ethereum's growth, maintaining high standards in smart contract practices is crucial. Bad practices increase risk, necessitating a study and detection framework.

Method: SCALM combines context-aware function-level slicing with semantic reasoning via vectorized pattern matching. It also uses a multi-layer verification system linking low-level patterns to security principles through syntax, design patterns, and architecture analysis.

Result: Experiments with various LLMs and datasets demonstrate SCALM's superior effectiveness in detecting bad practices compared to existing tools.

Conclusion: SCALM offers an innovative, effective solution for identifying and mitigating bad practices in Ethereum smart contracts, advancing security and standards in the ecosystem.
...
TL;DR: Bounty Hunter is an open-source plugin for Caldera enabling automated, autonomous adversary emulation with broad tactics coverage and multi-faceted behavior, reducing reliance on manual processes.


<details>
  <summary>Details</summary>
Motivation: Current adversary emulation methods remain manual, labor-intensive, and limited in autonomy/tactics coverage despite rising LLMs, hindering efficient security assessments.

Method: Developed Bounty Hunter as a Caldera plugin using diverse adversarial behaviors (e.g., variable detectability, dynamic attack paths) for autonomous emulation across pre-compromise, initial, and post-compromise phases without target knowledge.

Result: Demonstrated autonomous objective-achievement in simulated enterprise networks through repeated emulations with varied attack strategies, achieving comprehensive tactics coverage.

Conclusion: Bounty Hunter addresses automation gaps in cybersecurity, enabling scalable, realistic security assessments/training while advancing intrusion detection research through reproducible attack scenarios.
...
TL;DR: This paper demonstrates that airgapped embedded systems can be wirelessly infiltrated without hardware modification, using parasitic RF sensitivity in PCB traces and ADCs to receive external signals, enabling command-and-control over tens of meters with data rates up to 100 kbps.


<details>
  <summary>Details</summary>
Motivation: To address the critical need for airgap security by highlighting a new vulnerability that allows wireless infiltration of isolated systems without hardware modification, thereby challenging existing assumptions about their inherent isolation.

Method: The authors conducted experiments on twelve commercial embedded devices and two custom prototypes to identify configurations that allow parasitic RF reception via PCB traces and on-chip ADCs. They used a systematic methodology to evaluate the feasibility of wireless data reception under various conditions, including non-line-of-sight and signal strength.

Result: The paper found that unmodified embedded devices can receive radio signals in the 300-1000 MHz range with as little as 1 mW of signal power. Data reception was successfully demonstrated over tens of meters, with data rates up to 100 kbps even in non-line-of-sight conditions.

Conclusion: The work reveals a new, previously unexplored command-and-control vector for airgapped systems, demonstrating that physical isolation does not guarantee security and that existing defenses must evolve to address these unexpected RF sensitivities.
...
TL;DR: The study compares classical methods and deep learning (VGG16, DenseNet121) for detecting packed executables using grayscale byte plots. CNNs outperformed classical methods with high precision and recall, showing robust generalization against unknown packers.


<details>
  <summary>Details</summary>
Motivation: Detecting packed executables is critical in malware analysis, as packing obscures code and hinders static inspection. Existing methods have limitations in accuracy and computational efficiency.

Method: The study transforms binary executables into grayscale byte plots and employs CNNs (VGG16, DenseNet121) for classification of packed vs. non-packed binaries. It compares these deep learning approaches with classical models using Gabor jet features on a diverse dataset of PE files packed with various packers.

Result: CNNs outperformed classical methods with well-balanced precision, recall, and F1-scores. DenseNet121 showed higher precision and lower false positives, while VGG16 had slightly higher recall. Both models generalized well to unknown packers.

Conclusion: Grayscale byte plots combined with CNNs offer a reliable approach for detecting packed malware, enhancing malware analysis pipelines and automated antivirus inspection due to their high performance, complementary strengths, and robust generalization.
...
TL;DR: This paper proposes AIMformer, a transformer-based framework for real-time misbehaviour detection in vehicular platoons, addressing limitations of traditional methods through attention mechanisms and edge deployment optimization.


<details>
  <summary>Details</summary>
Motivation: Vehicular platoons face security risks from falsified data injected by authenticated vehicles. Conventional methods like plausibility checks and statistical analysis struggle with high false positives and lack capability to model complex temporal-spatial dynamics in multi-vehicle systems.

Method: The method uses multi-head self-attention to capture intra-vehicle dynamics and inter-vehicle correlations, incorporates vehicle-specific positional encoding for maneuvers, and introduces a BCE loss function to minimize false positives. The model is optimized for edge deployment using TensorFlow Lite, ONNX, and TensorRT.

Result: AIMformer achieves ≥0.93 detection accuracy across four platoon controllers and diverse attack scenarios. It attains sub-millisecond inference latency on edge devices, enabling real-time operation on resource-constrained platforms.

Conclusion: AIMformer effectively addresses misbehaviour detection in vehicular platoons with state-of-the-art performance and practical edge deployment capabilities, making it viable for both in-vehicle and roadside infrastructure use cases.
...
TL;DR: The paper proposes I2PERCEPTION, a low-cost method to de-anonymize I2P hidden services by monitoring router behavior and correlating live on-off patterns, validated through real-world experiments with 15 floodfill routers over eight months.


<details>
  <summary>Details</summary>
Motivation: Existing de-anonymization methods for I2P are ineffective at scaling across the large and diverse network, necessitating a more efficient and precise approach for identifying hidden service IP addresses.

Method: I2PERCEPTION uses floodfill routers to passively collect RouterInfo, inferring routers' on-off behaviors. It combines this with active probing targeting hidden services and correlates behavior patterns to identify matching routers.

Result: Experiments with 15 floodfill routers over eight months demonstrated I2PERCEPTION successfully de-anonymized all controlled hidden services, proving its precision and scalability.

Conclusion: I2PERCEPTION presents an effective, low-cost method for de-anonymizing I2P hidden services by exploiting live behavior analysis, highlighting vulnerabilities in the network's anonymity guarantees.
...
TL;DR: ComMark introduces a frequency-domain black-box model watermarking framework that achieves state-of-the-art covertness and robustness while extending applicability to diverse tasks beyond image recognition.


<details>
  <summary>Details</summary>
Motivation: Deep learning models are vulnerable to leakage, and existing watermarking methods struggle to balance covertness and robustness for practical IP protection.

Method: ComMark uses frequency-filtered compressed watermark generation and integrates simulated attack training with similarity loss to enhance robustness.

Result: ComMark outperforms existing methods in covertness and robustness across multiple datasets/architectures and demonstrates versatility in tasks like speech, video, and text analysis.

Conclusion: ComMark provides a practical, attack-resilient watermarking solution with broad applicability, addressing critical gaps in model IP protection.
...
TL;DR: The authors introduced a new protocol for handling linear queries on distributed data, named Distributed HDMM, which is secure and accurate.


<details>
  <summary>Details</summary>
Motivation: The need for a protocol that can answer workloads of linear queries on distributed data while maintaining privacy and without relying on a trusted curator.

Method: The proposed method uses a secure aggregation protocol to implement HDMM on distributed data, ensuring security against malicious aggregators and clients, with the assumption of an honest majority.

Result: Preliminary evaluation indicates that Distributed HDMM can be applied to real-world datasets and workloads with thousands of clients in under a minute.

Conclusion: Distributed HDMM is a promising protocol for processing linear queries on distributed data securely and efficiently.
...
TL;DR: This paper introduces BashArena, a testbed for evaluating AI control techniques in security-critical environments. It assesses LLMs on task completion, sabotage execution, and detection evasion, with Claude Sonnet 4.5 successfully evading GPT-4.1 monitoring 26% of the time.


<details>
  <summary>Details</summary>
Motivation: Autonomous AI agents with elevated privileges could cause harm if misaligned. Developing control techniques to prevent sabotage while maintaining utility is critical.

Method: Created BashArena with 637 Linux admin tasks and 4 sabotage objectives. Evaluated LLMs on task success, sabotage detection evasion, and detection accuracy via system CallGraph traces and probabilistic classifiers.

Result: Claude Sonnet 4.5 evaded GPT-4.1 monitoring during malware execution 26% of the time (4% false positive rate). Quantified performance gaps across models in both task completion and sabotage detection.

Conclusion: Established a baseline for AI control research in BashArena. Released dataset and task generation pipeline to enable benchmarking of control protocols against realistic attack scenarios.
...
### [31] [How Deep Does Your Dependency Tree Go? An Empirical Study of Dependency Amplification Across 10 Package Ecosystems](https://arxiv.org/abs/2512.14739)
*Jahidul Arafat*

Main category: cs.SE

TL;DR: This study compares dependency amplification across 10 ecosystems, finding Maven has highest amplification (24.70X), challenging assumptions about npm. Security strategies must be ecosystem-specific due to design-driven variations in amplification patterns.


<details>
  <summary>Details</summary>
Motivation: Dependency amplification impacts software supply chain security but has not been systematically compared across ecosystems. Prior assumptions (e.g., npm having highest amplification) lack data-driven validation.

Method: Conducted an empirical analysis of 500 projects across ten ecosystems (Maven, npm, Rust, Python, etc.), calculating dependency amplification ratios (transitive/direct). Used statistical comparisons (45 pairwise) with effect sizes to identify significant differences. Analyzed factors like dependency resolution and standard library completeness.

Result: Maven (24.70X amplification) dominates, contrasting with Go Modules (4.48X) and CocoaPods (0.32X). 28% of Maven projects exceed 10X amplification, compared to 14% for RubyGems and 12% for npm. Cargo, PyPI, and others show <1% projects above 10X. Ecosystem design (resolution behavior, platform constraints) explains differences.

Conclusion: The study concludes that dependency amplification varies significantly across ecosystems, necessitating tailored security strategies. Maven requires systematic auditing due to high amplification, while ecosystems likeCargo and PyPI maintain control with lower rates. The paper advocates for replication of the study with provided data.
...
### [32] [VDMN: A Graphical Notation for Modelling Value Driver Trees](https://arxiv.org/abs/2512.14740)
*Benjamin Matthies*

Main category: cs.SE

TL;DR: The paper proposes Value Driver Modelling Notation (VDMN), a graphical notation to systematize Value Driver Tree (VDT)$ modelling. It addresses the lack of standard guidelines through semantic constructs and intuitive syntax, validated via case studies and expert interviews.


<details>
  <summary>Details</summary>
Motivation: Existing VDT models lack systematic guidelines, hindering consistency and clarity in value-based business analysis. The study aims to fill this gap by enabling standardized, structured VDT creation.

Method: The authors developed VDMN with formal semantic constructs and graphical syntax. They validated its practical utility through two case studies and expert interviews to assess comprehension and effectiveness.

Result: VDMN successfully improved consistency and readability in VDT models. Case studies demonstrated its usability, while expert feedback confirmed its intuitive design and support for decision-making.

Conclusion: VDMN represents a critical step toward standardizing VDT modelling, offering a systematic framework that enhances clarity and consistency in value-based business analysis.
...
### [33] [Revisiting the Reliability of Language Models in Instruction-Following](https://arxiv.org/abs/2512.14754)
*Jianshuo Dong,Yutong Zhang,Yan Liu,Zhenyu Zhong,Tao Wei,Chao Zhang,Han Qiu*

Main category: cs.SE

TL;DR: The paper introduces IFEval++ to evaluate the nuance-oriented reliability of LLMs, revealing significant performance drops under subtle prompt changes and proposing improvement strategies.


<details>
  <summary>Details</summary>
Motivation: Current LLMs show high benchmark scores but lack real-world reliability due to variations in user phrasing and task formulations. The study aims to address this by focusing on nuanced prompt consistency.

Method: The authors developed the reliable@k metric and an automated pipeline for generating cousin prompts. They then built IFEval++ to test 20 proprietary and 26 open-source LLMs.

Result: Models show a performance drop of up to 61.8% with nuanced prompts. The paper identifies insufficiencies in nuance-oriented reliability and explores potential improvement methods.

Conclusion: Nuance-oriented reliability is a critical yet understudied aspect of LLM trustworthiness. The IFEval++ benchmark and findings offer a foundation for improving model dependability.
...
### [34] [Examining Software Developers' Needs for Privacy Enforcing Techniques: A survey](https://arxiv.org/abs/2512.14756)
*Ioanna Theophilou,Georgia M. Kapitsaki*

Main category: cs.SE

TL;DR: This paper explores the needs of 68 developers regarding data privacy law compliance, highlighting a demand for automated tools, especially among those with more privacy experience.


<details>
  <summary>Details</summary>
Motivation: The study is motivated by the growing need for data privacy law compliance in software systems and the lack of understanding of emerging developer needs that can aid in compliance.

Method: The authors conducted a survey with 68 developers to examine their needs for privacy law compliance and the factors influencing these needs.

Result: Most developers expressed a need for more automated tools, and privacy experience was found to influence their concerns about privacy tools.

Conclusion: The study concludes that there is an urgent need for privacy facilitators and that understanding developers' needs can help improve development practices for privacy law compliance.
...
### [35] [CAPE: Capability Achievement via Policy Execution](https://arxiv.org/abs/2512.14761)
*David Ball*

Main category: cs.SE

TL;DR: The paper introduces Capability Engineering and CAPE (Capability Achievement via Policy Execution) as a solution to ensure models reliably satisfy explicit constraints. CAPE provides an 8 times cost reduction and reduces violation rates by 81% using a specify-verify-correct-train loop, and it is open-sourced under Apache 2.0 with CapabilityBench.


<details>
  <summary>Details</summary>
Motivation: Modern AI systems lack a way to express and enforce requirements, leading to failures in real-world despite performance on benchmarks. Current pre and post-training processes do not guarantee compliance with explicit, context-dependent constraints.

Method: The paper introduces Capability Engineering, a systematic practice of converting requirements to executable specs using the CAPE protocol (Specify -> Verify -> Correct -> Train loop). CAPE is based on contextual objectivity, where subjective properties become objective with fixed context, and verification-fidelity scaling, where verification improves with model scale. CAPE uses reusable specifications instead of per-example annotation.

Result: CAPE is tested on 109,500 examples in six domains, showing 81% reduction in violation rate relative to DPO, with a standard deviation less than 0.3%. It also reduces costs by 5-20 times and shortens timelines from months to weeks.

Conclusion: The paper concludes that CAPE and Capability Engineering provide a systematic way to convert requirements into capabilities, and by open-sourcing components like CAPE protocol, CPL, and CapabilityBench, the community can measure and improve models' capability more effectively.
...
### [36] [Workflows vs Agents for Code Translation](https://arxiv.org/abs/2512.14762)
*Henry Gray,Tom Yotam,Octavian Udrea*

Main category: cs.SE

TL;DR: This paper compares two LLM-driven methods for syntax repair in MATLAB-to-HDL pipelines: an expert-designed flow and an autonomous agentic approach using MCP, finding the agentic approach more effective for mid-sized models.


<details>
  <summary>Details</summary>
Motivation: Translating MATLAB to HDL for FPGAs/ASICs is laborious due to LLMs' poor HDL syntax handling. Current methods are error-prone and lack robust automation.

Method: 1) Compare 42 signal-processing functions
2) Isolate syntax-repair stage
3) Test on three model scales
4) Use structured vs agentic (MCP) approaches
5) Measure simulation reach rate and error resolution

Result: Agentic approach: +20% simulation reach for mid-30B models. Shows: 
- Better upstream error resolution
- Short prompts + context management help
- 235B model gains minimal, RAG variant sometimes better
- 8B/30B benefit from conditional retrieval

Conclusion: Agentic frameworks effectively compensate for capacity limits in small/mid models, particularly improving mid-sized models' performance through context management and conditional retrieval. Large models gain less.
...
TL;DR: ADRS (AI-Driven Research for Systems) automates solution discovery in systems research using iterative AI feedback, demonstrating competitive solutions and best practices across ten case studies, while shifting researcher focus to strategic problem formulation.


<details>
  <summary>Details</summary>
Motivation: AI-driven research (ADRS) is motivated by the need for reliable verifiers in systems performance research, where candidate solutions can be evaluated through real/simulated implementations against workloads. This paradigm shifts research effort to problem formulation and oversight rather than solution design.

Method: The method involves an iterative cycle of generation, evaluation, and refinement (ADRS) using open-source frameworks like OpenEvolve, GEPA, and ShinkaEvolve. This approach is tested across ten case studies in various systems problems, utilizing real systems/simulators and predefined workloads for validation.

Result: ADRS-generated solutions outperformed or matched human state-of-the-art designs in ten case studies, including areas like cloud scheduling, load balancing, and transaction scheduling. Best practices for effective ADRS use are also outlined, such as prompt specification, feedback levels, and robust evaluation.

Conclusion: Although there is no universal recipe for ADRS yet, the paper provides guidance for future research, suggesting a shift in researcher effort towards problem formulation and strategic oversight. It highlights the potential of ADRS in systems research through case studies and best practices.
...
### [38] [Industry Expectations and Skill Demands in Quantum Software Testing](https://arxiv.org/abs/2512.14861)
*Ronnie de Souza Santos,Teresa Baldassarre,Cesar França*

Main category: cs.SE

TL;DR: Quantum software testing merges software QA and experimental physics, needing hybrid skills in programming, quantum tech, and collaboration, while emphasizing education-research alignment with industry needs.


<details>
  <summary>Details</summary>
Motivation: To investigate how the quantum software industry defines testing roles and what skills are expected from professionals in these positions.

Method: Analysis of 110 job postings from quantum software/hardware organizations to identify testing-related activities, competencies, and skill requirements.

Result: Testing in quantum contexts combines traditional software QA with experimental validation (calibration, control, hybrid quantum-classical verification), requiring interdisciplinary expertise in programming/automation, quantum-specific knowledge, and collaboration.

Conclusion: Quantum software testing is in an early but rapidly evolving stage, bridging software engineering and experimental physics, necessitating educational and research efforts aligned with industrial realities.
...
### [39] [Evaluating Code Reasoning Abilities of Large Language Models Under Real-World Settings](https://arxiv.org/abs/2512.14917)
*Changshu Liu,Alireza Ghazanfari,Yang Chen,Reyhaneh Jabbarvand*

Main category: cs.SE

TL;DR: This paper introduces RE2-Bench, a new realistic code reasoning benchmark that includes complex real-world code problems. Evaluation shows significant drops in LLM performance on hard problems compared to easy ones.


<details>
  <summary>Details</summary>
Motivation: Previous benchmarks for LLM code reasoning tasks are too simplistic and fail to represent real-world complexities, which may falsely inflate perceived capabilities of LLMs.

Method: RE2-Bench is designed using real-world code with inter-/intra-procedural dependencies and complex types, utilizing static and dynamic analysis. Problems are categorized into Easy and Hard based on code complexity metrics via a majority-vote mechanism.

Result: Evaluations with six LLMs on RE2-Bench tasks showed significant performance drops (51.5% for input prediction, 42.15% for output prediction) when moving from Easy to Hard problems.

Conclusion: RE2-Bench provides a realistic evaluation framework for LLM code reasoning, revealing a substantial overestimation of LLM capabilities by previous simplistic benchmarks.
...
### [40] [Imitation Game: Reproducing Deep Learning Bugs Leveraging an Intelligent Agent](https://arxiv.org/abs/2512.14990)
*Mehil B Shah,Mohammad Masudur Rahman,Foutse Khomh*

Main category: cs.SE

TL;DR: RepGen improves the reproduction of DL bugs with an 80.19% success rate, surpassing previous methods by 19.81%.


<details>
  <summary>Details</summary>
Motivation: Deep learning models are prone to bugs that are hard to reproduce due to nondeterminism and environment dependencies, with only 3% of bugs reproducible manually.

Method: RepGen uses a learning-enhanced context, an iterative generate-validate-refine strategy, and LLM-generated code to automatically reproduce DL bugs.

Result: RepGen achieves an 80.19% reproduction rate on 106 real-world bugs, a 19.81% improvement over existing techniques, and reduces reproduction time by 56.8% for developers.

Conclusion: RepGen effectively addresses DL bug reproduction challenges through an automated and intelligent approach, offering significant improvements in success rates, time efficiency, and developer workload.
...
### [41] [Toxicity Ahead: Forecasting Conversational Derailment on GitHub](https://arxiv.org/abs/2512.15031)
*Mia Mohammad Imran,Robert Zita,Rahat Rizvi Rahman,Preetha Chatterjee,Kostadin Damevski*

Main category: cs.SE

TL;DR: This paper addresses toxic interactions in open source communities by introducing an LLM-based framework for predicting conversational derailment using GitHub discussion data, achieving high F1-scores compared to NLP baselines.


<details>
  <summary>Details</summary>
Motivation: Toxicity in OSS communities threatens project sustainability, but existing moderation is manual and labor-intensive. Scalable proactive strategies require understanding how toxic conversations evolve.

Method: The authors 1.) curate a dataset of 159 toxic and 207 non-toxic GitHub threads 2.) develop a two-step LLM pipeline using Least-to-Most prompting to generate 'Summaries of Conversation Dynamics' and estimate derailment likelihood.

Result: Using Qwen and Llama models, the framework achieved F1-scores of 0.901/0.852 on the primary dataset and 0.797 on external validation (308 issue threads), outperforming established NLP baselines.

Conclusion: Structured LLM prompting enables effective early detection of toxic conversation patterns in OSS, supporting proactive moderation with explainable predictions.
...
### [42] [An Exploratory Study of Bayesian Prompt Optimization for Test-Driven Code Generation with Large Language Models](https://arxiv.org/abs/2512.15076)
*Shlok Tomar,Aryan Deshwal,Ethan Villalovoz,Mattia Fazzini,Haipeng Cai,Janardhan Rao Doppa*

Main category: cs.SE

TL;DR: BODE-GEN uses Bayesian optimization in continuous embedding space to optimize prompts for code generation, improving functional correctness compared to fixed or manually engineered prompts.


<details>
  <summary>Details</summary>
Motivation: Generated code must be functionally correct, but existing prompted LLM approaches often lack accuracy or require manual engineering, which is time-consuming and suboptimal.

Method: The approach formulates prompt search as Bayesian optimization using an auxiliary LLM to map between discrete prompts and continuous embeddings, with techniques like random projections and scaled priors enabling effective exploration of high-dimensional spaces.

Result: BODE-GEN achieves better code accuracy than fixed prompts or manual engineering on the HumanEval+ benchmark across multiple LLMs with sample efficiency, requiring few iterations to improve performance.

Conclusion: BODE-GEN represents an effective and automated method for improving code generation accuracy through data-driven prompt optimization.
...
### [43] [Aligning Academia with Industry: An Empirical Study of Industrial Needs and Academic Capabilities in AI-Driven Software Engineering](https://arxiv.org/abs/2512.15148)
*Hang Yu,Yuzhou Lai,Li Zhang,Xiaoli Lian,Fang Liu,Yanrui Dong,Ting Zhang,Zhi Jin,David Lo*

Main category: cs.SE

TL;DR: The study examines the alignment between academic SE research and industrial needs, analyzing papers and surveying organizations to highlight under-explored challenges.


<details>
  <summary>Details</summary>
Motivation: Rapid advancements in LLMs are changing SE, but the alignment of academic SE research with real industrial needs is unclear.

Method: The authors analyze 1,367 SE papers from 2022-2025 and conduct an empirical survey of 282 industry responses across six SE topics.

Result: Seven critical implications were identified, emphasizing under-addressed issues in requirements, architecture, reliability, explainability, evaluation, and ethics.

Conclusion: The study aims to refocus academic research on industrially relevant challenges to enhance the impact of SE research.
...
### [44] [Automating Execution and Verification of BPMN+DMN Business Processes](https://arxiv.org/abs/2512.15214)
*Giuseppe Della Penna,Igor Melatti*

Main category: cs.SE

TL;DR: This paper introduces a tool called BDTransTest that translates BPMN+DMN processes into Java programs, synthsizes testing plans, and assesses test coverage, addressing the lack of semantic analysis in current tools.


<details>
  <summary>Details</summary>
Motivation: Current BPMN+DMN frameworks focus on syntactic error detection, but overlook semantic (behavioral) faults. Designers must manually test processes with unspecified simulation translations, leading to inefficiency and potential errors.

Method: The authors designed a tool named BDTransTest that (i) translates BPMN+DMN processes into Java programs, (ii) synthesizes and executes testing plans, and (iii) analyzes coverage in terms of nodes and edges. The methodology is evaluated using existing BPMN+DMN processes from literature.

Result: BDTransTest successfully translated BPMN+DMN processes into Java, synthesized testing plans requiring designer input for disambiguation, and provided coverage analysis. Experimental evaluations demonstrated the tool's effectiveness on processes from the literature.

Conclusion: BDTransTest advances the state of the art by addressing semantic analysis and testing in BPMN+DMN processes. It offers an automated solution for translation, testing, and coverage analysis, improving the verification of business processes beyond syntactic checks.
...
### [45] [Heterogeneous Model Alignment in Digital Twin](https://arxiv.org/abs/2512.15281)
*Faima Abbasi,Jean-Sébastien Sottet,Cedric Pruski*

Main category: cs.SE

TL;DR: The paper introduces a framework for aligning heterogeneous models in multi-layered digital twins (DTs) by combining adaptive conformance mechanisms and a LLM-validated alignment process, enhancing flexibility and automating semantic correspondence discovery without manual updates.


<details>
  <summary>Details</summary>
Motivation: The motivation arises from the challenge of semantic mismatches and synchronization issues in multi-layer, model-driven DTs, where existing static mapping methods are inflexible and error-prone, risking data integrity.

Method: The proposed method integrates two components: adaptive conformance mechanisms for evolving model metamodel linkage and a LLM-based alignment process grounded in domain knowledge. This enables semantic coherence across abstraction layers, automated mappings, and scalability.

Result: The method was demonstrated using an air quality use case and validated with OAEI test cases, showing its effectiveness in automating semantic alignment, reducing manual effort, and maintaining consistency across layers.

Conclusion: The framework successfully addresses shortcomings in static and manual alignment methods by leveraging adaptability and LLM validation. This promotes scalable, accurate, and semantically aligned DT systems across abstraction layers.
...
### [46] [Can AI Generate more Comprehensive Test Scenarios? Review on Automated Driving Systems Test Scenario Generation Methods](https://arxiv.org/abs/2512.15422)
*Ji Zhou,Yongqi Zhao,Yixian Hu,Hexuan Li,Zhengguo Gu,Nan Xu,Arno Eichberger*

Main category: cs.SE

TL;DR: This review provides a comprehensive analysis of scenario-based testing methods for Automated Driving Systems (ADS), highlighting the shift from traditional to AI-assisted approaches and identifying key gaps in the field such as lack of standardized metrics, limited ethical considerations, and incomplete ODD coverage, while offering solutions like an updated taxonomy, ethical checklist, and scenario difficulty schema.


<details>
  <summary>Details</summary>
Motivation: Traditional verification methods for Automated Driving Systems (ADS) are costly and time-consuming, necessitating a more scalable and efficient solution. Scenario-based testing is emerging as a viable alternative, but existing reviews lack comprehensive coverage of recent advancements.

Method: The review systematically analyzes 31 primary studies and 10 surveys identified through a comprehensive search from 2015 to 2025. The in-depth methodological synthesis and comparative evaluation focus on recent frameworks (2023-2025), reflecting the surge in AI-assisted and multimodal approaches. The analysis compares traditional methods relying on expert knowledge and data with recent developments utilizing generative models and reinforcement learning.

Result: The review identifies three persistent gaps in scenario-based testing for ADS: absence of standardized evaluation metrics, limited integration of ethical and human factors, and insufficient coverage of multimodal and ODD-specific scenarios. The review contributes a refined taxonomy, an ethical and safety checklist for scenario design, and an ODD coverage map with scenario-difficulty schema to address these challenges.

Conclusion: This review provides methodological clarity and practical guidance for researchers and industry stakeholders by addressing key gaps in scenario-based testing for ADS. The proposed solutions, including a refined taxonomy, ethical checklist, and scenario difficulty schema, enable transparent benchmarking and reproducible evaluation, supporting the safe deployment of higher-level Automated Driving Systems.
...
### [47] [Insecure Ingredients? Exploring Dependency Update Patterns of Bundled JavaScript Packages on the Web](https://arxiv.org/abs/2512.15447)
*Ben Swierzy,Marc Ohm,Michael Meier*

Main category: cs.SE

TL;DR: This paper introduces Aletheia, a package-agnostic JavaScript version detection tool that improves upon prior methods by analyzing bundles through plagiarism detection algorithms. It reveals dependency update patterns, showing bundled packages update faster than CDN-included ones, but highlights that a small number of vendors drive most updates, indicating gaps in quantitative security metrics.


<details>
  <summary>Details</summary>
Motivation: Existing package version detection methods are limited to hand-selected popular packages or single-file resources, failing to accurately analyze large-scale web application update behavior. Security concerns persist due to unknown prevalence of vulnerable npm package versions in production.

Method: Aletheia dissects JavaScript bundles using plagiarism detection algorithms (e.g., similarity hashing, n-gram analysis) to identify package versions without relying on global namespace or popularity assumptions. This package-agnostic approach enables scalable, fine-grained analysis of dependency usage.

Result: 1) Aletheia outperforms existing tools in detection accuracy. 2-20\% of Tranco top 100k domains update dependencies within 16 weeks. 3- Bundled packages update 10x faster than CDN-included ones, resulting in fewer vulnerabilities. 4- A few vendors dominate update activity, suggesting quantitative metrics miss critical behavioral patterns.

Conclusion: While Aletheia demonstrates effective version detection, findings indicate security updates are uneven across ecosystems. Fast update rates for bundled packages coexist with vendor-driven update dynamics, emphasizing the need for complementary qualitative analysis alongside quantitative metrics to fully understand software supply chain security.
...
### [48] [A Container-based Approach For Proactive Asset Administration Shell Digital Twins](https://arxiv.org/abs/2512.15452)
*Carsten Ellwein,Jingxi Zhang,Andreas Wortmann,Antony Ayman Alfy Meckhael*

Main category: cs.SE

TL;DR: This paper proposes a submodel-based architecture to integrate containerized services into digital twins (Asset Administration Shells) for dynamic runtime adaptation. A case study on a 3-axis milling machine demonstrates this concept.


<details>
  <summary>Details</summary>
Motivation: Current digital twins (AAS) are treated as static information models, lacking dynamic service integration and proactive behavior, which limits their potential for runtime adaptation and system intelligence.

Method: The authors introduce a service-oriented submodel architecture for AAS, embedding behavioral definitions and trigger conditions. This allows containerized services to dynamically adapt AAS instances at runtime via an event-driven, modular design.

Result: A 3-axis milling machine case study validates the architecture, showing AAS can now act as an active interface for executing added-value services, moving beyond passive data representation.

Conclusion: The proposed architecture enhances AAS capabilities by enabling dynamic service integration, paving the way for future AI-driven adaptation and intelligence in digital twin ecosystems.
...
### [49] [On Assessing the Relevance of Code Reviews Authored by Generative Models](https://arxiv.org/abs/2512.15466)
*Robert Heumüller,Frank Ortmeier*

Main category: cs.SE

TL;DR: The paper introduces a multi-subjective ranking method for evaluating code review effectiveness of models like ChatGPT. It finds ChatGPT outperforms humans, even StackExchange accepted answers, while highlighting evaluation needs and integration risks.



<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for code review generation lack in capturing human perspective variability and are too subjective, so a new approach is needed for better assessment.


Method: Used 280 code review requests from CodeReview StackExchange. Multiple human judges ranked ChatGPT outputs against top human answers.

Result: ChatGPT responses were ranked better than human ones, including accepted answers on StackExchange, indicating potential AI superiority in code review

Conclusion: Multi-subjective ranking improves evaluation but warns about potential risks from over-trusting models in code reviews; need for further research on safety integration.
...
### [50] [How Do Semantically Equivalent Code Transformations Impact Membership Inference on LLMs for Code?](https://arxiv.org/abs/2512.15468)
*Hua Yang,Alejandro Velasco,Thanh Le-Cong,Md Nazmul Haque,Bowen Xu,Denys Poshyvanyk*

Main category: cs.SE

TL;DR: This paper explores how semantically equivalent code transformations can be used to weaken membership inference (MI) detection in large language models trained on code data. The study finds that these transformations, particularly variable renaming, significantly reduce MI effectiveness, exposing a major loophole in license compliance enforcement.


<details>
  <summary>Details</summary>
Motivation: Recent advancements in code-focused AI models have raised concerns about unauthorized use of licensed code. While MI techniques aim to detect such misuse, they remain vulnerable to code obfuscation via syntactic transformations. This work aims to evaluate the effectiveness of these techniques and their implications for IP compliance.

Method: The authors systematically apply semantically equivalent code transformations to datasets used for fine-tuning large language models. They measure the impact of these transformations on MI detection accuracy, using causal analysis to identify the most effective obfuscation strategies.

Result: Transformed datasets reduced MI detection accuracy by 1.5% per rule, with variable renaming decreasing detection success by 10.19%. Causal analysis confirmed variable renaming as the most effective strategy. However, combining multiple transformations did not improve obfuscation effectiveness.

Conclusion: The findings reveal a critical vulnerability in current MI-based compliance mechanisms for code-trained LLMs. Even simple syntactic transformations can significantly undermine detection capabilities, necessitating stronger enforcement strategies for IP protection in AI model development.
...
### [51] [WuppieFuzz: Coverage-Guided, Stateful REST API Fuzzing](https://arxiv.org/abs/2512.15554)
*Thomas Rooijakkers,Anne Nijsten,Cristian Daniele,Erieke Weitenberg,Ringo Groenewegen,Arthur Melissen*

Main category: cs.SE

TL;DR: The paper introduces WuppieFuzz, an open-source REST API fuzzer built on LibAFL, designed for white-box, grey-box, and black-box fuzzing. It uses OpenAPI specifications to generate and mutate request sequences for thorough testing of web services, aiming to reduce security risks through automated coverage-guided techniques.


<details>
  <summary>Details</summary>
Motivation: REST APIs are widely used in business processes, exposing endpoints that pose significant security risks. Manual testing is impractical due to the large number of endpoints, necessitating automated techniques like fuzzing to ensure robustness and reduce vulnerabilities.

Method: WuppieFuzz is built on LibAFL and supports multiple fuzzing modes. It utilizes OpenAPI specs to create an initial input corpus of request sequences, applies REST-specific and LibAFL mutators to explore code paths, and uses coverage feedback to guide further request generation. The tool automates harness creation and provides detailed reporting for bug identification.

Result: Evaluation on the Petstore API showed the effectiveness of white-box fuzzing and different power schedules. The monitoring of endpoint and code coverage over time demonstrated the approach's efficacy in improving API robustness.

Conclusion: WuppieFuzz is a promising tool for automated REST API testing that simplifies fuzzing by leveraging OpenAPI specifications and coverage feedback, reducing manual effort and enhancing the discovery of vulnerabilities in web services.
...
### [52] [A High-level Synthesis Toolchain for the Julia Language](https://arxiv.org/abs/2512.15679)
*Benedict Short,Ian McInerney,John Wickerson*

Main category: cs.SE

TL;DR: The paper proposes an MLIR-based compiler toolchain that translates Julia kernels into SystemVerilog for FPGA acceleration, addressing the two-language problem by unifying kernel development without additional directives.


<details>
  <summary>Details</summary>
Motivation: Problem sizes have grown due to Exascale computing and data-driven methods, leading to a need for hardware accelerators. Current FPGA development is hindered by the two-language problem, where algorithms are in high-level languages while accelerators use low-level, domain-specific ones.

Method: An MLIR-based compiler toolchain is developed to automatically convert Julia kernels to SystemVerilog. It supports dynamic/static scheduling, integrates with AXI4-Stream for memory interfacing, and generates vendor-agnostic RTL.

Result: A prototype toolchain was able to synthesize signal processing/math benchmarks on FPGAs at 100MHz, achieving 59.71-82.6% of the throughput of traditional C/C++ compiler toolchains.

Conclusion: The proposed toolchain enables domain experts to write kernels in Julia and target FPGAs without manual tuning or language modifications, reducing the complexity and expertise required in FPGA development.
